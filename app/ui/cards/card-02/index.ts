...
// Card 02 - Taxa de Sucesso Global
...
interface CardFacade {
  mount: (container: HTMLElement) => CardGlobalSuccessRate;
  unmount: (instance: CardGlobalSuccessRate | null | undefined) => void;
  refresh: (instance: CardGlobalSuccessRate | null | undefined) => Promise<void> | undefined;
  getStatus: (instance: CardGlobalSuccessRate | null | undefined) => CardStatusResult;
  isReady: (instance: CardGlobalSuccessRate | null | undefined) => boolean;
  info: (instance: CardGlobalSuccessRate | null | undefined) => CardInfoResult;
  healthCheck: (instance: CardGlobalSuccessRate | null | undefined) => HealthCheckResult;
}
...
export const MODULE_ID = 'card-02' as const;
export const CARD_ID = 'card-02' as const;

const _debug = (): boolean => window.APP_CONFIG?.app?.debug ?? false;
const _log = (level: LogLevel, ...args: unknown[]): void => {
  if (level === 'error') { console.error('[Card-02]', ...args); return; }
  if (level === 'warn') { console.warn('[Card-02]', ...args); return; }
  if (_debug()) console.log('[Card-02]', ...args);
};
...
export class CardGlobalSuccessRate {
  ...
  constructor(container: HTMLElement) {
    ...
    this.container = container; this.cardId = container.id || 'card-02';
  ...
  render(): void { this.container.innerHTML = `<div class="card-header"><svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg><span>Taxa Global</span></div><div class="card-body"><div class="card-value" data-el="percent">--</div><div class="card-label">Todos os jobs ativos</div></div><div class="card-status" data-el="status" role="status" aria-live="polite"></div>`; this.$ = { pct: this.container.querySelector('[data-el="percent"]'), status: this.container.querySelector('[data-el="status"]') }; }
  ...
  async loadData(): Promise<void> {
    return this.state.withLock('loadData', async (): Promise<void> => {
      ...
      try {
        ...
        const rate: number = parseFloat(String(data.data.success_rate ?? data.data.rate ?? 0));
        if (this.$?.pct) this.$.pct.textContent = Number.isFinite(rate) ? `${Math.round(rate)}%` : '--';
        this.setStatus(''); this.container.classList.remove('is-loading', 'has-error');
        this.state.setState('SUCCESS', { successRate: rate });
        _log('info', 'Dados atualizados', { successRate: rate, traceId: response.meta?.traceId });
      } catch (error: unknown) {
        ...
        if (this.container) { this.errorHandler.showCardError(this.container, 'Erro ao carregar taxa global'); this.container.classList.remove('is-loading'); this.container.classList.add('has-error'); }
  ...
  info(): CardInfoResult { return { name: MODULE_ID, version: VERSION, cardId: CARD_ID, title: 'Taxa Global', mounted: !!this.$, state: this.state.state, uptime: this._mountedAt ? Date.now() - this._mountedAt : 0, timestamp: Date.now() }; }
...
let _lastInstance: CardGlobalSuccessRate | null = null;
export const Card: CardFacade = {
  mount: (container: HTMLElement): CardGlobalSuccessRate => { _lastInstance = new CardGlobalSuccessRate(container); return _lastInstance; },
  unmount: (instance: CardGlobalSuccessRate | null | undefined): void => { instance?.destroy?.(); if (instance === _lastInstance) _lastInstance = null; },
  refresh: (instance: CardGlobalSuccessRate | null | undefined): Promise<void> | undefined => instance?.loadData?.(),
  getStatus: (instance: CardGlobalSuccessRate | null | undefined): CardStatusResult => instance?.getStatus?.() || { mounted: false },
  isReady: (instance: CardGlobalSuccessRate | null | undefined): boolean => instance?.isReady?.() ?? false,
  info: (instance: CardGlobalSuccessRate | null | undefined): CardInfoResult => instance?.info?.() || { name: MODULE_ID, version: VERSION, cardId: CARD_ID, mounted: false },
  healthCheck: (instance: CardGlobalSuccessRate | null | undefined): HealthCheckResult => instance?.healthCheck?.() || { status: 'UNHEALTHY', mounted: false, version: VERSION, moduleId: MODULE_ID }
};

export const mountCard = (container: HTMLElement): CardGlobalSuccessRate => Card.mount(container);
export const unmountCard = (instance: CardGlobalSuccessRate | null | undefined): void => Card.unmount(instance);
export const refreshCard = (instance: CardGlobalSuccessRate | null | undefined): Promise<void> | undefined => Card.refresh(instance);
export const getCardStatus = (instance: CardGlobalSuccessRate | null | undefined): CardStatusResult => Card.getStatus(instance);
