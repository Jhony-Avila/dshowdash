...
interface ApiResponse {
  ...
  meta?: { traceId?: string; [key: string]: unknown };
  [key: string]: unknown;
}
...
interface CardFacade {
  mount: (container: HTMLElement) => CardCompleted;
  unmount: (instance: CardCompleted | null | undefined) => void;
  refresh: (instance: CardCompleted | null | undefined) => Promise<void> | undefined;
  getStatus: (instance: CardCompleted | null | undefined) => CardStatusResult;
  isReady: (instance: CardCompleted | null | undefined) => boolean;
  info: (instance: CardCompleted | null | undefined) => CardInfoResult;
  healthCheck: (instance: CardCompleted | null | undefined) => HealthCheckResult;
}
...
export const MODULE_ID = 'card-05' as const;
export const CARD_ID = 'card-05' as const;

const _debug = (): boolean => window.APP_CONFIG?.app?.debug ?? false;
const _log = (level: LogLevel, ...args: unknown[]): void => {
  if (level === 'error') { console.error('[Card-05]', ...args); return; }
  if (level === 'warn') { console.warn('[Card-05]', ...args); return; }
  if (_debug()) console.log('[Card-05]', ...args);
};
...
class InternalState {
  ...
  getState(): CardStateValue { return this.state; } isPaused(): boolean { return this.state === 'PAUSED'; } isSuccess(): boolean { return this.state === 'SUCCESS'; }
}
...
export class CardCompleted {
  ...
  constructor(container: HTMLElement) {
    ...
    this.container = container; this.cardId = container.id || 'card-05';
    this.state = new InternalState(this.cardId); this.errorHandler = new InternalErrorHandler();
    this.apiEndpoint = '/api/modules/cards/card-05/api.php';
  ...
  async init(): Promise<void> {
    try {
      ...
      if (window.EventBus) window.EventBus.emit('card:ready', { cardId: 'card-05', name: 'Scripts Completados', timestamp: Date.now() } as CardReadyEvent);
      _log('info', 'Card inicializado com sucesso');
    } catch (error: unknown) { _log('error', 'Erro na inicializacao', { error: (error as Error).message }); if (window.EventBus) window.EventBus.emit('card:error', { cardId: 'card-05', error: (error as Error).message, timestamp: Date.now() } as CardErrorEvent); }
  }

  render(): void { this.container.classList.remove('is-loading'); this.container.removeAttribute('aria-busy'); this.container.innerHTML = `<div class="card-header"><svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true"><polyline points="20 6 9 17 4 12"></polyline></svg><span>Completados</span></div><div class="card-body"><div class="card-value" data-el="count">--</div><div class="card-label">Scripts finalizados</div></div><div class="card-status" data-el="status" role="status" aria-live="polite"></div>`; this.$ = { count: this.container.querySelector('[data-el="count"]'), status: this.container.querySelector('[data-el="status"]') }; }
  ...
  async loadData(): Promise<void> {
    return this.state.withLock('loadData', async (): Promise<void> => {
      try {
        const currentState: CardStateValue = this.state.getState(); const isRefresh: boolean = currentState === 'SUCCESS' || currentState === 'REFRESHING';
        ...
        const count: number = parseInt(String(data.data.value ?? 0), 10);
        ...
        this.state.setState('SUCCESS', { completed: count });
        _log('info', 'Dados atualizados', { completed: count });
      } catch (error: unknown) {
        if ((error as { name?: string }).name === 'AbortError' || (error as { code?: string }).code === 'REQUEST_ABORTED') { _log('info', 'Request cancelado'); return; }
        _log('error', 'Erro ao carregar dados', { error: (error as Error).message });
        if (this.container.classList.contains('is-loading')) this.render();
        this.errorHandler.showCardError(this.container, 'Erro ao carregar scripts completados');
  ...
  setupEventListeners(): void { if (window.EventBus) { window.EventBus.on('cards:refresh-all', this._onRefresh); window.EventBus.on('card-05:refresh', this._onRefresh); } document.addEventListener('visibilitychange', this._onVisibility); }
  ...
  destroy(): void { _log('info', 'Destruindo card'); if (this.intervalId) { clearInterval(this.intervalId); this.intervalId = null; } if (window.EventBus) { window.EventBus.off('cards:refresh-all', this._onRefresh); window.EventBus.off('card-05:refresh', this._onRefresh); } document.removeEventListener('visibilitychange', this._onVisibility); this.$ = null; this._mountedAt = null; }
  ...
  info(): CardInfoResult { return { name: MODULE_ID, version: VERSION, cardId: CARD_ID, title: 'Scripts Completados', mounted: !!this.$, state: this.state.state, uptime: this._mountedAt ? Date.now() - this._mountedAt : 0, timestamp: Date.now() }; }
...
let _lastInstance: CardCompleted | null = null;
export const Card: CardFacade = {
  mount: (container: HTMLElement): CardCompleted => { _lastInstance = new CardCompleted(container); return _lastInstance; },
  unmount: (instance: CardCompleted | null | undefined): void => { instance?.destroy?.(); if (instance === _lastInstance) _lastInstance = null; },
  refresh: (instance: CardCompleted | null | undefined): Promise<void> | undefined => instance?.loadData?.(),
  getStatus: (instance: CardCompleted | null | undefined): CardStatusResult => instance?.getStatus?.() || { mounted: false },
  isReady: (instance: CardCompleted | null | undefined): boolean => instance?.isReady?.() ?? false,
  info: (instance: CardCompleted | null | undefined): CardInfoResult => instance?.info?.() || { name: MODULE_ID, version: VERSION, cardId: CARD_ID, mounted: false },
  healthCheck: (instance: CardCompleted | null | undefined): HealthCheckResult => instance?.healthCheck?.() || { status: 'UNHEALTHY', mounted: false, version: VERSION, moduleId: MODULE_ID }
};

export const mountCard = (container: HTMLElement): CardCompleted => Card.mount(container);
export const unmountCard = (instance: CardCompleted | null | undefined): void => Card.unmount(instance);
export const refreshCard = (instance: CardCompleted | null | undefined): Promise<void> | undefined => Card.refresh(instance);
export const getCardStatus = (instance: CardCompleted | null | undefined): CardStatusResult => Card.getStatus(instance);
...
export async function init(selector: string = '#card-05-root'): Promise<CardCompleted | null> { const container: HTMLElement | null = document.querySelector(selector); if (!container || (container as HTMLElement & { dataset: DOMStringMap }).dataset.initialized === 'true') return null; const instance: CardCompleted = new CardCompleted(container); (container as HTMLElement & { dataset: DOMStringMap }).dataset.initialized = 'true'; (container as HTMLElement & { __cardInstance?: CardCompleted }).__cardInstance = instance; return instance; }
