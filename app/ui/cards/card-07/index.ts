...
interface CardFacade {
  mount: (container: HTMLElement) => CardRunning;
  unmount: (instance: CardRunning | null | undefined) => void;
  refresh: (instance: CardRunning | null | undefined) => Promise<void> | undefined;
  getStatus: (instance: CardRunning | null | undefined) => CardStatusResult;
  isReady: (instance: CardRunning | null | undefined) => boolean;
  info: (instance: CardRunning | null | undefined) => CardInfoResult;
  healthCheck: (instance: CardRunning | null | undefined) => HealthCheckResult;
}

// ── Constantes ──────────────────────────────────────────────────────

export const VERSION: string = '11.0.0-ENTERPRISE';
export const MODULE_ID: string = 'card-07';
export const CARD_ID: string = 'card-07';

// ── Helpers ─────────────────────────────────────────────────────────

const _debug = (): boolean => window.APP_CONFIG?.app?.debug ?? false;
const _log = (level: LogLevel, ...args: unknown[]): void => {
  if (level === 'error') { console.error('[Card-07]', ...args); return; }
  if (level === 'warn') { console.warn('[Card-07]', ...args); return; }
  if (_debug()) console.log('[Card-07]', ...args);
};
...
class InternalErrorHandler {
  ...
  showToast(message: string): void { _log('error', message); }
}
...
export class CardRunning {
  ...
  constructor(container: HTMLElement) {
    ...
    this.container = container; this.cardId = container.id || 'card-07';
    this.state = new InternalState(this.cardId); this.errorHandler = new InternalErrorHandler();
    this.apiEndpoint = '/api/modules/cards/card-07/api.php';
    this.refreshInterval = 30000; this.intervalId = null; this.$ = null; this._mountedAt = null;
  ...
  async init(): Promise<void> {
    try {
      _log('info', 'Inicializando card'); await this.loadCSS(); this.render(); await this.loadData(); this.startAutoRefresh(); this.setupEventListeners(); this._mountedAt = Date.now();
      if (window.EventBus) window.EventBus.emit('card:ready', { cardId: 'card-07', name: 'Rodando', timestamp: Date.now() });
      _log('info', 'Card inicializado com sucesso');
    } catch (error) { _log('error', 'Erro na inicializacao', { error: (error as Error).message }); }
  }

  async loadCSS(): Promise<void> { const linkId: string = 'card-07-styles'; if (document.getElementById(linkId)) return; return new Promise<void>((resolve: () => void, reject: (reason: Error) => void) => { const link: HTMLLinkElement = document.createElement('link'); link.id = linkId; link.rel = 'stylesheet'; link.href = '/components/cards/card-07/styles.css'; link.onload = (): void => resolve(); link.onerror = (): void => reject(new Error('Failed to load CSS')); document.head.appendChild(link); }); }

  render(): void { this.container.classList.remove('is-loading'); this.container.removeAttribute('aria-busy'); this.container.innerHTML = `<div class="card-header"><svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg><span>Em Execucao</span></div><div class="card-body"><div class="card-value" data-el="count">--</div><div class="card-label">Scripts rodando agora</div></div><div class="card-status" data-el="status" role="status" aria-live="polite"></div>`; this.$ = { count: this.container.querySelector('[data-el="count"]'), status: this.container.querySelector('[data-el="status"]') }; }
  ...
  async loadData(): Promise<void> {
    return this.state.withLock('loadData', async (): Promise<void> => {
      try {
        ...
        this.state.setState('SUCCESS', { running: count });
        _log('info', 'Dados atualizados', { running: count });
      } catch (error) {
        ...
        if (this.container.classList.contains('is-loading')) this.render();
        this.errorHandler.showCardError(this.container, 'Erro ao carregar scripts em execucao');
        ...
        if (window.EventBus) window.EventBus.emit('card:error', { cardId: 'card-07', error: err.message, timestamp: Date.now() });
      }
    }) as Promise<void>;
  }
  ...
  setupEventListeners(): void { if (window.EventBus) { window.EventBus.on('cards:refresh-all', this._onRefresh); window.EventBus.on('card-07:refresh', this._onRefresh); } document.addEventListener('visibilitychange', this._onVisibility); }
  ...
  destroy(): void { _log('info', 'Destruindo card'); if (this.intervalId) { clearInterval(this.intervalId); this.intervalId = null; } if (window.EventBus) { window.EventBus.off('cards:refresh-all', this._onRefresh); window.EventBus.off('card-07:refresh', this._onRefresh); } document.removeEventListener('visibilitychange', this._onVisibility); this.$ = null; this._mountedAt = null; }
  ...
  info(): CardInfoResult { return { name: MODULE_ID, version: VERSION, cardId: CARD_ID, title: 'Em Execucao', mounted: !!this.$, state: this.state.state, uptime: this._mountedAt ? Date.now() - this._mountedAt : 0, timestamp: Date.now() }; }
...
let _lastInstance: CardRunning | null = null;
export const Card: CardFacade = {
  mount: (container: HTMLElement): CardRunning => { _lastInstance = new CardRunning(container); return _lastInstance; },
  unmount: (instance: CardRunning | null | undefined): void => { instance?.destroy?.(); if (instance === _lastInstance) _lastInstance = null; },
  refresh: (instance: CardRunning | null | undefined): Promise<void> | undefined => instance?.loadData?.(),
  getStatus: (instance: CardRunning | null | undefined): CardStatusResult => instance?.getStatus?.() || { mounted: false },
  isReady: (instance: CardRunning | null | undefined): boolean => instance?.isReady?.() ?? false,
  info: (instance: CardRunning | null | undefined): CardInfoResult => instance?.info?.() || { name: MODULE_ID, version: VERSION, cardId: CARD_ID, mounted: false },
  healthCheck: (instance: CardRunning | null | undefined): HealthCheckResult => instance?.healthCheck?.() || { status: 'UNHEALTHY', mounted: false, version: VERSION, moduleId: MODULE_ID }
};

export const mountCard = (container: HTMLElement): CardRunning => Card.mount(container);
export const unmountCard = (instance: CardRunning | null | undefined): void => Card.unmount(instance);
export const refreshCard = (instance: CardRunning | null | undefined): Promise<void> | undefined => Card.refresh(instance);
export const getCardStatus = (instance: CardRunning | null | undefined): CardStatusResult => Card.getStatus(instance);
