...
interface CardFacade {
  mount: (container: HTMLElement) => CardAverageTime;
  unmount: (instance: CardAverageTime | null | undefined) => void;
  refresh: (instance: CardAverageTime | null | undefined) => Promise<void> | undefined;
  getStatus: (instance: CardAverageTime | null | undefined) => CardStatusResult;
  isReady: (instance: CardAverageTime | null | undefined) => boolean;
  info: (instance: CardAverageTime | null | undefined) => CardInfoResult;
  healthCheck: (instance: CardAverageTime | null | undefined) => HealthCheckResult;
}

interface HTMLElementWithCardInstance extends HTMLElement {
  __cardInstance?: CardAverageTime;
}

// ── Constantes ──────────────────────────────────────────────────────

export const VERSION: string = '12.0.0-ENTERPRISE';
export const MODULE_ID: string = 'card-08';
export const CARD_ID: string = 'card-08';

// ── Helpers ─────────────────────────────────────────────────────────

const _debug = (): boolean => window.APP_CONFIG?.app?.debug ?? false;
const _log = (level: LogLevel, ...args: unknown[]): void => {
  if (level === 'error') { console.error('[Card-08]', ...args); return; }
  if (level === 'warn') { console.warn('[Card-08]', ...args); return; }
  if (_debug()) console.log('[Card-08]', ...args);
};
...
export class CardAverageTime {
  ...
  constructor(container: HTMLElement) {
    ...
    this.container = container; this.cardId = container.id || 'card-08';
    this.state = new InternalState(this.cardId); this.errorHandler = new InternalErrorHandler();
    this.apiEndpoint = '/api/modules/cards/card-08/api.php';
  ...
  async init(): Promise<void> {
    try {
      ...
      if (window.EventBus) window.EventBus.emit('card:ready', { cardId: 'card-08', name: 'Media por Hora', timestamp: Date.now() });
      _log('info', 'Card inicializado com sucesso');
    } catch (error) { _log('error', 'Erro na inicializacao', { error: (error as Error).message }); if (window.EventBus) window.EventBus.emit('card:error', { cardId: 'card-08', error: (error as Error).message, timestamp: Date.now() }); }
  }

  render(): void { this.container.classList.remove('is-loading'); this.container.removeAttribute('aria-busy'); this.container.innerHTML = `<div class="card-header"><svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg><span>Media/Hora</span></div><div class="card-body"><div class="card-value" data-el="time">--</div><div class="card-label">Execucoes por hora</div></div><div class="card-status" data-el="status" role="status" aria-live="polite"></div>`; this.$ = { time: this.container.querySelector('[data-el="time"]'), status: this.container.querySelector('[data-el="status"]') }; }

  setStatus(msg: string): void { if (this.$ && this.$.status) this.$.status.textContent = msg || ''; }
  formatValue(value: number): string { if (!Number.isFinite(value) || value < 0) return '--'; return new Intl.NumberFormat('pt-BR', { maximumFractionDigits: 1 }).format(value); }
  ...
  async loadData(): Promise<void> {
    return this.state.withLock('loadData', async (): Promise<void> => {
      try {
        ...
        const avgValue: number = parseFloat(String(data.data.value ?? 0));
        if (this.$ && this.$.time) this.$.time.textContent = this.formatValue(avgValue);
        this.setStatus(''); this.container.classList.remove('is-loading', 'has-error');
        this.state.setState('SUCCESS', { averagePerHour: avgValue });
        _log('info', 'Dados atualizados', { averagePerHour: avgValue });
      } catch (error) {
        ...
        if (this.container.classList.contains('is-loading')) this.render();
        this.errorHandler.showCardError(this.container, 'Erro ao carregar media por hora');
  ...
  setupEventListeners(): void { if (window.EventBus) { window.EventBus.on('cards:refresh-all', this._onRefresh); window.EventBus.on('card-08:refresh', this._onRefresh); } document.addEventListener('visibilitychange', this._onVisibility); }
  ...
  destroy(): void { _log('info', 'Destruindo card'); if (this.intervalId) { clearInterval(this.intervalId); this.intervalId = null; } if (window.EventBus) { window.EventBus.off('cards:refresh-all', this._onRefresh); window.EventBus.off('card-08:refresh', this._onRefresh); } document.removeEventListener('visibilitychange', this._onVisibility); this.$ = null; this._mountedAt = null; }
  ...
  info(): CardInfoResult { return { name: MODULE_ID, version: VERSION, cardId: CARD_ID, title: 'Media por Hora', mounted: !!this.$, state: this.state.state, uptime: this._mountedAt ? Date.now() - this._mountedAt : 0, timestamp: Date.now() }; }
...
let _lastInstance: CardAverageTime | null = null;
export const Card: CardFacade = {
  mount: (container: HTMLElement): CardAverageTime => { _lastInstance = new CardAverageTime(container); return _lastInstance; },
  unmount: (instance: CardAverageTime | null | undefined): void => { instance?.destroy?.(); if (instance === _lastInstance) _lastInstance = null; },
  refresh: (instance: CardAverageTime | null | undefined): Promise<void> | undefined => instance?.loadData?.(),
  getStatus: (instance: CardAverageTime | null | undefined): CardStatusResult => instance?.getStatus?.() || { mounted: false },
  isReady: (instance: CardAverageTime | null | undefined): boolean => instance?.isReady?.() ?? false,
  info: (instance: CardAverageTime | null | undefined): CardInfoResult => instance?.info?.() || { name: MODULE_ID, version: VERSION, cardId: CARD_ID, mounted: false },
  healthCheck: (instance: CardAverageTime | null | undefined): HealthCheckResult => instance?.healthCheck?.() || { status: 'UNHEALTHY', mounted: false, version: VERSION, moduleId: MODULE_ID }
};

export const mountCard = (container: HTMLElement): CardAverageTime => Card.mount(container);
export const unmountCard = (instance: CardAverageTime | null | undefined): void => Card.unmount(instance);
export const refreshCard = (instance: CardAverageTime | null | undefined): Promise<void> | undefined => Card.refresh(instance);
export const getCardStatus = (instance: CardAverageTime | null | undefined): CardStatusResult => Card.getStatus(instance);
...
export async function init(selector: string = '#card-08-root'): Promise<CardAverageTime | null> { const container = document.querySelector(selector) as HTMLElementWithCardInstance | null; if (!container || container.dataset.initialized === 'true') return null; const instance: CardAverageTime = new CardAverageTime(container); container.dataset.initialized = 'true'; container.__cardInstance = instance; return instance; }
