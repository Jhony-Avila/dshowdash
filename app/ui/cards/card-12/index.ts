...
interface CardFacade {
  mount: (container: HTMLElement) => CardCacheHitRate;
  unmount: (instance: CardCacheHitRate | null | undefined) => void;
  refresh: (instance: CardCacheHitRate | null | undefined) => Promise<void> | undefined;
  getStatus: (instance: CardCacheHitRate | null | undefined) => CardStatusResult;
  isReady: (instance: CardCacheHitRate | null | undefined) => boolean;
  info: (instance: CardCacheHitRate | null | undefined) => CardInfoResult;
  healthCheck: (instance: CardCacheHitRate | null | undefined) => HealthCheckResult;
}

interface HTMLElementWithCardInstance extends HTMLElement {
  __cardInstance?: CardCacheHitRate;
}

// ── Constantes ──────────────────────────────────────────────────────

export const VERSION: string = '11.0.0-ENTERPRISE';
export const MODULE_ID: string = 'card-12';
export const CARD_ID: string = 'card-12';

// ── Helpers ─────────────────────────────────────────────────────────

const _debug = (): boolean => window.APP_CONFIG?.app?.debug ?? false;
const _log = (level: LogLevel, ...args: unknown[]): void => {
  if (level === 'error') { console.error('[Card-12]', ...args); return; }
  if (level === 'warn') { console.warn('[Card-12]', ...args); return; }
  if (_debug()) console.log('[Card-12]', ...args);
};
...
export class CardCacheHitRate {
  ...
  constructor(container: HTMLElement) {
    ...
    this.container = container; this.cardId = container.id || 'card-12';
    this.state = new InternalState(this.cardId); this.errorHandler = new InternalErrorHandler(this.cardId);
    this.apiEndpoint = '/api/modules/cards/card-12/api.php';
  ...
  async init(): Promise<void> {
    try {
      ...
      if (window.EventBus) window.EventBus.emit('card:ready', { cardId: 'card-12', name: 'Taxa de Cache', timestamp: Date.now() });
      _log('info', 'Card inicializado com sucesso');
    } catch (error) { _log('error', 'Erro na inicializacao', { error: (error as Error).message }); if (window.EventBus) window.EventBus.emit('card:error', { cardId: 'card-12', error: (error as Error).message, timestamp: Date.now() }); }
  }

  render(): void { this.container.classList.remove('is-loading'); this.container.removeAttribute('aria-busy'); this.container.innerHTML = `<div class="card-header"><svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"></rect><line x1="8" y1="21" x2="16" y2="21"></line><line x1="12" y1="17" x2="12" y2="21"></line></svg><span>Cache</span></div><div class="card-body"><div class="card-value" data-el="rate">--</div><div class="card-label">Taxa de acerto</div></div><div class="card-status" data-el="status" role="status" aria-live="polite"></div>`; this.$ = { rate: this.container.querySelector('[data-el="rate"]'), status: this.container.querySelector('[data-el="status"]') }; }
  ...
  async loadData(): Promise<void> {
    return this.state.withLock('loadData', async (): Promise<void> => {
      try {
        ...
        const hitRate: number = parseFloat(String(data.data.hit_rate ?? data.data.value ?? 0));
        if (this.$ && this.$.rate) this.$.rate.textContent = Number.isFinite(hitRate) ? `${hitRate.toFixed(1)}%` : '--';
        this.setStatus(''); this.container.classList.remove('is-loading', 'has-error');
        this.state.setState('SUCCESS', { cacheHitRate: hitRate });
        _log('info', 'Dados atualizados', { cacheHitRate: hitRate, traceId: response.meta?.traceId });
      } catch (error) {
        ...
        if (this.container.classList.contains('is-loading')) this.render();
        this.errorHandler.showCardError(this.container, 'Erro ao carregar taxa de cache');
        this.errorHandler.showToast(`Card 12: ${err.message}`, 'error', 5000);
  ...
  setupEventListeners(): void { if (window.EventBus) { window.EventBus.on('cards:refresh-all', this._onRefresh); window.EventBus.on('card-12:refresh', this._onRefresh); } document.addEventListener('visibilitychange', this._onVisibility); }
  ...
  destroy(): void { _log('info', 'Destruindo card'); if (this.intervalId) { clearInterval(this.intervalId); this.intervalId = null; } if (window.EventBus) { window.EventBus.off('cards:refresh-all', this._onRefresh); window.EventBus.off('card-12:refresh', this._onRefresh); } document.removeEventListener('visibilitychange', this._onVisibility); this.$ = null; this._mountedAt = null; _log('info', 'Card destruido'); }
  ...
  info(): CardInfoResult { return { name: MODULE_ID, version: VERSION, cardId: CARD_ID, title: 'Taxa de Cache', mounted: !!this.$, state: this.state.state, uptime: this._mountedAt ? Date.now() - this._mountedAt : 0, timestamp: Date.now() }; }
...
let _lastInstance: CardCacheHitRate | null = null;
export const Card: CardFacade = {
  mount: (container: HTMLElement): CardCacheHitRate => { _lastInstance = new CardCacheHitRate(container); return _lastInstance; },
  unmount: (instance: CardCacheHitRate | null | undefined): void => { instance?.destroy?.(); if (instance === _lastInstance) _lastInstance = null; },
  refresh: (instance: CardCacheHitRate | null | undefined): Promise<void> | undefined => instance?.loadData?.(),
  getStatus: (instance: CardCacheHitRate | null | undefined): CardStatusResult => instance?.getStatus?.() || { mounted: false },
  isReady: (instance: CardCacheHitRate | null | undefined): boolean => instance?.isReady?.() ?? false,
  info: (instance: CardCacheHitRate | null | undefined): CardInfoResult => instance?.info?.() || { name: MODULE_ID, version: VERSION, cardId: CARD_ID, mounted: false },
  healthCheck: (instance: CardCacheHitRate | null | undefined): HealthCheckResult => instance?.healthCheck?.() || { status: 'UNHEALTHY', mounted: false, version: VERSION, moduleId: MODULE_ID }
};

export const mountCard = (container: HTMLElement): CardCacheHitRate => Card.mount(container);
export const unmountCard = (instance: CardCacheHitRate | null | undefined): void => Card.unmount(instance);
export const refreshCard = (instance: CardCacheHitRate | null | undefined): Promise<void> | undefined => Card.refresh(instance);
export const getCardStatus = (instance: CardCacheHitRate | null | undefined): CardStatusResult => Card.getStatus(instance);
...
export async function init(selector: string = '#card-12-root'): Promise<CardCacheHitRate | null> { const container = document.querySelector(selector) as HTMLElementWithCardInstance | null; if (!container || container.dataset.initialized === 'true') return null; const instance: CardCacheHitRate = new CardCacheHitRate(container); container.dataset.initialized = 'true'; container.__cardInstance = instance; return instance; }
