...
- [x] C3 §73–§74 ✅ (c198294a): CANAIS DE COR POR CAMADA — paleta LOCAL injetada por peça no motor (a arte recebe outra Paleta pela MESMA injeção; nada foi redesenhado): trocar os Detalhes da roupa não vaza para aura/emblema/moldura (que usam o destaque global). `AvatarConfig.coresCamada` byte-estável; canais = usaCores do item (Cor principal/Detalhes); UI: chips §73.1 (hover=preview, PRÓPRIO, Restaurar) + 8 paletas §74 + Original. Salvar paleta própria → F4 (§136 presets).
- [ ] C3-resto → outros donos: §72 editor de roupas por PARTES exige slots+arte de vestuário (F9/P11 conteúdo; slots §122.1 já existem no vocabulário unificado só p/ 3D futuro) · §75 materiais 2D "textura e iluminação" exige filtros/arte (F5 3D tem materiais físicos reais; 2D fica p/ F9 se houver demanda).
- [x] P1' ✅ (turno 12): §56 FILTROS EM POPOVER (raridade com contadores §56.2 no contexto atual + só favoritos + ocultar bloqueados + ordenação + Limpar tudo; badge de ativos; chips removíveis §56.1) · §90 ALEATÓRIO INTELIGENTE (aleatorioInteligente no catálogo: determinístico por semente, modos completo/categoria/cores/favoritos, bloqueios §70.1 NUNCA trocam — nem preenchem slot vazio protegido; regulagens §71/§73 sobrevivem onde o item não mudou; menu no header do shell, aplica sem modal — a proteção já ocorreu no sorteio) · §65.3 COMPARAÇÃO POR TECLA (segurar V ou o botão "Original" no palco mostra o PERSISTIDO; badge de estado).
- [ ] **PENDÊNCIAS F3 (P1 de §627, não bloqueiam F4):** §67 drawer completo de detalhes (a Dica hover cobre parte) · §65.1/§65.2 comparação lado a lado/sequencial · §89 recomendações contextuais (pede telemetria de uso — melhor depois do §619 ligado). Retomar na passada de polimento (F7) ou junto do registry.
