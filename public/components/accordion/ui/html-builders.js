...
const MODULE_ID = "components.accordion.ui.html-builders";
...
let _uarpsRegion = null;
function setUarpsRegion(region) {
  _uarpsRegion = region;
}
function getUarpsRegion() {
  return _uarpsRegion;
}
function buildIcon(iconName, className) {
  ...
  if (!iconSvg || iconSvg.includes('circle cx="12" cy="12" r="10"')) {
    iconSvg = FALLBACK_ICONS[iconName] || FALLBACK_ICONS.file;
  }
...
function buildBadge(badge) {
  ...
  if (badge.type === "count" && badge.value != null) {
    ...
    badgeContent = badge.value || badge.label || "";
    ...
    badgeContent = escapeHTML(badge.label);
  }
  if (badge.pulse) badgeClass += " dsd-sidebar__badge--pulse";
  return `<span class="${badgeClass}" aria-label="${badgeContent} notifica\xE7\xF5es">${badgeContent}</span>`;
}
function buildItem(item, state, sectionId, uarpsEnabled) {
...
function buildItems(items, state, sectionId, uarpsEnabled) {
  if (!items || items.length === 0) return "";
  return items.filter((item) => isItemVisible(item)).sort((a, b) => Number(a.order ?? 100) - Number(b.order ?? 100)).map((item) => buildItem(item, state, sectionId, uarpsEnabled)).join("");
}
function buildSection(section, state, uarpsEnabled) {
...
function buildEmptyState(uarpsEnabled) {
  const regionAttr = buildRegionAttr(uarpsEnabled, _uarpsRegion);
  return `<div class="dsd-sidebar__nav-content" role="navigation" aria-label="Accordion navigation" ${regionAttr}><div class="dsd-sidebar__search-empty"><span class="dsd-sidebar__search-empty-icon">${FALLBACK_ICONS.empty}</span><span class="dsd-sidebar__search-empty-text">Nenhum item dispon\xEDvel</span></div></div>`;
}
function buildSkeleton(count, uarpsEnabled) {
  count = count || 3;
  let skeletons = "";
...
function buildErrorState(uarpsEnabled) {
  const regionAttr = buildRegionAttr(uarpsEnabled, _uarpsRegion);
  return `<div class="dsd-sidebar__nav-content dsd-sidebar--error" role="navigation" ${regionAttr}><div class="dsd-sidebar__search-empty"><span class="dsd-sidebar__search-empty-icon">${FALLBACK_ICONS.empty}</span><span class="dsd-sidebar__search-empty-text">Erro ao carregar navega\xE7\xE3o</span></div></div>`;
}
function buildHTML(structure, state, uarpsEnabled) {
  ...
  const sectionsHTML = structure.sections.filter((section) => isSectionVisible(section)).sort((a, b) => Number(a.order ?? 100) - Number(b.order ?? 100)).map((section) => buildSection(section, state, uarpsEnabled)).join("");
...
export {
  ...
  buildSection,
  buildSkeleton,
  html_builders_default as default,
  getIconResolver,
  getUarpsRegion,
  healthCheck,
  info,
