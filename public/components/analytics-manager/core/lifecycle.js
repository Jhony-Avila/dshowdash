...
const MODULE_ID = "components.analytics-manager.core.lifecycle";
const hasWindow = typeof window !== "undefined";
let initialized = false;
let initializing = false;
let flushInterval = null;
let pageViewListener = null;
const _metrics = {
  initCount: 0,
  shutdownCount: 0,
  resetCount: 0,
...
const AnalyticsLifecycle = {
  init(options = {}) {
    ...
    return new Promise((resolve) => {
      try {
        ...
        initializing = false;
        trackAnalyticsEvent("analytics:lifecycle:init:error", { error: error.message });
        resolve(false);
      }
    });
  },
  ...
  _startFlushInterval(intervalMs) {
    if (!hasWindow) return;
...
var lifecycle_default = AnalyticsLifecycle;
export {
  AnalyticsLifecycle,
  MODULE_ID,
  VERSION,
