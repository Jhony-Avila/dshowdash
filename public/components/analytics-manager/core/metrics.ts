...
// ═══════════════════════════════════════════════════════════════
// MODULE: components.analytics-manager.core.metrics
// PURPOSE: Metrics tracking and aggregation for analytics
// ───────────────────────────────────────────────────────────────
// @contract TRACK - track(name, value) sets a metric value
// @contract INCREMENT - increment(name, amount) increments a metric
// @contract DECREMENT - decrement(name, amount) decrements a metric
// @contract GET - get(name) returns metric value
// @contract GET_ALL - getAll() returns all metrics
// @contract RESET - reset(name) resets a single metric
// @contract RESET_ALL - resetAll() resets all metrics
// @contract TRACK_TIMING - trackTiming(name, durationMs) tracks timing metric
// @contract TRACK_PAGE_VIEW - trackPageView(path) tracks page view
// @contract TRACK_USER_ACTION - trackUserAction(action, label, value) tracks user action
// @contract GET_SUMMARY - getSummary() returns metrics summary
// @contract HEALTH - healthCheck() and info() for observability
// ───────────────────────────────────────────────────────────────
// IMPORTS: analyticsStore from state/store.js
// IMPORTS: trackAnalyticsEvent from telemetry/reporter.js
// PROVIDES: MetricsManager, VERSION, MODULE_ID
...
export const MODULE_ID = 'components.analytics-manager.core.metrics';

const _metrics: { trackCount: number; incrementCount: number; resetCount: number; pageViewCount: number; userActionCount: number; lastTrackAt: number | null } = {
  trackCount: 0,
  incrementCount: 0,
  resetCount: 0,
  pageViewCount: 0,
...
export const MetricsManager = {
  ...
  decrement(name: string, amount = 1) {
  ...
  trackUserAction(action: string, label: string | null = null, value: number | null = null) {
