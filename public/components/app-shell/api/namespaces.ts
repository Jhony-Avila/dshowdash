...
// ═══════════════════════════════════════════════════════════════
// MODULE: app-shell/api/namespaces
// PURPOSE: AppShell Frozen API Namespaces (44 Object.freeze blocks)
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   All UI, DevTools, Utils, Core, State, Adapter modules
//
// PROVIDES:
//   VERSION — module constant
//   buildNamespaces() — exported function (returns object with all frozen namespaces)
//   buildDebugNamespace() — exported function
//   buildDevToolsExposure() — exported function
//   setDebug() — exported function
//   getLogs() — exported function
//
// RECEIVES (via buildNamespaces): deps = { all imported modules }
// EMITS (eventos): (none)
...
import { fadeOutPreloader, setLoadingState, setErrorState, getTransitionState, prefersReducedMotion } from '../ui/transitions.js';
import { getRegion, listRegions, getAllRegions, getRegionsHealth, validateRegions, clearCache, getAccessCounts, REGION_MAP, getCacheConfig, setCacheConfig, getCacheStats, invalidateCache, setCacheTTL, getCacheTTL } from '../core/dom-regions/index.js';
...
import { collectAdapterInfos, getConnectedAdaptersList, GlobalStateAdapter, LayoutAdapter, ResponsiveAdapter } from '../adapters/adapter-manager.js';
...
export const MODULE_ID = 'app-shell/api/namespaces';
...
// ── Build All Frozen Namespaces ─────────────────────────────
export function buildNamespaces(ctx: DynObj) {
  return {
    focus: Object.freeze({ save: saveFocus, restore: restoreFocus, main: focusMain, region: focusRegion, announce }),

    theme: Object.freeze({
      getTheme: ThemeIntegration.getTheme, getResolvedTheme: ThemeIntegration.getResolvedTheme, setTheme: ThemeIntegration.setTheme, toggleTheme: ThemeIntegration.toggleTheme, useSystemTheme: ThemeIntegration.useSystemTheme,
      isDarkMode: ThemeIntegration.isDarkMode, isLightMode: ThemeIntegration.isLightMode, isSystemTheme: ThemeIntegration.isSystemTheme, getSystemPreference: ThemeIntegration.getSystemPreference,
      setRegionTheme: ThemeIntegration.setRegionTheme, clearRegionTheme: ThemeIntegration.clearRegionTheme, getRegionTheme: ThemeIntegration.getRegionTheme, getRegionThemes: ThemeIntegration.getRegionThemes,
      getThemeVariable: ThemeIntegration.getThemeVariable, setThemeVariable: ThemeIntegration.setThemeVariable, getThemeVariables: ThemeIntegration.getThemeVariables,
      THEMES, subscribe: ThemeIntegration.subscribe, healthCheck: ThemeIntegration.healthCheck, info: ThemeIntegration.info
    }),

    events: Object.freeze({
      emit: RegionEvents.emit, on: RegionEvents.on, off: RegionEvents.off, onAny: RegionEvents.onAny, onGlobal: RegionEvents.onGlobal, once: RegionEvents.once, waitFor: RegionEvents.waitFor,
      getHistory: RegionEvents.getHistory, clearHistory: RegionEvents.clearHistory, setHistoryLimit: RegionEvents.setHistoryLimit, getListenerCounts: RegionEvents.getListenerCounts,
      EVENTS: REGION_EVENTS, healthCheck: RegionEvents.healthCheck, info: RegionEvents.info
    }),

    slots: Object.freeze({
      registerSlot: RegionSlots.registerSlot, unregisterSlot: RegionSlots.unregisterSlot, injectContent: RegionSlots.injectContent, clearSlot: RegionSlots.clearSlot, clearRegionSlots: RegionSlots.clearRegionSlots, updateSlot: RegionSlots.updateSlot,
      getSlot: RegionSlots.getSlot, getRegionSlots: RegionSlots.getRegionSlots, getSlotContent: RegionSlots.getSlotContent, hasContent: RegionSlots.hasContent, findSlotsByName: RegionSlots.findSlotsByName,
      inject: RegionSlots.inject, createPersistentSlot: RegionSlots.createPersistentSlot, POSITIONS: SLOT_POSITIONS, subscribe: RegionSlots.subscribe, healthCheck: RegionSlots.healthCheck, info: RegionSlots.info
    }),

    perf: Object.freeze({
      mark: PerformanceMetrics.mark, measure: PerformanceMetrics.measure, clearMarks: PerformanceMetrics.clearMarks, clearMeasures: PerformanceMetrics.clearMeasures,
      getMetrics: PerformanceMetrics.getMetrics, getMarks: PerformanceMetrics.getMarks, getMeasures: PerformanceMetrics.getMeasures, getSampleHistory: PerformanceMetrics.getSampleHistory,
      getWebVitals: PerformanceMetrics.getWebVitals, getResourceBreakdown: PerformanceMetrics.getResourceBreakdown,
      subscribe: PerformanceMetrics.subscribe, healthCheck: PerformanceMetrics.healthCheck, info: PerformanceMetrics.info
    }),
    ...
    notify: Object.freeze({
      show: NotificationCenter.show, dismiss: NotificationCenter.dismiss, dismissAll: NotificationCenter.dismissAll, update: NotificationCenter.update,
      info: (msg: string, opts: DynObj) => NotificationCenter.show(Object.assign({ type: 'info', message: msg }, opts || {})),
      success: (msg: string, opts: DynObj) => NotificationCenter.show(Object.assign({ type: 'success', message: msg }, opts || {})),
      warning: (msg: string, opts: DynObj) => NotificationCenter.show(Object.assign({ type: 'warning', message: msg }, opts || {})),
      error: (msg: string, opts: DynObj) => NotificationCenter.show(Object.assign({ type: 'error', message: msg, duration: 0 }, opts || {})),
      loading: (msg: string, opts: DynObj) => NotificationCenter.show(Object.assign({ type: 'loading', message: msg, duration: 0, dismissible: false }, opts || {})),
      promise: NotificationCenter.promise,
      get: NotificationCenter.get, getAll: NotificationCenter.getAll, getByType: NotificationCenter.getByType, getQueueSize: NotificationCenter.getQueueSize,
      configure: NotificationCenter.configure, getConfig: NotificationCenter.getConfig, setPosition: NotificationCenter.setPosition, subscribe: NotificationCenter.subscribe,
      TYPES: NOTIFICATION_TYPES, POSITIONS: NOTIFICATION_POSITIONS, PRIORITIES: NOTIFICATION_PRIORITIES, getMetrics: NotificationCenter.getMetrics, healthCheck: NotificationCenter.healthCheck
    }),

    shortcuts: Object.freeze({
      register: KeyboardShortcuts.register, unregister: KeyboardShortcuts.unregister, unregisterGroup: KeyboardShortcuts.unregisterGroup, registerMany: KeyboardShortcuts.registerMany,
      setScope: KeyboardShortcuts.setScope, restoreScope: KeyboardShortcuts.restoreScope, getScope: KeyboardShortcuts.getScope,
      enable: KeyboardShortcuts.enable, disable: KeyboardShortcuts.disable, isEnabled: KeyboardShortcuts.isEnabled, setEnabled: KeyboardShortcuts.setEnabled,
      get: KeyboardShortcuts.get, getAll: KeyboardShortcuts.getAll, getByGroup: KeyboardShortcuts.getByGroup, getGroups: KeyboardShortcuts.getGroups, isRegistered: KeyboardShortcuts.isRegistered,
      trigger: KeyboardShortcuts.trigger, showHelp: KeyboardShortcuts.showHelp, hideHelp: KeyboardShortcuts.hideHelp,
      configure: KeyboardShortcuts.configure, getConfig: KeyboardShortcuts.getConfig, subscribe: KeyboardShortcuts.subscribe, destroy: KeyboardShortcuts.destroy,
      MODIFIER_KEYS, SCOPES: SHORTCUT_SCOPES, getMetrics: KeyboardShortcuts.getMetrics, healthCheck: KeyboardShortcuts.healthCheck, info: KeyboardShortcuts.info
    }),

    gestures: Object.freeze({
      on: GestureHandler.on, off: GestureHandler.off, once: GestureHandler.once, offAll: GestureHandler.offAll,
      addToElement: GestureHandler.addToElement, removeFromElement: GestureHandler.removeFromElement,
      onSwipe: GestureHandler.onSwipe, onSwipeLeft: GestureHandler.onSwipeLeft, onSwipeRight: GestureHandler.onSwipeRight, onSwipeUp: GestureHandler.onSwipeUp, onSwipeDown: GestureHandler.onSwipeDown,
      onTap: GestureHandler.onTap, onDoubleTap: GestureHandler.onDoubleTap, onLongPress: GestureHandler.onLongPress, onPinch: GestureHandler.onPinch, onPan: GestureHandler.onPan,
      enable: GestureHandler.enable, disable: GestureHandler.disable, isEnabled: GestureHandler.isEnabled,
      configure: GestureHandler.configure, getConfig: GestureHandler.getConfig, subscribe: GestureHandler.subscribe, destroy: GestureHandler.destroy,
      GESTURES, DIRECTIONS: GESTURE_DIRECTIONS, getMetrics: GestureHandler.getMetrics, healthCheck: GestureHandler.healthCheck, info: GestureHandler.info
    }),

    debugPresets: Object.freeze({
      apply: DebugPresets.apply, applyCustom: DebugPresets.applyCustom, disable: DebugPresets.disable, revert: DebugPresets.revert,
      getCurrent: DebugPresets.getCurrent, getCurrentConfig: DebugPresets.getCurrentConfig, getPrevious: DebugPresets.getPrevious, isEnabled: DebugPresets.isEnabled,
      getPresetConfig: DebugPresets.getPresetConfig, listPresets: DebugPresets.listPresets, getHistory: DebugPresets.getHistory,
      configure: DebugPresets.configure, getConfig: DebugPresets.getConfig, subscribe: DebugPresets.subscribe,
      minimal: DebugPresets.minimal, standard: DebugPresets.standard, verbose: DebugPresets.verbose, performance: DebugPresets.performance,
      network: DebugPresets.network, memory: DebugPresets.memory, events: DebugPresets.events, regions: DebugPresets.regions,
      PRESETS: DEBUG_PRESETS, getMetrics: DebugPresets.getMetrics, healthCheck: DebugPresets.healthCheck, info: DebugPresets.info
    }),

    configExporter: Object.freeze({
      exportConfig: ConfigExporter.exportConfig, exportToFile: ConfigExporter.exportToFile, exportToClipboard: ConfigExporter.exportToClipboard,
      importConfig: ConfigExporter.importConfig, importFromFile: ConfigExporter.importFromFile, importFromClipboard: ConfigExporter.importFromClipboard,
      getLastExport: ConfigExporter.getLastExport, getLastImport: ConfigExporter.getLastImport,
      configure: ConfigExporter.configure, getConfig: ConfigExporter.getConfig, subscribe: ConfigExporter.subscribe,
      FORMATS: EXPORT_FORMATS, SCOPES: EXPORT_SCOPES, getMetrics: ConfigExporter.getMetrics, healthCheck: ConfigExporter.healthCheck, info: ConfigExporter.info
    }),

    regionMetrics: Object.freeze({
      trackRender: RegionMetrics.trackRender, trackUpdate: RegionMetrics.trackUpdate, trackVisibility: RegionMetrics.trackVisibility,
      trackInteraction: RegionMetrics.trackInteraction, trackError: RegionMetrics.trackError, trackLoad: RegionMetrics.trackLoad,
      startTimer: RegionMetrics.startTimer, endTimer: RegionMetrics.endTimer,
      getRegionMetrics: RegionMetrics.getRegionMetrics, getAllMetrics: RegionMetrics.getAllMetrics, getPerformanceSummary: RegionMetrics.getPerformanceSummary, getProblematicRegions: RegionMetrics.getProblematicRegions,
      enable: RegionMetrics.enable, disable: RegionMetrics.disable, isEnabled: RegionMetrics.isEnabled, reset: RegionMetrics.reset, resetRegion: RegionMetrics.resetRegion,
      configure: RegionMetrics.configure, getConfig: RegionMetrics.getConfig, subscribe: RegionMetrics.subscribe,
      METRIC_TYPES, getMetrics: RegionMetrics.getMetrics, healthCheck: RegionMetrics.healthCheck, info: RegionMetrics.info
    }),

    commandPalette: Object.freeze({
      show: CommandPalette.show, hide: CommandPalette.hide, toggle: CommandPalette.toggle, isVisible: CommandPalette.isVisible,
      register: CommandPalette.register, registerMany: CommandPalette.registerMany, unregister: CommandPalette.unregister, execute: CommandPalette.execute,
      getCommand: CommandPalette.getCommand, getAllCommands: CommandPalette.getAllCommands, getCommandsByType: CommandPalette.getCommandsByType,
      getRecentCommands: CommandPalette.getRecentCommands, clearRecent: CommandPalette.clearRecent,
      configure: CommandPalette.configure, getConfig: CommandPalette.getConfig, subscribe: CommandPalette.subscribe,
      TYPES: COMMAND_TYPES, getMetrics: CommandPalette.getMetrics, healthCheck: CommandPalette.healthCheck, info: CommandPalette.info
    }),

    stateSnapshots: Object.freeze({
      capture: StateSnapshots.capture, restore: StateSnapshots.restore, compare: StateSnapshots.compare,
      get: StateSnapshots.get, getAll: StateSnapshots.getAll, getLatest: StateSnapshots.getLatest, getByLabel: StateSnapshots.getByLabel, count: StateSnapshots.count,
      remove: StateSnapshots.remove, clear: StateSnapshots.clear, exportSnapshot: StateSnapshots.exportSnapshot, exportAll: StateSnapshots.exportAll, importSnapshot: StateSnapshots.importSnapshot,
      startAutoSnapshot: StateSnapshots.startAutoSnapshot, stopAutoSnapshot: StateSnapshots.stopAutoSnapshot, isAutoSnapshotRunning: StateSnapshots.isAutoSnapshotRunning,
      configure: StateSnapshots.configure, getConfig: StateSnapshots.getConfig, subscribe: StateSnapshots.subscribe,
      getMetrics: StateSnapshots.getMetrics, healthCheck: StateSnapshots.healthCheck, info: StateSnapshots.info
    }),

    docs: Object.freeze({
      show: () => { DebugPanel.open(); DebugPanel.setActiveTab('docs'); return true; },
      hide: () => { DebugPanel.close(); return true; },
      toggle: () => { if (DebugPanel.isOpen()) { DebugPanel.close(); } else { DebugPanel.open(); DebugPanel.setActiveTab('docs'); } return true; },
      isVisible: () => DebugPanel.isOpen(),
      goToSection: (section: DynObj) => { DebugPanel.open(); DebugPanel.setActiveTab('docs'); return true; },
...
export function buildDebugNamespace(ctx: DynObj) {
  return Object.freeze({
    store: getState, lifecycle: getLifecycleInfo, readiness: getReadinessInfo, regions: getRegionsHealth, transitions: getTransitionState, systemPages: getSystemPagesInfo, adapters: collectAdapterInfos, metrics: () => ({ ...ctx.metrics, connectedAdapters: getConnectedAdaptersList() }), validateRegions, transitionHistory: getTransitionHistory, phaseTimings: getPhaseTimings, accessCounts: getAccessCounts, layoutPrefs: LayoutPersistence.info, visibility: RegionVisibility.info, responsive: ResponsiveAdapter.info, keyboard: KeyboardNavigation.info, resize: RegionResize.info, loading: RegionLoading.info, theme: ThemeIntegration.info, events: RegionEvents.info, slots: RegionSlots.info, performance: PerformanceMetrics.info, debugPanel: DebugPanel.info, autoHealthCheck: AutoHealthCheck.info, logger: Logger.getInfo, rateLimiter: RateLimiter.info, configValidator: ConfigValidator.info, cacheStats: getCacheStats, apiMetrics: APIUsageMetrics.info, memoryLeaks: MemoryLeakDetector.info, analytics: AnalyticsExporter.info, circuitBreaker: CircuitBreaker.info, lazyLoader: LazyLoader.info, bundleAnalyzer: BundleAnalyzer.info, layoutPresets: LayoutPresets.info, slotPersistence: SlotPersistence.info, skeleton: SkeletonLoader.info, maintenance: MaintenanceMode.info, focusManager: FocusManager.info, a11y: AccessibilityPresets.info, animations: AnimationAPI.info, serviceWorker: ServiceWorkerManager.info, virtualizer: ContentVirtualizer.info, offline: OfflineManager.info, notifications: NotificationCenter.info, shortcuts: KeyboardShortcuts.info, gestures: GestureHandler.info, debugPresets: DebugPresets.info, configExporter: ConfigExporter.info, regionMetrics: RegionMetrics.info, commandPalette: CommandPalette.info, stateSnapshots: StateSnapshots.info, icons: Icons.info
  });
}
