...
// ═══════════════════════════════════════════════════════════════
// MODULE: maintenance-mode/banner
// PURPOSE: Criação e gerenciamento do banner de manutenção
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   MAINTENANCE_TYPES, SEVERITY from ./constants.js
//   state, config, bannerElement from ./state.js
// EXPORTS:
//   getDefaultMessage — Retorna mensagem padrão por tipo
//   updateTimeRemaining — Atualiza tempo restante
//   createBanner — Cria banner DOM
//   removeBanner — Remove banner DOM
// BROWSER APIs: document.createElement, document.body
// ═══════════════════════════════════════════════════════════════
/**
 * @module MaintenanceModeBanner
 * @description Banner visual para modo de manutenção
 * @version 1.0.0-AAA
 * @since 2025-02-02
 */
'use strict';
...
export const MODULE_ID = 'app-shell.core.maintenance-mode.banner';

function getSeverityColor() {
    switch (state.severity) {
        case SEVERITY.CRITICAL: return '#dc3545';
        case SEVERITY.WARNING: return '#fd7e14';
...
function getSeverityIcon() {
    switch (state.severity) {
        case SEVERITY.CRITICAL: return '⚠️';
...
/**
 * Retorna mensagem padrão baseada no tipo de manutenção
 * @returns {string} Mensagem
 */
export function getDefaultMessage() {
    switch (state.type) {
        case MAINTENANCE_TYPES.FULL: return 'Sistema em manutencao. Voltaremos em breve.';
        case MAINTENANCE_TYPES.PARTIAL: return 'Algumas funcionalidades estao temporariamente indisponiveis.';
        case MAINTENANCE_TYPES.SCHEDULED: return 'Manutencao programada em andamento.';
        case MAINTENANCE_TYPES.EMERGENCY: return 'Manutencao de emergencia. Desculpe o transtorno.';
        case MAINTENANCE_TYPES.FEATURE: return 'Recurso temporariamente indisponivel.';
        default: return 'Sistema em manutencao.';
    }
}

/**
 * Atualiza display de tempo restante
 */
...
/**
 * Cria banner de manutenção no DOM
 */
