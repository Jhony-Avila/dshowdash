...
// ═══════════════════════════════════════════════════════════════
// MODULE: devtools/panel/tabs/diff-viewer
// PURPOSE: Snapshot diff comparison
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   icon, sanitizeAttr, formatDate, getAppShell, makeSectionHtml from ../helpers.js
// EXPORTS:
//   setDiffLeft, setDiffRight — Define snapshots para comparação
//   getDiffLeft, getDiffRight — Retorna snapshots selecionados
//   clearDiff — Limpa seleção
//   renderDiffViewer — Renderiza UI de comparação
// ═══════════════════════════════════════════════════════════════
/**
 * @module DevToolsPanelTabsDiffViewer
 * @description Comparação de snapshots
 * @version 1.0.0-AAA
 * @since 2025-02-02
 */
'use strict';
...
export const MODULE_ID = 'app-shell.devtools.panel.tabs.diff-viewer';

let _diffLeftId: DynObj = null;
let _diffRightId: DynObj = null;

export function setDiffLeft(id: DynObj) { _diffLeftId = id; }
export function setDiffRight(id: DynObj) { _diffRightId = id; }
export function getDiffLeft() { return _diffLeftId; }
export function getDiffRight() { return _diffRightId; }
export function clearDiff() { _diffLeftId = null; _diffRightId = null; }

function _deepDiff(left: DynObj, right: DynObj, path: DynObj) {
    let results: DynObj[] = [];
    if (!path) path = '';
    if (left === right) return results;
    if (left === null || left === undefined || right === null || right === undefined || typeof left !== typeof right) {
    ...
    const allKeys = {};
    Object.keys(left).forEach(k => { (allKeys as DynObj)[k] = true; });
    Object.keys(right).forEach(k => { (allKeys as DynObj)[k] = true; });
    Object.keys(allKeys).forEach(key => {
        const childPath = path ? `${path}.${key}` : key;
        if (!(key in left)) {
            results.push({ path: childPath, type: 'added', left: undefined, right: right[key] });
        } else if (!(key in right)) {
            results.push({ path: childPath, type: 'removed', left: left[key], right: undefined });
        } else {
...
function _formatValue(val: DynObj) {
...
export function renderDiffViewer() {
    ...
    let snapshots = [];
    try { snapshots = shell.stateSnapshots.getAll(); } catch (e) { return '<div class="dsd-ui-empty">Error loading snapshots</div>'; }
    ...
    const selectHtml = snapshots.slice(-10).reverse().map((s: DynObj) => `<option value="${sanitizeAttr(s.id)}">${sanitizeAttr(s.label)} (${formatDate(s.timestamp)})</option>`).join('');
    ...
    if (_diffLeftId && _diffRightId) {
        const left = snapshots.find((s: DynObj) => s.id === _diffLeftId);
        const right = snapshots.find((s: DynObj) => s.id === _diffRightId);
        if (left && right) {
            ...
            if (diffs.length === 0) {
                ...
                diffResultHtml = makeSectionHtml('diff-summary', 'checkCircle', `Summary: ${diffs.length} differences`,
                ...
                makeSectionHtml('diff-details', 'fileText', 'Details',
...
export default { renderDiffViewer, setDiffLeft, setDiffRight, getDiffLeft, getDiffRight, clearDiff };
