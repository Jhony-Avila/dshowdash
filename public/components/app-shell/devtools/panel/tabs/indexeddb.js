...
const MODULE_ID = "app-shell.devtools.panel.tabs.indexeddb";
let _databases = [];
let _lastScan = null;
...
function _processDatabase(name, version) {
  return new Promise((resolve) => {
    try {
...
function _countRecords(db, storeName) {
  return new Promise((resolve) => {
    try {
