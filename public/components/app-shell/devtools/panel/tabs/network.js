...
const MODULE_ID = "app-shell.devtools.panel.tabs.network";
let _requests = [];
...
function interceptNetwork() {
  ...
  if (window.fetch && window.fetch.__dsdIntercepted) return;
  _intercepted = true;
  const safeFetch = _origFetch;
  window.fetch = function(input, init) {
    const url = typeof input === "string" ? input : input && input.url ? input.url : String(input);
    const method = init && init.method ? init.method.toUpperCase() : "GET";
    const entry = { url, method, status: 0, duration: 0, size: 0, type: "fetch", timestamp: Date.now(), error: null };
    const t0 = typeof performance !== "undefined" ? performance.now() : Date.now();
    try {
      return safeFetch.apply(this, arguments).then((response) => {
  ...
  window.fetch.__dsdIntercepted = true;
  ...
  XMLHttpRequest.prototype.send = function() {
    ...
    const entry = { url: meta.url || "", method: meta.method || "GET", status: 0, duration: 0, size: 0, type: "xhr", timestamp: Date.now(), error: null };
    const t0 = typeof performance !== "undefined" ? performance.now() : Date.now();
...
function renderNetworkTab() {
  ...
  const listHtml = reqs.length === 0 ? '<div class="dsd-ui-empty">No requests captured yet. Navigate the app to see requests.</div>' : reqs.map((r) => {
...
var network_default = { renderNetworkTab, interceptNetwork, restoreNetwork, getNetworkRequests, clearNetworkRequests, getNetworkStats };
export {
  MODULE_ID,
  VERSION,
