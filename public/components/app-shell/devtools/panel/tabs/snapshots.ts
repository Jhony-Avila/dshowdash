...
// ═══════════════════════════════════════════════════════════════
// MODULE: devtools/panel/tabs/snapshots
// PURPOSE: Renderização da tab Snapshots no Debug Panel
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   icon, sanitizeAttr, formatDate, getAppShell from ../helpers.js
// EXPORTS:
//   renderSnapshotsTab — Retorna HTML da tab de snapshots
// ═══════════════════════════════════════════════════════════════
/**
 * @module DevtoolsPanelTabSnapshots
 * @description Tab de State Snapshots no Debug Panel
 * @version 1.0.0-AAA
 * @since 2025-02-02
 */
'use strict';
...
export const MODULE_ID = 'app-shell.devtools.panel.tabs.snapshots';

/**
 * Renderiza a tab de Snapshots
 * @returns {string} HTML da tab
 */
export function renderSnapshotsTab() {
    ...
    try {
        ...
        const snapshotsList = snapshots.length === 0 
            ? '<div class="dsd-ui-empty">No snapshots captured yet</div>'
            : snapshots.slice(-10).reverse().map((s: DynObj) => `<div class="dsd-ui-snapshot-item" data-id="${sanitizeAttr(s.id)}"><div class="dsd-ui-snapshot-item__info"><div class="dsd-ui-snapshot-item__label">${sanitizeAttr(s.label)}</div><div class="dsd-ui-snapshot-item__time">${formatDate(s.timestamp)}</div></div><div class="dsd-ui-snapshot-item__actions"><button class="dsd-ui-btn" data-action="restore" data-id="${sanitizeAttr(s.id)}">${icon('undo', 14)}</button><button class="dsd-ui-btn" data-action="delete" data-id="${sanitizeAttr(s.id)}">${icon('trash', 14)}</button></div></div>`).join('');
