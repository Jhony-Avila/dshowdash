...
// ═══════════════════════════════════════════════════════════════
// MODULE: app-shell-command-palette
// PURPOSE: Command Palette - Unified UI
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   Icons from ./icons.js
//
...
//   registerMany() — exported function
...
//   getCommandsByType() — exported function
//   getRecentCommands() — exported function
//   clearRecent() — exported function
...
export const VERSION = '2.0.0-P2-ENTERPRISE';
export const MODULE_ID = 'app-shell-command-palette';

export const COMMAND_TYPES = { ACTION: 'action', NAVIGATION: 'navigation', SEARCH: 'search', SETTING: 'setting', DEBUG: 'debug' };
...
let _recentCommands: DynObj[] = [];
let _isVisible = false;
let _selectedIndex = 0;
let _searchQuery = '';
let _filteredCommands: DynObj[] = [];
let _panelElement: DynObj = null;
let _overlayElement: DynObj = null;
...
function _icon(name: string, size: DynObj) { return Icons.icon(name, size || 16); }

function _getCommandIcon(cmd: DynObj) {
  const iconMap = {
    'toggle-theme': 'sun', 'toggle-sidebar': 'sidebar', 'open-debug-panel': 'tool',
    'health-check': 'activity', 'export-config': 'download', 'show-info': 'info',
    'clear-cache': 'trash', 'fullscreen': 'maximize'
  };
  return (iconMap as DynObj)[cmd.id] || 'terminal';
}

function _loadRecent() {
  if (!_config.persistRecent) return;
  try { const saved = localStorage.getItem(STORAGE_KEY); if (saved) _recentCommands = JSON.parse(saved).slice(0, _config.maxRecent); } catch (e: any) { }
}

function _saveRecent() {
  if (!_config.persistRecent) return;
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify(_recentCommands.slice(0, _config.maxRecent))); } catch (e: any) { }
}

function _addToRecent(commandId: string) {
...
function _filterCommands(query: string) {
...
function _executeSelected() {
  ...
  try {
    ...
    hide();
  } catch (e: any) { _metrics.errors++; console.debug('%c[ERROR]%c [CommandPalette] Execute error:', 'color:#ef4444;font-weight:bold', 'color:inherit', e.message || e); }
}

function _handleKeydown(e: KeyboardEvent) {
  if (!_isVisible) return;
  switch (e.key) {
    case 'ArrowDown': e.preventDefault(); _selectedIndex = Math.min(_selectedIndex + 1, _filteredCommands.length - 1); _render(); break;
    case 'ArrowUp': e.preventDefault(); _selectedIndex = Math.max(_selectedIndex - 1, 0); _render(); break;
    case 'Enter': e.preventDefault(); _executeSelected(); break;
...
function _handleGlobalKeydown(e: KeyboardEvent) {
  ...
  if (isCtrlK) { e.preventDefault(); toggle(); }
}
...
function _render() {
  ...
  const listHtml = _filteredCommands.length === 0 
    ? '<div class="dsd-ui-empty">Nenhum comando encontrado</div>'
    : _filteredCommands.map((cmd, i) => {
  ...
  if (input) {
    input.focus();
    input.oninput = (e: DynObj) => { _filterCommands(e.target.value); _render(); };
  }
...
function _registerDefaultCommands() {
  const defaults = [
    { id: 'toggle-theme', title: 'Alternar tema claro/escuro', type: COMMAND_TYPES.SETTING, shortcut: 'Alt+T', handler() { const s = (window as any).AppShell; if (s && s.theme) s.theme.toggleTheme(); }, priority: 100 },
    { id: 'toggle-sidebar', title: 'Alternar sidebar', type: COMMAND_TYPES.ACTION, handler() { const s = (window as any).AppShell; if (s && s.layoutPrefs) s.layoutPrefs.toggleSidebar(); }, priority: 90 },
    { id: 'open-debug-panel', title: 'Abrir Debug Panel', type: COMMAND_TYPES.DEBUG, shortcut: 'Alt+F12', handler() { const s = (window as any).AppShell; if (s && s.debugPanel) s.debugPanel.open(); }, priority: 85 },
    { id: 'health-check', title: 'Executar Health Check', type: COMMAND_TYPES.DEBUG, handler() { const s = (window as any).AppShell; if (s) { const r = s.healthCheck(); console.log('Health:', r); } }, priority: 80 },
    { id: 'export-config', title: 'Exportar configuração', type: COMMAND_TYPES.ACTION, handler() { const s = (window as any).AppShell; if (s && s.configExporter) s.configExporter.exportToFile(); }, priority: 70 },
    { id: 'show-info', title: 'Mostrar informações do sistema', type: COMMAND_TYPES.DEBUG, handler() { const s = (window as any).AppShell; if (s) console.log('Info:', s.info()); }, priority: 65 },
    { id: 'clear-cache', title: 'Limpar cache de regiões', type: COMMAND_TYPES.ACTION, handler() { const s = (window as any).AppShell; if (s && s.cache) s.cache.clear(); }, priority: 60 },
    { id: 'fullscreen', title: 'Entrar em tela cheia', type: COMMAND_TYPES.ACTION, handler() { if (document.documentElement.requestFullscreen) document.documentElement.requestFullscreen(); }, priority: 50 }
  ];
  defaults.forEach(cmd => { if (!_commands.has(cmd.id)) _commands.set(cmd.id, cmd); });
}
...
export function register(command: DynObj) {
  if (!command || !command.id || !command.title) return false;
  _commands.set(command.id, command);
  return true;
}

export function registerMany(commands: DynObj) { return commands.map(register).filter(Boolean).length; }

export function unregister(commandId: string) { return _commands.delete(commandId); }

export function execute(commandId: string) {
  const cmd = _commands.get(commandId);
  if (!cmd || typeof cmd.handler !== 'function') return false;
  try { cmd.handler(); _addToRecent(commandId); _metrics.executions++; _notify('execute', { command: commandId }); return true; }
  catch (e: any) { _metrics.errors++; return false; }
}
...
export function toggle() { return _isVisible ? hide() : show(); }
export function isVisible() { return _isVisible; }
export function getCommand(id: DynObj) { return _commands.get(id); }
export function getAllCommands() { return Array.from(_commands.values()); }
export function getCommandsByType(type: DynObj) { return getAllCommands().filter(c => c.type === type); }
export function getRecentCommands() { return _recentCommands.map(id => _commands.get(id)).filter(Boolean); }
export function clearRecent() { _recentCommands = []; _saveRecent(); }
export function configure(options: DynObj) { Object.assign(_config, options); }
export function getConfig() { return { ..._config }; }
export function subscribe(callback: DynObj) { if (typeof callback === 'function') _subscribers.push(callback); return () => { _subscribers = _subscribers.filter(cb => cb !== callback); }; }
...
export default { VERSION, MODULE_ID, COMMAND_TYPES, init, destroy, register, registerMany, unregister, execute, show, hide, toggle, isVisible, getCommand, getAllCommands, getCommandsByType, getRecentCommands, clearRecent, configure, getConfig, subscribe, getMetrics, healthCheck, info };
