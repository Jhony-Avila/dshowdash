...
const MODULE_ID = "app-shell.ui.content-virtualizer.virtualizer";
import { SCROLL_DIRECTION } from "./index.js";
function Virtualizer(container, options, metrics) {
...
Virtualizer.prototype._render = function() {
  if (!this.renderItem) return;
  const isVertical = this.direction === SCROLL_DIRECTION.VERTICAL;
  const newRendered = /* @__PURE__ */ new Map();
  ...
  for (let i = this.visibleStart; i < this.visibleEnd; i++) {
    ...
    if (this.renderedItems.has(key)) {
      ...
      element.style.cssText = isVertical ? `position: absolute; left: 0; right: 0; height: ${this.itemHeight}px;` : `position: absolute; top: 0; bottom: 0; width: ${this.itemWidth}px; display: inline-block;`;
    }
  ...
  this.renderedItems.forEach(function(el) {
...
Virtualizer.prototype.setItems = function(items) {
  this.items = items || [];
  const isVertical = this.direction === SCROLL_DIRECTION.VERTICAL;
  const itemSize = isVertical ? this.itemHeight : this.itemWidth;
...
Virtualizer.prototype.scrollToIndex = function(index, align) {
  align = align || "start";
  const isVertical = this.direction === SCROLL_DIRECTION.VERTICAL;
  const itemSize = isVertical ? this.itemHeight : this.itemWidth;
  const containerSize = isVertical ? this.container.clientHeight : this.container.clientWidth;
...
Virtualizer.prototype.scrollToItem = function(predicate, align) {
