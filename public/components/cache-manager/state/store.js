...
const MODULE_ID = "components.cache-manager.state.store";
...
const DEFAULT_TTL = 3e5;
const createInitialData = () => ({ entries: /* @__PURE__ */ new Map(), stats: { hits: 0, misses: 0, sets: 0, deletes: 0, evictions: 0, cleanedExpired: 0 }, maxSize: DEFAULT_MAX_SIZE, defaultTTL: DEFAULT_TTL });
let cacheData = createInitialData();
const storeListeners = /* @__PURE__ */ new Set();
let destroyed = false;
let _metrics = { notifyCount: 0, notifyErrors: 0, lastNotifyAt: null };
const cacheStore = {
  setDebug(debug) {
    _debug = debug;
  },
  ...
  get(key) {
    if (key === "stats") return { ...cacheData.stats };
  ...
  set(key, value) {
    ...
    else cacheData[key] = value;
    this._notify("set", key, value, null);
    return true;
  },
  getEntry(key) {
    return cacheData.entries.get(key) || null;
  },
  ...
  deleteEntry(key) {
    const existed = cacheData.entries.delete(key);
    if (existed) cacheData.stats.deletes++;
    this._notify("entry-deleted", key, null, null);
    return existed;
  },
  hasEntry(key) {
    return cacheData.entries.has(key);
  },
  getSize() {
    return cacheData.entries.size;
  },
  clear() {
    const previousSize = cacheData.entries.size;
    cacheData.entries.clear();
    this._notify("cleared", null, { previousSize }, null);
    return true;
  },
  getAllKeys() {
    return Array.from(cacheData.entries.keys());
  },
  getStats() {
  ...
  incrementHits() {
    ...
    this._notify("cache-hit", null, null, null);
  },
  incrementMisses() {
    cacheData.stats.misses++;
    this._notify("cache-miss", null, null, null);
  },
  incrementEvictions() {
    cacheData.stats.evictions++;
  },
  incrementCleanedExpired() {
    cacheData.stats.cleanedExpired++;
  },
  addCleanedExpired(count) {
    cacheData.stats.cleanedExpired += count;
  },
  getListenerCount() {
  ...
  _notify(action, key, newValue, oldValue) {
    _metrics.notifyCount++;
    _metrics.lastNotifyAt = Date.now();
    storeListeners.forEach((l) => {
      try {
        l({ action, key, newValue, oldValue });
      } catch (e) {
        _metrics.notifyErrors++;
  ...
  info() {
    const portsSnapshot = Ports.snapshot();
    return { version: VERSION, moduleId: MODULE_ID, debug: _debug, destroyed, size: cacheData.entries.size, maxSize: cacheData.maxSize, defaultTTL: cacheData.defaultTTL, listenerCount: storeListeners.size, metrics: { ..._metrics }, healthCheck: this.healthCheck(), portsInitialized: portsSnapshot._initialized, strictMode: isStrict(), timestamp: Date.now() };
  },
  reset() {
    cacheData = createInitialData();
    destroyed = false;
    _metrics = { notifyCount: 0, notifyErrors: 0, lastNotifyAt: null };
    this._notify("reset", null, null, null);
  ...
  toJSON() {
    const entries = {};
    cacheData.entries.forEach((v, k) => {
      entries[k] = { createdAt: v.createdAt, expiresAt: v.expiresAt, accessCount: v.accessCount, lastAccess: v.lastAccess, hasValue: v.value !== null && v.value !== void 0, isExpired: Date.now() > v.expiresAt };
    });
