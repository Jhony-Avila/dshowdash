...
const MODULE_ID = "components.cards.card-01";
...
class CardSuccessRate {
  ...
  loadCSS() {
    ...
    return new Promise((resolve, reject) => {
      ...
      link.id = linkId;
      link.rel = "stylesheet";
      link.href = "/components/cards/card-01/styles.css";
      link.onload = () => resolve();
  ...
  render() {
    ...
    if (this.$?.sparkline && typeof window !== "undefined" && window.Sparkline) {
      this.sparkline = new window.Sparkline(this.$.sparkline, { width: 120, height: 30, color: "#22c55e", lineWidth: 2, showDots: true, dotRadius: 2, animate: true, gradient: true });
    }
  }
  setStatus(msg) {
  ...
  loadData() {
    return this.state.withLock("loadData", () => {
      ...
      return API.get(this.apiEndpoint, { timeout: CONFIG.API_TIMEOUT, retries: CONFIG.API_RETRIES, headers: { "Accept": "application/json" } }).then((response) => {
        ...
        if (this.$) {
          if (this.$.pct) this.$.pct.textContent = formatPercent(parsed.successRate);
          if (this.$.ok) this.$.ok.textContent = formatNumber(parsed.successCount);
          if (this.$.err) this.$.err.textContent = formatNumber(parsed.errorCount);
        }
        if (data.history && this.sparkline) this.sparkline.setData(parseSparklineData(data.history));
        else this.sparkline?.setData?.([]);
        this.setStatus("");
  ...
  handleVisibility() {
    if (document.hidden) this.state.setState(STATES.PAUSED);
...
var card_01_default = CardSuccessRate;
export {
  CARD_ID,
  CARD_NAME,
  CardSuccessRate,
