...
const MODULE_ID = "components.cards.card-02";
...
class CardGlobalSuccessRate {
  ...
  loadCSS() {
    const linkId = "card-02-styles";
    if (document.getElementById(linkId)) return Promise.resolve();
    return new Promise((resolve, reject) => {
      const link = document.createElement("link");
      link.id = linkId;
      link.rel = "stylesheet";
      link.href = "/components/cards/card-02/styles.css";
      link.onload = () => resolve();
  ...
  render() {
    ...
    this.$ = { pct: this.container.querySelector('[data-el="percent"]'), status: this.container.querySelector('[data-el="status"]') };
  ...
  loadData() {
    return this.state.withLock("loadData", () => {
      ...
      return API.get(this.apiEndpoint, { timeout: CONFIG.API_TIMEOUT, retries: CONFIG.API_RETRIES, headers: { "Accept": "application/json" } }).then((response) => {
        ...
        this.state.setState(STATES.SUCCESS, { successRate: parsed.successRate });
        this.tracker.trackLoad(duration, true);
        _log("debug", "Dados atualizados", { successRate: parsed.successRate });
      }).catch((error) => {
        ...
        if (this.container.classList.contains("is-loading")) this.render();
        this.container.innerHTML = renderError("Erro ao carregar taxa global");
  ...
  setupEventListeners() {
    ...
    if (eb) eb.on(EVENTS.REFRESH, this._onRefresh);
  ...
  destroy() {
    ...
    const eb = _getPort("eventBus");
    if (eb) eb.off(EVENTS.REFRESH, this._onRefresh);
...
var card_02_default = CardGlobalSuccessRate;
export {
  CARD_ID,
  CARD_NAME,
  CardGlobalSuccessRate,
  MODULE_ID,
  VERSION,
  card_02_default as default,
  getPorts,
  getStatus,
  getVersion,
  healthCheck,
