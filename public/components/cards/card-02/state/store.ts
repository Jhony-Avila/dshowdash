...
// ═══════════════════════════════════════════════════════════════
// MODULE: components.cards.card-02.state.store
// PURPOSE: Reactive state management for Card 02 with lock support
// ───────────────────────────────────────────────────────────────
// @contract STATE_MACHINE - setState/getState/is/isLoading/isSuccess/isError/isPaused/isIdle
...
export const MODULE_ID = 'components.cards.card-02.state.store';
...
export class CardState {
  ...
  subscribe(fn: (state: unknown) => void) { this._listeners.add(fn); return () => this._listeners.delete(fn); }
  _notify(event: unknown) { this._listeners.forEach((fn: (...args: unknown[]) => void) => { try { fn(this.getState(), event); } catch {} }); }
