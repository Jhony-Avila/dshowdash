...
const MODULE_ID = "components.cards.card-03";
...
class CardPerformanceScore {
  ...
  loadCSS() {
    const linkId = "card-03-styles";
    if (document.getElementById(linkId)) return Promise.resolve();
    return new Promise((resolve, reject) => {
      const link = document.createElement("link");
      link.id = linkId;
      link.rel = "stylesheet";
      link.href = "/components/cards/card-03/styles.css";
      link.onload = () => resolve();
  ...
  loadData() {
    return this.state.withLock("loadData", () => {
      ...
      return API.get(this.apiEndpoint, { timeout: CONFIG.API_TIMEOUT, retries: CONFIG.API_RETRIES, headers: { "Accept": "application/json" } }).then((response) => {
        ...
        if (this.$?.score) this.$.score.textContent = formatScore(parsed.score);
        this.setStatus("");
        this.container.classList.remove("is-loading", "has-error");
        this._metrics.lastLoadAt = Date.now();
        this.state.setState(STATES.SUCCESS, { score: parsed.score });
        this.tracker.trackLoad(duration, true);
        _log("debug", "Dados atualizados", { score: parsed.score });
      }).catch((error) => {
        ...
        if (this.container.classList.contains("is-loading")) this.render();
        this.container.innerHTML = renderError("Erro ao carregar performance score");
...
var card_03_default = CardPerformanceScore;
export {
  CARD_ID,
  CARD_NAME,
