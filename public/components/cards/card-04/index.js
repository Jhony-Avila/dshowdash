...
const MODULE_ID = "components.cards.card-04";
...
class CardPeakHour {
  ...
  loadCSS() {
    const linkId = "card-04-styles";
    if (document.getElementById(linkId)) return Promise.resolve();
    return new Promise((resolve, reject) => {
      const link = document.createElement("link");
      link.id = linkId;
      link.rel = "stylesheet";
      link.href = "/components/cards/card-04/styles.css";
      link.onload = resolve;
  ...
  render() {
    ...
    this.$ = { hour: this.container.querySelector('[data-el="hour"]'), status: this.container.querySelector('[data-el="status"]') };
  ...
  loadData() {
    return this.state.withLock("loadData", () => {
      ...
      return API.get(this.apiEndpoint, { timeout: CONFIG.API_TIMEOUT, retries: CONFIG.API_RETRIES, headers: { "Accept": "application/json" } }).then((response) => {
        ...
        if (this.$?.hour) this.$.hour.textContent = formatHour(parsed.peakHour);
        this.setStatus("");
        this.container.classList.remove("is-loading", "has-error");
        this._metrics.lastLoadAt = Date.now();
        this.state.setState(STATES.SUCCESS, { peakHour: parsed.peakHour });
        this.tracker.trackLoad(duration, true);
        _log("debug", "Dados atualizados", { peakHour: parsed.peakHour });
      }).catch((error) => {
        ...
        if (this.container.classList.contains("is-loading")) this.render();
        this.container.innerHTML = renderError("Erro ao carregar pico do dia");
...
var card_04_default = CardPeakHour;
export {
  CARD_ID,
  CARD_NAME,
  CardPeakHour,
