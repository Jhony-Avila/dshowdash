...
// ═══════════════════════════════════════════════════════════════
// MODULE: components.cards.card-04.telemetry.tracker
// PURPOSE: Telemetry tracking for Card 04 with metrics collection
// ───────────────────────────────────────────────────────────────
...
export const MODULE_ID = 'components.cards.card-04.telemetry.tracker';
...
export class CardTelemetryTracker {
  ...
  track(event: string, data?: unknown) {
    ...
    const payload = {
      event: `${NAMESPACE}:${this.cardId}:${event}`,
      cardId: this.cardId,
      timestamp: Date.now(),
...
export const info = () => ({
  moduleId: MODULE_ID,
  version: VERSION,
  exports: ['CardTelemetryTracker', 'createTracker'],
  timestamp: Date.now()
});
