...
const MODULE_ID = "components.cards.card-05";
...
class CardCompleted {
  ...
  loadCSS() {
    const linkId = "card-05-styles";
    if (document.getElementById(linkId)) return Promise.resolve();
    return new Promise((resolve, reject) => {
      const link = document.createElement("link");
      link.id = linkId;
      link.rel = "stylesheet";
      link.href = "/components/cards/card-05/styles.css";
      link.onload = resolve;
  ...
  loadData() {
    return this.state.withLock("loadData", () => {
      ...
      return API.get(this.apiEndpoint, { timeout: CONFIG.API_TIMEOUT, retries: CONFIG.API_RETRIES, headers: { "Accept": "application/json" } }).then((response) => {
        ...
        if (this.$?.count) this.$.count.textContent = formatNumber(parsed.completed);
        this.setStatus("");
        this.container.classList.remove("is-loading", "has-error");
        this._metrics.lastLoadAt = Date.now();
        this.state.setState(STATES.SUCCESS, { completed: parsed.completed });
        this.tracker.trackLoad(duration, true);
        _log("debug", "Dados atualizados", { completed: parsed.completed });
      }).catch((error) => {
        ...
        if (this.container.classList.contains("is-loading")) this.render();
        this.container.innerHTML = renderError("Erro ao carregar completados");
...
var card_05_default = CardCompleted;
export {
  CARD_ID,
  CARD_NAME,
  CardCompleted,
