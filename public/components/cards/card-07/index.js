...
const MODULE_ID = "components.cards.card-07";
...
class CardRunning {
  ...
  loadCSS() {
    const linkId = "card-07-styles";
    if (document.getElementById(linkId)) return Promise.resolve();
    return new Promise((resolve, reject) => {
      const link = document.createElement("link");
      link.id = linkId;
      link.rel = "stylesheet";
      link.href = "/components/cards/card-07/styles.css";
      link.onload = resolve;
  ...
  loadData() {
    return this.state.withLock("loadData", () => {
      ...
      return API.get(this.apiEndpoint, { timeout: CONFIG.API_TIMEOUT, retries: CONFIG.API_RETRIES, headers: { "Accept": "application/json" } }).then((response) => {
        ...
        if (this.$?.count) this.$.count.textContent = formatNumber(parsed.running);
        this.setStatus("");
        this.container.classList.remove("is-loading", "has-error");
        this._metrics.lastLoadAt = Date.now();
        this.state.setState(STATES.SUCCESS, { running: parsed.running });
        this.tracker.trackLoad(duration, true);
        _log("debug", "Dados atualizados", { running: parsed.running });
      }).catch((error) => {
        ...
        if (this.container.classList.contains("is-loading")) this.render();
        this.container.innerHTML = renderError("Erro ao carregar em execu\xE7\xE3o");
...
var card_07_default = CardRunning;
export {
  CARD_ID,
  CARD_NAME,
  CardRunning,
