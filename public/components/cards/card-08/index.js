...
const MODULE_ID = "components.cards.card-08";
...
class CardAverageTime {
  ...
  loadCSS() {
    const linkId = "card-08-styles";
    if (document.getElementById(linkId)) return Promise.resolve();
    return new Promise((resolve, reject) => {
      const link = document.createElement("link");
      link.id = linkId;
      link.rel = "stylesheet";
      link.href = "/components/cards/card-08/styles.css";
      link.onload = resolve;
  ...
  render() {
    ...
    this.$ = { value: this.container.querySelector('[data-el="value"]'), status: this.container.querySelector('[data-el="status"]') };
  ...
  loadData() {
    return this.state.withLock("loadData", () => {
      ...
      return API.get(this.apiEndpoint, { timeout: CONFIG.API_TIMEOUT, retries: CONFIG.API_RETRIES, headers: { "Accept": "application/json" } }).then((response) => {
        ...
        if (this.$?.value) this.$.value.textContent = formatDecimal(parsed.averagePerHour);
        this.setStatus("");
        this.container.classList.remove("is-loading", "has-error");
        this._metrics.lastLoadAt = Date.now();
        this.state.setState(STATES.SUCCESS, { averagePerHour: parsed.averagePerHour });
        this.tracker.trackLoad(duration, true);
        _log("debug", "Dados atualizados", { averagePerHour: parsed.averagePerHour });
      }).catch((error) => {
        ...
        if (this.container.classList.contains("is-loading")) this.render();
        this.container.innerHTML = renderError("Erro ao carregar m\xE9dia por hora");
...
var card_08_default = CardAverageTime;
export {
  CARD_ID,
  CARD_NAME,
  CardAverageTime,
