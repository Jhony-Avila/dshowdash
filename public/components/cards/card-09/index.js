...
const MODULE_ID = "components.cards.card-09";
...
class CardTotalExecutions {
  ...
  loadCSS() {
    const linkId = "card-09-styles";
    if (document.getElementById(linkId)) return Promise.resolve();
    return new Promise((resolve, reject) => {
      const link = document.createElement("link");
      link.id = linkId;
      link.rel = "stylesheet";
      link.href = "/components/cards/card-09/styles.css";
      link.onload = resolve;
  ...
  render() {
    ...
    this.$ = { count: this.container.querySelector('[data-el="count"]'), status: this.container.querySelector('[data-el="status"]') };
  ...
  loadData() {
    return this.state.withLock("loadData", () => {
      ...
      return API.get(this.apiEndpoint, { timeout: CONFIG.API_TIMEOUT, retries: CONFIG.API_RETRIES, headers: { "Accept": "application/json" } }).then((response) => {
        ...
        if (this.$?.count) this.$.count.textContent = formatNumber(parsed.total);
        this.setStatus("");
        this.container.classList.remove("is-loading", "has-error");
        this._metrics.lastLoadAt = Date.now();
        this.state.setState(STATES.SUCCESS, { total: parsed.total });
        this.tracker.trackLoad(duration, true);
        _log("debug", "Dados atualizados", { total: parsed.total });
      }).catch((error) => {
        ...
        if (this.container.classList.contains("is-loading")) this.render();
        this.container.innerHTML = renderError("Erro ao carregar total");
  ...
  healthCheck() {
    ...
    return { status: score >= 5 ? "HEALTHY" : score >= 3 ? "DEGRADED" : "UNHEALTHY", score, maxScore, scoreDisplay: `${score}/${maxScore}`, checks, state: this.state.state, portsInitialized: portsSnapshot._initialized, version: VERSION, moduleId: MODULE_ID, strictMode: isStrict(), timestamp: Date.now() };
  }
  info() {
    const portsSnapshot = Ports.snapshot();
...
var card_09_default = CardTotalExecutions;
export {
  CARD_ID,
  CARD_NAME,
  CardTotalExecutions,
