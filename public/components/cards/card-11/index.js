...
const MODULE_ID = "components.cards.card-11";
...
let _instance = null;
class CardAverageExecutionTime {
  ...
  loadCSS() {
    const linkId = "card-11-styles";
    if (document.getElementById(linkId)) return Promise.resolve();
    return new Promise((resolve, reject) => {
      const link = document.createElement("link");
      link.id = linkId;
      link.rel = "stylesheet";
      link.href = "/components/cards/card-11/styles.css";
      link.onload = resolve;
  ...
  render() {
    ...
    this.$ = { time: this.container.querySelector('[data-el="time"]'), status: this.container.querySelector('[data-el="status"]') };
  ...
  loadData() {
    return this.state.withLock("loadData", () => {
      ...
      return API.get(this.apiEndpoint, { timeout: CONFIG.API_TIMEOUT, retries: CONFIG.API_RETRIES, headers: { "Accept": "application/json" } }).then((response) => {
        ...
        if (this.$?.time) this.$.time.textContent = formatDuration(parsed.averageTime);
        this.setStatus("");
        this.container.classList.remove("is-loading", "has-error");
        this._metrics.lastLoadAt = Date.now();
        this.state.setState(STATES.SUCCESS, { averageTime: parsed.averageTime });
        this.tracker.trackLoad(duration, true);
        _log("debug", "Dados atualizados", { averageTime: parsed.averageTime });
      }).catch((error) => {
        ...
        if (this.container.classList.contains("is-loading")) this.render();
        this.container.innerHTML = renderError("Erro ao carregar tempo m\xE9dio");
...
var card_11_default = CardAverageExecutionTime;
export {
  CARD_ID,
  CARD_NAME,
