...
const MODULE_ID = "components.cards.card-12";
...
class CardCacheHitRate {
  ...
  loadCSS() {
    const linkId = "card-12-styles";
    if (document.getElementById(linkId)) return Promise.resolve();
    return new Promise((resolve, reject) => {
      const link = document.createElement("link");
      link.id = linkId;
      link.rel = "stylesheet";
      link.href = "/components/cards/card-12/styles.css";
      link.onload = resolve;
  ...
  render() {
    ...
    this.$ = { rate: this.container.querySelector('[data-el="rate"]'), status: this.container.querySelector('[data-el="status"]') };
  ...
  loadData() {
    return this.state.withLock("loadData", () => {
      ...
      return API.get(this.apiEndpoint, { timeout: CONFIG.API_TIMEOUT, retries: CONFIG.API_RETRIES, headers: { "Accept": "application/json" } }).then((response) => {
        ...
        if (this.$?.rate) this.$.rate.textContent = formatPercent(parsed.cacheHitRate);
        this.setStatus("");
        this.container.classList.remove("is-loading", "has-error");
        this._metrics.lastLoadAt = Date.now();
        this.state.setState(STATES.SUCCESS, { cacheHitRate: parsed.cacheHitRate });
        this.tracker.trackLoad(duration, true);
        _log("debug", "Dados atualizados", { cacheHitRate: parsed.cacheHitRate });
      }).catch((error) => {
        ...
        if (this.container.classList.contains("is-loading")) this.render();
        this.container.innerHTML = renderError("Erro ao carregar taxa de cache");
...
var card_12_default = CardCacheHitRate;
export {
  CARD_ID,
  CARD_NAME,
  CardCacheHitRate,
