// ═══════════════════════════════════════════════════════════════
// DEPENDENCY CONTRACT (1.3.0-ICON-REGISTRY)
// ═══════════════════════════════════════════════════════════════
// MODULE: footer-shared-icons
// PURPOSE: Footer Shared - Icons SVG
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export function getIcon(name: string): string {
    ...
    // 1. Local lookup
    if ((ICONS as Record<string, string>)[name]) return (ICONS as Record<string, string>)[name];
    // 2. Registry lookup (namespace:name)
    if (name.includes(':')) {
      try {
    ...
    // 3. Resolve short name across namespaces
    try {
...
export function getMetrics() { return { ..._metrics, totalIcons: Object.keys(ICONS).length }; }
export function info() { return { moduleId: MODULE_ID, version: VERSION, availableIcons: Object.keys(ICONS), metrics: getMetrics() }; }
...
export default { ICONS, getIcon, getMetrics, info, healthCheck, VERSION, MODULE_ID };
