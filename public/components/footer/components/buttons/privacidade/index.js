...
const MODULE_ID = "footer/components/buttons/privacidade";
class PrivacidadeLink {
  ...
  info() {
    return { id: this._config.id, label: this._config.label, href: this._config.href, kind: this._config.kind, mounted: this._mounted, initialized: this._initialized, version: VERSION, moduleId: MODULE_ID, emits: [UI_EVENTS.ACTION], actionPayload: ACTION_PAYLOAD };
  }
  getElement() {
...
function getInstance() {
  if (!_instance) _instance = new PrivacidadeLink();
  return _instance;
}
function createPrivacidadeLink() {
  return new PrivacidadeLink();
}
...
var privacidade_default = { PrivacidadeLink, getInstance, createPrivacidadeLink, init, mount, unmount, healthCheck, info, VERSION, MODULE_ID };
export {
  MODULE_ID,
  VERSION,
  createPrivacidadeLink,
  privacidade_default as default,
  getInstance,
  healthCheck,
  info,
  init,
