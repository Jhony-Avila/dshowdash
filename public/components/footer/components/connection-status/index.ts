...
//
// PROVIDES:
//   ConnectionStatus constructor, createConnectionStatus(),
//   mount(), unmount(), checkNow(), info(), healthCheck(),
//   VERSION, MODULE_ID, injectPorts(), getPorts()
//
// RECEIVES (via constructor config):
//   config.size, config.colors, config.thresholds
//
// BROWSER APIs (legítimo — connection monitor):
//   document.createElement, document.head.appendChild — DOM
//   fetch(/api/metrics/server.php) — latency check
//   performance.now — latency measurement
//   AbortController — timeout control
//   setInterval — periodic monitoring (30s)
// ═══════════════════════════════════════════════════════════════
// ConnectionStatus Component - Autocontained Enterprise
// @version 1.3.1-P18EC-ES6
// @changelog v1.3.1-P18EC - Fixed duplicate export of injectPorts/getPorts
// @changelog P18EC - Events Contracts Migration
// P18EC: Events Contracts Migration
// Ícone Wi-Fi com 3 arcos + ponto central
// Estados visuais baseados em qualidade de conexão
'use strict';
...
.cs-container {\
    ...
    width: ${TOKENS.size}px;\
    height: ${TOKENS.size}px;\
    border-radius: 50%;\
    cursor: pointer;\
...
.cs-container[data-status="excellent"] .cs-glow {\
    box-shadow: 0 0 12px rgba(34, 197, 94, 0.4);\
    opacity: 1;\
}\
.cs-container[data-status="good"] .cs-glow {\
    box-shadow: 0 0 10px rgba(132, 204, 22, 0.35);\
    opacity: 1;\
}\
.cs-container[data-status="fair"] .cs-glow {\
    box-shadow: 0 0 8px rgba(245, 158, 11, 0.3);\
    opacity: 1;\
}\
.cs-container[data-status="poor"] .cs-glow {\
    box-shadow: 0 0 8px rgba(239, 68, 68, 0.35);\
    opacity: 1;\
}\
.cs-container[data-status="offline"] .cs-glow {\
    opacity: 0;\
}\
.cs-tooltip {\
    position: absolute;\
...
function ConnectionStatus(this: any, config: Record<string,unknown>) {
...
ConnectionStatus.prototype.mount = function(container: HTMLElement|null) {
...
ConnectionStatus.prototype._render = function() {
    ...
    wrapper.setAttribute('aria-label', 'Status de conexão');

    wrapper.innerHTML = `\
        <div class="cs-glow"></div>\
        <svg class="cs-icon" viewBox="0 0 510.875 510.875" xmlns="http://www.w3.org/2000/svg">\
            <path class="cs-arc cs-arc-3" d="M434.975,219.174c-98.952-98.952-259.373-98.952-358.325,0c-17.242,17.242-45.728,17.242-63.719,0c-17.242-17.242-17.242-45.728,0-63.719c134.184-134.184,351.578-134.184,485.013,0c17.242,17.242,17.242,45.728,0,63.719C481.452,236.416,452.966,236.416,434.975,219.174z" fill="${TOKENS.colors.inactive}"/>\
        ...
        <div class="cs-tooltip">Verificando conexão...</div>\
    `;
...
ConnectionStatus.prototype._updateStatus = function(latency: unknown) {
    ...
    if (latency < thresholds.excellent) {
        status = 'excellent'; color = colors.excellent; arcsActive = 3;
    // @ts-expect-error strict migration — TS18046
    } else if (latency < thresholds.good) {
        status = 'good'; color = colors.good; arcsActive = 2;
    // @ts-expect-error strict migration — TS18046
    } else if (latency < thresholds.fair) {
        status = 'fair'; color = colors.fair; arcsActive = 1;
    } else {
        status = 'poor'; color = colors.poor; arcsActive = 1;
    }
...
ConnectionStatus.prototype._applyVisualState = function(color: string, arcsActive: boolean, status: string, latency: unknown) {
    ...
    els.tooltip.textContent = `${(statusText as Record<string,unknown>)[status as string]} • ${latency}ms`;
};
...
ConnectionStatus.prototype._setOffline = function() {
    ...
    els.tooltip.textContent = 'Sem conexão';
};

ConnectionStatus.prototype.checkNow = function() { this._checkConnection(); return this; };
...
function createConnectionStatus(config: Record<string,unknown>) { return (new (ConnectionStatus as unknown as { new(..._args: unknown[]): {[k:string]:Function} })(config)); }

export { ConnectionStatus, createConnectionStatus, MODULE_ID, VERSION };
