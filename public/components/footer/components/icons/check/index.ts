...
// ═══════════════════════════════════════════════════════════════
// MODULE: footer-icon-check
// PURPOSE: check Icon - Entry Point
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
const MODULE_ID = 'footer-icon-check';
...
function _log(level: string, ...args: unknown[]) {
  if (Logger && typeof (Logger as Record<string,unknown>)[level as string] === 'function') {
    ...
    (Logger as Record<string,unknown>)[level as string]('[CHECK]', ...args);
  }
}
...
if (typeof window !== 'undefined') {
  ...
  window.__dev.footerIconCheck = { getVersion: () => VERSION, info: () => CheckIcon.info(), healthCheck: () => CheckIcon.healthCheck() };
}

export default CheckIcon;
export { CheckIcon, MODULE_ID, VERSION };
