...
// ═══════════════════════════════════════════════════════════════
// MODULE: footer-icon-clock
// PURPOSE: clock Icon - Entry Point
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
const MODULE_ID = 'footer-icon-clock';
...
function _log(level: string, ...args: unknown[]) {
  if (Logger && typeof (Logger as Record<string,unknown>)[level as string] === 'function') {
    ...
    (Logger as Record<string,unknown>)[level as string]('[CLOCK]', ...args);
  }
}
...
if (typeof window !== 'undefined') {
  ...
  window.__dev.footerIconClock = { getVersion: () => VERSION, info: () => ClockIcon.info(), healthCheck: () => ClockIcon.healthCheck() };
}

export default ClockIcon;
export { ClockIcon, MODULE_ID, VERSION };
