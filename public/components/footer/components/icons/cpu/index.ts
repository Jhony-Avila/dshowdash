...
// ═══════════════════════════════════════════════════════════════
// MODULE: footer-icon-cpu
// PURPOSE: cpu Icon - Entry Point
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
const MODULE_ID = 'footer-icon-cpu';
...
function _log(level: string, ...args: unknown[]) {
  if (Logger && typeof (Logger as Record<string,unknown>)[level as string] === 'function') {
    ...
    (Logger as Record<string,unknown>)[level as string]('[CPU]', ...args);
  }
}
...
if (typeof window !== 'undefined') {
  ...
  window.__dev.footerIconCpu = { getVersion: () => VERSION, info: () => CpuIcon.info(), healthCheck: () => CpuIcon.healthCheck() };
}

export default CpuIcon;
export { CpuIcon, MODULE_ID, VERSION };
