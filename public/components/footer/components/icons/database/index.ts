...
// ═══════════════════════════════════════════════════════════════
// MODULE: footer-icon-database
// PURPOSE: database Icon - Entry Point
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
const MODULE_ID = 'footer-icon-database';
...
function _log(level: string, ...args: unknown[]) {
  if (Logger && typeof (Logger as Record<string,unknown>)[level as string] === 'function') {
    ...
    (Logger as Record<string,unknown>)[level as string]('[DATABASE]', ...args);
  }
}
...
if (typeof window !== 'undefined') {
  ...
  window.__dev.footerIconDatabase = { getVersion: () => VERSION, info: () => DatabaseIcon.info(), healthCheck: () => DatabaseIcon.healthCheck() };
}

export default DatabaseIcon;
export { DatabaseIcon, MODULE_ID, VERSION };
