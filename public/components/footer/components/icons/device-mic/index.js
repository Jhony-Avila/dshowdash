...
const DeviceMicIcon = {
  ...
  setMode(mode) {
    const el = Lifecycle.getContainer()?.querySelector(".dsd-icon--device-mic");
    if (el) {
  ...
  toggle() {
    ...
    this.setMode(current === "on" ? "off" : "on");
  },
  isOn() {
    return Store.getMode() === "on";
  },
  async unmount() {
  ...
  info() {
    return { moduleId: MODULE_ID, version: VERSION, lifecycle: Lifecycle.info(), currentMode: Store.getMode(), isOn: this.isOn(), metrics: Tracker.getMetrics(), timestamp: Date.now() };
  }
};
if (typeof window !== "undefined") {
  window.__dev = window.__dev || {};
  window.__dev.footerIconDeviceMic = { getVersion: () => VERSION, info: () => DeviceMicIcon.info(), healthCheck: () => DeviceMicIcon.healthCheck(), toggle: () => DeviceMicIcon.toggle() };
}
var device_mic_default = DeviceMicIcon;
export {
  DeviceMicIcon,
  MODULE_ID,
  VERSION,
  device_mic_default as default
};
