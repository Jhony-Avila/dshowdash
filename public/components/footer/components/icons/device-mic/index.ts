...
// ═══════════════════════════════════════════════════════════════
// MODULE: footer-icon-device-mic
// PURPOSE: device-mic Icon - Entry Point
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
const MODULE_ID = 'footer-icon-device-mic';
const VERSION = '1.0.1-ENTERPRISE';
// @ts-expect-error TS migration - TS2349
function _log(level: string, ...args: unknown[]) { if (Logger && typeof (Logger as Record<string,unknown>)[level as string] === 'function') { (Logger as Record<string,unknown>)[level as string]('[DEVICE-MIC]', ...args); } }
const DeviceMicIcon = {
  ...
  setMode(mode) { const el = Lifecycle.getContainer()?.querySelector('.dsd-icon--device-mic'); if (el) { const oldMode = Store.getMode(); States.applyMode(el, mode); Store.setMode(mode); Tracker.trackModeChange(); _log('info', 'Mode changed:', oldMode, '->', mode); } },
...
if (typeof window !== 'undefined') { window.__dev = window.__dev || {}; window.__dev.footerIconDeviceMic = { getVersion: () => VERSION, info: () => DeviceMicIcon.info(), healthCheck: () => DeviceMicIcon.healthCheck(), toggle: () => DeviceMicIcon.toggle() }; }
export default DeviceMicIcon;
export { DeviceMicIcon, MODULE_ID, VERSION };
