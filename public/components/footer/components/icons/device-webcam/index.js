...
function _log(level, ...args) {
  if (Logger && typeof Logger[level] === "function") {
    Logger[level]("[DEVICE-WEBCAM]", ...args);
  }
}
const DeviceWebcamIcon = {
  ...
  setMode(mode) {
    const el = Lifecycle.getContainer()?.querySelector(".dsd-icon--device-webcam");
    if (el) {
...
if (typeof window !== "undefined") {
  ...
  window.__dev.footerIconDeviceWebcam = { getVersion: () => VERSION, info: () => DeviceWebcamIcon.info(), healthCheck: () => DeviceWebcamIcon.healthCheck(), toggle: () => DeviceWebcamIcon.toggle() };
}
var device_webcam_default = DeviceWebcamIcon;
export {
  DeviceWebcamIcon,
  MODULE_ID,
  VERSION,
  device_webcam_default as default
};
