...
// ═══════════════════════════════════════════════════════════════
// MODULE: footer.icon.dollar-sign.tracker
// PURPOSE: dollar-sign Icon - Telemetry Tracker
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
const MODULE_ID = 'footer.icon.dollar-sign.tracker';
...
export const Tracker = { trackRender(p: Record<string,unknown>) { _m.renders++; this._emit(TELEMETRY_EVENTS.COMPONENT_RENDER, { props: p }); }, trackClick(p: Record<string,unknown>) { _m.clicks++; this._emit(TELEMETRY_EVENTS.COMPONENT_INTERACTION, { props: p, action: 'click' }); }, trackMount() { _m.mounts++; this._emit(TELEMETRY_EVENTS.COMPONENT_MOUNT, {}); }, trackUnmount() { _m.unmounts++; this._emit(TELEMETRY_EVENTS.COMPONENT_UNMOUNT, {}); }, trackError(e: Event) { _m.errors++; this._emit(TELEMETRY_EVENTS.ERROR, { message: e && e.message ? e.message : String(e) }); }, _emit(eventType: string, d: unknown) { const t = Ports.get('telemetry'); if (t && t.track) t.track(eventType, Object.assign({ moduleId: MODULE_ID }, d)); }, getMetrics() { return Object.assign({}, _m); }, reset() { for (const k in _m) (_m as Record<string,unknown>)[k] = 0; } };
