...
// ═══════════════════════════════════════════════════════════════
// MODULE: footer-icon-globe
// PURPOSE: globe Icon - Entry Point
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
const MODULE_ID = 'footer-icon-globe';
...
function _log(level: string, ...args: unknown[]) {
  if (Logger && typeof (Logger as Record<string,unknown>)[level as string] === 'function') {
    ...
    (Logger as Record<string,unknown>)[level as string]('[GLOBE]', ...args);
  }
}
...
if (typeof window !== 'undefined') {
  ...
  window.__dev.footerIconGlobe = { getVersion: () => VERSION, info: () => GlobeIcon.info(), healthCheck: () => GlobeIcon.healthCheck() };
}

export default GlobeIcon;
export { GlobeIcon, MODULE_ID, VERSION };
