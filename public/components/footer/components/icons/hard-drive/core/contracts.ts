...
// ═══════════════════════════════════════════════════════════════
// MODULE: footer-icon-hard-drive-contracts
// PURPOSE: hard-drive Icon - Contracts
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
// ═══════════════════════════════════════════════════════════════
'use strict';
const MODULE_ID = 'footer-icon-hard-drive-contracts';
const VERSION = '1.1.0-ENTERPRISE';
export const CONTRACTS = {
  EVENTS: { MOUNTED: 'footer.icon.hard-drive:mounted', UNMOUNTED: 'footer.icon.hard-drive:unmounted', RENDERED: 'footer.icon.hard-drive:rendered', CLICKED: 'footer.icon.hard-drive:clicked', ERROR: 'footer.icon.hard-drive:error' },
  PROPS: { size: { type: 'string', values: ['xs', 'sm', 'md', 'lg', 'xl'], default: 'md' }, variant: { type: 'string', values: ['primary', 'secondary', 'brand', 'success', 'warning', 'danger', 'muted'], default: 'primary' }, state: { type: 'string', values: ['default', 'active', 'disabled', 'loading'], default: 'default' }, decorative: { type: 'boolean', default: false }, ariaLabel: { type: 'string', default: 'hard drive' }, clickable: { type: 'boolean', default: false } },
  SIZES: { xs: 12, sm: 14, md: 16, lg: 20, xl: 24 },
  CSS_CLASSES: { wrapper: 'dsd-icon dsd-icon--hard-drive', svg: 'dsd-icon__svg' }
};
