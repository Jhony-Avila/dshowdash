...
// ═══════════════════════════════════════════════════════════════
// MODULE: footer-icon-hard-drive
// PURPOSE: hard-drive Icon - Entry Point
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
const MODULE_ID = 'footer-icon-hard-drive';
const VERSION = '1.0.1-ENTERPRISE';
// @ts-expect-error TS migration - TS2349
function _log(level: string, ...args: unknown[]) { if (Logger && typeof (Logger as Record<string,unknown>)[level as string] === 'function') { (Logger as Record<string,unknown>)[level as string]('[HARD-DRIVE]', ...args); } }
const HardDriveIcon = {
...
if (typeof window !== 'undefined') { window.__dev = window.__dev || {}; window.__dev.footerIconHardDrive = { getVersion: () => VERSION, info: () => HardDriveIcon.info(), healthCheck: () => HardDriveIcon.healthCheck() }; }
export default HardDriveIcon;
export { HardDriveIcon, MODULE_ID, VERSION };
