...
// ═══════════════════════════════════════════════════════════════
// MODULE: footer-icon-logo
// PURPOSE: logo Icon - Entry Point
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
const MODULE_ID = 'footer-icon-logo';
...
function _log(level: string, ...args: unknown[]) {
  if (Logger && typeof (Logger as Record<string,unknown>)[level as string] === 'function') {
    ...
    (Logger as Record<string,unknown>)[level as string]('[LOGO]', ...args);
  }
}
...
if (typeof window !== 'undefined') {
  ...
  window.__dev.footerIconLogo = { getVersion: () => VERSION, info: () => LogoIcon.info(), healthCheck: () => LogoIcon.healthCheck() };
}

export default LogoIcon;
export { LogoIcon, MODULE_ID, VERSION };
