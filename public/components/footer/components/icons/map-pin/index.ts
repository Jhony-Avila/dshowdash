...
// ═══════════════════════════════════════════════════════════════
// MODULE: footer-icon-map-pin
// PURPOSE: map-pin Icon - Entry Point
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
const MODULE_ID = 'footer-icon-map-pin';
...
function _log(level: string, ...args: unknown[]) {
  if (Logger && typeof (Logger as Record<string,unknown>)[level as string] === 'function') {
    ...
    (Logger as Record<string,unknown>)[level as string]('[MAP-PIN]', ...args);
  }
}
...
if (typeof window !== 'undefined') {
  ...
  window.__dev.footerIconMapPin = { getVersion: () => VERSION, info: () => MapPinIcon.info(), healthCheck: () => MapPinIcon.healthCheck() };
}

export default MapPinIcon;
export { MapPinIcon, MODULE_ID, VERSION };
