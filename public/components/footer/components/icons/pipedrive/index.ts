...
// ═══════════════════════════════════════════════════════════════
// MODULE: footer-icon-pipedrive
// PURPOSE: pipedrive Icon - Entry Point
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
const MODULE_ID = 'footer-icon-pipedrive';
...
function _log(level: string, ...args: unknown[]) {
  if (Logger && typeof (Logger as Record<string,unknown>)[level as string] === 'function') {
    ...
    (Logger as Record<string,unknown>)[level as string]('[PIPEDRIVE]', ...args);
  }
}
...
if (typeof window !== 'undefined') {
  ...
  window.__dev.footerIconPipedrive = { getVersion: () => VERSION, info: () => PipedriveIcon.info(), healthCheck: () => PipedriveIcon.healthCheck() };
}

export default PipedriveIcon;
export { PipedriveIcon, MODULE_ID, VERSION };
