...
// ═══════════════════════════════════════════════════════════════
// MODULE: footer-icon-search
// PURPOSE: search Icon - Entry Point
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
const MODULE_ID = 'footer-icon-search';
const VERSION = '1.0.1-ENTERPRISE';
// @ts-expect-error TS migration - TS2349
function _log(level: string, ...args: unknown[]) { if (Logger && typeof (Logger as Record<string,unknown>)[level as string] === 'function') { (Logger as Record<string,unknown>)[level as string]('[SEARCH]', ...args); } }
const SearchIcon = {
...
if (typeof window !== 'undefined') { window.__dev = window.__dev || {}; window.__dev.footerIconSearch = { getVersion: () => VERSION, info: () => SearchIcon.info(), healthCheck: () => SearchIcon.healthCheck() }; }
export default SearchIcon;
export { SearchIcon, MODULE_ID, VERSION };
