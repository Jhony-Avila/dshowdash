...
// ═══════════════════════════════════════════════════════════════
// MODULE: footer-icon-server
// PURPOSE: server Icon - Entry Point
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
const MODULE_ID = 'footer-icon-server';
...
function _log(level: string, ...args: unknown[]) {
  if (Logger && typeof (Logger as Record<string,unknown>)[level as string] === 'function') {
    ...
    (Logger as Record<string,unknown>)[level as string]('[SERVER]', ...args);
  }
}
...
if (typeof window !== 'undefined') {
  ...
  window.__dev.footerIconServer = { getVersion: () => VERSION, info: () => ServerIcon.info(), healthCheck: () => ServerIcon.healthCheck() };
}

export default ServerIcon;
export { ServerIcon, MODULE_ID, VERSION };
