...
// ═══════════════════════════════════════════════════════════════
// MODULE: footer-icon-status-mode-store
// PURPOSE: status-mode Icon - Store
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
const MODULE_ID = 'footer-icon-status-mode-store';
const VERSION = '1.2.0-ENTERPRISE';

const _state = { props: (null as Record<string,unknown>|null), isVisible: true, isHovered: false, lastRender: (null as unknown|null), mode: 'on' };
let _metrics = { propsSet: 0, resets: 0, modeChanges: 0 };

export const Store = {
  getState() { return { ..._state }; },
  setProps(p: Record<string,unknown>) { _state.props = p; _metrics.propsSet++; },
  getProps() { return _state.props; },
  setVisible(v: unknown) { _state.isVisible = !!v; },
  setHovered(v: unknown) { _state.isHovered = !!v; },
  // @ts-expect-error TS migration - TS2322
  setMode(m: unknown) { _state.mode = m; _metrics.modeChanges++; },
  getMode() { return _state.mode; },
  markRender() { _state.lastRender = Date.now(); },
  reset() { _state.props = null; _state.isVisible = true; _state.isHovered = false; _state.lastRender = null; _state.mode = 'on'; _metrics.resets++; }
};
