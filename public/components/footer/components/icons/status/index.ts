...
// ═══════════════════════════════════════════════════════════════
// MODULE: footer-icon-status
// PURPOSE: status Icon - Entry Point
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
const MODULE_ID = 'footer-icon-status';
...
function _log(level: string, ...args: unknown[]) {
  if (Logger && typeof (Logger as Record<string,unknown>)[level as string] === 'function') {
    ...
    (Logger as Record<string,unknown>)[level as string]('[STATUS]', ...args);
  }
}
...
if (typeof window !== 'undefined') {
  ...
  window.__dev.footerIconStatus = { getVersion: () => VERSION, info: () => StatusIcon.info(), healthCheck: () => StatusIcon.healthCheck() };
}

export default StatusIcon;
export { StatusIcon, MODULE_ID, VERSION };
