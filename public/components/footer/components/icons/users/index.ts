...
// ═══════════════════════════════════════════════════════════════
// MODULE: footer-icon-users
// PURPOSE: users Icon - Entry Point
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
const MODULE_ID = 'footer-icon-users';
const VERSION = '1.0.1-ENTERPRISE';
// @ts-expect-error TS migration - TS2349
function _log(level: string, ...args: unknown[]) { if (Logger && typeof (Logger as Record<string,unknown>)[level as string] === 'function') { (Logger as Record<string,unknown>)[level as string]('[USERS]', ...args); } }
const UsersIcon = {
...
if (typeof window !== 'undefined') { window.__dev = window.__dev || {}; window.__dev.footerIconUsers = { getVersion: () => VERSION, info: () => UsersIcon.info(), healthCheck: () => UsersIcon.healthCheck() }; }
export default UsersIcon;
export { UsersIcon, MODULE_ID, VERSION };
