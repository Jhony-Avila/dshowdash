// ═══════════════════════════════════════════════════════════════
// DEPENDENCY CONTRACT (v31.0.0-METRICS-API)
// ═══════════════════════════════════════════════════════════════
// IMPORTS:
//   createUiPorts from /core/runtime/ports-profiles.js
//   createProgressCircle from ../progress-circle/index.js
//   createConnectionStatus from ../connection-status/index.js
//   Animacoes from /assets/animacoes/index.js
//   FooterButtonsMounter from ../buttons/component-mounter.js
//   IconsOrchestrator from ../icons-orchestrator.js
//   FooterAnnounceLiveRegion from ../announce-live-region/index.js
//   LanguageSelectorComponent from ../status-lang/index.js
//   FooterLinksUseful from ../links-useful/index.js
//   FooterVersionEnv from ../version-env/index.js
//
// PROVIDES:
//   FooterComponentsRegistry constructor with prototype methods:
//   mountAll(), destroyAll(), announce(),
//   getComponent(), getAllComponents(), getVersion(),
//   getMetrics(), getManifest(), healthCheck(), info(),
//   setDebug()
//   VERSION, MODULE_ID, SLOTS, injectPorts(), getPorts()
//
// RECEIVES (via mountAll):
//   container — DOM Element
//   config.dock.items — icon dock configuration
//   config.buildDate — build metadata
//   integrations — external integrations object
//
// BROWSER APIs (legítimo):
//   document.createElement, container.querySelector — DOM mounting
//   fetch (via _fetchWithTimeout) — IP geolocation API, Server metrics API
// ═══════════════════════════════════════════════════════════════
// Footer Components Registry - Enterprise V9 P3-AAA
'use strict';
// @version 31.0.0-METRICS-API
// @changelog v31.0.0-METRICS-API - Replaced MetricsManager Ports dependency with direct polling to /api/metrics/server.php (30s interval)
// @changelog v30.0.0-STATUS-LAYOUT - Removed status-lang from TOP status area (kept only in BOTTOM controls to avoid duplication)
// @changelog v29.3.0-IMPORT-FIX - Fixed: Added missing IconsOrchestrator import
// @changelog v29.2.0-FIX - Fixed: Pass dock.items config to IconsOrchestrator.mountAll()
// @changelog v29.1.0-FIX - Fixed IconsOrchestrator usage (object singleton, not class)
// @changelog v29.0.0-ENTERPRISE - Removed window.MetricsPollingManager fallback, using only Ports (100% DI compliance)
// @changelog v28.0.0-ENTERPRISE - Replaced direct fetch with resilient pattern + fallback
...
export const VERSION = '31.0.0-METRICS-API';
export const MODULE_ID = 'footer.registry';
...
const _log = function(level: string, msg: string, data?: unknown) { var logger = _getPort('logger'); if (!logger) return; var prefix = '[' + MODULE_ID + ']'; if (level === 'error' && logger.error) logger.error(prefix, msg, data || ''); else if (level === 'warn' && logger.warn) logger.warn(prefix, msg, data || ''); else if (level === 'info' && logger.info) logger.info(prefix, msg, data || ''); else if (logger.debug) logger.debug(prefix, msg, data || ''); };

const METRICS_API_URL = '/api/metrics/server.php';
const METRICS_POLL_INTERVAL = 30000;

const _metrics = { ipFetchAttempts: 0, ipFetchSuccess: 0, ipFetchErrors: 0, metricsPolls: 0, metricsPollErrors: 0, metricsSource: (null as unknown|null) };

function _fetchWithTimeout(url: string, options?: RequestInit, timeout?: number) {
...
function FooterComponentsRegistry(this: any) { this._mounted = false; this._mountedAt = null; this._cleanupFns = []; this._intervals = []; this._sessionStart = Date.now(); this._ipData = null; this._connectionStatus = null; this._cpuCircle = null; this._memoryCircle = null; this._diskCircle = null; this._metricsInterval = null; this._brandAnimation = null; this._buttonsMounted = false; this._iconsMounted = false; this._announceRegion = null; this._statusLang = null; this._linksUseful = null; this._versionEnv = null; this._metaContainer = null; this._config = null; }

FooterComponentsRegistry.prototype.mountAll = function(container: HTMLElement|null, config: Record<string,unknown>, integrations: Record<string,unknown>) {
  let self = this;
  config = config || {};
...
FooterComponentsRegistry.prototype._mountConnectionStatus = function(container: HTMLElement|null) {
  const wrapper = document.createElement('div');
  wrapper.className = 'dsd-footer__status-connection';
  wrapper.setAttribute('data-component', 'connection-status');
  container!.appendChild(wrapper);
  this._connectionStatus = (createConnectionStatus as any)();
  this._connectionStatus.mount(wrapper);
  _log('info', 'Connection status mounted');
};

FooterComponentsRegistry.prototype._mountProgressCircles = function(container: HTMLElement|null) {
...
// v31.0.0: Direct API polling replaces MetricsManager Ports dependency
FooterComponentsRegistry.prototype._startMetricsPolling = function() {
  let self = this;

  const fetchMetrics = function() {
    _metrics.metricsPolls++;
    _fetchWithTimeout(METRICS_API_URL, { method: 'GET', headers: { 'Accept': 'application/json' } }, 5000)
      ...
      .then(function(json) {
        ...
        if (data) {
          if (data.cpu !== undefined && self._cpuCircle) self._cpuCircle.setValue(data.cpu);
          if (data.memory !== undefined && self._memoryCircle) self._memoryCircle.setValue(data.memory);
          if (data.disk !== undefined && self._diskCircle) self._diskCircle.setValue(data.disk);
  ...
  this._metricsInterval = setInterval(function() { if (document.hidden) return; fetchMetrics(); }, METRICS_POLL_INTERVAL);
  this._intervals.push(this._metricsInterval);

  _log('info', 'Metrics polling started (' + (METRICS_POLL_INTERVAL / 1000) + 's interval)');
};

FooterComponentsRegistry.prototype._mountDecorativeIcons = function(container: HTMLElement|null, config: Record<string,unknown>) {
  let self = this;
  ...
  if (config.dock && config.dock.items && Array.isArray(config.dock.items)) {
    // @ts-expect-error TS migration - TS2339
    dockItems = config.dock.items.filter(function(item: Record<string,unknown>) {
  ...
  // @ts-expect-error TS migration - TS2339
  _log('info', 'Mounting decorative icons', { count: dockItems.length, items: dockItems.map(function(i: unknown) { return i.id; }) });
  ...
  return Promise.resolve().then(function() {
    ...
    _log('info', 'Icons mounted', { mounted: mounted, failed: failed });
  }).catch(function(err) {
    _log('error', 'Failed to mount icons', err.message);
  });
};

FooterComponentsRegistry.prototype._mountButtons = function(container: HTMLElement|null) {
  let self = this;
...
FooterComponentsRegistry.prototype._mountBrandLottie = function(container: HTMLElement|null) {
  let self = this;
...
FooterComponentsRegistry.prototype._mountAnnounceRegion = function(container: HTMLElement, integrations?: Record<string, unknown>) {
  let self = this;
  return Promise.resolve().then(function() {
    self._announceRegion = new (FooterAnnounceLiveRegion as any)();
...
FooterComponentsRegistry.prototype._mountStatusLang = function(container: HTMLElement, integrations?: Record<string, unknown>) {
  let self = this;
  return Promise.resolve().then(function() {
    ...
    self._statusLang = (new (LanguageSelectorComponent as unknown as { new(..._args: unknown[]): {[k:string]:Function} })({ container: langContainer }));
...
FooterComponentsRegistry.prototype._mountLinksUseful = function(container: HTMLElement, integrations?: Record<string, unknown>) {
  let self = this;
  return Promise.resolve().then(function() {
    self._linksUseful = new (FooterLinksUseful as any)();
...
FooterComponentsRegistry.prototype._mountVersionEnv = function(container: HTMLElement, integrations?: Record<string, unknown>) {
  let self = this;
  return Promise.resolve().then(function() {
    ...
    self._versionEnv = (new (FooterVersionEnv as unknown as { new(..._args: unknown[]): {[k:string]:Function} })());
...
FooterComponentsRegistry.prototype._mountSessionTimer = function(container: HTMLElement|null) {
  let self = this;
  ...
  const sessionInterval = setInterval(function() {
    ...
    const timeStr = [hours, mins, secs].map(function(n) { return String(n).padStart(2, '0'); }).join(':');
...
FooterComponentsRegistry.prototype._mountMetaInfo = function(container: HTMLElement|null, config: Record<string,unknown>) {
  const buildEl = container!.querySelector('[data-value="build"]');
  if (buildEl) buildEl.textContent = 'Build: ' + (config.buildDate || new Date().toISOString().split('T')[0]);
  this._fetchIpInfo(container);
};

FooterComponentsRegistry.prototype._fetchIpInfo = function(container: HTMLElement|null) {
  const self = this;
  ...
  _fetchWithTimeout('https://ipapi.co/json/', { method: 'GET', headers: { 'Accept': 'application/json' } }, 5000)
    .then(function(response) {
      if ((response as any).ok) return (response as any).json();
      throw new Error('Response not OK: ' + (response as any).status);
    })
...
FooterComponentsRegistry.prototype.destroyAll = function() {
  ...
  if (this._announceRegion && this._announceRegion.destroy) { this._announceRegion.destroy(); this._announceRegion = null; }
  if (this._statusLang && this._statusLang.destroy) { this._statusLang.destroy(); this._statusLang = null; }
  if (this._metricsInterval) { clearInterval(this._metricsInterval); this._metricsInterval = null; }
  if (this._brandAnimation) { Animacoes.destroyAnimation('cards'); this._brandAnimation = null; }
  if (this._connectionStatus) { this._connectionStatus.unmount(); this._connectionStatus = null; }
  if (this._cpuCircle) { this._cpuCircle.unmount(); this._cpuCircle = null; }
  if (this._memoryCircle) { this._memoryCircle.unmount(); this._memoryCircle = null; }
  if (this._diskCircle) { this._diskCircle.unmount(); this._diskCircle = null; }
  if (this._versionEnv && this._versionEnv.destroy) { this._versionEnv.destroy(); this._versionEnv = null; }
  if (this._linksUseful && this._linksUseful.destroy) { this._linksUseful.destroy(); this._linksUseful = null; }
  let i;
...
FooterComponentsRegistry.prototype.announce = function(message: string) { if (this._announceRegion) this._announceRegion.announce(message); };

FooterComponentsRegistry.prototype.getComponent = function(name: string) {
  const map = {
  ...
  return (map as Record<string,unknown>)[name as string] || null;
};
...
FooterComponentsRegistry.prototype.getAllComponents = function() {
  ...
  return all.filter(function(c) { return c.instance; });
};

FooterComponentsRegistry.prototype.getVersion = function() { return VERSION; };
...
FooterComponentsRegistry.prototype.getManifest = function() {
  ...
  return {
    ...
    // @ts-expect-error TS migration - TS2339
    items: components.map(function(c: unknown) { return c.name; }),
    mounted: this._mounted,
...
FooterComponentsRegistry.prototype.info = function() {
  return {
    ...
    mounted: this._mounted,
    // @ts-expect-error TS migration - TS2339
    components: this.getAllComponents().map(function(c: unknown) { return c.name; }),
    metrics: this.getMetrics(),
    p23Governance: true,
...
FooterComponentsRegistry.prototype.setDebug = function(enabled: boolean) { };

FooterComponentsRegistry.VERSION = VERSION;
FooterComponentsRegistry.SLOTS = SLOTS;

export default FooterComponentsRegistry;
