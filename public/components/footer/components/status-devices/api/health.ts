...
// ═══════════════════════════════════════════════════════════════
// MODULE: footer/components/status-devices/api/health
// PURPOSE: Status Devices - Health API (Enterprise)
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'footer/components/status-devices/api/health';
...
export class HealthAPI {
  ...
  constructor(options: { baseURL?: string } = {}) { this.fetch = new FetchAdapter({ baseURL: options.baseURL || '/api' }); this._metrics = { checkCount: 0, successCount: 0, errorCount: 0, lastCheckAt: null }; }
  async checkHealth() { this._metrics.checkCount++; this._metrics.lastCheckAt = Date.now(); try { const response = await this.fetch.get('/health/check.php'); this._metrics.successCount++; return { ok: response.ok || false }; } catch (error) { this._metrics.errorCount++; return { ok: false }; } }
  healthCheck() { const checks = { fetchReady: !!this.fetch, goodSuccessRate: this._metrics.checkCount === 0 || (this._metrics.successCount / this._metrics.checkCount) > 0.5 }; const passed = Object.values(checks).filter(Boolean).length; return { status: passed === 2 ? 'HEALTHY' : 'DEGRADED', score: passed, maxScore: 2, scoreDisplay: `${passed}/2`, checks, version: VERSION, moduleId: MODULE_ID, timestamp: new Date().toISOString() }; }
