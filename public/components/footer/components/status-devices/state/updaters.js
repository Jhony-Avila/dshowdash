...
const MODULE_ID = "footer/components/status-devices/state/updaters";
let _debug = false;
let _logBuffer = [];
...
class StateUpdaters {
  ...
  // @ts-expect-error TS migration - TS2698
  static setWebcamActive(currentState, active) {
    return { ...currentState, webcamActive: active, lastUpdate: Date.now() };
  }
  // @ts-expect-error TS migration - TS2698
  static setMicActive(currentState, active) {
    return { ...currentState, micActive: active, lastUpdate: Date.now() };
  }
  static healthCheck() {
