...
const MODULE_ID = "footer/components/status-devices/state/validators";
let _debug = false;
let _logBuffer = [];
...
class StateValidators {
  ...
  static validateDeviceStatus(status) {
    if (typeof status !== "boolean") throw new Error("Invalid device status");
    return true;
  }
  static healthCheck() {
