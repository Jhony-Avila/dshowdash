...
const MODULE_ID = "footer/components/status-lang/state/updaters";
let _debug = false;
let _logBuffer = [];
...
class StateUpdaters {
  ...
  // @ts-expect-error TS migration - TS2698
  static setLanguage(currentState, language) {
    return { ...currentState, currentLanguage: language, lastUpdate: Date.now() };
  }
  static healthCheck() {
