...
// ═══════════════════════════════════════════════════════════════
// MODULE: UNKNOWN
// PURPOSE: SHARED: Header Panel PollingManager Factory
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
//   createModulePollingManager() — exported function
//
...
export function createModulePollingManager(moduleId: string) {
  const MODULE_ID = moduleId;

  const _metrics = { starts: 0, stops: 0, ticks: 0, errors: 0, lastTickAt: (null as unknown|null) };

  class PollingManager {
  [key: string]: any;
    constructor(options: { interval?: number; callback?: Function } = {}) {
    ...
    setInterval(ms) { this.interval = ms; if (this._isRunning) { this.stop(); this.start(); } }
    isRunning() { return this._isRunning; }
    getTickCount() { return this._tickCount; }
    resetTickCount() { this._tickCount = 0; }
    ...
    info() { return { version: VERSION, moduleId: MODULE_ID, isRunning: this._isRunning, interval: this.interval, tickCount: this._tickCount, metrics: { ..._metrics } }; }
  }

  function getMetrics() { return { ..._metrics }; }
  function resetMetrics() { _metrics.starts = 0; _metrics.stops = 0; _metrics.ticks = 0; _metrics.errors = 0; _metrics.lastTickAt = null; }
