...
const VERSION = "8.8.0-P1-UARPS";
const id = "errors-status";
const capabilities = { type: "indicator", reorderable: false, hideable: false, critical: true, rendersUI: true };
const MODULE_ID = "header/components/errors-status";
...
(function loadCSS() {
  const cssPath = "/components/header/components/errors-status/component.css";
...
const emitUIAction = (data = {}) => {
  ...
  eventBus.emit(UI_EVENTS.ACTION, { actionId: "header:errors-status", source: MODULE_ID, timestamp: Date.now(), kind: "ui", meta: Object.assign({ label: "Erros do Sistema" }, data) });
};
...
function ErrorsStatusComponent(options) {
  options = options || {};
  ...
  this.logger = new Logger({ prefix: "[Errors]", debug: cfg && cfg.app && cfg.app.debug });
...
ErrorsStatusComponent.prototype.render = function() {
  ...
  this.element.addEventListener("click", () => {
    ...
    emitUIAction({ clicked: true, value: self.store && self.store.getState ? self.store.getState().errorCount : 0 });
  });
...
ErrorsStatusComponent.prototype.updateUI = function(state) {
  const badgeEl = this.element.querySelector(".errors-badge");
  if (state.errorCount !== null) {
    this.element.dataset.count = String(state.errorCount);
    this.element.dataset.status = state.errorCount > 0 ? "has-errors" : "ok";
    badgeEl.textContent = state.errorCount > 99 ? "99+" : state.errorCount;
  } else {
    this.element.dataset.count = "0";
...
ErrorsStatusComponent.prototype.fetchErrors = function() {
  ...
  return this.api.fetchErrors().then((data) => {
    if (self.isDestroyed) return;
    self.store.setState({ errorCount: data.error_count || 0, status: "ok", lastUpdate: (/* @__PURE__ */ new Date()).toISOString() });
    const eb = _getPort("eventBus");
...
ErrorsStatusComponent.prototype.startPolling = function() {
  const self = this;
  if (this.isDestroyed) return;
  this.polling = new PollingCoordinator({ interval: 3e4 });
  this.polling.onPoll(() => self.fetchErrors());
  this.polling.start();
};
...
ErrorsStatusComponent.prototype.info = function() {
  return { version: VERSION, moduleId: MODULE_ID, id, capabilities, portsInitialized: Ports.isInitialized(), mounted: !!this.element, errorCount: this.store.getState().errorCount, metrics: this._metrics };
};
var errors_status_default = ErrorsStatusComponent;
export {
  ErrorsStatusComponent,
  MODULE_ID,
  VERSION,
  capabilities,
  errors_status_default as default,
  getPorts,
  id,
  injectPorts
};
