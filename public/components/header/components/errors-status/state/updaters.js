...
const MODULE_ID = "header/components/errors-status/state/updaters";
let _debug = false;
let _logBuffer = [];
...
class StateUpdaters {
  ...
  // @ts-expect-error TS migration - TS2698
  static updateErrorCount(currentState, count) {
    return { ...currentState, errorCount: count, lastUpdate: Date.now() };
  }
  // @ts-expect-error TS migration - TS2698, TS2339
  static incrementErrors(currentState) {
    return { ...currentState, errorCount: (currentState.errorCount || 0) + 1, lastUpdate: Date.now() };
  }
  static healthCheck() {
