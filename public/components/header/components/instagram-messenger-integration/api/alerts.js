...
const MODULE_ID = "header/components/instagram-messenger-integration/api/alerts";
let _debug = false;
let _logBuffer = [];
...
class AlertsAPI {
  ...
  async getAlerts() {
    ...
    try {
      ...
      const alerts = [];
      if (data.unread_count >= this.unreadThreshold) alerts.push({ type: "high_unread", message: `${data.unread_count} DMs n\xE3o lidas` });
