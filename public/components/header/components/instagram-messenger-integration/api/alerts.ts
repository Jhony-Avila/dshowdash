...
// ═══════════════════════════════════════════════════════════════
// MODULE: header/components/instagram-messenger-integration/api/alerts
// PURPOSE: Alerts API for monitoring unread Instagram DM thresholds
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
// ═══════════════════════════════════════════════════════════════
// Instagram Messenger Integration - Alerts API (Enterprise)
// @version 5.1.0-ENTERPRISE
'use strict';
export const VERSION = '5.1.0-ENTERPRISE';
export const MODULE_ID = 'header/components/instagram-messenger-integration/api/alerts';
...
export class AlertsAPI {
  ...
  async getAlerts() {
    ...
    try { const data = await this.api.fetchStatus(); this._metrics.successCount++; const alerts = []; if (data.unread_count >= this.unreadThreshold) alerts.push({ type: 'high_unread', message: `${data.unread_count} DMs não lidas` }); return { ok: true, alerts }; }
