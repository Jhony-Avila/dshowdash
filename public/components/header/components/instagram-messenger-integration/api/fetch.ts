...
// ═══════════════════════════════════════════════════════════════
// MODULE: header.instagram-messenger-integration.api.fetch
// PURPOSE: Boot-safe fetch API for Instagram integration with AbortError fallback
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   createUiPorts from /core/runtime/ports-profiles.js
// PROVIDES:
//   IntegrationAPI — Constructor for fetching Instagram API status
//   VERSION, MODULE_ID, getVersion() — Module metadata
//   injectPorts() — Inject port dependencies
//   getPorts() — Get current ports snapshot
// ═══════════════════════════════════════════════════════════════
// Instagram Messenger Integration - IntegrationAPI (Enterprise AAA)
// @version 6.1.0-ES6
...
const MODULE_ID = 'header.instagram-messenger-integration.api.fetch';
...
function _debug() {
  ...
  return config && config.app && config.app.debug ? true : false;
}
...
function IntegrationAPI(this: any, endpoint?: string) {
  this.endpoint = endpoint || '/api/integrations/instagram/status.php';
  this.timeout = 12000;
