...
// ═══════════════════════════════════════════════════════════════
// MODULE: header/components/instagram-messenger-integration
// PURPOSE: Main Instagram Messenger integration component with UARPS governance
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
// PROVIDES:
//   InstagramMessengerComponent() — Main component constructor (mount/render/unmount)
//   VERSION, id, capabilities — Component metadata
//   injectPorts() — Inject port dependencies
//   getPorts() — Get current ports snapshot
// ═══════════════════════════════════════════════════════════════
// Instagram Messenger - Enterprise Premium
...
export const id = 'instagram-messenger-integration';
export const capabilities = { type: 'integration', reorderable: true, hideable: true, critical: false, rendersUI: true };
export const MODULE_ID = 'header/components/instagram-messenger-integration';
...
const emitUIAction = (data = {}) => { const eventBus = _getPort('eventBus'); if (!eventBus || !eventBus.emit) return; eventBus.emit(UI_EVENTS.ACTION, { actionId: 'header:instagram-messenger-integration', source: MODULE_ID, timestamp: Date.now(), meta: Object.assign({ kind: 'ui' }, data) }); };

(function loadCSS() { const cssPath = '/components/header/components/instagram-messenger-integration/component.css'; if (!document.querySelector(`link[href="${cssPath}"]`)) { const link = document.createElement('link'); link.rel = 'stylesheet'; link.href = cssPath; document.head.appendChild(link); } })();
...
export function InstagramMessengerComponent(this: any, options: Record<string,unknown>) {
  options = options || {};
  this.container = options.container;
  this.store = new StateStore({ unreadCount: 0, status: 'loading' });
  this.lifecycle = new LifecycleManager(this);
  this.logger = (new (Logger as unknown as { new(..._args: unknown[]): {[k:string]:Function} })({ prefix: '[Instagram]', debug: false }));
...
InstagramMessengerComponent.prototype.updateUI = function(state: Record<string,unknown>) {
...
InstagramMessengerComponent.prototype.info = function() { return { version: VERSION, moduleId: MODULE_ID, id, capabilities, mounted: !!this.element, unreadCount: this.store.getState().unreadCount, portsInitialized: Ports.isInitialized(), metrics: this._metrics }; };

export default InstagramMessengerComponent;
