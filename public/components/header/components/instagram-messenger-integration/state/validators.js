...
const MODULE_ID = "header/components/instagram-messenger-integration/state/validators";
let _debug = false;
let _logBuffer = [];
...
class StateValidators {
  static validateCount(count) {
    if (count !== null && (typeof count !== "number" || count < 0)) throw new Error(`Invalid count: ${count}`);
    return true;
  }
