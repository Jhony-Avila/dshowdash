...
// ═══════════════════════════════════════════════════════════════
// MODULE: header/components/instagram-messenger-integration/utils/helpers
// PURPOSE: General helper utilities for Instagram integration (ID generation, formatting)
// ───────────────────────────────────────────────────────────────
// PROVIDES:
//   Helpers — Static class with generateId() and formatUsername()
//   setDebug() — Toggle debug mode
// ═══════════════════════════════════════════════════════════════
// Instagram Messenger Integration - Helper Utilities (Enterprise)
// @version 5.1.0-ENTERPRISE
// @changelog v5.1.0 - Removed console.* for enterprise compliance
'use strict';

export const VERSION = '5.1.0-ENTERPRISE';
export const MODULE_ID = 'header/components/instagram-messenger-integration/utils/helpers';
...
export class Helpers { [key: string]: any;
  static generateId(prefix = 'insta') { return `${prefix}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`; }
  // @ts-expect-error TS migration - TS2339
  static formatUsername(username: unknown) { return username ? `@${username.replace('@', '')}` : ''; }
  static healthCheck() {
