// ═══════════════════════════════════════════════════════════════
// DEPENDENCY CONTRACT (6.2.0-AAA)
// ═══════════════════════════════════════════════════════════════
// MODULE: header.notifications.api.fetch
// PURPOSE: Notifications - API Adapter (Enterprise AAA)
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   createUiPorts from /core/runtime/ports-profiles.js
//
// PROVIDES:
//   NotificationsAPI() — exported function
...
function NotificationsAPI(this: any, options?: { baseURL?: string; timeout?: number }) {
  if (!options) options = {};
...
NotificationsAPI.prototype.get = function(endpoint: string) {
  const self = this;
  ...
  return fetch(self.baseURL + endpoint, { signal: controller.signal, credentials: 'include' })
    .then(response => {
...
NotificationsAPI.prototype.getMetrics = function() { return Object.assign({}, this._metrics); };
NotificationsAPI.prototype.resetMetrics = function() { this._metrics = { requestCount: 0, successCount: 0, errorCount: 0, contractViolations: 0, authErrors: 0, lastRequestAt: null }; };

function getVersion() { return VERSION; }

export { NotificationsAPI, VERSION, MODULE_ID, getVersion, injectPorts, getPorts };
function destroy() { }
