...
const MODULE_ID = "header/components/notifications/state/updaters";
let _debug = false;
let _logBuffer = [];
...
class StateUpdaters {
  ...
  // @ts-expect-error TS migration - TS2698
  static setUnreadCount(currentState, count) {
    return { ...currentState, unreadCount: count, lastUpdate: Date.now() };
  }
  // @ts-expect-error TS migration - TS2698, TS2339
  static incrementUnread(currentState) {
    return { ...currentState, unreadCount: (currentState.unreadCount || 0) + 1, lastUpdate: Date.now() };
  }
  // @ts-expect-error TS migration - TS2698
  static clearUnread(currentState) {
    return { ...currentState, unreadCount: 0, lastUpdate: Date.now() };
  }
  static healthCheck() {
