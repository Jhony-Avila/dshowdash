...
// ═══════════════════════════════════════════════════════════════
// MODULE: header-panel-loja-integrada-state-store
// PURPOSE: Reactive state store with subscribe/notify pattern
// ───────────────────────────────────────────────────────────────
// PROVIDES:
//   StateStore — class with getState(), setState(), subscribe(), healthCheck()
...
export const MODULE_ID = 'header-panel-loja-integrada-state-store';
...
export class StateStore {
  [key: string]: any;
  constructor(initial = {}) { this._state = { ...initial }; this._listeners = new Set(); }
...
export function healthCheck() { return { status: (_state?._initialized !== false) ? 'HEALTHY' : 'DEGRADED', version: VERSION, moduleId: MODULE_ID, checks: { storeReady: true } }; }
