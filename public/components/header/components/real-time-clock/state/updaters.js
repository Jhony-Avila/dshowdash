...
const MODULE_ID = "header/components/real-time-clock/state/updaters";
let _debug = false;
let _logBuffer = [];
...
class StateUpdaters {
  ...
  // @ts-expect-error TS migration - TS2698
  static updateTime(currentState, time) {
    return { ...currentState, currentTime: time, lastUpdate: Date.now() };
  }
  static healthCheck() {
