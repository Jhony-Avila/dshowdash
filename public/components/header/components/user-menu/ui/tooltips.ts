...
// ═══════════════════════════════════════════════════════════════
// MODULE: header/components/user-menu/ui/tooltips
// PURPOSE: User Menu - Tooltip Manager (Enterprise)
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'header/components/user-menu/ui/tooltips';
...
TooltipManager.prototype._show = function(element: HTMLElement|null, text: string) { const self = this; this._hide(); this.timeoutId = setTimeout(() => { const el = document.createElement('div'); el.className = 'user-menu-tooltip'; el.textContent = text; el.style.position = 'fixed'; el.style.pointerEvents = 'none'; getTooltipRoot().appendChild(el); const rect = element!.getBoundingClientRect(); el.style.left = `${rect.left + rect.width / 2}px`; el.style.top = `${rect.top - 30}px`; el.style.transform = 'translateX(-50%)'; self.activeTooltip = el; self._metrics.showCount++; self._metrics.lastShowAt = Date.now(); }, this.delay); };
