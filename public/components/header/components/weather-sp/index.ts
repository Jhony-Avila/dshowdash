// =============================================================
// DEPENDENCY CONTRACT (v9.7.0-P1-UARPS)
// =============================================================
// MODULE: header/components/weather-sp
// PURPOSE: Main weather SP component with Ports, polling, lifecycle, and event bus
// -------------------------------------------------------------
// IMPORTS:
...
// PROVIDES:
//   WeatherSpComponent (constructor)
//   VERSION, id, capabilities
//   injectPorts(p), getPorts()
// EVENTS:
//   EMITS UI_EVENTS.ACTION on click interaction
//   EMITS COMPONENT_EVENTS.MOUNTED on mount
//   EMITS COMPONENT_EVENTS.DATA_UPDATED on weather fetch
//   EMITS COMPONENT_EVENTS.UNMOUNTED on unmount
// =============================================================
// Weather SP Component - Enterprise Premium
// @version 9.7.0-P1-UARPS
// @changelog v9.7.0-P1-UARPS - Added data-uarps-trigger for UARPS governance
// @changelog v9.6.0-ENTERPRISE - ES6 conversion, removed duplicate getVersion export
...
const emitUIAction = (data = {}) => { const eventBus = _getPort('eventBus'); if (!eventBus || !eventBus.emit) return; eventBus.emit(UI_EVENTS.ACTION, { actionId: 'header:weather-sp', source: MODULE_ID, timestamp: Date.now(), meta: Object.assign({ kind: 'ui' }, data) }); };

(function loadCSS() { const cssPath = '/components/header/components/weather-sp/component.css'; if (!document.querySelector(`link[href="${cssPath}"]`)) { const link = document.createElement('link'); link.rel = 'stylesheet'; link.href = cssPath; document.head.appendChild(link); } })();
...
const getWeatherIcon = (code: unknown, isDay = true) => { if (code === 0) return isDay ? ICONS.sun : ICONS.moon; if (code >= 1 && code <= 48) return ICONS.cloud; return ICONS.cloudRain; };

export function WeatherSpComponent(this: any, options: Record<string,unknown>) {
  options = options || {};
  ...
  this.logger = (new (Logger as unknown as { new(..._args: unknown[]): {[k:string]:Function} })({ prefix: '[Weather]', debug: cfg && cfg.app && cfg.app.debug }));
  this.telemetry = new TelemetryTracker();
  this.api = (new (WeatherAPI as unknown as { new(..._args: unknown[]): {[k:string]:Function} })());
  this.element = null;
...
WeatherSpComponent.prototype.render = function() {
  ...
  this.element.title = 'Clima São Paulo';
  this.element.dataset.status = 'loading';
  this.element.setAttribute('data-uarps-trigger', 'trigger:header:open-weather-sp');
  this.element.innerHTML = `<div class="weather-icon">${ICONS.sun}</div><span class="weather-temp">--°C</span>`;
  if (this.container) this.container.appendChild(this.element);
  this.element.style.cursor = 'pointer';
  this.element.addEventListener('click', () => { emitUIAction({ clicked: true, value: self.store && self.store.getState ? self.store.getState().temp : null }); });
...
WeatherSpComponent.prototype.updateUI = function(state: Record<string,unknown>) {
  const iconEl = this.element.querySelector('.weather-icon');
  const tempEl = this.element.querySelector('.weather-temp');
  // @ts-expect-error TS migration - TS2345
  if (state.temp !== null) { iconEl.innerHTML = getWeatherIcon(state.code, state.isDay); tempEl.textContent = `${Math.round(state.temp)}°C`; this.element.dataset.status = 'ok'; }
  else { tempEl.textContent = '--°C'; this.element.dataset.status = 'offline'; }
};
...
WeatherSpComponent.prototype.fetchWeather = function() {
  ...
  return this.api.fetchWeather().then((data: Record<string,unknown>) => {
    if (self.isDestroyed) return;
    self.store.setState({ temp: data.temperature, code: data.weathercode || 0, isDay: data.is_day !== 0, status: 'ok', lastUpdate: new Date().toISOString() });
    const eb = _getPort('eventBus');
...
WeatherSpComponent.prototype.startPolling = function() { const self = this; if (this.isDestroyed) return; if (this.polling) { this.polling.stop(); this.polling = null; } this.polling = new PollingCoordinator({ interval: 300000 }); this.polling.onPoll(() => self.fetchWeather()); this.polling.start(); };
...
WeatherSpComponent.prototype.info = function() { return { version: VERSION, moduleId: MODULE_ID, id, capabilities, portsInitialized: Ports.isInitialized(), mounted: !!this.element, temp: this.store.getState().temp, metrics: this._metrics }; };

export default WeatherSpComponent;
