...
// ═══════════════════════════════════════════════════════════════
// MODULE: header/components/wechat-integration
// PURPOSE: Main WeChat integration component with UARPS governance
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
const emitUIAction = (data = {}) => { const eventBus = _getPort('eventBus'); if (!eventBus || !eventBus.emit) return; eventBus.emit(UI_EVENTS.ACTION, { actionId: 'header:wechat-integration', source: MODULE_ID, timestamp: Date.now(), meta: Object.assign({ kind: 'ui' }, data) }); };

(function loadCSS() { const cssPath = '/components/header/components/wechat-integration/component.css'; if (!document.querySelector(`link[href="${cssPath}"]`)) { const link = document.createElement('link'); link.rel = 'stylesheet'; link.href = cssPath; document.head.appendChild(link); } })();
...
export function WeChatIntegrationComponent(this: any, options: Record<string,unknown>) {
  options = options || {};
  this.container = options.container;
  this.store = new StateStore({ unreadCount: 0, status: 'loading' });
  this.lifecycle = new LifecycleManager(this);
  this.logger = (new (Logger as unknown as { new(..._args: unknown[]): {[k:string]:Function} })({ prefix: '[WeChat]', debug: false }));
...
WeChatIntegrationComponent.prototype.updateUI = function(state: Record<string,unknown>) {
...
WeChatIntegrationComponent.prototype.info = function() { return { version: VERSION, moduleId: MODULE_ID, id, capabilities, mounted: !!this.element, unreadCount: this.store.getState().unreadCount, portsInitialized: Ports.isInitialized(), metrics: this._metrics }; };

export default WeChatIntegrationComponent;
