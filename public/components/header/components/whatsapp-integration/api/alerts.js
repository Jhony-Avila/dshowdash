...
const MODULE_ID = "header/components/whatsapp-integration/api/alerts";
let _debug = false;
let _logBuffer = [];
...
class AlertsAPI {
  ...
  async getAlerts() {
    ...
    try {
      ...
      const alerts = [];
      if (data.unread_count >= this.unreadThreshold) alerts.push({ type: "high_unread", message: `${data.unread_count} mensagens WhatsApp n\xE3o lidas` });
