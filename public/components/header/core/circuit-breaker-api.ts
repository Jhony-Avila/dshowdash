...
// ═══════════════════════════════════════════════════════════════
// MODULE: header/core/circuit-breaker-api
// PURPOSE: Circuit breaker por componente para chamadas de API
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   createCorePorts from /core/runtime/ports-profiles.js
//   CIRCUIT_STATE from ./constants.js
// PROVIDES:
//   CircuitBreakerAPI(name, opts) — constructor do breaker por componente
//     .execute(fn, fallback) — executa com protecao do breaker
//     .canExecute() — verifica se breaker permite execucao
//     .recordSuccess() / .recordFailure(err) — registra resultado
//     .reset() / .forceOpen() / .forceClose() — controle de estado
//     .onStateChange(cb) — listener de mudanca de estado
//     .getStatus() / .getMetrics() — consulta estado e metricas
//   getBreaker(name, opts) — obtem ou cria breaker para componente
//   hasBreaker(name) / removeBreaker(name) — gestao de breakers
//   getAllBreakers() / getOpenBreakers() — consulta breakers
//   resetAll() — reseta todos os breakers
//   healthCheck() — status de saude do modulo
//   info() — informacoes completas do modulo
//   injectPorts(p) / getPorts() — gestao de ports
// EVENTS EMITTED:
//   header:circuit-breaker:state-change — quando estado muda
// ═══════════════════════════════════════════════════════════════
// Header - Circuit Breaker API (Per-Component)
// @version 1.2.0-ES6
// @changelog v1.2.0-ES6 - Task 10.1 B11: var → const/let
// @changelog v1.1.0 - healthCheck ajustado para não penalizar quando não há breakers
'use strict';
...
export const MODULE_ID = 'header/core/circuit-breaker-api';
...
const DEFAULT_OPTIONS = { maxFailures: 3, resetTimeout: 60000, halfOpenMaxAttempts: 2, successThreshold: 2, timeout: 5000 };

function CircuitBreakerAPI(this: any, componentName: string, options: Record<string,unknown>) {
  options = options || {};
...
CircuitBreakerAPI.prototype._setState = function(newState: Record<string,unknown>) {
...
CircuitBreakerAPI.prototype.execute = function(fn: Function, fallback: unknown) {
  const self = this;
  ...
  return self._executeWithTimeout(fn).then((result: unknown) => {
    self.recordSuccess();
    return result;
  }).catch((error: unknown) => {
...
CircuitBreakerAPI.prototype._executeWithTimeout = function(fn: Function) {
  const self = this;
  return new Promise((resolve, reject) => {
    const timeoutId = setTimeout(() => {
      self._metrics.timeouts++;
      reject(new Error(`Timeout após ${self.timeout}ms`));
    }, self.timeout);
...
CircuitBreakerAPI.prototype.recordFailure = function(error: unknown) {
  ...
  if (this.state === CIRCUIT_STATE.HALF_OPEN) {
    ...
    if (this.failures >= this.maxFailures) {
      this._setState(CIRCUIT_STATE.OPEN);
      _log('warn', 'CircuitBreaker ABERTO após', this.failures, 'falhas:', this.componentName);
    }
  }
...
CircuitBreakerAPI.prototype.forceOpen = function() { this._setState(CIRCUIT_STATE.OPEN); this.lastFailureTime = Date.now(); };
CircuitBreakerAPI.prototype.forceClose = function() { this._setState(CIRCUIT_STATE.CLOSED); this.failures = 0; };

CircuitBreakerAPI.prototype.onStateChange = function(callback: Function) {
  if (typeof callback !== 'function') return () => {};
  this._listeners.push(callback);
  const self = this;
  return () => { const idx = self._listeners.indexOf(callback); if (idx > -1) self._listeners.splice(idx, 1); };
};

CircuitBreakerAPI.prototype._notifyListeners = function(event: string) {
  this._listeners.forEach((cb: Function) => { try { cb(event); } catch (e: any) { _log('error', 'Listener error:', e.message); } });
};
...
CircuitBreakerAPI.prototype.getMetrics = function() { return Object.assign({}, this._metrics, this.getStatus()); };

function getBreaker(componentName: string, options: Record<string,unknown>) {
  _initPorts();
  if (_breakers.has(componentName)) return _breakers.get(componentName);
  const breaker = (new (CircuitBreakerAPI as unknown as { new(..._args: unknown[]): {[k:string]:Function} })(componentName, options));
  _breakers.set(componentName, breaker);
  _globalMetrics.totalCreated++;
  return breaker;
}

function hasBreaker(componentName: string) { return _breakers.has(componentName); }
function removeBreaker(componentName: string) { return _breakers.delete(componentName); }
function getAllBreakers() { const result = {}; _breakers.forEach((breaker, name) => { (result as Record<string,unknown>)[name as string] = breaker.getStatus(); }); return result; }
function getOpenBreakers() { const open: any[] = []; _breakers.forEach((breaker, name) => { if (breaker.state === CIRCUIT_STATE.OPEN) open.push(name); }); return open; }
function resetAll() { _breakers.forEach(breaker => { breaker.reset(); }); _log('info', 'Todos os CircuitBreakers resetados'); }
function getGlobalMetrics() { return Object.assign({}, _globalMetrics, { activeBreakers: _breakers.size, openBreakers: getOpenBreakers().length }); }
...
function info() { return { version: VERSION, moduleId: MODULE_ID, totalBreakers: _breakers.size, breakers: getAllBreakers(), globalMetrics: getGlobalMetrics(), portsInitialized: _portsInitialized, healthCheck: healthCheck() }; }

export { CircuitBreakerAPI, getBreaker, hasBreaker, removeBreaker, getAllBreakers, getOpenBreakers, resetAll, getGlobalMetrics, healthCheck, info, DEFAULT_OPTIONS, CIRCUIT_STATE };

export default { VERSION, MODULE_ID, CircuitBreakerAPI, getBreaker, hasBreaker, removeBreaker, getAllBreakers, getOpenBreakers, resetAll, getGlobalMetrics, healthCheck, info };
