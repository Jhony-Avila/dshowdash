...
// ═══════════════════════════════════════════════════════════════
// MODULE: header/core/component-fallback-ui
// PURPOSE: UI de fallback para componentes críticos com falha no Header
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   escapeHtml from ../utils/html-helpers.js
// PROVIDES:
//   createFallback(name, container, error, options) — cria UI fallback
//   removeFallback(name, animate) — remove fallback
//   removeAllFallbacks(animate) — remove todos os fallbacks
//   hasFallback(name) — verifica se fallback existe
//   getActiveFallbacks() — lista fallbacks ativos
//   getMetrics() — métricas
//   resetMetrics() — reseta métricas
//   healthCheck() — auto health check
//   info() — informações do módulo
// WINDOW ACCESS:
//   document.getElementById — verificar styles carregados
//   document.createElement — criar elementos de fallback
//   document.head.appendChild — injetar CSS de fallback
// ═══════════════════════════════════════════════════════════════

// Component Fallback UI
// @version 1.1.0-ES6
// @changelog v1.1.0-ES6 - Task 10.1 B11: var → const/let
// UI de fallback para componentes criticos do Header
'use strict';

import { escapeHtml } from '../utils/html-helpers.js';

export const VERSION = '1.1.0-ES6';
export const MODULE_ID = 'header/core/component-fallback-ui';
...
const _activeFallbacks = new Map();

function _createFallbackHTML(componentName: string, errorMessage: string, options: Record<string,unknown>) {
  options = options || {};
  const showRetry = options.showRetry !== false;
  const showError = options.showError !== false;
  const compact = options.compact || false;
  
  const iconKey = (COMPONENT_ICONS as Record<string,unknown>)[componentName as string] || 'generic';
  const icon = (ICONS as Record<string,unknown>)[iconKey as string] || ICONS.generic;
  const label = (COMPONENT_LABELS as Record<string,unknown>)[componentName as string] || componentName;
...
function _ensureStyles() {
  ...
  const css = '.header-component-fallback{display:inline-flex;align-items:center;gap:6px;padding:4px 8px;' +
    'background:var(--color-warning-bg,#fff3cd);border:1px solid var(--color-warning-border,#ffc107);' +
    'border-radius:4px;font-size:12px;color:var(--color-warning-text,#856404);max-width:150px;' +
    'animation:hcf-fade-in 0.3s ease}' +
    '.header-component-fallback .hcf-icon{flex-shrink:0;opacity:0.7}' +
    '.header-component-fallback .hcf-content{display:flex;flex-direction:column;gap:1px;overflow:hidden}' +
    '.header-component-fallback .hcf-label{font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}' +
    '.header-component-fallback .hcf-error{font-size:10px;opacity:0.8;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}' +
    '.header-component-fallback .hcf-retry{display:flex;align-items:center;justify-content:center;' +
    'width:24px;height:24px;padding:0;border:none;background:transparent;cursor:pointer;' +
    'border-radius:50%;transition:background 0.2s}' +
    '.header-component-fallback .hcf-retry:hover{background:rgba(0,0,0,0.1)}' +
    '.header-component-fallback .hcf-retry:focus{outline:2px solid var(--color-focus,#0066cc);outline-offset:2px}' +
    '.header-component-fallback.hcf-removing{animation:hcf-fade-out 0.3s ease forwards}' +
    '@keyframes hcf-fade-in{from{opacity:0;transform:scale(0.9)}to{opacity:1;transform:scale(1)}}' +
    '@keyframes hcf-fade-out{from{opacity:1;transform:scale(1)}to{opacity:0;transform:scale(0.9)}}' +
    '@media(max-width:768px){.header-component-fallback{padding:2px 4px}.header-component-fallback .hcf-content{display:none}}';
...
export function createFallback(componentName: string, container: HTMLElement|null, errorMessage: string, options: Record<string,unknown>) {
  options = options || {};
...
export function removeFallback(componentName: string, animate?: boolean) {
...
export function removeAllFallbacks(animate: unknown) {
...
export function hasFallback(componentName: string) {
...
export function getMetrics() { return Object.assign({}, _metrics, { activeFallbacks: _activeFallbacks.size }); }
export function resetMetrics() { _metrics.fallbacksCreated = 0; _metrics.fallbacksRemoved = 0; _metrics.retryClicks = 0; }
