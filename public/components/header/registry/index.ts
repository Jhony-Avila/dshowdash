...
// ═══════════════════════════════════════════════════════════════
// DEPENDENCY CONTRACT (v4.1.0-ES6)
// ═══════════════════════════════════════════════════════════════
// Header Registry
// @version 4.1.0-ES6
// @changelog v4.1.0-ES6 - Task 10.1 B13: var → const/let
// @changelog v4.0.0-PORTSFACTORY - Migração para PortsFactory/PortsProfiles
'use strict';
...
const BASE_COMPONENTS = [
  { component_key: 'logo', component_type: 'brand', label: 'Logo', order_index: 0, group_position: 'left', uarps_trigger_id: (null as unknown|null), is_active: 1 },
...
let _registry = [];
let _loaded = false;
let _userPermissions: unknown = null;
const _loadedComponents = new Map();
let _orderSource = 'fallback';
const _metrics = { apiCalls: 0, cacheHits: 0, cacheMisses: 0, lastLoadAt: (null as unknown|null) };
...
function hasAccess(component: Record<string,unknown>) {
  const triggerId = component.uarps_trigger_id;
  if (!triggerId) return true;
  if (!_userPermissions) return false;
  
  // @ts-expect-error TS migration - TS2339
  const permission = _userPermissions.triggers.find((t: unknown) => t.trigger_id === triggerId);
...
function setComponentLoaded(key: string, loaded: unknown, instance: Record<string,unknown>) {
...
function register(component: Record<string,unknown>) {
...
export { load, loadFromAPI, loadUserPermissions, getOrderedComponents, getOrderInfo, setComponentLoaded, invalidateCache, getComponentsForUser, getAllComponents, getComponent, register, unregister, refresh, hasAccess, getMetrics, resetMetrics, healthCheck, info, VERSION, MODULE_ID };

export default { load, loadFromAPI, loadUserPermissions, getOrderedComponents, getOrderInfo, setComponentLoaded, invalidateCache, getComponentsForUser, getAllComponents, getComponent, register, unregister, refresh, hasAccess, getMetrics, resetMetrics, healthCheck, info, VERSION, MODULE_ID };
