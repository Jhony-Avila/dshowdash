...
const _log = (level, msg) => {
  ...
  if (!logger) return;
  if (level === "error") {
    if (logger.error) logger.error(`[${MODULE_ID}]`, msg);
    return;
  }
  if (level === "warn") {
    if (logger.warn) logger.warn(`[${MODULE_ID}]`, msg);
    return;
  }
  if (_debug && logger.debug) logger.debug(`[${MODULE_ID}]`, msg);
};
const BreakpointManager = {
  ...
  configure(customBreakpoints) {
    this.breakpoints = Object.assign({}, defaultBreakpoints, customBreakpoints);
    _metrics.configureCount++;
    _log("info", "Breakpoints configured");
  },
  reset() {
    this.breakpoints = Object.assign({}, defaultBreakpoints);
    _metrics.resetCount++;
    _log("info", "Breakpoints reset to defaults");
  },
  ...
  getAll() {
    return Object.assign({}, this.breakpoints);
  },
  // @ts-expect-error strict migration — TS7053
  detect(width) {
    _metrics.detectCount++;
    for (let i = 0; i < breakpointOrder.length; i++) {
      const name = breakpointOrder[i];
      const bp = this.breakpoints[name];
      if (bp && width >= bp.min && width <= bp.max) return name;
    }
    return "xl";
  },
  isMobile(breakpoint) {
  ...
  isTablet(breakpoint) {
    return TABLET_BREAKPOINTS.indexOf(breakpoint) !== -1;
  },
  ...
  isAtMost(current, target) {
    const currentIndex = breakpointOrder.indexOf(current);
    const targetIndex = breakpointOrder.indexOf(target);
    return currentIndex <= targetIndex;
  },
  // @ts-expect-error strict migration — TS7053
  getMediaQuery(breakpointName) {
    const bp = this.breakpoints[breakpointName];
    if (!bp) return "";
    if (bp.max === Infinity) return `(min-width: ${bp.min}px)`;
    return `(min-width: ${bp.min}px) and (max-width: ${bp.max}px)`;
  },
  // @ts-expect-error strict migration — TS2683
  healthCheck() {
    const logger = _getPort("logger");
    const checks = { hasBreakpoints: Object.keys(this.breakpoints).length > 0, hasAllDefaults: breakpointOrder.every(function(bp) {
      return !!this.breakpoints[bp];
    }, this), validOrder: breakpointOrder.length === 6, loggerAvailable: !!logger, portsInitialized: Ports.isInitialized() };
    let passed = 0;
    for (const k in checks) {
      if (checks[k]) passed++;
    }
    return { status: passed === Object.keys(checks).length ? "HEALTHY" : "DEGRADED", score: `${passed}/${Object.keys(checks).length}`, checks, version: VERSION, moduleId: MODULE_ID, timestamp: Date.now() };
  },
  info() {
    return { version: VERSION, moduleId: MODULE_ID, debug: _debug, portsInitialized: Ports.isInitialized(), breakpointCount: Object.keys(this.breakpoints).length, breakpointOrder, breakpoints: Object.assign({}, this.breakpoints), ranges: { mobile: MOBILE_BREAKPOINTS, tablet: TABLET_BREAKPOINTS, desktop: DESKTOP_BREAKPOINTS }, metrics: Object.assign({}, _metrics), healthCheck: this.healthCheck() };
  },
  resetMetrics() {
    _metrics = { detectCount: 0, configureCount: 0, resetCount: 0 };
  },
  getVersion() {
    return VERSION;
  }
};
var breakpoints_default = BreakpointManager;
export {
  BreakpointManager,
  MODULE_ID,
  VERSION,
