...
// ═══════════════════════════════════════════════════════════════
// DEPENDENCY CONTRACT (4.3.0-INIT-SAFE)
// ═══════════════════════════════════════════════════════════════
// MODULE: layout-lifecycle
// PURPOSE: Layout Manager Lifecycle
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   createCorePorts from /core/runtime/ports-profiles.js
//   layoutStore from ../state/store.js
//   BreakpointManager from ./breakpoints.js
//   LayoutDetector from ./detector.js
//   trackLayoutEvent from ../telemetry/tracker.js
//   loadLayoutPreference from ../utils/helpers.js
//
// PROVIDES:
...
//   LayoutLifecycle — exported value
//   BODY_CLASSES — exported value
//
...
export const VERSION = '4.3.0-INIT-SAFE';
export const MODULE_ID = 'layout-lifecycle';
...
let _debug = false;
let _metrics: { initCount: number; shutdownCount: number; resetCount: number; modeChanges: number; fullscreenToggles: number; bodyClassUpdates: number; lastInitAt: number | null; lastShutdownAt: number | null } = { initCount: 0, shutdownCount: 0, resetCount: 0, modeChanges: 0, fullscreenToggles: 0, bodyClassUpdates: 0, lastInitAt: null, lastShutdownAt: null };
...
const LayoutLifecycle = {
  ...
  async init(options: Record<string, unknown> = {}) {
    // FIXED: Return success object instead of false when already initialized
    if (initialized) {
    ...
    if (options.breakpoints) BreakpointManager.configure(options.breakpoints as Record<string, unknown>);
  ...
  setSidebarMobileOpen(value: boolean) {
  ...
  setPreloaderActive(value: boolean) {
  ...
  healthCheck() {
    ...
    const detectorHealth: Record<string, unknown> = LayoutDetector.healthCheck?.() || {};
    const storeHealth: Record<string, unknown> = layoutStore.healthCheck?.() || {};
  ...
  _applyBodyClasses() {
    ...
    // Remove all mode classes and add current
    ...
    _log('info', 'Body classes applied (P1A AAA OWNER):', {
      bp, compact, sidebarCollapsed, sidebarMobileOpen,
      fullscreen, globalLoading, scrollLocked, preloaderActive, modalOpen, mode
    });
  },
...
export { LayoutLifecycle, BODY_CLASSES };
export default LayoutLifecycle;
