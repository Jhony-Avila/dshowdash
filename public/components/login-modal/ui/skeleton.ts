...
// ═══════════════════════════════════════════════════════════════
// MODULE: login-modal-skeleton
// PURPOSE: Login Modal - Loading Skeleton
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
// PROVIDES:
//   LoadingSkeleton — exported class
...
export class LoadingSkeleton {
  ...
  show(container: HTMLElement | null) {
    this._metrics.shows++;
  ...
  hide(container: HTMLElement | null, delay = 300) {
    this._metrics.hides++;
    const target = container || this.container;
    if (!target) return;
  ...
  info() { return { moduleId: MODULE_ID, version: VERSION, mounted: this.mounted, metrics: { ...this._metrics }, timestamp: Date.now() }; }
