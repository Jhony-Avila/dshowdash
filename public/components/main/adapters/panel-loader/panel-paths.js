...
const ITEM_TO_PANEL = Object.freeze({
  "principal": "panel-01",
  "geral": "panel-02",
  "automacoes": "panel-03",
  "bling": "panel-04",
  ...
  "panel-pipedrive": "/components/panels/panel-pipedrive/index.js",
  "panel-ads": "/components/panels/panel-ads/index.js",
...
const CRITICAL_PANELS = Object.freeze(["panel-01", "panel-cards", "panel-dashboard", "panel-09"]);
function resolvePanelPath(panelId) {
  if (PANEL_ID_PATHS[panelId]) {
    return PANEL_ID_PATHS[panelId];
  }
  const mappedPanelId = ITEM_TO_PANEL[panelId];
  if (mappedPanelId && PANEL_ID_PATHS[mappedPanelId]) {
    return PANEL_ID_PATHS[mappedPanelId];
  }
  if (panelId.startsWith("status-")) {
    return PANEL_ID_PATHS["panel-status"];
  }
...
function resolvePanelId(rawId) {
  if (PANEL_ID_PATHS[rawId]) {
    return rawId;
  }
  if (ITEM_TO_PANEL[rawId]) {
    return ITEM_TO_PANEL[rawId];
  }
  return rawId;
}
function info() {
...
export {
  ...
  PANEL_ID_PATHS,
  VERSION,
  panel_paths_default as default,
  healthCheck,
  info,
  resolvePanelId,
  resolvePanelPath
