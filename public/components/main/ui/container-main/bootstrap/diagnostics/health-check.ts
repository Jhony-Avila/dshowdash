...
export async function performHealthCheck(refs: Record<string, unknown>) {
  const state = refs.state as string;
  const kernel = refs.kernel as ManagerRef | null;
  const errors = refs.errors as unknown[];
  ...
  return {
    status, version: VERSION, moduleId: MODULE_ID, bootstrapState: state, errorCount: errors.length,
    ...
    kernel: kernelHealth, contracts: contractsHealth
  };
}
