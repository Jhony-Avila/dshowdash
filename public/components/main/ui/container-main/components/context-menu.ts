...
// ═══════════════════════════════════════════════════════════════
// MODULE: container-context-menu
// PURPOSE: Container Context Menu Component
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
//   createContextMenu() — exported function
//   info() — exported function
//   healthCheck() — exported function
//   DEFAULT_ITEMS — exported value
//
...
export const MODULE_ID = 'container-context-menu';
...
function _validateOptions(options: Record<string, unknown>) {
  ...
  if (options.customClass !== undefined && typeof options.customClass !== 'string') errors.push('customClass must be a string');
...
export function createContextMenu(container: HTMLElement, options: Record<string, any> = {}) {
  ...
  let _focusedIndex = -1;
  let _boundContextHandler: EventListener | null = null;
  let _boundClickOutside: EventListener | null = null;
  let _boundKeydown: EventListener | null = null;
  let _menuItems: HTMLElement[] = [];
  ...
  function _createMenuElement() {
    ...
    items.forEach((item: Record<string, unknown>, idx: number) => {
      ...
      li.addEventListener('click', (e) => { if (item.disabled) return; e.stopPropagation(); _handleAction(String(item.id)); });
      ul.appendChild(li);
      _menuItems.push(li);
    });
  ...
  function _positionMenu(x: number, y: number) {
    if (!_menuEl) return;
  ...
  function _handleAction(actionId: string) {
    contextMenu.hide();
  ...
  function _handleClickOutside(e: Event) {
    if (_menuEl && !_menuEl.contains(e.target as Node)) contextMenu.hide();
  }

  const contextMenu = {
    init() {
      if (_initialized) return this;
      _boundContextHandler = ((e: MouseEvent) => { e.preventDefault(); contextMenu.show(e.clientX, e.clientY); }) as EventListener;
      container.addEventListener('contextmenu', _boundContextHandler as EventListener);
      _initialized = true;
      return this;
    },
    show(x: number, y: number) {
      if (!_menuEl) { _menuEl = _createMenuElement(); document.body.appendChild(_menuEl); }
      _positionMenu(x, y);
      ...
      document.addEventListener('click', _boundClickOutside as EventListener, true);
      document.addEventListener('keydown', _boundKeydown as EventListener);
      _menuEl.focus();
      return this;
    },
    hide() {
      ...
      _isVisible = false;
      _focusedIndex = -1;
      if (_boundClickOutside) document.removeEventListener('click', _boundClickOutside as EventListener, true);
      if (_boundKeydown) document.removeEventListener('keydown', _boundKeydown as EventListener);
      _boundClickOutside = null;
      _boundKeydown = null;
      return this;
    },
    isVisible() { return _isVisible; },
    isInitialized() { return _initialized; },
    destroy() {
      this.hide();
      if (_boundContextHandler) container.removeEventListener('contextmenu', _boundContextHandler as EventListener);
...
export default { createContextMenu, injectEventBus, info, healthCheck, VERSION, MODULE_ID, DEFAULT_ITEMS };
