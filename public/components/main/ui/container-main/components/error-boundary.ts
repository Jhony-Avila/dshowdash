...
// ═══════════════════════════════════════════════════════════════
// MODULE: container-error-boundary
// PURPOSE: Container Error Boundary Component
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'container-error-boundary';
...
export const ERROR_SEVERITY = { LOW: 'low', MEDIUM: 'medium', HIGH: 'high', CRITICAL: 'critical' };

// Exponential backoff helper
function _calculateBackoffDelay(attempt: number, baseDelay = 1000, maxDelay = 30000) {
  const delay = Math.min(baseDelay * Math.pow(2, attempt - 1), maxDelay);
  const jitter = delay * 0.1 * Math.random();
  return Math.round(delay + jitter);
}

export function createErrorBoundary(container: HTMLElement, options: Record<string, any> = {}) {
  const { maxErrors = 10, resetTimeout = 30000, showFallback = true, recoveryAttempts = 3, baseRetryDelay = 1000, maxRetryDelay = 30000, onError, onRecover, onFatalError, fallbackContent = null, eventBus } = options;
  ...
  let _fallbackEl: HTMLElement | null = null;
  let _originalContent: string | null = null;
  let _recoveryTimeout: ReturnType<typeof setTimeout> | null = null;
  let _resetTimeout: ReturnType<typeof setTimeout> | null = null;
  let _retryBtnHandler: EventListener | null = null;
  let _detailsBtnHandler: EventListener | null = null;
  ...
  function _createFallbackElement() {
    ...
    if (detailsBtn && detailsEl) {
      ...
      _detailsBtnHandler = () => { const isHidden = detailsEl.hidden; detailsEl.hidden = !isHidden; detailsBtn.textContent = isHidden ? 'Ocultar detalhes' : 'Ver detalhes'; };
      detailsBtn.addEventListener('click', _detailsBtnHandler);
    }
    return fallback;
  }

  function _classifyError(error: Error | Record<string, unknown>) {
  ...
  function _recordError(error: Error | Record<string, unknown>, context: Record<string, unknown> = {}) {
    const classification = _classifyError(error);
    const errorRecord = { id: `err-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, message: (error as Error)?.message || String(error), stack: (error as Error)?.stack, timestamp: Date.now(), ...classification, context, recovered: false };
  ...
  function _showFallback() {
    ...
    if (detailsPre && _errors.length > 0) {
      ...
      detailsPre.textContent = `${(lastError as Record<string, unknown>).message}\n\n${(lastError as Record<string, unknown>).stack || 'No stack trace'}`;
    }
  ...
  function _hideFallback() {
    if (_fallbackEl) {
      ...
      const detailsBtn = (_fallbackEl as HTMLElement).querySelector('.dsd-error-boundary__btn--details');
  ...
  const errorBoundary = {
    init() {
    ...
    catch(error: Error | Record<string, unknown>, context: Record<string, unknown> = {}) {
      ...
      if (!record.recoverable || _recoveryCount >= recoveryAttempts) { _showFallback(); }
    ...
    wrap(fn: (...args: unknown[]) => unknown, context: Record<string, unknown> = {}) {
      return (...args: unknown[]) => {
        try {
          ...
          if (result && typeof (result as Record<string, unknown>).catch === 'function') return (result as Promise<unknown>).catch((error: Error) => { this.catch(error, { ...context, type: 'async' }); throw error; });
          return result;
        } catch (error) { this.catch(error as Error, { ...context, type: 'sync' }); throw error; }
      };
    },
    recover() {
      ...
      _hideFallback();
      if (_errors.length > 0) (_errors[_errors.length - 1] as Record<string, unknown>).recovered = true;
      ...
      _resetTimeout = setTimeout(() => { if (!_isFatalState) _recoveryCount = 0; }, resetTimeout);
      return true;
    },
    fatal(errorRecord: Record<string, unknown>) {
      _isFatalState = true;
      if (_recoveryTimeout) { clearTimeout(_recoveryTimeout); _recoveryTimeout = null; }
      _showFallback();
      if (_fallbackEl) {
        ...
        const retryBtn = _fallbackEl!.querySelector('.dsd-error-boundary__btn--retry');
        if (title) title.textContent = 'Erro crítico';
        if (message) message.textContent = 'Não foi possível recuperar. Por favor, recarregue a página.';
        if (retryBtn) {
          if (_retryBtnHandler) retryBtn.removeEventListener('click', _retryBtnHandler);
          _retryBtnHandler = () => window.location.reload();
          retryBtn.textContent = 'Recarregar página';
          retryBtn.addEventListener('click', _retryBtnHandler);
        }
      }
    ...
    reset() {
      ...
      _errors = []; _errorCount = 0; _recoveryCount = 0; _isFatalState = false;
      _hideFallback();
      return this;
    },
    getErrors() { return [..._errors]; },
    getLastError() { return _errors[_errors.length - 1] || null; },
    getStats() { return { totalErrors: _errorCount, recoveryAttempts: _recoveryCount, maxRecoveryAttempts: recoveryAttempts, isFatalState: _isFatalState, recentErrors: _errors.length }; },
    isFatal() { return _isFatalState; },
...
export function info() { return { moduleId: MODULE_ID, version: VERSION, hasInjectedEventBus: !!_injectedEventBus, backoffEnabled: true }; }
export function healthCheck() { return { status: 'HEALTHY', version: VERSION, moduleId: MODULE_ID, hasInjectedEventBus: !!_injectedEventBus, backoffEnabled: true }; }

export default { createErrorBoundary, injectEventBus, info, healthCheck, VERSION, MODULE_ID, ERROR_SEVERITY };
