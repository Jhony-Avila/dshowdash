...
// ═══════════════════════════════════════════════════════════════
// MODULE: container-split-view
// PURPOSE: Container Split View Component
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
//   SPLIT_DIRECTION — exported value
//   createSplitView() — exported function
...
export const MODULE_ID = 'container-split-view';
...
function _validateOptions(options: Record<string, unknown>) {
  const errors = [];
  if (options.direction !== undefined && !['horizontal', 'vertical'].includes(options.direction as string)) errors.push('direction must be horizontal|vertical');
  if (options.initialSizes !== undefined && !Array.isArray(options.initialSizes)) errors.push('initialSizes must be an array');
  if (options.minSize !== undefined && (typeof options.minSize !== 'number' || options.minSize < 0)) errors.push('minSize must be a positive number');
  if (options.gutterSize !== undefined && (typeof options.gutterSize !== 'number' || options.gutterSize < 1)) errors.push('gutterSize must be a positive number');
  if (errors.length > 0) logger.warn('Invalid options', { errors });
  return errors.length === 0;
}

export const SPLIT_DIRECTION = { HORIZONTAL: 'horizontal', VERTICAL: 'vertical' };

export function createSplitView(container: HTMLElement, options: Record<string, any> = {}) {
  ...
  let _gutters: HTMLElement[] = [];
  let _splitViewEl: HTMLElement | null = null;
  let _isDragging = false;
  let _activeGutterIndex = -1;
  let _startPos = 0;
  let _startSizes: number[] = [];
  let _panelHandlers: { el: Element | null; handler: EventListener }[] = [];
  ...
  function _createSplitView() {
    ...
    splitView.setAttribute('aria-label', 'Painéis redimensionáveis');
    _sizes.forEach((size, index) => {
      const panel = _createPanel(index, size as number) as HTMLElement;
  ...
  function _createPanel(index: number, size: number) {
  ...
  function _createGutter(index: number) {
  ...
  function _onGutterMouseDown(e: MouseEvent, index: number) {
    e.preventDefault();
    _isDragging = true;
  ...
  function _onMouseMove(e: MouseEvent) {
    ...
    const containerSize = _direction === SPLIT_DIRECTION.HORIZONTAL ? (_splitViewEl as HTMLElement).offsetWidth : (_splitViewEl as HTMLElement).offsetHeight;
    ...
    if (newSize1 < minPercent) { newSize1 = minPercent; newSize2 = _startSizes[_activeGutterIndex] + _startSizes[_activeGutterIndex + 1] - minPercent; }
    if (newSize2 < minPercent) { newSize2 = minPercent; newSize1 = _startSizes[_activeGutterIndex] + _startSizes[_activeGutterIndex + 1] - minPercent; }
    if (newSize1 < snapThreshold / containerSize * 100) { newSize1 = 0; newSize2 = _startSizes[_activeGutterIndex] + _startSizes[_activeGutterIndex + 1]; }
    if (newSize2 < snapThreshold / containerSize * 100) { newSize2 = 0; newSize1 = _startSizes[_activeGutterIndex] + _startSizes[_activeGutterIndex + 1]; }
  ...
  function _onGutterKeydown(e: KeyboardEvent, index: number) {
    const step = e.shiftKey ? 10 : 2;
    let handled = false;
    if (_direction === SPLIT_DIRECTION.HORIZONTAL) {
      if (e.key === 'ArrowLeft') { _adjustSize(index, -step); handled = true; }
      else if (e.key === 'ArrowRight') { _adjustSize(index, step); handled = true; }
    } else {
      if (e.key === 'ArrowUp') { _adjustSize(index, -step); handled = true; }
      else if (e.key === 'ArrowDown') { _adjustSize(index, step); handled = true; }
    }
    if (e.key === 'Home') { _sizes[index] = 0; _sizes[index + 1] = initialSizes[index] + initialSizes[index + 1]; _applyPanelSizes(); handled = true; }
    else if (e.key === 'End') { _sizes[index] = initialSizes[index] + initialSizes[index + 1]; _sizes[index + 1] = 0; _applyPanelSizes(); handled = true; }
    if (handled) { e.preventDefault(); _gutters[index]?.setAttribute('aria-valuenow', String(_sizes[index])); onResize?.([..._sizes]); }
  }

  function _adjustSize(index: number, delta: number) {
  ...
  function _resetSizes() { _sizes = [...initialSizes]; _applyPanelSizes(); onResize?.([..._sizes]); }
  ...
  const splitViewApi = {
    init() { if (_initialized) return this; _splitViewEl = _createSplitView() as HTMLElement | null; _initialized = true; return this; },
    addPanel(opts: Record<string, any> = {}) {
      ...
      _sizes.push(size);
      if (_panels.length > 0) { const gutter = _createGutter(index - 1); _splitViewEl.appendChild(gutter); _gutters.push(gutter); }
      const panel = _createPanel(index, size as number) as HTMLElement;
    ...
    removePanel(index: number) {
      ...
      if (_gutters[index]) { _gutters[index].remove(); _gutters.splice(index, 1); }
      else if (_gutters[index - 1]) { _gutters[index - 1].remove(); _gutters.splice(index - 1, 1); }
      _sizes.splice(index, 1);
      const factor = 100 / (100 - removedSize);
      _sizes = _sizes.map(s => s * factor);
      _panels.forEach((panel, i) => { panel.dataset.panelIndex = String(i); panel.setAttribute('aria-label', `Painel ${i + 1}`); });
      _gutters.forEach((gutter, i) => { gutter.dataset.gutterIndex = String(i); gutter.setAttribute('aria-label', `Separador entre painel ${i + 1} e ${i + 2}`); });
      _applyPanelSizes();
      return true;
    },
    toggleCollapse(index: number) {
      ...
      if (isCollapsed) {
        ...
        if (remaining.length > 0) { const addEach = collapsedSize / remaining.length; _sizes = _sizes.map((s, i) => i !== index && s > 0 ? s + addEach : s); }
        onCollapse?.(index);
      }
      _applyPanelSizes();
    },
    ...
    setSizes(sizes: unknown) { if (!Array.isArray(sizes) || sizes.length !== _sizes.length) return; _sizes = [...sizes]; _applyPanelSizes(); },
    getSizes() { return [..._sizes]; },
    getPanelContent(index: number) { return _panels[index]?.querySelector('[data-slot="content"]') || null; },
...
export default { createSplitView, injectEventBus, info, healthCheck, VERSION, MODULE_ID, SPLIT_DIRECTION };
