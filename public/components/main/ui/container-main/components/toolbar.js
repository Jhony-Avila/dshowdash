...
const MODULE_ID = "container-toolbar";
...
function _validateOptions(options) {
  const errors = [];
  if (options.position !== void 0 && !["top", "bottom", "left", "right"].includes(options.position)) errors.push("position must be top|bottom|left|right");
  if (options.items !== void 0 && !Array.isArray(options.items)) errors.push("items must be an array");
  if (options.size !== void 0 && !["small", "medium", "large"].includes(options.size)) errors.push("size must be small|medium|large");
  if (options.collapsible !== void 0 && typeof options.collapsible !== "boolean") errors.push("collapsible must be a boolean");
...
const TOOLBAR_POSITION = { TOP: "top", BOTTOM: "bottom", LEFT: "left", RIGHT: "right" };
function createToolbar(container, options = {}) {
  ...
  function _createToolbarElement() {
    ...
    if (collapsible) {
      ...
      if (toggle) {
        _boundToggleHandler = () => toolbarApi.toggle();
        toggle.addEventListener("click", _boundToggleHandler);
      }
    }
    return toolbar;
  }
  function _handleAction(item) {
  ...
  function _handleKeyboardNav(e, currentIndex) {
    const isVertical = position === "left" || position === "right";
    const prev = isVertical ? "ArrowUp" : "ArrowLeft";
    const next = isVertical ? "ArrowDown" : "ArrowRight";
    let newIndex = currentIndex;
    if (e.key === prev) {
      newIndex = currentIndex > 0 ? currentIndex - 1 : _focusableItems.length - 1;
      e.preventDefault();
    } else if (e.key === next) {
      newIndex = currentIndex < _focusableItems.length - 1 ? currentIndex + 1 : 0;
      e.preventDefault();
    } else if (e.key === "Home") {
      newIndex = 0;
      e.preventDefault();
    } else if (e.key === "End") {
      newIndex = _focusableItems.length - 1;
      e.preventDefault();
    }
  ...
  function _renderItems() {
    ...
    _items.forEach((item, idx) => {
      ...
      if (item.type === "group") {
        ...
        item.items?.forEach((subItem) => {
          const btn2 = _createButton(subItem);
          group.appendChild(btn2);
          if (!subItem.disabled) _focusableItems.push(btn2);
        });
    ...
    _focusableItems.forEach((btn, i) => {
      ...
      const keyHandler = (e) => _handleKeyboardNav(e, _focusableItems.indexOf(btn));
      btn.addEventListener("keydown", keyHandler);
      _buttonHandlers.push({ btn, handler: keyHandler, type: "keydown" });
    });
  }
  function _createButton(item) {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "dsd-toolbar__btn";
    if (item.active) {
      btn.classList.add("dsd-toolbar__btn--active");
  ...
  const toolbarApi = {
    init() {
      ...
      _toolbarEl = _createToolbarElement();
      _renderItems();
      _initialized = true;
      return this;
    },
    setItems(items2) {
      if (!Array.isArray(items2)) return this;
      _items = [...items2];
      _renderItems();
      return this;
    },
    ...
    expand() {
      ...
      if (toggle) {
        ...
        toggle.setAttribute("aria-label", "Recolher barra de ferramentas");
      }
      return this;
    },
    collapse() {
      ...
      if (toggle) {
        ...
        toggle.setAttribute("aria-label", "Expandir barra de ferramentas");
      }
      return this;
    },
    isCollapsed() {
    ...
    destroy() {
      ...
      if (_toolbarEl && _boundToggleHandler) {
        const toggle = _toolbarEl.querySelector(".dsd-toolbar__toggle");
        toggle?.removeEventListener("click", _boundToggleHandler);
      }
...
function info() {
  return { moduleId: MODULE_ID, version: VERSION, hasInjectedEventBus: !!_injectedEventBus, usesCentralizedIcons: true, hasValidation: true, hasKeyboardNav: true };
}
function healthCheck() {
  return { status: "HEALTHY", version: VERSION, moduleId: MODULE_ID, hasInjectedEventBus: !!_injectedEventBus, usesCentralizedIcons: true, hasValidation: true, hasKeyboardNav: true };
}
var toolbar_default = { createToolbar, injectEventBus, info, healthCheck, VERSION, MODULE_ID, TOOLBAR_POSITION };
export {
  MODULE_ID,
  TOOLBAR_POSITION,
  VERSION,
