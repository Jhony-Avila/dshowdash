const VERSION = "10.0.0-PHASE7";
const MODULE_ID = "container-main:dependency-map";
const DEPENDENCY_TYPES = Object.freeze({ REQUIRED: "required", OPTIONAL: "optional", PEER: "peer", DEV: "dev" });
...
let _loadedModules = /* @__PURE__ */ new Set();
let _loadOrder = [];
function registerLoaded(moduleId) {
  if (!_loadedModules.has(moduleId)) {
    _loadedModules.add(moduleId);
    _loadOrder.push({ moduleId, timestamp: Date.now() });
  }
}
...
function getDependencies(moduleId) {
  const deps = DEPENDENCY_GRAPH[moduleId];
  if (!deps) return null;
  return { ...deps, moduleId };
}
function checkDependencies(moduleId) {
  const deps = DEPENDENCY_GRAPH[moduleId];
  if (!deps) return { satisfied: true, missing: [], optional: [] };
  const missing = deps.requires.filter((d) => !_loadedModules.has(d));
  const optionalMissing = deps.optional.filter((d) => !_loadedModules.has(d));
  return { satisfied: missing.length === 0, missing, optional: optionalMissing, loaded: deps.requires.filter((d) => _loadedModules.has(d)) };
}
function getLoadOrder(moduleId, visited = /* @__PURE__ */ new Set()) {
  if (visited.has(moduleId)) return [];
  visited.add(moduleId);
  const deps = DEPENDENCY_GRAPH[moduleId];
  if (!deps) return [moduleId];
  const order = [];
  for (const dep of deps.requires) {
    order.push(...getLoadOrder(dep, visited));
  }
  order.push(moduleId);
  return [...new Set(order)];
}
function detectCircular(moduleId, visited = /* @__PURE__ */ new Set(), path = []) {
  if (visited.has(moduleId)) {
    const cycleStart = path.indexOf(moduleId);
    if (cycleStart !== -1) return { hasCircular: true, cycle: path.slice(cycleStart).concat(moduleId) };
  ...
  if (deps) {
    for (const dep of deps.requires) {
      const result = detectCircular(dep, visited, [...path]);
...
function getDependents(moduleId) {
  const dependents = [];
  for (const [id, deps] of Object.entries(DEPENDENCY_GRAPH)) {
    if (deps.requires.includes(moduleId) || deps.optional.includes(moduleId)) {
      dependents.push({ moduleId: id, type: deps.requires.includes(moduleId) ? "required" : "optional" });
    }
  }
  return dependents;
}
function generateReport() {
  const modules = Object.keys(DEPENDENCY_GRAPH);
  const loaded = Array.from(_loadedModules);
  const notLoaded = modules.filter((m) => !_loadedModules.has(m));
  const circularChecks = modules.map((m) => ({ moduleId: m, ...detectCircular(m) }));
  const hasCircular = circularChecks.filter((c) => c.hasCircular);
  return { totalModules: modules.length, loadedModules: loaded.length, notLoaded: notLoaded.length, loadOrder: _loadOrder, circularDependencies: hasCircular, modules: modules.map((m) => ({ moduleId: m, loaded: _loadedModules.has(m), dependencies: checkDependencies(m) })) };
}
function reset() {
...
function info() {
  return { moduleId: MODULE_ID, version: VERSION, totalModules: Object.keys(DEPENDENCY_GRAPH).length, loadedModules: _loadedModules.size, dependencyTypes: Object.keys(DEPENDENCY_TYPES) };
}
function healthCheck() {
  const report = generateReport();
  const hasCircular = report.circularDependencies.length > 0;
  return { status: hasCircular ? "WARNING" : "HEALTHY", version: VERSION, moduleId: MODULE_ID, loadedModules: _loadedModules.size, totalModules: Object.keys(DEPENDENCY_GRAPH).length, circularCount: report.circularDependencies.length };
}
var dependency_map_default = { VERSION, MODULE_ID, DEPENDENCY_TYPES, DEPENDENCY_GRAPH, registerLoaded, isLoaded, getDependencies, checkDependencies, getLoadOrder, detectCircular, getDependents, generateReport, reset, info, healthCheck };
export {
  DEPENDENCY_GRAPH,
  DEPENDENCY_TYPES,
  MODULE_ID,
  VERSION,
  checkDependencies,
  dependency_map_default as default,
  detectCircular,
  generateReport,
  getDependencies,
  getDependents,
