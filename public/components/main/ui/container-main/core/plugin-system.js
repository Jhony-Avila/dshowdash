...
const MODULE_ID = "container-main:plugin-system";
...
const PLUGIN_SCHEMA = {
  ...
  dependencies: { type: TYPES.ARRAY, default: [] },
...
function createPluginSystem(options = {}) {
  ...
  function _checkDependencies(plugin) {
    const missing = [];
  ...
  function _registerHooks(plugin) {
  ...
  function _unregisterHooks(pluginId) {
    for (const [hookName, handlers] of _hooks) {
      _hooks.set(hookName, handlers.filter((h) => h.pluginId !== pluginId));
    }
  }
  async function _executeHook(hookName, data = {}) {
  ...
  const system = {
    ...
    async executeHook(hookName, data = {}) {
      return _executeHook(hookName, data);
    },
    get(pluginId) {
      const plugin = _plugins.get(pluginId);
    ...
    listActive() {
      return this.list().filter((p) => p.state === PLUGIN_STATES.ACTIVE);
    },
    has(pluginId) {
      return _plugins.has(pluginId);
    },
    isActive(pluginId) {
      return _plugins.get(pluginId)?.state === PLUGIN_STATES.ACTIVE;
    },
    getHookHandlers(hookName) {
      return (_hooks.get(hookName) || []).map((h) => h.pluginId);
    },
    healthCheck() {
    ...
    info() {
      return {
        ...
        activePlugins: this.listActive().length,
        availableHooks: Object.keys(PLUGIN_HOOKS),
        pluginTypes: Object.keys(PLUGIN_TYPES)
      };
    },
    async destroy() {
...
function getPluginSystem(options = {}) {
  if (!_instance) {
    _instance = createPluginSystem(options);
  }
  return _instance;
}
