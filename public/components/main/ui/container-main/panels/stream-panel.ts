...
// ═══════════════════════════════════════════════════════════════
// MODULE: container-main:panels:stream
// PURPOSE: Stream Panel - Painel adaptativo para dados em tempo real
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   createPanel, PANEL_CATEGORIES, PANEL_CAPABILITIES from ../contracts/panel-con...
//   createTimerResource from ../contracts/resource-contract.js
//
// PROVIDES:
//   VERSION — module constant
//   MODULE_ID — module constant
//   STREAM_TYPES — exported value
//   createStreamPanel() — exported function
...
export const MODULE_ID = 'container-main:panels:stream';
...
// Cria um painel de stream
export function createStreamPanel(config: Record<string, any> = {}) {
  const {
  ...
  let _timerResource: Record<string, (...args: unknown[]) => unknown> | null = null;
  let _connectionResource: Record<string, (...args: unknown[]) => unknown> | null = null;
  ...
  const implementation = {
    async render(element: HTMLElement) {
      ...
      _listEl = _containerEl!.querySelector('.dsd-stream-panel__list') as HTMLElement;
      ...
      await (_timerResource!.load as (...args: unknown[]) => Promise<unknown>)(async () => ({ memoryEstimate: 1024 * 1024 }));
    },

    pause() {
      _disconnect();
      (_timerResource?.pause as (() => void) | undefined)?.();
    },

    resume() {
      (_timerResource?.resume as (() => void) | undefined)?.();
    ...
    async destroy() {
      _disconnect();
      await (_timerResource?.dispose as (() => Promise<void>) | undefined)?.();
      // @ts-expect-error strict migration — TS2339
      await (_connectionResource?.dispose as (() => Promise<void>) | undefined)?.();
    ...
    healthCheck() {
      return {
        ...
        timerResource: (_timerResource?.healthCheck as (() => Record<string, unknown>) | undefined)?.() || null
      };
    }
  };
  ...
  function _disconnect() {
    ...
    (_timerResource?.clearAll as (() => void) | undefined)?.();
    _setConnected(false);
  }
  ...
  function _startPolling() {
    ...
    _fetchData();
    (_timerResource?.setInterval as ((...a: unknown[]) => void) | undefined)?.(() => { if (document.hidden) return; _fetchData(); }, pollingInterval);
  }
  ...
  function _addItem(data: Record<string, unknown>) {
    const item = {
  ...
  function _renderItem(item: Record<string, unknown>) {
    if (!_listEl) return;
    
    const li = document.createElement('li');
    li.className = 'dsd-stream-panel__item';
    li.setAttribute('data-id', item.id as string);
    li.innerHTML = `
  ...
  function _setConnected(status: boolean) {
    _connected = status;
