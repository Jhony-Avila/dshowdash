...
// PURPOSE: Layout Position Calculator
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   DOCK_ZONES, SPLIT_MODES from ./constants.js
//
// PROVIDES:
//   createPositionCalculator() — exported function
//
...
export const MODULE_ID = 'main.ui.container-main.resources.layout-manager.position-calculator';

export function createPositionCalculator() {
  return {
    // Calcula posição de dock
    calculateDockPosition(zone: string, containerRect: { width: number; height: number }) {
      const positions = {
        [DOCK_ZONES.LEFT]: { 
          x: 0, 
          y: 0, 
      ...
      return (positions as Record<string, { x: number; y: number; width: number; height: number }>)[zone] || positions[DOCK_ZONES.CENTER];
    },

    // Calcula posição de split
    calculateSplitPosition(mode: string, index: number, containerRect: { width: number; height: number }) {
    ...
    getContentContainer(container: HTMLElement) {
      if (!container) return null;
      return container.querySelector('.dsd-container__content') || 
             container.querySelector('[data-slot="content"]') || 
             container;
    },

    // Obtém rect do container
    getContainerRect(container: HTMLElement) {
...
export default { createPositionCalculator };
