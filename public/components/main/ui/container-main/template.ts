...
// ═══════════════════════════════════════════════════════════════
// MODULE: container-main-template
// PURPOSE: ContainerMain Template Enterprise
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
//   createContainerTemplate() — exported function
//   createSkeletonTemplate() — exported function
//   createEmptyTemplate() — exported function
//   createErrorTemplate() — exported function
//   getIcons() — exported function
...
export const MODULE_ID = 'container-main-template';
...
export function createContainerTemplate(options: Record<string, unknown> = {}) {
  const {
    id = 'container',
    title = 'Container',
    icon = '',
  ...
  if (showControls) {
    ...
    controlsHtml += `<button type="button" class="dsd-container__control dsd-container__control--menu" data-action="menu" aria-label="Menu" title="Mais opções"><span class="dsd-container__control-icon">${ICONS.menu}</span></button>`;
    controlsHtml += '</div>';
  }
...
export function createSkeletonTemplate(type = 'dashboard') {
  if (type === 'dashboard') {
...
export function createEmptyTemplate(options: Record<string, unknown> = {}) {
  const {
    ...
    hint = 'Selecione um item no menu para começar'
  } = options;
...
export function createErrorTemplate(options: Record<string, unknown> = {}) {
  const {
    icon = ICONS.warning,
