...
// ═══════════════════════════════════════════════════════════════
// MODULE: container-main:clipboard-manager
// PURPOSE: Clipboard Manager - Gerenciamento de área de transferência
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
//   createClipboardManager() — exported function
//   getClipboardManager() — exported function
//   resetClipboardManager() — exported function
...
export const MODULE_ID = 'container-main:clipboard-manager';

export function createClipboardManager(options: Record<string, any> = {}) {
  ...
  const manager = {
    // Copia texto
    async copy(text: string, options: Record<string, any> = {}) {
      try {
        ...
        onCopy?.(text);
        if (showNotification && options.notify !== false) {
    ...
    // Copia HTML
    async copyHtml(html: string, plainText = '') {
      try {
    ...
    // Copia imagem
    async copyImage(imageUrl: unknown) {
      try {
    ...
    async read() {
      try {
        if (navigator.clipboard?.read) {
          ...
          const result = { text: null as string | null, html: null as string | null, image: null as HTMLImageElement | null };
        ...
        return { text: await this.paste(), html: null as string | null, image: null as Record<string, unknown> | null };
      } catch (e) {
        _metrics.errors++;
    ...
    // Copia de elemento
    async copyFromElement(element: HTMLElement, options: Record<string, any> = {}) {
      // @ts-expect-error strict migration — TS2322
      if (typeof element === 'string') element = document.querySelector(element);
      if (!element) return false;
      // @ts-expect-error TS migration - TS2339
      const text = options.html ? element.innerHTML : (element.textContent || element.value);
      return options.html ? this.copyHtml(element.innerHTML, element.textContent) : this.copy(text, options);
    },
    ...
    resetMetrics() { _metrics = { copies: 0, pastes: 0, errors: 0 }; },
...
export function getClipboardManager(options: Record<string, any> = {}) { if (!_instance) _instance = createClipboardManager(options); return _instance; }
export function resetClipboardManager() { _instance = null; }

export async function copyText(text: string) { return (getClipboardManager().copy as (...args: unknown[]) => unknown)(text); }
export async function pasteText() { return (getClipboardManager().paste as (...args: unknown[]) => unknown)(); }
...
export default { VERSION, MODULE_ID, createClipboardManager, getClipboardManager, resetClipboardManager, copyText, pasteText, info, healthCheck };
