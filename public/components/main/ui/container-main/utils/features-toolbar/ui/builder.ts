// ═══════════════════════════════════════════════════════════════
// DEPENDENCY CONTRACT (8.0.0-NEW-AAA)
// ═══════════════════════════════════════════════════════════════
// MODULE: builder
// PURPOSE: Features Toolbar - Builder
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   ICONS from ../icons.js
//   _createButton, _createGroup, _createDropdown, _createOverflowButton, setupKey...
//   addCleanup from ../state.js
//
// PROVIDES:
//   _buildToolbar() — exported function
//   _appendDynamicGroup() — exported function
//
...
export const MODULE_ID = 'main.ui.container-main.utils.features-toolbar.ui.builder';

export function _buildToolbar() {
  ...
  // #4-G: Inicializar keyboard navigation (roving tabindex)
...
// ============================================================================
// #23-G — OVERFLOW OBSERVER (debounce via requestAnimationFrame)
// ============================================================================

const _currentOverflowRef = { toolbar: null as HTMLElement | null, overflow: null as Record<string, unknown> | null };

function _setupOverflowObserver(toolbar: HTMLElement, overflow: HTMLElement) {
  if (typeof ResizeObserver === 'undefined') return;

  // #15-H: Guardar referencia para _appendDynamicGroup disparar update
  _currentOverflowRef.toolbar = toolbar;
  // @ts-expect-error TS migration - TS2352
  _currentOverflowRef.overflow = overflow as Record<string, unknown>;
  ...
  setTimeout(() => { _updateOverflow(toolbar, overflow); }, 100);
}

function _updateOverflow(toolbar: HTMLElement, overflow: HTMLElement) {
  ...
  groupArray.forEach(g => { g.style.display = ''; });
...
// ============================================================================
// #28 — APPEND DYNAMIC GROUP
// #15-H: Dispara _updateOverflow apos inserir grupo dinamico
// ============================================================================

export function _appendDynamicGroup(toolbar: HTMLElement, groupId: string, buttonDefs: unknown) {
  ...
  // #15-H: Atualizar overflow imediatamente apos novo grupo
