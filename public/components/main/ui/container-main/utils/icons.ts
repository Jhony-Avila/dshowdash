...
// ═══════════════════════════════════════════════════════════════
// MODULE: container-main-icons
// PURPOSE: Container-Main - Centralized SVG Icons
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   ICONS, getIcon from ../utils/icons.js
//
// PROVIDES:
...
//   getIconSized() — exported function
//   getIconNames() — exported function
...
export const MODULE_ID = 'container-main-icons';
...
export function getIcon(name: string, fallback = '') {
  return (ICONS as Record<string, unknown>)[name] || fallback;
}

// Helper to get icon with custom size
export function getIconSized(name: string, width: number, height: number) {
  const icon = (ICONS as Record<string, unknown>)[name];
  if (!icon) return '';
  return (icon as string).replace(/width="(\d+)"/, `width="${width}"`).replace(/height="(\d+)"/, `height="${height}"`);
}
...
export default { ICONS, getIcon, getIconSized, getIconNames, info, healthCheck, VERSION, MODULE_ID };
