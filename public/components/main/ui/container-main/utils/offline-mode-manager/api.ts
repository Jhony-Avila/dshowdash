...
// ═══════════════════════════════════════════════════════════════
// MODULE: api
// PURPOSE: Offline Mode Manager - Public API
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   VERSION, MODULE_ID, OFFLINE_STATES, CACHE_STRATEGIES, DEFAULT_CONFIG from ./c...
//   _instance, setInstance, getConfig, setConfig, getState, setState, isInitializ...
//   _log, _emit, _loadState, _loadCacheMetadata, _saveCacheMetadata from ./helper...
//   _openCache, _cacheResponse, _getCachedResponse from ./cache/index.js
//   _updateOfflineIndicator from ./ui/index.js
//   cachedFetch, _processOfflineQueue, queueRequest from ./network/index.js
//
// PROVIDES:
//   createOfflineModeManager() — exported function
//   getOfflineModeManager() — exported function
//   destroy() — exported function
//   getCacheSize() — exported function
//   setStrategy() — exported function
...
import {
  ...
  getState, setState, isInitialized, setIsInitialized,
  getCache, setCache, getSyncTimer, setSyncTimer,
  getOfflineQueue, setOfflineQueue, getCacheMetadata, setCacheMetadata,
...
export function createOfflineModeManager(options: Record<string, unknown> = {}) {
...
export function getOfflineModeManager(options: Record<string, unknown> = {}) {
  if (!_instance) {
...
export function getCacheSize() {
  ...
  const totalSize = Object.values(metadata)
    ...
    .reduce((sum, meta) => sum + (parseInt((meta as any).size) || 0), 0);

  return {
    entries,
    bytes: totalSize,
    formatted: (totalSize as number) > 1024 * 1024
      ? `${((totalSize as number) / (1024 * 1024)).toFixed(2)} MB`
      : `${((totalSize as number) / 1024).toFixed(2)} KB`
  };
}

export function setStrategy(strategy: Record<string, unknown>) {
  // @ts-expect-error TS migration - TS2345
...
// Re-exports
export { cachedFetch, queueRequest };
