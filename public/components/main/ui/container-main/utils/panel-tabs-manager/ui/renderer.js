...
const MODULE_ID = "main.ui.container-main.utils.panel-tabs-manager.ui.renderer";
function _renderTabs() {
  ...
  tabsList.innerHTML = tabs.map((tab) => {
    ...
    const stateClass = tab.state !== TAB_STATES.ACTIVE && tab.state !== TAB_STATES.INACTIVE ? `dsd-pt-tab--${tab.state}` : "";
    const icon = config.showTabIcons ? `<span class="dsd-pt-tab-icon">${tab.icon || "\u{1F4C4}"}</span>` : "";
...
function _renderContent() {
  ...
  contentContainer.querySelectorAll(".dsd-pt-panel").forEach((panelEl) => {
    const tabId = panelEl.dataset.panelId;
