const VERSION = "1.0.0";
const MODULE_ID = "overlay-layer-debug-panel.devtools.debug-panel";
...
function hide() {
  ...
  if (_state.refreshIntervalId) {
    clearInterval(_state.refreshIntervalId);
    _state.refreshIntervalId = null;
  }
  return { ok: true };
}
function toggle() {
  if (_state.isVisible) hide();
  else show();
...
function toggleCollapse() {
  ...
  if (_state.panelElement) {
    _state.panelElement.classList.toggle("collapsed", _config.collapsed);
    refresh();
  }
...
function closeAll() {
  if (_overlayLayer?.closeAll) {
    _overlayLayer.closeAll({ reason: "debug-panel" });
...
function exportInfo() {
  if (_overlayLayer?.info) {
    console.log("[OverlayDebug] Full Info:", JSON.stringify(_overlayLayer.info(), null, 2));
...
function init(overlayLayerRef) {
  ...
  if (typeof window !== "undefined") {
    window.__overlayDebugPanel = { toggle: toggleCollapse, closeAll, scanOrphans, exportInfo };
  }
...
function destroy() {
  hide();
  if (_state.panelElement?.parentNode) {
    _state.panelElement.parentNode.removeChild(_state.panelElement);
    _state.panelElement = null;
  }
  const styleEl = document.getElementById?.("overlay-debug-styles");
  if (styleEl) styleEl.remove();
  if (typeof window !== "undefined") delete window.__overlayDebugPanel;
