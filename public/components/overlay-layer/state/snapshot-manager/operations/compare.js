...
const MODULE_ID = "overlay-layer.state.snapshot-manager.operations.compare";
function compare(snapshot1, snapshot2) {
  ...
  const common = stack1.filter((id) => stack2.indexOf(id) !== -1);
  const onlyIn1 = stack1.filter((id) => stack2.indexOf(id) === -1);
  const onlyIn2 = stack2.filter((id) => stack1.indexOf(id) === -1);
  const modified = [];
