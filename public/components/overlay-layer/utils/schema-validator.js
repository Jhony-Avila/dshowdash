const VERSION = "1.0.0";
const MODULE_ID = "overlay-layer-schema-validator";
...
const DESCRIPTOR_SCHEMA = {
  ...
  content: { type: ["string", "object", "null"], required: false, default: null },
...
function applyDefaults(descriptor, schema) {
  const result = { ...descriptor };
  for (const [key, fieldDef] of Object.entries(schema)) {
    if (result[key] === void 0 && fieldDef.default !== void 0) {
      result[key] = typeof fieldDef.default === "object" ? { ...fieldDef.default } : fieldDef.default;
    }
...
function validate(descriptor, options = {}) {
  ...
  if (isValid) {
    ...
    _metrics.failed++;
    _metrics.errors.push(...errors.slice(0, 3));
  }
...
function healthCheck() {
  const passRate = _metrics.validations > 0 ? _metrics.passed / _metrics.validations * 100 : 100;
  const checks = {
...
export {
  MODULE_ID,
  VERSION,
  schema_validator_default as default,
  getConfigSchema,
  getMetrics,
  getSchema,
  getValidScopes,
