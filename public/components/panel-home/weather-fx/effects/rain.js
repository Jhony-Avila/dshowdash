...
const MODULE_ID = "panel-home.weather-fx.effects.rain";
...
class RainEffect extends EffectBase {
  ctx = null;
  w = 0;
  h = 0;
  isDay = true;
  intensity = 0.55;
  paletteKey = "rain";
  // 'storm' = céu escuro (tempestade); default preserva Lote A
  counted = false;
  drops = [];
  splashes = [];
  activeDrops = 0;
  ...
  init(ctx, env) {
    ...
    for (let p = 0; p < MAX_DROPS && pre > 0; p++) {
      if (!this.drops[p].active) {
        this._spawnDrop(this.drops[p], false);
        this.activeDrops++;
        pre--;
      }
    }
  ...
  update(dt, env) {
    ...
    if (this.activeDrops < this.targetActive) {
      ...
      for (let a = 0; a < MAX_DROPS && toAdd > 0; a++) {
        if (!this.drops[a].active) {
          this._spawnDrop(this.drops[a], true);
          this.activeDrops++;
          toAdd--;
        }
      }
    ...
    for (let s = 0; s < MAX_SPLASH; s++) {
      ...
      if (sp.kind === 0) {
        ...
        sp.x += sp.vx * dt;
        sp.y += sp.vy * dt;
        if (sp.y > this.h) sp.active = false;
      }
  ...
  _applyIntensity(env) {
    ...
    let base;
    if (v < 0.3) base = lerp(20, 220, v / 0.3);
    else if (v < 0.7) base = lerp(220, 900, (v - 0.3) / 0.4);
    else base = lerp(900, MAX_DROPS, (v - 0.7) / 0.3);
    const rm = env.reducedMotion ? 0.15 : 1;
    this.targetActive = Math.round(base * env.densityScale * rm);
    this.speedMul = lerp(0.85, 1.25, v);
    this.wind = lerp(0.06, 0.24, v);
    this.fogAlpha = Math.pow(v, 1.1) * 0.95;
  }
  _spawnDrop(d, fromTop) {
    d.active = true;
    const r = Math.random();
    if (r < 0.16) d.plane = FRONT;
    else if (r < 0.56) d.plane = MID;
    else d.plane = BACK;
    if (d.plane === FRONT) {
      d.len = rand(26, 50);
      d.speed = rand(520, 700);
      d.thick = rand(2.6, 4.2);
      d.alpha = rand(0.62, 0.85);
      d.drawW = d.thick * 2;
      d.splash = true;
    } else if (d.plane === MID) {
      d.len = rand(13, 24);
      d.speed = rand(330, 460);
      d.thick = rand(1.4, 2.2);
      d.alpha = rand(0.26, 0.44);
      d.drawW = d.thick * 2;
      d.splash = false;
    } else {
      d.len = rand(6, 12);
      d.speed = rand(160, 260);
      d.thick = rand(0.7, 1.2);
      d.alpha = rand(0.1, 0.22);
      d.drawW = 0;
      d.splash = false;
    }
  ...
  _spawnSplashCluster(x) {
    this._spawnSplash(1, x, this.h - 2, 0, 0, rand(0.13, 0.2), rand(9, 13));
    const n = 2 + (Math.random() < 0.5 ? 0 : 1);
    for (let k = 0; k < n; k++) this._spawnSplash(0, x + rand(-4, 4), this.h - 3, rand(-120, 120), -rand(150, 300), rand(0.22, 0.4), rand(1.6, 3.4));
  }
  _spawnSplash(kind, x, y, vx, vy, life, size) {
  ...
  _buildSky(env) {
    ...
    this.w = env.width;
    this.h = env.height;
    const pal = this.paletteKey === "storm" ? this.isDay ? SKY.stormDay : SKY.stormNight : this.isDay ? SKY.day : SKY.night;
    ...
    m.addColorStop(0, "rgba(150,175,205,0)");
    m.addColorStop(1, "rgba(150,175,205,0.10)");
    this.mistGrad = m;
  }
  _buildDropSprite(withHighlight) {
    ...
    if (this.isDay) {
      grad.addColorStop(0, "rgba(214,230,255,0.98)");
      grad.addColorStop(0.14, "rgba(202,222,252,0.90)");
      grad.addColorStop(0.55, "rgba(190,214,245,0.38)");
      grad.addColorStop(1, "rgba(188,212,244,0.00)");
    } else {
      grad.addColorStop(0, "rgba(170,196,232,0.95)");
      grad.addColorStop(0.14, "rgba(156,184,224,0.86)");
      grad.addColorStop(0.55, "rgba(140,170,214,0.34)");
      grad.addColorStop(1, "rgba(138,168,212,0.00)");
    }
    g.fillStyle = grad;
    g.beginPath();
    g.moveTo(cx, h);
    ...
    g.fill();
    if (withHighlight) {
      g.strokeStyle = "rgba(242,248,255,0.55)";
      g.lineWidth = w * 0.09;
      g.lineCap = "round";
      g.beginPath();
      g.moveTo(cx - w * 0.13, h * 0.3);
      g.lineTo(cx - w * 0.13, h * 0.72);
      g.stroke();
    }
    return c;
  }
  _buildDot() {
    ...
    g.addColorStop(0, "rgba(220,234,255,0.98)");
    g.addColorStop(0.5, "rgba(198,220,250,0.5)");
  ...
  _buildArc() {
    const w = 48, h = 28, c = document.createElement("canvas");
    c.width = w;
    c.height = h;
    const a = c.getContext("2d");
    a.strokeStyle = "rgba(214,230,255,0.95)";
    a.lineCap = "round";
    const cx = w / 2, base = h - 3;
    a.lineWidth = 1.6;
    a.beginPath();
    a.ellipse(cx, base, 20, 8, 0, Math.PI, Math.PI * 2);
    a.stroke();
    a.lineWidth = 1.9;
    a.strokeStyle = "rgba(232,242,255,0.98)";
    a.beginPath();
    a.arc(cx, base, 10, Math.PI * 1.12, Math.PI * 1.88);
    a.stroke();
    return c;
  }
  _buildFog(ctx) {
    ...
    f.fillStyle = "rgba(172,196,230,0.03)";
    f.fillRect(0, 0, TILE, TILE);
    f.lineCap = "round";
    const slope = 0.16;
    for (let i = 0; i < 60; i++) {
