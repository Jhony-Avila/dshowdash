...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/_shared/core/circuit-breaker
// PURPOSE: Panels Shared - Circuit Breaker
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
export const MODULE_ID = 'panels/_shared/core/circuit-breaker';
...
export class CircuitBreaker {
  ...
  healthCheck() {
    const checks = {
      stateClosed: this.state === 'closed',
      noRecentFailures: this.failures < this.threshold,
      hasSuccesses: this.successes > 0 || this._metrics.totalCalls === 0
    };
...
export function createCircuitBreaker(options: { threshold?: number; timeout?: number; halfOpenMax?: number } = {}) {
