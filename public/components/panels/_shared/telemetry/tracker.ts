...
// ═══════════════════════════════════════════════════════════════
// MODULE: _shared.telemetry.tracker
// PURPOSE: Shared Telemetry Tracker Enterprise
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
const MODULE_ID = '_shared.telemetry.tracker';
...
PanelTelemetryTracker.prototype.trackError = function(error: unknown, context: Record<string, unknown> = {}) { this.metrics.errorCount++; const err = error as { message?: string; name?: string } | null; return this.track(LIFECYCLE_EVENTS.ERROR, { error: err?.message || String(error), errorType: err?.name || 'Error', context, totalErrors: this.metrics.errorCount }); };
