...
const MODULE_ID = "panel-01/core/template";
function renderStructure(container) {
  container.innerHTML = `
    <div class="p01-wrapper" role="region" aria-label="Gest\xE3o de Requisi\xE7\xF5es">
      
      <!-- HEADER: IDENTIDADE + KPIs MINI + A\xC7\xD5ES -->
      <header class="p01-header">
        <div class="p01-identity">
          ...
          <h2 class="p01-title">Gest\xE3o de Requisi\xE7\xF5es</h2>
        </div>

        <div class="p01-health-summary" data-status-badges>
          <span class="p01-health-item p01-health-item--pendente" data-badge="pendente" data-tooltip="Pendente Lan\xE7amento">
          ...
          <span class="p01-health-item p01-health-item--total" data-badge="total" data-tooltip="Total Requisi\xE7\xF5es">
        ...
        <div class="p01-actions">
          ...
          <span class="p01-timestamp" data-last-update data-tooltip="\xDAltima atualiza\xE7\xE3o">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>
            <span>--:--:--</span>
          </span>
        </div>
      ...
      <div class="p01-filters" data-filters>
        <div class="p01-filter-chips">
          <div class="p01-filter-chip" data-filter-chip="situacao">
            ...
            <select data-filter="situacao">
              <option value="">Situa\xE7\xE3o</option>
            </select>
          ...
          <div class="p01-filter-chip p01-filter-chip--date" data-filter-chip="data">
            ...
            <input type="date" data-filter="dataFim" placeholder="At\xE9">
          </div>
        </div>
        ...
        <div class="p01-search">
          ...
          <input type="text" class="p01-search-input" data-filter="q" placeholder="Buscar por descri\xE7\xE3o, fornecedor..." autocomplete="off">
        ...
        <div class="p01-grid-controls">
          <div class="p01-density-cluster" data-tooltip="Densidade">
            ...
            <button class="p01-density-btn" data-density="comfortable" type="button" aria-label="Confort\xE1vel">
          ...
          <select class="p01-group-select" data-action="group-by" data-tooltip="Agrupar">
            ...
            <option value="situacao">Por Situa\xE7\xE3o</option>
      ...
      <!-- BULK ACTIONS (sele\xE7\xE3o m\xFAltipla) -->
...
function populateFilterOptions(container, options) {
  if (!container || !options) return;
  const situacaoSelect = container.querySelector('[data-filter="situacao"]');
  if (situacaoSelect && options.situacoes) {
    const current = situacaoSelect.value;
    situacaoSelect.innerHTML = `<option value="">Situa\xE7\xE3o</option>${options.situacoes.map((s) => `<option value="${s.id}">${s.nome} (${s.qtd})</option>`).join("")}`;
    situacaoSelect.value = current;
  }
  const centroSelect = container.querySelector('[data-filter="centro"]');
  if (centroSelect && options.centros) {
    const current = centroSelect.value;
    centroSelect.innerHTML = `<option value="">Centro Custo</option>${options.centros.map((c) => `<option value="${c.id}">${c.nome} (${c.qtd})</option>`).join("")}`;
