...
const MODULE_ID = "panel-01/ui/date-range-picker";
const PICKER_SVGS = {
...
function DateRangePicker(container, options = {}) {
  ...
  this._dropdown = null;
  this._visible = false;
  this._currentMonth = /* @__PURE__ */ new Date();
...
DateRangePicker.prototype._render = function() {
  ...
  this.container.innerHTML = `<div class="p01-date-range"><button class="p01-date-range-trigger" data-action="toggle"><span class="p01-date-range-icon">${PICKER_SVGS.calendar}</span><span class="p01-date-range-label">Selecionar per\xEDodo</span><span class="p01-date-range-arrow">${PICKER_SVGS.chevronDown}</span></button></div>`;
  this._createDropdown();
};
DateRangePicker.prototype._createDropdown = function() {
  this._dropdown = document.createElement("div");
  this._dropdown.className = "p01-date-range-dropdown";
  this._dropdown.style.display = "none";
  this.container.querySelector(".p01-date-range").appendChild(this._dropdown);
};
...
DateRangePicker.prototype._renderCalendar = function() {
  ...
  ["D", "S", "T", "Q", "Q", "S", "S"].forEach((d) => {
    html += `<span class="p01-calendar-weekday">${d}</span>`;
  });
  html += '</div><div class="p01-calendar-days">';
  for (let i = 0; i < firstDay; i++) {
    html += '<span class="p01-calendar-day p01-calendar-day--empty"></span>';
  }
...
DateRangePicker.prototype._bindEvents = function() {
  ...
  document.addEventListener("click", (e) => {
    if (self._visible && !self.container.contains(e.target)) {
      self.hide();
    }
  }, { signal });
};
DateRangePicker.prototype._bindDropdownEvents = function() {
  ...
  this._dropdown.querySelectorAll("[data-preset]").forEach((btn) => {
    btn.addEventListener("click", () => {
      const preset = PRESETS.find((p) => p.id === btn.dataset.preset);
      if (preset) {
        const range = preset.getValue();
        self._startDate = range.start;
        self._endDate = range.end;
        self._updateInputs();
        self._updateCalendar();
      }
    }, { signal });
  });
  this._dropdown.querySelectorAll("[data-date]").forEach((btn) => {
    btn.addEventListener("click", () => {
      const date = new Date(btn.dataset.date);
      self._selectDate(date);
    }, { signal });
  });
  this._dropdown.querySelector('[data-action="prev-month"]').addEventListener("click", () => {
    self._currentMonth.setMonth(self._currentMonth.getMonth() - 1);
    self._updateCalendar();
  }, { signal });
  this._dropdown.querySelector('[data-action="next-month"]').addEventListener("click", () => {
    self._currentMonth.setMonth(self._currentMonth.getMonth() + 1);
    self._updateCalendar();
  }, { signal });
  this._dropdown.querySelector('[data-action="clear"]').addEventListener("click", () => {
    self.clear();
  }, { signal });
  this._dropdown.querySelector('[data-action="apply"]').addEventListener("click", () => {
    self._apply();
  }, { signal });
  this._dropdown.querySelector('[data-input="start"]').addEventListener("change", (e) => {
    const date = self._parseDate(e.target.value);
    if (date) self._startDate = date;
    self._updateCalendar();
  }, { signal });
  this._dropdown.querySelector('[data-input="end"]').addEventListener("change", (e) => {
    const date = self._parseDate(e.target.value);
    if (date) self._endDate = date;
    self._updateCalendar();
  }, { signal });
};
DateRangePicker.prototype._selectDate = function(date) {
  if (this._selecting === "start" || !this._startDate || date < this._startDate) {
...
DateRangePicker.prototype._updateInputs = function() {
  this._dropdown.querySelector('[data-input="start"]').value = this._formatDate(this._startDate);
  this._dropdown.querySelector('[data-input="end"]').value = this._formatDate(this._endDate);
};
DateRangePicker.prototype._updateCalendar = function() {
  this._dropdown.querySelector(".p01-calendar").innerHTML = this._renderCalendar();
  this._dropdown.querySelector(".p01-calendar-title").textContent = `${this._getMonthName(this._currentMonth)} ${this._currentMonth.getFullYear()}`;
  this._bindDropdownEvents();
};
DateRangePicker.prototype._apply = function() {
  this._updateLabel();
  this.hide();
  this.onChange({ start: this._startDate, end: this._endDate });
};
DateRangePicker.prototype._updateLabel = function() {
  const label = this.container.querySelector(".p01-date-range-label");
  if (this._startDate && this._endDate) {
    label.textContent = `${this._formatDate(this._startDate)} - ${this._formatDate(this._endDate)}`;
  } else if (this._startDate) {
    label.textContent = `A partir de ${this._formatDate(this._startDate)}`;
  } else {
    label.textContent = "Selecionar per\xEDodo";
  }
};
DateRangePicker.prototype._isToday = (date) => {
  const today = /* @__PURE__ */ new Date();
  return date.toDateString() === today.toDateString();
};
DateRangePicker.prototype._isSelected = function(date) {
  return this._startDate && date.toDateString() === this._startDate.toDateString() || this._endDate && date.toDateString() === this._endDate.toDateString();
};
DateRangePicker.prototype._isInRange = function(date) {
  if (!this._startDate || !this._endDate) return false;
  return date > this._startDate && date < this._endDate;
};
DateRangePicker.prototype._isRangeStart = function(date) {
  return this._startDate && date.toDateString() === this._startDate.toDateString();
};
DateRangePicker.prototype._isRangeEnd = function(date) {
  return this._endDate && date.toDateString() === this._endDate.toDateString();
};
DateRangePicker.prototype._formatDate = (date) => {
  if (!date) return "";
...
DateRangePicker.prototype._parseDate = (str) => {
  if (!str) return null;
  const parts = str.split("/");
  if (parts.length !== 3) return null;
  return new Date(parseInt(parts[2]), parseInt(parts[1]) - 1, parseInt(parts[0]));
};
DateRangePicker.prototype._getMonthName = (date) => date.toLocaleDateString("pt-BR", { month: "long" });
...
DateRangePicker.prototype.toggle = function() {
...
DateRangePicker.prototype.setValue = function(start, end) {
...
function healthCheck() {
  return { status: "HEALTHY", moduleId: MODULE_ID, version: VERSION, checks: { cleanupAvailable: true } };
}
var date_range_picker_default = { DateRangePicker, PRESETS, PICKER_SVGS, info, healthCheck };
export {
  DateRangePicker,
  MODULE_ID,
  PRESETS,
