...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-01/ui/filter-presets
// PURPOSE: Panel-01 - Filter Presets
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
//   FilterPresetsManager() — exported function
...
export const MODULE_ID = 'panel-01/ui/filter-presets';
...
export const DEFAULT_PRESETS = [
  ...
  { id: 'high-value', name: 'Alto Valor', icon: 'dollarSign', filters: { valor_min: 10000 }, system: true }
];

const ICON_OPTIONS = ['bookmark', 'star', 'mapPin', 'target', 'briefcase', 'barChart', 'flame', 'gem'];

function _getIconSvg(iconId: string) { return (PRESET_SVGS as Record<string, string>)[iconId] || PRESET_SVGS.bookmark; }

export function FilterPresetsManager(this: any, options: Record<string, unknown> = {}) {
...
FilterPresetsManager.prototype._loadPresets = function() { try { const saved = localStorage.getItem(this.storageKey); this._presets = saved ? JSON.parse(saved) : []; } catch (e) { this._presets = []; } };

FilterPresetsManager.prototype._savePresets = function() { try { localStorage.setItem(this.storageKey, JSON.stringify(this._presets)); } catch (e) {} this.onChange(this.getAllPresets()); };

FilterPresetsManager.prototype.getAllPresets = function() { return DEFAULT_PRESETS.concat(this._presets); };

FilterPresetsManager.prototype.getUserPresets = function() { return this._presets.slice(); };

FilterPresetsManager.prototype.getPreset = function(id: string) { return this.getAllPresets().find((p: Record<string, unknown>) => p.id === id); };

FilterPresetsManager.prototype.savePreset = function(name: string, filters: Record<string, unknown>, icon: string) {
...
FilterPresetsManager.prototype.updatePreset = function(id: string, updates: Record<string, unknown>) { const preset = this._presets.find((p: Record<string, unknown>) => p.id === id); if (preset) { Object.assign(preset, updates); this._savePresets(); } };

FilterPresetsManager.prototype.deletePreset = function(id: string) { this._presets = this._presets.filter((p: Record<string, unknown>) => p.id !== id); this._savePresets(); };

FilterPresetsManager.prototype.applyPreset = function(id: string) { const preset = this.getPreset(id); if (preset) { this.onApply(preset.filters, preset); } };

FilterPresetsManager.prototype.render = function(container: HTMLElement) {
  const self = this;
  if (!container) return;
  ...
  presets.forEach((preset: Record<string, unknown>) => { html += `<button class="p01-preset-btn" data-preset="${preset.id}" title="${preset.name}"><span class="p01-preset-icon">${_getIconSvg(preset.icon as string)}</span><span class="p01-preset-name">${preset.name}</span></button>`; });
  html += '</div></div>';
  container.innerHTML = html;
  container.querySelectorAll('[data-preset]').forEach((btn: Element) => { btn.addEventListener('click', () => { self.applyPreset((btn as HTMLButtonElement).dataset.preset as string); }); });
  const manageBtn = container.querySelector('[data-action="manage"]');
  if (manageBtn) { manageBtn.addEventListener('click', () => { self.openModal(); }); }
};

FilterPresetsManager.prototype.openSaveModal = function(currentFilters: Record<string, unknown>) {
  const self = this;
...
FilterPresetsManager.prototype.openModal = function() {
  ...
  this._modal = document.createElement('div');
  this._modal.className = 'p01-modal-overlay';
  const presetsHtml = this._presets.length === 0 ? '<p class="p01-presets-empty">Nenhum filtro personalizado salvo</p>' : this._presets.map((preset: Record<string, unknown>) => `<div class="p01-preset-manage-item" data-id="${preset.id}"><span class="p01-preset-icon">${_getIconSvg(preset.icon as string)}</span><span class="p01-preset-name">${preset.name}</span><span class="p01-preset-date">${new Date(preset.createdAt as number).toLocaleDateString('pt-BR')}</span><button class="p01-preset-delete" data-action="delete">${PRESET_SVGS.x}</button></div>`).join('');
  ...
  document.body.appendChild(this._modal);
  this._modal.querySelectorAll('[data-action="delete"]').forEach((btn: Element) => { btn.addEventListener('click', () => { const item = btn.closest('[data-id]') as HTMLElement; self.deletePreset(item.dataset.id as string); item.remove(); }); });
  this._modal.querySelectorAll('[data-action="close"]').forEach((btn: Element) => { btn.addEventListener('click', () => { self.closeModal(); }); });
  this._modal.addEventListener('click', (e: MouseEvent) => { if ((e.target as Element).classList.contains('p01-modal-overlay')) self.closeModal(); });
  setTimeout(() => { self._modal.classList.add('open'); }, 10);
};

FilterPresetsManager.prototype.closeModal = function() { const self = this; if (this._modal) { this._modal.classList.remove('open'); setTimeout(() => { if (self._modal && self._modal.parentNode) self._modal.parentNode.removeChild(self._modal); self._modal = null; }, 200); } };

FilterPresetsManager.prototype.destroy = function() { this.closeModal(); this._presets = []; };

export function info() { return { moduleId: MODULE_ID, version: VERSION }; }
export function healthCheck() { return { status: 'HEALTHY', moduleId: MODULE_ID, version: VERSION }; }
export default { FilterPresetsManager, DEFAULT_PRESETS, PRESET_SVGS, info, healthCheck };
