...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-01/ui/numeric-range-filter
// PURPOSE: Panel-01 - Numeric Range Filter
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
//   NumericRangeFilter() — exported function
...
export const MODULE_ID = 'panel-01/ui/numeric-range-filter';

export function NumericRangeFilter(this: any, container: HTMLElement, options: Record<string, unknown> = {}) {
...
NumericRangeFilter.prototype.setRange = function(dataMin: number, dataMax: number) {
  this.min = dataMin;
  this.max = dataMax;
...
NumericRangeFilter.prototype._render = function() {
  ...
  this.container.innerHTML = `<div class="p01-range-filter"><div class="p01-range-header"><span class="p01-range-label">${this.label}</span><span class="p01-range-values">${this._formatValue(this._minValue)} - ${this._formatValue(this._maxValue)}</span></div><div class="p01-range-inputs"><input type="number" class="p01-input p01-input--sm" data-input="min" value="${this._minValue}" min="${this.min}" max="${this.max}" step="${this.step}"><span class="p01-range-separator">até</span><input type="number" class="p01-input p01-input--sm" data-input="max" value="${this._maxValue}" min="${this.min}" max="${this.max}" step="${this.step}"></div><div class="p01-range-slider"><div class="p01-range-track"></div><div class="p01-range-selected" style="left: ${minPercent}%; right: ${100 - maxPercent}%"></div><div class="p01-range-handle p01-range-handle--min" style="left: ${minPercent}%" data-handle="min"></div><div class="p01-range-handle p01-range-handle--max" style="left: ${maxPercent}%" data-handle="max"></div></div><div class="p01-range-presets"><button class="p01-range-preset" data-preset="all">Todos</button><button class="p01-range-preset" data-preset="low">Baixo</button><button class="p01-range-preset" data-preset="medium">Médio</button><button class="p01-range-preset" data-preset="high">Alto</button></div></div>`;
};
...
NumericRangeFilter.prototype._bindEvents = function() {
  ...
  this._onMouseMove = function(e: MouseEvent) {
  ...
  // Input events
  this.container.querySelector('[data-input="min"]').addEventListener('change', (e: Event) => {
    const val = parseFloat((e.target as HTMLInputElement).value) || self.min;
    self._minValue = Math.max(self.min, Math.min(val, self._maxValue));
    self._update();
  });

  this.container.querySelector('[data-input="max"]').addEventListener('change', (e: Event) => {
    const val = parseFloat((e.target as HTMLInputElement).value) || self.max;
  ...
  handles.forEach((handle: Element) => {
    handle.addEventListener('mousedown', (e: Event) => {
      e.preventDefault();
      self._dragging = (handle as HTMLElement).dataset.handle;
      if (self._dragAbortController) {
        self._dragAbortController.abort();
      }
  ...
  // Preset events
  this.container.querySelectorAll('[data-preset]').forEach((btn: Element) => {
    btn.addEventListener('click', () => {
      self._applyPreset((btn as HTMLElement).dataset.preset);
    });
  });
};

NumericRangeFilter.prototype._applyPreset = function(preset: string | undefined) {
...
NumericRangeFilter.prototype._formatValue = function(value: number) {
...
NumericRangeFilter.prototype.setValue = function(min: number, max: number) {
  this._minValue = min;
  this._maxValue = max;
  this._updateUI();
};
...
export default { NumericRangeFilter, info, healthCheck };
