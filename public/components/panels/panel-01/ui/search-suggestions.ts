...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-01/ui/search-suggestions
// PURPOSE: Panel-01 - Search Suggestions (Autocomplete)
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
//   SearchSuggestions() — exported function
...
export const MODULE_ID = 'panel-01/ui/search-suggestions';

export function SearchSuggestions(this: any, inputElement: HTMLInputElement, options: Record<string, unknown> = {}) {
...
SearchSuggestions.prototype.setData = function(data: unknown[]) {
  this._data = data || [];
  this._buildIndex();
};
...
SearchSuggestions.prototype._capitalizeFirst = (str: string) => str.charAt(0).toUpperCase() + str.slice(1);
...
SearchSuggestions.prototype._onKeydown = function(e: KeyboardEvent) {
...
SearchSuggestions.prototype._updateSuggestions = function() {
  ...
  matches.sort((a, b) => {
    const aStarts = (a.text as string).toLowerCase().indexOf(query) === 0;
    const bStarts = (b.text as string).toLowerCase().indexOf(query) === 0;
...
SearchSuggestions.prototype._render = function() {
  ...
  if (this._suggestions.length === 0) {
    this._dropdown.innerHTML = '<div class="p01-suggestion-empty">Nenhuma sugestão encontrada</div>';
    return;
  }
  
  const html = this._suggestions.map((item: Record<string, unknown>, index: number) => {
...
SearchSuggestions.prototype._highlight = (text: string, query: string) => {
...
SearchSuggestions.prototype._bindItemEvents = function() {
  const self = this;
  this._dropdown.querySelectorAll('.p01-suggestion-item').forEach((item: Element) => {
    item.addEventListener('mousedown', (e: Event) => {
      e.preventDefault();
      self._selectedIndex = parseInt((item as HTMLElement).dataset.index as string);
      self._selectCurrent();
    });
...
SearchSuggestions.prototype._updateSelection = function() {
  const self = this;
  this._dropdown.querySelectorAll('.p01-suggestion-item').forEach((item: Element, index: number) => {
...
export default { SearchSuggestions, info, healthCheck };
