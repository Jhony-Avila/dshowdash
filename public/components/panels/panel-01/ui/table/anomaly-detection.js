...
const MODULE_ID = "panel-01/ui/anomaly-detection";
const SVGS = {
...
const ANOMALY_TYPES = { HIGH_VALUE: "high_value", LOW_VALUE: "low_value", OUTLIER: "outlier", UNUSUAL_PATTERN: "unusual_pattern" };
function AnomalyDetector(options) {
  ...
  this.onAnomalyFound = options.onAnomalyFound || (() => {
  });
  this._data = [];
  this._anomalies = [];
  this._stats = null;
}
AnomalyDetector.prototype.setData = function(data) {
  this._data = data || [];
...
AnomalyDetector.prototype._calculateStats = function() {
  const self = this;
  const values = this._data.map((item) => parseFloat(String(item[self.valueField] || item.valor_total || item.Valor_Total || 0))).filter((v) => !isNaN(v));
  if (values.length === 0) {
    this._stats = { mean: 0, stdDev: 0, median: 0, q1: 0, q3: 0, iqr: 0 };
    return;
  }
...
AnomalyDetector.prototype._percentile = (sorted, p) => {
...
AnomalyDetector.prototype._detectAnomalies = function() {
  ...
  this._data.forEach((item, index) => {
    ...
    let anomaly = null;
    if (value > upperBound || zScore > zThreshold) {
      anomaly = { type: ANOMALY_TYPES.HIGH_VALUE, severity: "high", reason: "Valor muito acima da m\xE9dia" };
    } else if (value < lowerBound || zScore < -zThreshold) {
      anomaly = { type: ANOMALY_TYPES.LOW_VALUE, severity: "medium", reason: "Valor muito abaixo da m\xE9dia" };
    } else if (Math.abs(zScore) > zThreshold * 0.7) {
      anomaly = { type: ANOMALY_TYPES.OUTLIER, severity: "low", reason: "Valor fora do padr\xE3o" };
    }
    if (anomaly) {
      anomaly.item = item;
      anomaly.index = index;
      ...
      anomaly.deviation = ((value - self._stats.mean) / self._stats.mean * 100).toFixed(1);
      self._anomalies.push(anomaly);
    }
  });
  this._anomalies.sort((a, b) => Math.abs(b.zScore) - Math.abs(a.zScore));
  if (this._anomalies.length > 0) {
    this.onAnomalyFound(this._anomalies);
  }
};
AnomalyDetector.prototype.getAnomalies = function() {
  return this._anomalies.slice();
};
AnomalyDetector.prototype.getStats = function() {
  return Object.assign({}, this._stats);
};
AnomalyDetector.prototype.isAnomaly = function(item) {
  const id = item.id || item.Id_Requisicao;
  return this._anomalies.some((a) => {
    const aItem = a.item;
    const aId = aItem.id || aItem.Id_Requisicao;
    return aId === id;
  });
};
AnomalyDetector.prototype.getAnomalyInfo = function(item) {
  const id = item.id || item.Id_Requisicao;
  return this._anomalies.find((a) => {
    const aItem = a.item;
    const aId = aItem.id || aItem.Id_Requisicao;
    return aId === id;
  });
};
AnomalyDetector.prototype.getRowStyle = function(item) {
  const anomaly = this.getAnomalyInfo(item);
  ...
  return styles[anomaly.severity] || null;
};
AnomalyDetector.prototype.renderBadge = function(item) {
  const anomaly = this.getAnomalyInfo(item);
  if (!anomaly) return "";
  const icons = { high: SVGS.high, medium: SVGS.medium, low: SVGS.low };
  const colors = { high: "#ef4444", medium: "#f59e0b", low: "#3b82f6" };
  return `<span class="p01-anomaly-badge" style="color: ${colors[anomaly.severity]}" title="${anomaly.reason} (${anomaly.deviation}%)">${icons[anomaly.severity]}</span>`;
};
AnomalyDetector.prototype.setThreshold = function(threshold) {
  this.threshold = threshold;
  this._detectAnomalies();
};
AnomalyDetector.prototype.destroy = function() {
  this._data = [];
  this._anomalies = [];
  this._stats = null;
};
...
var anomaly_detection_default = { AnomalyDetector, ANOMALY_TYPES, info, healthCheck };
export {
  ANOMALY_TYPES,
  AnomalyDetector,
  MODULE_ID,
  VERSION,
  anomaly_detection_default as default,
  healthCheck,
  info
};
