// ═══════════════════════════════════════════════════════════════
// DEPENDENCY CONTRACT (9.2.0-ENTERPRISE-AAA)
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-01/ui/table/render/header
// PURPOSE: Panel-01 Table Header Render
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panel-01/ui/table/render/header';
...
export function renderHeader(options: Record<string, unknown> = {}) {
  const opts = options || {};
  const columns = (opts.columns || COLUMNS) as Record<string, unknown>[];
  const sort = (opts.sort || {}) as Record<string, unknown>;
  const allSelected = opts.allSelected || false;
  const multiSorts = (opts.multiSorts || []) as Record<string, unknown>[];

  const visibleCols = columns.filter((c: Record<string, unknown>) => c.visible);

  const cells = visibleCols.map((col: Record<string, unknown>, _index: number) => {
    if (col.id === 'select') {
...
export default { renderHeader, SORT_SVGS };
