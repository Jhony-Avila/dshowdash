...
const MODULE_ID = "panel-01/ui/row-hover-menu";
...
function RowHoverMenu(options) {
  ...
  this._bound = {
    onMouseEnter: (e) => self._onMouseEnter(e),
    onMouseLeave: (e) => self._onMouseLeave(e),
    onMenuClick: (e) => self._onMenuClick(e),
    onTableMouseEnter: null,
    onTableMouseLeave: null
  };
}
RowHoverMenu.prototype.init = function(tableContainer) {
...
RowHoverMenu.prototype._renderActions = function() {
  return this.actions.map((action) => {
    ...
    return `<button class="p01-row-hover-action${dangerClass}" data-action="${action.id}" title="${action.label}"><span class="p01-row-hover-icon">${ICONS[action.icon] || ""}</span><span class="p01-row-hover-label">${action.label}</span>${shortcut}</button>`;
  }).join("");
};
RowHoverMenu.prototype._bindTableEvents = function() {
  if (!this._tableContainer) return;
  const self = this;
  this._bound.onTableMouseEnter = (e) => {
    const row = e.target.closest("tr[data-id]");
    if (row) self._showMenu(row);
  };
  this._bound.onTableMouseLeave = (e) => {
    const row = e.target.closest("tr[data-id]");
...
RowHoverMenu.prototype._showMenu = function(row) {
...
RowHoverMenu.prototype._onMenuClick = function(e) {
  const btn = e.target.closest("[data-action]");
  if (btn && this._currentRowId) {
    this.onAction(btn.dataset.action, this._currentRowId);
    this._hideMenu();
  }
};
...
var row_hover_menu_default = { RowHoverMenu, DEFAULT_ACTIONS, info, healthCheck };
export {
  DEFAULT_ACTIONS,
  MODULE_ID,
  RowHoverMenu,
  VERSION,
  row_hover_menu_default as default,
  healthCheck,
  info
};
