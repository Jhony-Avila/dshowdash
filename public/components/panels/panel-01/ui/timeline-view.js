...
const MODULE_ID = "panel-01/ui/timeline-view";
function TimelineViewManager(options = {}) {
...
TimelineViewManager.prototype.setData = function(data) {
  this._data = data || [];
  this._groupByDate();
};
...
TimelineViewManager.prototype._renderModal = function() {
  ...
  return `<div class="p01-modal p01-modal--xl"><div class="p01-modal-header"><h2>Timeline de Requisi\xE7\xF5es</h2><button class="p01-modal-close" data-action="close">&times;</button></div><div class="p01-modal-body"><div class="p01-timeline-stats"><div class="p01-timeline-stat"><span class="p01-timeline-stat-value">${stats.totalDays}</span><span class="p01-timeline-stat-label">Dias</span></div><div class="p01-timeline-stat"><span class="p01-timeline-stat-value">${stats.totalItems}</span><span class="p01-timeline-stat-label">Requisi\xE7\xF5es</span></div><div class="p01-timeline-stat"><span class="p01-timeline-stat-value">R$ ${self._formatNumber(stats.totalValue)}</span><span class="p01-timeline-stat-label">Total</span></div><div class="p01-timeline-stat"><span class="p01-timeline-stat-value">R$ ${self._formatNumber(stats.avgPerDay)}</span><span class="p01-timeline-stat-label">M\xE9dia/Dia</span></div></div><div class="p01-timeline">${timelineHtml}</div></div><div class="p01-modal-footer"><button class="p01-btn p01-btn--secondary" data-action="close">Fechar</button></div></div>`;
};
TimelineViewManager.prototype._renderDay = function(dateKey, group) {
  const self = this;
  const date = group.date;
  const dayName = date.toLocaleDateString("pt-BR", { weekday: "short" });
  const dayNum = date.getDate();
  const month = date.toLocaleDateString("pt-BR", { month: "short" });
  let itemsHtml = group.items.slice(0, 5).map((item) => {
  ...
  const moreCount = group.items.length - 5;
  if (moreCount > 0) {
    itemsHtml += `<div class="p01-timeline-more">+ ${moreCount} mais</div>`;
  }
  return `<div class="p01-timeline-day"><div class="p01-timeline-date"><span class="p01-timeline-day-name">${dayName}</span><span class="p01-timeline-day-num">${dayNum}</span><span class="p01-timeline-month">${month}</span></div><div class="p01-timeline-connector"><div class="p01-timeline-dot"></div><div class="p01-timeline-line"></div></div><div class="p01-timeline-content"><div class="p01-timeline-summary"><span class="p01-timeline-count">${group.count} requisi\xE7\xF5es</span><span class="p01-timeline-total">R$ ${this._formatNumber(group.total)}</span></div><div class="p01-timeline-items">${itemsHtml}</div></div></div>`;
};
TimelineViewManager.prototype._calculateStats = function() {
...
TimelineViewManager.prototype._formatNumber = (num) => {
...
TimelineViewManager.prototype._truncate = (str, len) => {
  if (!str) return "";
...
var timeline_view_default = { TimelineViewManager, info, healthCheck };
export {
  MODULE_ID,
  TimelineViewManager,
  VERSION,
  timeline_view_default as default,
  healthCheck,
  info
};
