...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-01/ui/toolbar
// PURPOSE: Panel-01 Toolbar Component
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
export const MODULE_ID = 'panel-01/ui/toolbar';
...
export class ToolbarComponent {
  ...
  render(state: { loading?: boolean; autoRefresh?: boolean; countdown?: number; density?: string } = {}) {
      ...
      <div class="p01-toolbar">
        ...
        <div class="p01-toolbar-center">
          <div class="p01-density-cluster">
            ...
            <button class="p01-density-btn ${state.density === 'comfortable' ? 'active' : ''}" data-density="comfortable" title="Confortável">
  ...
  _bindEvents() {
    ...
    this._listener = (e: MouseEvent) => {
      ...
      const density = (e.target as Element).closest('[data-density]') as HTMLElement | null;
      if (density) {
  ...
  setDensity(density: string) {
    this.container?.querySelectorAll('[data-density]').forEach((btn: Element) => {
      btn.classList.toggle('active', (btn as HTMLElement).dataset.density === density);
    });
  }
