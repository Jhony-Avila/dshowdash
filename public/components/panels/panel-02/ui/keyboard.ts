...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-02/ui/keyboard
// PURPOSE: Panel-02 Keyboard Navigation
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
export const MODULE_ID = 'panel-02/ui/keyboard';

export class KeyboardNavigation {
  container: HTMLElement;
  onRowSelect: (row: Element) => void;
  onRowActivate: (row: Element) => void;
  currentIndex: number;
  ...
  constructor(container: HTMLElement, options: Record<string, unknown> = {}) {
    ...
    this.onRowSelect = (options.onRowSelect as (row: Element) => void) || (() => {});
    this.onRowActivate = (options.onRowActivate as (row: Element) => void) || (() => {});
    this.currentIndex = -1;
  ...
  init() {
    this._keyHandler = (e: KeyboardEvent) => {
      ...
      const tbody = this.container.querySelector('.p02-tbody');
      if (!tbody) return;
      const rows: Element[] = Array.from(tbody.querySelectorAll('tr[data-job-id]'));
  ...
  navigate(rows: Element[], direction: number) {
  ...
  goTo(rows: Element[], index: number) {
    rows.forEach((r: Element) => r.classList.remove('p02-row-selected'));
    this.currentIndex = index;
    const row = rows[index];
    if (row) {
      row.classList.add('p02-row-selected');
      (row as HTMLElement).scrollIntoView({ block: 'nearest', behavior: 'smooth' });
      this.onRowSelect(row);
    }
  }

  clearSelection(rows: Element[]) {
    rows.forEach((r: Element) => r.classList.remove('p02-row-selected'));
    this.currentIndex = -1;
  }
