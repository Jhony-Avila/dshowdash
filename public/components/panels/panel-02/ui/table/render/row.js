...
function createJobRow(job, index, columns, expandedRows, groupKey = null, isHidden = false) {
  const jobId = job.id || "";
  ...
  const rateClass = getRateClass(successRate, job.sla);
  ...
  const cells = visibleCols.map((col) => {
    switch (col.id) {
      case "select":
        return `<td class="p02-checkbox-col" role="cell"><input type="checkbox" class="p02-row-checkbox" data-job-id="${jobId}" aria-label="Selecionar job ${jobName}"></td>`;
      case "expand":
        return `<td class="p02-expand-col" role="cell"><button class="p02-expand-btn ${isExpanded ? "expanded" : ""}" aria-expanded="${isExpanded}" aria-label="Expandir detalhes" type="button"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"/></svg></button></td>`;
      case "job_name":
        return `<td class="p02-job-name" title="${jobName}" role="cell">${jobName}</td>`;
      case "job_type":
        return `<td role="cell"><span class="p02-job-type ${typeClass}">${typeIcon}${jobType}</span></td>`;
      case "health_status":
        return `<td class="center" role="cell"><span class="p02-status-indicator ${statusData.className}"><span class="p02-status-dot" aria-hidden="true"></span>${statusData.text}</span></td>`;
      case "last_execution":
        return `<td class="p02-datetime" role="cell">${lastExec}</td>`;
      case "total_executions":
        return `<td class="p02-executions right" role="cell">${formatNumber(totalExec)}</td>`;
      case "success_rate":
        return `<td class="center" role="cell"><span class="p02-success-rate ${rateClass}">${successRate.toFixed(1)}%</span></td>`;
      case "avg_execution_time":
        return `<td class="p02-avg-time right" role="cell">${avgTime}</td>`;
      case "error_count":
        return `<td class="p02-errors ${errorClass} right" role="cell">${errors}</td>`;
      case "actions":
        return `<td class="p02-actions-col" role="cell"><div class="p02-inline-actions">
...
function renderRows(jobs, columns, expandedRows) {
  return jobs.map((job, idx) => createJobRow(job, idx, columns, expandedRows)).join("");
}
function renderGroupedRows(jobs, columns, expandedRows, collapsedGroups, groupBy) {
...
function renderEmpty(colCount) {
  return `<tr><td colspan="${colCount}" class="p02-empty" role="cell">Nenhum job encontrado</td></tr>`;
}
var row_default = { createJobRow, renderRows, renderGroupedRows, renderEmpty };
const MODULE_ID = "panel-02/ui/table/render/row";
const VERSION = "9.3.0-P2-ENTERPRISE";
function info() {
  return { moduleId: MODULE_ID, version: VERSION };
}
function healthCheck() {
  return { status: "HEALTHY", moduleId: MODULE_ID, version: VERSION, checks: { rowReady: true } };
}
export {
  MODULE_ID,
  VERSION,
