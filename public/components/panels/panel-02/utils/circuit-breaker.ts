...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-02/utils/circuit-breaker
// PURPOSE: Panel-02 - Circuit Breaker Enterprise
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
export const MODULE_ID = 'panel-02/utils/circuit-breaker';
...
export class CircuitBreaker {
  ...
  recordFailure(error: Error | unknown) { this.failures++; this.lastFailureTime = Date.now(); if (this.failures >= this.threshold && this.state !== STATES.OPEN) { this.setState(STATES.OPEN); this.logger?.warn?.('circuit.open', { failures: this.failures, threshold: this.threshold, error: (error as Error)?.message }); } }
