...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-02.utils.logger
// PURPOSE: Panel-02 - Logger Enterprise
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panel-02.utils.logger';
...
Logger.prototype._shouldLog = function(level: string) { return (LOG_LEVELS as Record<string, number>)[level] >= (LOG_LEVELS as Record<string, number>)[this.minLevel]; };
...
Logger.prototype.log = function(level: string, event: string, data?: Record<string, unknown>) { if (!this._shouldLog(level)) return; const entry = this._formatEntry(level, event, data); this.buffer.push(entry); if (this.buffer.length > this.maxBuffer) this.buffer.shift(); const logger = _getPort('logger') as Record<string, unknown> | null; if (logger) { const fn = (logger[level] || logger['info']) as ((...a: unknown[]) => void) | undefined; if (typeof fn === 'function') fn.call(logger, `[${this.panelId}] ${event}`, entry); } };
