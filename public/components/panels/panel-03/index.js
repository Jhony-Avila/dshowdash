...
const _log = (level, ...args) => {
  ...
  if (typeof fn === "function") fn(`[Panel-03]`, ...args);
};
...
(() => {
  ...
  if (assetLoader?.loadCSS) assetLoader.loadCSS(cssPath);
    ...
    link.rel = "stylesheet";
    link.href = cssPath;
    link.setAttribute("data-painel", "panel-03");
  ...
  setupStateSubscription() {
    const unsubscribe = this.store.subscribe((state) => {
      if (state.loading) {
        ...
        if (!state.data && this.uiComponent) this.uiComponent.showError(getErrorMessage(String(state.error)));
...
const healthCheck = () => {
  ...
  const checks = { instanceExists: !!painelInstance, mounted: status.mounted === true, notDestroyed: status.destroyed !== true, notDegraded: status.isDegraded !== true, lowErrorCount: (status.consecutiveErrors || 0) < 5, portsInitialized: Ports.isInitialized(), isAuthenticated: _isAuthenticated(), abortControllerActive: !!painelInstance?.abortController && !painelInstance.abortController.signal.aborted, noOrphanTimers: status.destroyed !== true || !painelInstance?.countdownInterval, noOrphanPolling: status.destroyed !== true || !painelInstance?.autoRefreshEnabled };
...
var panel_03_default = { mount, unmount, destroy, getStatus, getVersion, healthCheck, info, injectPorts, getPorts };
export {
  MODULE_ID,
  VERSION,
  panel_03_default as default,
  destroy,
  getPorts,
  getStatus,
  getVersion,
