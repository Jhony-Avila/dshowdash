...
const MODULE_ID = "panel-03:ui:component";
const ICONS = {
...
class UIComponent {
  ...
  showLoading() {
    if (this.hasData) {
      this.container.classList.add("p03-loading-overlay");
      return;
    }
    this.container.innerHTML = renderSkeleton();
  }
  hideLoading() {
    this.container.classList.remove("p03-loading-overlay");
  }
  showError(msg) {
      ...
      <div class="p03-dashboard">
        ...
        <div class="p03-chart-section">
          <div class="p03-section-header">
            <h3 class="p03-section-title">${ICONS.barChart} <span>Execu\xE7\xF5es por Hora</span></h3>
  ...
  _renderChart() {
    ...
    const max = Math.max(...this.hourly.map((h) => h.total));
    const bars = this.hourly.slice(0, 24).map((h) => {
      const total = h.total;
      const pct = max > 0 ? total / max * 100 : 0;
      const hour = h.hour.split(" ")[1]?.slice(0, 5) || h.hour.slice(11, 16);
      return `<div class="p03-bar-group"><div class="p03-bar" style="height:${pct}%" title="${total} execu\xE7\xF5es"><div class="p03-bar-success" style="height:${h.success_rate}%"></div></div><span class="p03-bar-label">${hour}</span></div>`;
    }).join("");
    return `<div class="p03-bar-chart">${bars}</div>`;
  }
  _renderTable() {
    const rows = this.hourly.slice(0, 12).map((h) => {
      const rate = h.success_rate;
      const rateClass = rate >= 95 ? "p03-rate--high" : rate >= 80 ? "p03-rate--medium" : "p03-rate--low";
      return `<tr class="p03-tr"><td class="p03-td">${this._formatHour(h.hour)}</td><td class="p03-td">${h.total}</td><td class="p03-td p03-td--success">${h.success}</td><td class="p03-td p03-td--error">${h.errors}</td><td class="p03-td"><span class="p03-rate ${rateClass}">${rate}%</span></td><td class="p03-td">${h.avg_time?.toFixed(1)}s</td></tr>`;
    }).join("");
  ...
  _export() {
    ...
    const rows = this.hourly.map((h) => [h.hour, h.total, h.success, h.errors, h.success_rate, h.avg_time]);
    this._downloadCSV(["Hora", "Total", "Sucesso", "Erros", "Taxa (%)", "Tempo M\xE9dio"], rows, "execucoes-por-hora");
  ...
  _formatHour(h) {
    if (!h) return "--";
    return h.split(" ")[1]?.slice(0, 5) || h.slice(11, 16);
  }
  async destroy() {
