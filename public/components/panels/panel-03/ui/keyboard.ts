...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-03/ui/keyboard
// PURPOSE: Panel-03 Keyboard Navigation
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
export const MODULE_ID = 'panel-03/ui/keyboard';
...
KeyboardNavigation.prototype.init = function() {
    ...
    self._keyHandler = (e: KeyboardEvent) => {
        ...
        const tbody = self.container.querySelector('.p03-tbody');
        if (!tbody) return;
...
KeyboardNavigation.prototype.goTo = function(rows: HTMLElement[], index: number) { rows.forEach((r: HTMLElement) => r.classList.remove('p03-row-selected')); this.currentIndex = index; const row = rows[index]; if (row) { row.classList.add('p03-row-selected'); row.scrollIntoView({ block: 'nearest', behavior: 'smooth' }); this.onRowSelect(row); } };
KeyboardNavigation.prototype.clearSelection = function(rows: HTMLElement[]) { rows.forEach((r: HTMLElement) => r.classList.remove('p03-row-selected')); this.currentIndex = -1; };
