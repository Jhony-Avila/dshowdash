...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-03.utils.logger
// PURPOSE: Panel-03 - Logger Enterprise
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panel-03.utils.logger';
...
Logger.prototype.log = function(level: string, event: string, data: Record<string, unknown>) { if (!this._shouldLog(level)) return; const entry = this._formatEntry(level, event, data); this.buffer.push(entry); if (this.buffer.length > this.maxBuffer) this.buffer.shift(); const logger = _getPort('logger') as Record<string, unknown>; if (logger) { const fn = logger[level] as ((...args: unknown[]) => void) | undefined; if (typeof fn === 'function') fn.call(logger, `[${this.panelId}] ${event}`, entry); } };
