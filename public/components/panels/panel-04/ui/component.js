...
class UIComponent {
  constructor(container, options = {}) {
    ...
    this._expandedJobs = /* @__PURE__ */ new Set();
  ...
  _playNotificationSound() {
    ...
    try {
      this._notificationSound.play().catch(() => {
      });
    } catch {
    }
  }
  ...
  _getFilteredData() {
    ...
    if (this._filterText) {
      data = data.filter(
        (d) => d.job_name?.toLowerCase().includes(this._filterText) || d.message?.toLowerCase().includes(this._filterText)
      );
    }
    data.sort((a, b) => {
