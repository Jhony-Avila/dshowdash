...
const MODULE_ID = "panel-04-ui-table";
...
function renderTable(container, data, options = {}) {
  if (!container) return;
  const expandedRows = options && options.expandedRows || /* @__PURE__ */ new Set();
  ...
  if (countEl) {
    countEl.textContent = `${totalCount} registro${totalCount !== 1 ? "s" : ""}`;
  }
  ...
  const rows = pageData.map((d) => {
    const alertConfig = getAlertConfig(d.type);
    ...
    const recent = isRecent(d.created_at, 5);
    const jobName = escapeHtml(d.job_name || "--");
  ...
  if (onRowClick) {
    tbody.querySelectorAll("[data-row]").forEach((el) => {
      ...
      row.addEventListener("click", () => {
        ...
        if (!isNaN(id)) onRowClick(id);
      });
      row.addEventListener("mouseenter", () => {
        row.style.background = "#1a1a24";
      });
      row.addEventListener("mouseleave", () => {
        const recent = row.classList.contains("p04-row-recent");
        row.style.background = recent ? "rgba(99,102,241,0.05)" : "transparent";
      });
    });
  }
}
function showLoading(container) {
  ...
  let skeletonRows = "";
  for (let i = 0; i < 5; i++) {
    skeletonRows += '<tr><td style="padding:10px 14px;"><div class="p04-skeleton" style="width:60px;height:20px;"></div></td><td style="padding:10px 14px;"><div class="p04-skeleton" style="width:100px;height:16px;"></div></td><td style="padding:10px 14px;"><div class="p04-skeleton" style="width:200px;height:16px;"></div></td><td style="padding:10px 14px;"><div class="p04-skeleton" style="width:60px;height:16px;"></div></td></tr>';
  }
...
var table_default = { renderTable, showLoading, showError, clearTable };
export {
  MODULE_ID,
  VERSION,
  clearTable,
