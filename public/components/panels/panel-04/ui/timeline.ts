...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-04-ui-timeline
// PURPOSE: Panel-04 UI - Timeline & Top Jobs
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
//   clearTimeline() — exported function
//   clearTopJobs() — exported function
...
export const MODULE_ID = 'panel-04-ui-timeline';
export const VERSION = '9.3.0-P2-ENTERPRISE';

const SEVERITY_COLORS = { critical: '#dc2626', high: '#ef4444', medium: '#f59e0b', low: '#22c55e', default: '#6366f1' };

export function renderTimeline(container: HTMLElement, timeline: Record<string, unknown>[]) {
  if (!container) return;
  const timelineEl = container.querySelector('[data-timeline]');
  if (!timelineEl) return;

  if (!timeline || !timeline.length) {
    timelineEl.innerHTML = '<div style="height:100%;display:flex;align-items:center;justify-content:center;color:#606070;font-size:11px;"><span>Sem dados no período</span></div>';
    return;
  }

  const maxTotal = Math.max(...timeline.map((t: Record<string, unknown>) => (t.total as number) || 0).concat([1]));

  const bars = timeline.map((t: Record<string, unknown>, i: number) => {
    const height = Math.max((((t.total as number) || 0) / maxTotal) * 100, 3);
    const hour = t.hour ? new Date(t.hour as string | number).getHours() : i;
    let color = SEVERITY_COLORS.default;
    if ((t.critical as number) > 0) color = SEVERITY_COLORS.critical;
    else if ((t.high as number) > 0) color = SEVERITY_COLORS.high;
    else if ((t.medium as number) > 0) color = SEVERITY_COLORS.medium;
    else if ((t.low as number) > 0) color = SEVERITY_COLORS.low;
    const showLabel = timeline.length <= 12 || i % Math.ceil(timeline.length / 8) === 0;
    return `<div class="p04-timeline-bar" style="display:flex;flex-direction:column;align-items:center;flex:1;min-width:0;cursor:pointer;" title="${t.total || 0} alertas às ${hour}h&#10;Crítico: ${t.critical || 0}&#10;Alto: ${t.high || 0}&#10;Médio: ${t.medium || 0}&#10;Baixo: ${t.low || 0}"><div style="width:70%;height:${height}%;background:${color};border-radius:2px 2px 0 0;min-height:2px;transition:all 0.3s ease;opacity:0.8;" onmouseover="this.style.opacity='1';this.style.transform='scaleX(1.2)'" onmouseout="this.style.opacity='0.8';this.style.transform='scaleX(1)'"></div>${showLabel ? `<span style="font-size:8px;color:#606070;margin-top:2px;">${hour}h</span>` : ''}</div>`;
  }).join('');

  timelineEl.innerHTML = `<div style="display:flex;align-items:flex-end;height:65px;gap:1px;padding:0 2px;">${bars}</div>`;
}

export function renderTopJobs(container: HTMLElement, topJobs: Record<string, unknown>[]) {
  if (!container) return;
  ...
  const maxAlerts = Math.max(...topJobs.map((j: Record<string, unknown>) => parseInt(String(j.total_alerts)) || 0).concat([1]));

  const items = topJobs.slice(0, 5).map((job: Record<string, unknown>, index: number) => {
    const total = parseInt(String(job.total_alerts)) || 0;
    const pct = (total / maxAlerts) * 100;
    const jobName = (job.job_name as string) || 'Unknown';
...
export function clearTimeline(container: HTMLElement) {
  if (!container) return;
  const timelineEl = container.querySelector('[data-timeline]');
  if (timelineEl) timelineEl.innerHTML = '';
}

export function clearTopJobs(container: HTMLElement) {
  if (!container) return;
  const topJobsEl = container.querySelector('[data-top-jobs]');
...
export default { renderTimeline, renderTopJobs, clearTimeline, clearTopJobs, healthCheck, info };
