...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-05:managers:export-controller
// PURPOSE: Panel-05 - Export Controller
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   store from ../state/store.js
//   exportManager from ../utils/export-manager.js
//   toastManager from ../ui/toast.js
//   log from ../utils/lifecycle.js
//
// PROVIDES:
...
export const MODULE_ID = 'panel-05:managers:export-controller';
...
export async function exportData(format: string) {
    ...
    try {
        await exportManager.export(format, clientes, { filename: `clientes_${new Date().toISOString().split('T')[0]}`, columns: DEFAULT_COLUMNS });
        toastManager.success('Exportação concluída!');
...
export function info() { return { moduleId: MODULE_ID, version: VERSION, columns: DEFAULT_COLUMNS.length, p25Compliant: true }; }

export default { exportData, healthCheck, info, VERSION, MODULE_ID };
