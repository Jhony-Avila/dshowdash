...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-05:charts
// PURPOSE: Panel-05 Charts Renderer - Enterprise Premium AAA
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panel-05:charts';
...
class ChartsRenderer {
  ...
  renderReceitaMensal(data: Array<Record<string, unknown>> = [], options: Record<string, unknown> = {}) {
  ...
  renderDonutChart(data: Array<Record<string, unknown>> = [], options: Record<string, unknown> = {}) {
  ...
  renderTopClientes(clientes: Array<Record<string, unknown>> = [], options: Record<string, unknown> = {}) {
  ...
  renderVendedores(vendedores: Array<Record<string, unknown>> = []) {
    if (!vendedores?.length) return `<div class="p05-chart-empty"><p>Sem dados de vendedores</p></div>`;
    return `<div class="p05-chart p05-chart-table"><div class="p05-chart-header"><h4 class="p05-chart-title">Performance Vendedores</h4></div><div class="p05-chart-body"><table class="p05-vendedores-table"><thead><tr><th>Vendedor</th><th>Receita</th><th>Ganhos</th><th>Conv.</th><th>Ticket</th><th>Clientes</th></tr></thead><tbody>${vendedores.map(v => { const conversao = Number(v.conversao) || 0; return `<tr><td class="p05-td-nome">${this._escapeHtml(v.nome)}</td><td class="p05-td-value">${this._formatCurrencyCompact(v.receita)}</td><td class="p05-td-value">${this._formatNumber(v.ganhos)}</td><td><span class="p05-badge ${conversao >= 30 ? 'p05-badge-success' : conversao >= 15 ? 'p05-badge-warning' : 'p05-badge-danger'}">${conversao.toFixed(1)}%</span></td><td class="p05-td-value">${this._formatCurrencyCompact(v.ticketMedio)}</td><td class="p05-td-value">${this._formatNumber(v.clientes)}</td></tr>`; }).join('')}</tbody></table></div></div>`;
  }

  renderPorUF(data: Array<Record<string, unknown>> = [], options: Record<string, unknown> = {}) {
  ...
  renderTimeline(eventos: Array<Record<string, unknown>> = [], options: Record<string, unknown> = {}) {
    const { limit = 10 } = options;
    if (!eventos?.length) return `<div class="p05-chart-empty"><p>Sem eventos recentes</p></div>`;
    const eventIcons: Record<string, { icon: string; className: string }> = { criacao: { icon: icon('charts:activity'), className: 'p05-event-info' }, ganho: { icon: icon('ui:check-circle'), className: 'p05-event-success' }, perdido: { icon: icon('ui:x-circle'), className: 'p05-event-danger' }, andamento: { icon: icon('system:clock'), className: 'p05-event-warning' } };
    return `<div class="p05-chart p05-chart-timeline"><div class="p05-chart-header"><h4 class="p05-chart-title">Timeline</h4></div><div class="p05-chart-body"><div class="p05-timeline">${eventos.slice(0, Number(limit)).map(evento => {
      const config = eventIcons[evento.tipo as string] || eventIcons.criacao;
  ...
  renderSparkline(data: Array<Record<string, unknown> | number> = [], options: Record<string, unknown> = {}) {
    ...
    const values = data.map(d => typeof d === 'object' ? Number((d as Record<string, unknown>).value) : Number(d)).filter(v => !isNaN(v));
  ...
  _formatRelativeTime(date) { if (!date) return '—'; const d = date instanceof Date ? date : new Date(date); if (isNaN(d.getTime())) return '—'; const now = new Date(); const diffMs = now - d; const diffMin = Math.floor(diffMs / 60000); const diffHour = Math.floor(diffMin / 60); const diffDay = Math.floor(diffHour / 24); if (diffMin < 60) return `${diffMin}min`; if (diffHour < 24) return `${diffHour}h`; if (diffDay < 7) return `${diffDay}d`; return d.toLocaleDateString('pt-BR'); }
