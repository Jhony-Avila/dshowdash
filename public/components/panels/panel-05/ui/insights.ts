...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-05:insights
// PURPOSE: Panel-05 Insights Renderer - Enterprise Premium AAA
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
//   insightsRenderer — exported value
//
...
export const MODULE_ID = 'panel-05:insights';

// Icon helper - busca no IconRegistry
...
class InsightsRenderer {
  ...
  renderInsightsPanel(insights: Array<Record<string, unknown>> = []) {
    if (!insights?.length) {
  ...
  _renderInsightCard(insight: Record<string, unknown>) {
    const { tipo = 'info', nivel = 'info', titulo, descricao, valor, variacao, acao } = insight;
    const iconName = INSIGHT_ICON_MAP[tipo as string] || 'info';
    const icon = getIcon(iconName);
    
    const nivelClassMap: Record<string, string> = {
    ...
    const nivelClass = nivelClassMap[nivel as string] || 'p05-insight-info';
        ...
        <div class="p05-insight-content">
          ...
          ${valor !== undefined ? `
            <div class="p05-insight-value">
              ...
              ${variacao !== undefined ? this._renderVariacao(Number(variacao)) : ''}
            </div>
          ` : ''}
  ...
  _renderVariacao(variacao: number) {
  ...
  renderComparativo(data = {}, options: Record<string, unknown> = {}) {
    ...
    return `
      <div class="p05-comparativo">
        ...
        <div class="p05-comparativo-body">
          ${metricas.length ? (metricas as Array<Record<string, unknown>>).map((m: Record<string, unknown>) => this._renderComparativoMetrica(m)).join('') : '<p class="p05-comparativo-empty">Sem dados para comparacao</p>'}
        </div>
      </div>
    `;
  }

  _renderComparativoMetrica(metrica: Record<string, unknown>) {
    const { label, atual, anterior, formato = 'number' } = metrica;
    const variacao = Number(metrica.variacao ?? 0);
    const sparkline = (metrica.sparkline as Array<unknown>) || [];
    const isPositive = variacao > 0;
  ...
  _renderMiniSparkline(data: Array<unknown>) {
    if (!data?.length) return '';
    const width = 60, height = 20, padding = 2;
    const values = data.map((d: unknown) => typeof d === 'object' ? (d as Record<string, unknown>).value as number : d as number);
  ...
  renderQuickStats(stats: Array<Record<string, unknown>> = []) {
  ...
  renderAlertBanner(alert: { tipo?: string; titulo?: string; mensagem?: string; dismissible?: boolean; acoes?: Array<Record<string, unknown>> } = {}) {
  ...
  _formatValue(value: unknown, formato: unknown = 'number') {
    if (value === null || value === undefined) return '—';
    switch (formato as string) {
      case 'currency': return this._formatCurrency(value);
      case 'percent': return `${Number(value).toFixed(1)}%`;
      case 'compact': return this._formatCompact(value);
      default: return this._formatNumber(value);
    }
  }

  _formatNumber(value: unknown) {
    if (value === null || value === undefined || isNaN(Number(value))) return '—';
    return new Intl.NumberFormat('pt-BR').format(Number(value));
  }

  _formatCurrency(value: unknown) {
    if (value === null || value === undefined || isNaN(Number(value))) return '—';
    return new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(Number(value));
  }

  _formatCompact(value: unknown) {
    if (value === null || value === undefined || isNaN(Number(value))) return '—';
