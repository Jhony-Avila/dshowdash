...
const MODULE_ID = "panel-05:export-manager";
const icon = (name) => IconRegistry.get(name) || "";
const EXPORT_FORMATS = {
  csv: { id: "csv", label: "CSV", mime: "text/csv", ext: ".csv", description: "Arquivo separado por v\xEDrgulas" },
...
class ExportManager {
  ...
  getFormats() {
    return Object.values(EXPORT_FORMATS).map((f) => ({ ...f, icon: f.id === "csv" ? icon("business:file-text") : f.id === "excel" ? icon("table:file-spreadsheet") : icon("table:file-pdf") }));
  }
  isProcessing() {
  ...
  async exportCSV(data, options = {}) {
    ...
    try {
      const csv = this._dataToCSV(data, columns, includeHeaders);
      this._downloadFile(csv, `${filename}.csv`, "text/csv;charset=utf-8");
      return { success: true, rows: data.length, format: "csv" };
    } finally {
      this._processing = false;
    }
  }
  async exportExcel(data, options = {}) {
    ...
    try {
      if (apiEndpoint) {
        const response = await fetch(String(apiEndpoint), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ data, sheetName, columns }), signal: this._abortController.signal });
        if (!response.ok) throw new Error("Erro ao gerar Excel");
        const blob = await response.blob();
        this._downloadBlob(blob, `${filename}.xlsx`);
      } else {
        const csv = this._dataToCSV(data, columns, true);
        this._downloadFile(csv, `${filename}.csv`, "text/csv;charset=utf-8");
      }
  ...
  async exportPDF(data, options = {}) {
    const { filename = "export", title = "Relat\xF3rio", apiEndpoint = null, orientation = "portrait", columns = null } = options;
    if (!data?.length) throw new Error("Nenhum dado para exportar");
    this._processing = true;
    this._abortController = new AbortController();
    try {
      if (apiEndpoint) {
        const response = await fetch(String(apiEndpoint), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ data, title, orientation, columns }), signal: this._abortController.signal });
        if (!response.ok) throw new Error("Erro ao gerar PDF");
        const blob = await response.blob();
        this._downloadBlob(blob, `${filename}.pdf`);
      } else {
        this._printData(data, String(title), columns);
      }
  ...
  async export(format, data, options = {}) {
    switch (format) {
      ...
      default:
        throw new Error(`Formato n\xE3o suportado: ${format}`);
    }
  }
  renderExportButton(options = {}) {
  ...
  _dataToCSV(data, columns, includeHeaders) {
    if (!data?.length) return "";
    const keys = columns || Object.keys(data[0]);
    const rows = [];
    if (includeHeaders) rows.push(keys.map((k) => this._escapeCSV(this._formatHeader(k))).join(";"));
    for (const item of data) {
      const row = keys.map((k) => {
        const val = item[k];
        return this._escapeCSV(this._formatValue(val));
      });
      rows.push(row.join(";"));
    }
    return `\uFEFF${rows.join("\n")}`;
  ...
  _formatValue(val) {
    if (val === null || val === void 0) return "";
    if (typeof val === "number") return new Intl.NumberFormat("pt-BR").format(val);
  ...
  _printData(data, title, columns) {
    const keys = columns || Object.keys(data[0] || {});
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${this._escapeHtml(title)}</title><style>body { font-family: Arial, sans-serif; font-size: 12px; margin: 20px; } h1 { font-size: 18px; margin-bottom: 16px; } table { width: 100%; border-collapse: collapse; } th, td { border: 1px solid #ddd; padding: 8px; text-align: left; } th { background: #f5f5f5; font-weight: 600; } tr:nth-child(even) { background: #fafafa; } .footer { margin-top: 20px; font-size: 10px; color: #666; }</style></head><body><h1>${this._escapeHtml(title)}</h1><table><thead><tr>${keys.map((k) => `<th>${this._escapeHtml(this._formatHeader(k))}</th>`).join("")}</tr></thead><tbody>${data.map((row) => `<tr>${keys.map((k) => `<td>${this._escapeHtml(this._formatValue(row[k]))}</td>`).join("")}</tr>`).join("")}</tbody></table><div class="footer">Gerado em ${(/* @__PURE__ */ new Date()).toLocaleString("pt-BR")} | Total: ${data.length} registros</div></body></html>`;
  ...
  info() {
    return { moduleId: MODULE_ID, version: VERSION, formats: Object.keys(EXPORT_FORMATS), processing: this._processing, source: "IconRegistry" };
  }
  healthCheck() {
...
var export_manager_default = exportManager;
export {
  MODULE_ID,
  VERSION,
