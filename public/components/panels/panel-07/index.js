...
(() => {
  ...
  if (typeof window !== "undefined" && window.AssetLoader) window.AssetLoader.loadCSS(cssPath);
    ...
    link.rel = "stylesheet";
    link.href = cssPath;
    link.setAttribute("data-painel", "panel-07");
...
class Painel07 {
  ...
  unmount(container, deps) {
    ...
    return p.then(() => {
      ...
      eb?.emit?.(PANEL_EVENTS.UNMOUNTED, { panelId: PAINEL_ID, version: VERSION, timestamp: Date.now(), source: MODULE_ID });
    });
  }
  markLoaded() {
    const eb = _getPort("eventBus");
...
const mount = (container, deps = {}) => {
  if (painelInstance) {
    const cur = painelInstance.container;
    if (cur && document.contains(cur) && cur === container && painelInstance.mounted) return Promise.resolve();
    return unmount().then(() => {
      painelInstance = new Painel07();
...
const getStatus = () => painelInstance?.getStatus?.() ?? { panelId: PAINEL_ID, moduleId: MODULE_ID, mounted: false, p22Compliant: true };
const getVersion = () => VERSION;
const healthCheck = () => {
  const status = getStatus();
  const checks = { instanceExists: !!painelInstance, mounted: status.mounted === true, notDestroyed: status.destroyed !== true, notDegraded: status.isDegraded !== true, lowErrorCount: (status.consecutiveErrors || 0) < 5, isAuthenticated: _isAuthenticated(), abortControllerActive: !!painelInstance?.abortController && !painelInstance.abortController.signal.aborted, noOrphanTimers: status.destroyed !== true || !painelInstance?.countdownInterval, noOrphanPolling: status.destroyed !== true || !painelInstance?.autoRefreshEnabled };
...
const info = () => ({ panelId: PAINEL_ID, moduleId: MODULE_ID, version: VERSION, isAuthenticated: _isAuthenticated(), isDocumentVisible: _isDocumentVisible(), p22Compliant: true, ...getStatus() });
const destroy = () => unmount();
var panel_07_default = { mount, unmount, destroy, getStatus, getVersion, healthCheck, info, injectPorts, getPorts, MODULE_ID, VERSION };
export {
  MODULE_ID,
  VERSION,
  panel_07_default as default,
  destroy,
  getPorts,
  getStatus,
  getVersion,
