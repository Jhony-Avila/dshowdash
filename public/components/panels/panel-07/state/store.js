...
class StateStore {
  ...
  healthCheck() {
    const s = this.state;
    return { status: Ports.snapshot()._initialized ? "HEALTHY" : "DEGRADED", moduleId: MODULE_ID, version: VERSION, state: { hasData: !!s.data, loading: s.loading, hasError: !!s.error }, portsInitialized: Ports.snapshot()._initialized };
  }
  info() {
