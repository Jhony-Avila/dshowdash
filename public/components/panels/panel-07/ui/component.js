...
const MODULE_ID = "panel-07:ui:component";
const ICONS = {
...
class UIComponent {
  ...
  showLoading() {
    if (this.hasData) {
      this.container.classList.add("p07-loading-overlay");
      return;
    }
    this.container.innerHTML = renderSkeleton();
  }
  hideLoading() {
    this.container.classList.remove("p07-loading-overlay");
  }
  showError(msg) {
  ...
  update(data) {
    ...
    try {
      ...
      this.recentCompleted = payload?.recent_completed || [];
      this.statistics = payload?.statistics || {};
      if (!this.running.length && !this.recentCompleted.length) {
        this.showEmpty();
        return;
      }
        ...
        <div class="p07-kpi-grid">
          <div class="p07-kpi-card">
            ...
            <div class="p07-kpi-content">
              ...
              <span class="p07-kpi-label">Em Execu\xE7\xE3o</span>
            </div>
          </div>
          <div class="p07-kpi-card">
  ...
  _renderRunningSection() {
    ...
    }).join("") || '<tr><td colspan="5" class="p07-empty-table">Nenhum job em execu\xE7\xE3o</td></tr>';
    return `<div class="p07-section"><div class="p07-section-header"><h3 class="p07-section-title">${ICONS.play} <span>Em Execu\xE7\xE3o</span></h3><button class="p07-btn-icon" data-action="export-running">${ICONS.download}</button></div><div class="p07-section-body"><table class="p07-table"><thead><tr><th class="p07-th">Job</th><th class="p07-th">Tipo</th><th class="p07-th">Status</th><th class="p07-th">Tempo</th><th class="p07-th">M\xE9dia</th></tr></thead><tbody>${rows}</tbody></table></div></div>`;
  }
  _renderRecentSection() {
    const rows = this.recentCompleted.map((j) => {
  ...
  _exportRunning() {
    ...
    const rows = this.running.map((j) => [j.job_name, j.job_type, j.status, j.running_for_seconds, j.avg_execution_time]);
    this._downloadCSV(["Job", "Tipo", "Status", "Tempo (s)", "M\xE9dia (s)"], rows, "jobs-em-execucao");
    this.toastManager?.success("Exportado");
  }
  _exportRecent() {
    if (!this.recentCompleted.length) {
      this.toastManager?.warning("Nenhum dado");
      return;
    }
    const rows = this.recentCompleted.map((j) => [j.job_name, j.status, j.duration, j.rows_affected, j.finished_at]);
    this._downloadCSV(["Job", "Status", "Dura\xE7\xE3o", "Rows", "Fim"], rows, "execucoes-recentes");
    this.toastManager?.success("Exportado");
  }
  ...
  _formatTime(d) {
    if (!d) return "--";
    try {
      return new Date(String(d)).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" });
    } catch {
      return String(d);
    }
  }
