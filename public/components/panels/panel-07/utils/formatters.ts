...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-07/utils/formatters
// PURPOSE: Panel-07 - Formatters Centralizados
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
export const MODULE_ID = 'panel-07/utils/formatters';
...
export function formatPercent(value: unknown, decimals: number) {
...
export function getRateClass(rate: unknown, threshold: number) {
  if (threshold === undefined) threshold = 95;
...
export function getHealthClass(health: string) {
  const map: Record<string, string> = { 'healthy': 'status-active', 'warning': 'status-warning', 'critical': 'status-error', 'inactive': 'status-inactive' };
  return map[health] || 'status-inactive';
}

export function getHealthText(health: string) {
  const map: Record<string, string> = { 'healthy': 'Saudável', 'warning': 'Atenção', 'critical': 'Crítico', 'inactive': 'Inativo' };
  return map[health] || 'Inativo';
}

export function healthCheck() {
  const checks = { formattersAvailable: true, escapeHtmlWorks: escapeHtml('<test>') === '&lt;test&gt;' };
...
export function info() { return { moduleId: MODULE_ID, version: VERSION, formattersCount: 10, p25Compliant: true }; }

export default { formatDuration, formatDateTime, formatDateTimeFull, escapeHtml, formatNumber, formatPercent, formatBytes, getRateClass, getHealthClass, getHealthText, healthCheck, info };
