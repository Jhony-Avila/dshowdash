...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-07.utils.logger
// PURPOSE: Panel-07 - Logger Enterprise
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panel-07.utils.logger';
...
Logger.prototype.log = function(level: string, event: string, data: Record<string, unknown>) { if (!this.shouldLog(level)) return; const entry = this.formatEntry(level, event, data); this.buffer.push(entry); if (this.buffer.length > this.maxBuffer) this.buffer.shift(); const logger = _getPort('logger'); if (logger) { const fn = (logger as Record<string, unknown>)[level]; if (typeof fn === 'function') { (fn as Function).call(logger, `[${this.panelId}] ${event}`, entry); } } };
