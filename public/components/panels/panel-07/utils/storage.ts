...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-07/utils/storage
// PURPOSE: Panel-07 - Filter Storage Enterprise
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
export const MODULE_ID = 'panel-07/utils/storage';
export const VERSION = '9.3.0-P2-ENTERPRISE';
const STORAGE_KEY = 'p07-filters';
const STORAGE_VERSION = 1;

export function FilterStorage(this: any, logger: Record<string, unknown>) {
  this.logger = logger;
...
FilterStorage.prototype.checkAvailability = function() {
  try {
    ...
    if (this.logger && this.logger.warn) this.logger.warn('storage.unavailable');
    return false;
  }
};
...
FilterStorage.prototype.save = function(filters: Record<string, unknown>) {
  ...
  try {
    ...
    if (this.logger && this.logger.warn) this.logger.warn('storage.save-failed', { error: e.message });
    return false;
  }
};
...
FilterStorage.prototype.load = function() {
  ...
  try {
    ...
    if (data.version !== STORAGE_VERSION) {
      if (this.logger && this.logger.info) this.logger.info('storage.version-mismatch', { stored: data.version, current: STORAGE_VERSION });
      this.clear();
      return null;
    }
    const maxAge = 7 * 24 * 60 * 60 * 1000;
    if (Date.now() - data.timestamp > maxAge) {
      if (this.logger && this.logger.info) this.logger.info('storage.expired');
      this.clear();
      return null;
    }
    ...
    if (this.logger && this.logger.warn) this.logger.warn('storage.load-failed', { error: e.message });
    return null;
  }
};
...
FilterStorage.prototype.clear = function() {
  ...
  try {
    ...
    if (this.logger && this.logger.debug) this.logger.debug('storage.cleared');
    return true;
...
FilterStorage.prototype.info = function() {
  ...
  try {
    ...
    const data = JSON.parse(raw);
    return { available: true, hasData: true, version: data.version, age: Date.now() - data.timestamp, size: raw.length, p25Compliant: true };
  } catch (e: any) { return { available: true, hasData: false, corrupted: true }; }
};

FilterStorage.prototype.healthCheck = function() {
  const checks = { storageAvailable: this.available, localStorageWorks: this.checkAvailability() };
