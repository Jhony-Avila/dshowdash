...
function _loadCSS() {
  ...
  if (hasDocument && document.querySelector(`link[href*="${MODULE_ID}/styles/index.css"]`)) {
    _cssLoaded = true;
    return;
  }
  if (hasDocument) {
...
function _renderStructure(container) {
...
function mount(container, config) {
  ...
  return Controller.init(contentEl, config).then(() => Controller.mount()).then(() => {
    ...
    if (eb && eb.emit) eb.emit(PANEL_EVENTS.MOUNTED, { panelId: MODULE_ID, timestamp: Date.now(), source: MODULE_ID });
    return { ok: true };
  });
}
function unmount() {
  ...
  tracker.unmounted();
  const eb = _getPort("eventBus");
  if (eb && eb.emit) eb.emit(PANEL_EVENTS.UNMOUNTED, { panelId: MODULE_ID, timestamp: Date.now(), source: MODULE_ID });
...
function healthCheck() {
  return Object.assign({}, Controller.healthCheck(), { mounted: _mounted, cssLoaded: _cssLoaded });
}
var lifecycle_default = { VERSION, mount, unmount, refresh, getVersion, healthCheck, injectPorts, getPorts };
export {
  VERSION,
