...
class UIComponent {
  ...
  showLoading() {
    ...
    if (this.hasData) {
      this.logger?.debug?.("ui.loading-skipped", { reason: "has-data" });
      return;
    }
  ...
  hideLoading() {
    if (!this.container) return;
    this.container.classList.remove("p08-loading-overlay");
  }
  showError(message) {
    if (!this.container) return;
    if (this.hasData) {
      this._showErrorBadge(message);
      return;
    }
  ...
  update(data) {
    ...
    if (newHash === this._lastDataHash && this.hasData) {
      this._updateTimestampOnly();
      this.logger?.debug?.("ui.update-skipped", { reason: "data-unchanged" });
      return;
    }
    ...
    try {
      ...
      if (Array.isArray(inner)) {
        this._renderAlertsList(inner, data.meta || {});
      } else if (inner && typeof inner === "object" && inner.overview) {
        this._renderJobsPanel(inner);
      } else if (inner && typeof inner === "object") {
        this._renderMetricsCards(inner);
      } else {
        this._renderGenericData(data);
      }
      this.hasData = true;
  ...
  _updateTimestampOnly() {
    const timestampEl = this.container.querySelector("[data-timestamp]");
    if (timestampEl) {
      timestampEl.textContent = this._formatTime(Date.now());
    }
  }
  _renderAlertsList(alerts, meta) {
    const self = this;
    meta = meta || {};
    ...
    this.container.innerHTML = `<div class="p08-alerts"><div class="p08-alerts-summary"><div class="p08-summary-card p08-summary-card--total"><span class="p08-summary-value">${count}</span><span class="p08-summary-label">Total</span></div><div class="p08-summary-card p08-summary-card--error"><span class="p08-summary-value">${errorCount}</span><span class="p08-summary-label">Erros</span></div><div class="p08-summary-card p08-summary-card--warning"><span class="p08-summary-value">${warningCount}</span><span class="p08-summary-label">Avisos</span></div><div class="p08-summary-card p08-summary-card--period"><span class="p08-summary-value">${meta?.period || "24h"}</span><span class="p08-summary-label">Per\xEDodo</span></div></div><div class="p08-alerts-list">${alerts.slice(0, 10).map((alert) => {
      const alertType = String(alert.alert_type || "info");
      const alertSeverity = String(alert.severity || "low");
      return `<div class="p08-alert-item p08-alert-item--${alertType.toLowerCase()}"><div class="p08-alert-icon">${alertType === "ERROR" ? SVGS.x : alertType === "WARNING" ? SVGS.warning : SVGS.info}</div><div class="p08-alert-content"><div class="p08-alert-header"><span class="p08-alert-job">${self._escapeHtml(String(alert.job_name || "-"))}</span><span class="p08-alert-badge p08-alert-badge--${alertSeverity.toLowerCase()}">${self._escapeHtml(alertSeverity)}</span></div><div class="p08-alert-message">${self._escapeHtml(String(alert.message || "-"))}</div><div class="p08-alert-time">${self._formatDateTime(String(alert.created_at || ""))}</div></div></div>`;
    }).join("")}</div><div class="p08-alerts-footer"><span class="p08-timestamp" data-timestamp>Atualizado: ${this._formatTime(Date.now())}</span></div></div>`;
  }
  _renderJobsPanel(data) {
    const self = this;
    const overview = data.overview || {};
    const activity = data.recent_activity || [];
    this.container.innerHTML = `<div class="p08-jobs"><div class="p08-jobs-grid"><div class="p08-metric-card"><span class="p08-metric-value">${this._formatNumber(overview.total_jobs)}</span><span class="p08-metric-label">Total de Jobs</span></div><div class="p08-metric-card"><span class="p08-metric-value">${this._formatNumber(overview.active_jobs)}</span><span class="p08-metric-label">Jobs Ativos</span></div><div class="p08-metric-card"><span class="p08-metric-value">${this._formatNumber(overview.running_jobs)}</span><span class="p08-metric-label">Em Execu\xE7\xE3o</span></div><div class="p08-metric-card"><span class="p08-metric-value">${this._formatPercentage(overview.avg_success_rate)}</span><span class="p08-metric-label">Taxa de Sucesso</span></div></div><div class="p08-activity"><h3 class="p08-activity-title">Atividade Recente</h3><div class="p08-activity-list">${activity.slice(0, 8).map((item) => `<div class="p08-activity-item p08-activity-item--${item.status || "unknown"}"><span class="p08-activity-icon">${item.status === "success" ? SVGS.check : SVGS.x}</span><span class="p08-activity-name">${self._escapeHtml(item.job_name)}</span><span class="p08-activity-time">${self._formatDateTime(item.execution_start)}</span></div>`).join("")}</div></div><div class="p08-footer"><span class="p08-timestamp" data-timestamp>Atualizado: ${this._formatTime(Date.now())}</span></div></div>`;
  }
  _renderMetricsCards(data) {
    const self = this;
  ...
  _renderGenericData(data) {
    this.container.innerHTML = `<div class="p08-generic"><pre class="p08-json">${this._escapeHtml(JSON.stringify(data, null, 2))}</pre><div class="p08-footer"><span class="p08-timestamp" data-timestamp>Atualizado: ${this._formatTime(Date.now())}</span></div></div>`;
  }
  _formatNumber(value) {
  ...
  async destroy() {
    if (this.container) {
      ...
      this.container.classList.remove("p08-loading-overlay", "p08-has-error");
    }
    this.mounted = false;
    this.hasData = false;
    this._lastDataHash = null;
...
const MODULE_ID = "panels-panel-08-ui-component";
