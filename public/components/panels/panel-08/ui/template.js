...
const MODULE_ID = "panel-08-template";
...
function renderSummary(alerts, meta) {
  const total = alerts.length;
  const errors = alerts.filter((a) => a.alert_type === "ERROR").length;
  const warnings = alerts.filter((a) => a.alert_type === "WARNING").length;
  const period = meta && meta.period ? meta.period : "24h";
  return `<div class="${CSS_PREFIX}__summary"><div class="${CSS_PREFIX}__summary-card ${CSS_PREFIX}__summary-card--total"><span class="${CSS_PREFIX}__summary-value">${total}</span><span class="${CSS_PREFIX}__summary-label">Total</span></div><div class="${CSS_PREFIX}__summary-card ${CSS_PREFIX}__summary-card--error"><span class="${CSS_PREFIX}__summary-value">${errors}</span><span class="${CSS_PREFIX}__summary-label">Erros</span></div><div class="${CSS_PREFIX}__summary-card ${CSS_PREFIX}__summary-card--warning"><span class="${CSS_PREFIX}__summary-value">${warnings}</span><span class="${CSS_PREFIX}__summary-label">Avisos</span></div><div class="${CSS_PREFIX}__summary-card ${CSS_PREFIX}__summary-card--period"><span class="${CSS_PREFIX}__summary-value">${period}</span><span class="${CSS_PREFIX}__summary-label">Per\xEDodo</span></div></div>`;
}
function renderAlertItem(alert, isExpanded, isAcknowledged) {
...
function render(container, state, config) {
  ...
  const acknowledgedIds = state.acknowledgedIds || /* @__PURE__ */ new Set();
  ...
  const alertsHtml = alerts.slice(0, 15).map((alert) => renderAlertItem(alert, expandedIds.has(alert.id), acknowledgedIds.has(alert.id))).join("");
...
var template_default = { MODULE_ID, VERSION, render, getVersion, info, healthCheck };
export {
  MODULE_ID,
  VERSION,
