const PAINEL_ID = "panel-09";
const MODULE_ID = "panel-09.core.constants";
const VERSION = "9.3.0-P2-ENTERPRISE";
const PANEL_TITLE = "An\xE1lise Comparativa";
const REFRESH_INTERVAL_BASE = 3e5;
const REFRESH_INTERVAL_DEGRADED = 6e5;
const REQUEST_TIMEOUT = 15e3;
const MAX_CONSECUTIVE_ERRORS = 3;
const CIRCUIT_BREAKER_THRESHOLD = 5;
const CIRCUIT_BREAKER_TIMEOUT = 3e4;
const CSS_PATH = "/components/panels/panel-09/styles/index.css";
...
const COMPARISON_TYPES = {
  today_vs_yesterday: { label: "Hoje vs Ontem", icon: SVGS.calendar, period: "di\xE1rio" },
  this_week_vs_last_week: { label: "Esta Semana vs Anterior", icon: SVGS.calendarDays, period: "semanal" },
  this_month_vs_last_month: { label: "Este M\xEAs vs Anterior", icon: SVGS.calendarRange, period: "mensal" }
};
const METRICS = {
  total: { label: "Execu\xE7\xF5es", icon: SVGS.zap, format: "number" },
  success: { label: "Sucessos", icon: SVGS.checkCircle, format: "number" },
  avg_time: { label: "Tempo M\xE9dio", icon: SVGS.clock, format: "time" }
};
function info() {
  return { moduleId: "panels-panel-09-core-constants", version: VERSION };
}
function healthCheck() {
  return { status: "HEALTHY", moduleId: "panels-panel-09-core-constants", version: VERSION, checks: { constantsLoaded: true } };
}
export {
