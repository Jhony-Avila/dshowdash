...
class ComparisonsGrid {
  ...
  render() {
    ...
    this.element.className = "p09-grid";
    this.element.id = "p09-comparisons-grid";
    this.container.appendChild(this.element);
    return this.element;
  }
  update(comparisons) {
    if (!this.element || !comparisons || comparisons.length === 0) {
      this.showEmpty();
      return;
    }
    this.element.innerHTML = comparisons.map((comp) => this.renderCard(comp)).join("");
  }
  renderCard(comparison) {
    ...
    return `<div class="p09-comparison-card ${statusClass}"><div class="p09-card-title">${comparison.title}</div><div class="p09-metrics"><div class="p09-metric current"><div class="p09-metric-label">${comparison.currentLabel}</div><div class="p09-metric-value">${this.formatNumber(currentTotal)}</div></div><div class="p09-metric previous"><div class="p09-metric-label">${comparison.previousLabel}</div><div class="p09-metric-value">${this.formatNumber(previousTotal)}</div></div><div class="p09-metric variation"><div class="p09-metric-label">Varia\xE7\xE3o</div><div class="p09-metric-value ${statusClass}">${arrow} ${Math.abs(diffPercent)}%</div></div></div></div>`;
  }
  formatNumber(value) {
...
const MODULE_ID = "panels-panel-09-ui-comparisons";
const VERSION = "9.3.0-P2-ENTERPRISE";
const info = () => ({ moduleId: MODULE_ID, version: VERSION });
const healthCheck = () => ({ status: "HEALTHY", moduleId: MODULE_ID, version: VERSION, checks: { ready: true } });
export {
  ComparisonsGrid,
  MODULE_ID,
  VERSION,
