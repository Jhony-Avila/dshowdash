...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels-ui-render-comparison
// PURPOSE: Panel-09 Render Comparison
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   formatNumber, formatTime from ../helpers.js
//
// PROVIDES:
//   renderComparison() — exported function
...
export function renderComparison(container: HTMLElement, data: Record<string, unknown>, activeTab: string) {
    ...
    if (!periodData) { compEl.innerHTML = '<div style="grid-column:1/-1;text-align:center;color:#606070;">Sem dados</div>'; return; }

    const metrics = [
        { key: 'total', label: 'Execuções', icon: SVGS.zap, current: periodData.current?.total, previous: periodData.previous?.total, change: periodData.change?.total },
        ...
        { key: 'avg_time', label: 'Tempo Médio', icon: SVGS.clock, current: periodData.current?.avg_time, previous: periodData.previous?.avg_time, change: periodData.change?.avg_time, isTime: true, invertColor: true }
    ];

    compEl.innerHTML = metrics.map((m) => {
        const change = parseFloat(m.change as string) || 0;
        ...
        const currentVal = m.isTime ? formatTime(m.current as string) : formatNumber(m.current as string);
        const previousVal = m.isTime ? formatTime(m.previous as string) : formatNumber(m.previous as string);
        const currentNum = parseFloat(m.current as string) || 0;
        const previousNum = parseFloat(m.previous as string) || 1;
...
export const MODULE_ID = 'panels-ui-render-comparison';
