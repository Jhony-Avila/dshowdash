...
function renderStatusDistribution(container, data) {
  const distEl = container.querySelector("[data-status-dist]");
  if (!distEl) return;
  const dist = data.status_distribution || [];
  if (!dist.length) {
    distEl.innerHTML = '<div style="text-align:center;color:#606070;font-size:11px;">Sem dados</div>';
    return;
  }
  const total = dist.reduce((sum, d) => sum + parseInt(d.count), 0);
  distEl.innerHTML = dist.map((d) => {
    const pct = total > 0 ? (parseInt(d.count) / total * 100).toFixed(1) : 0;
    const color = STATUS_COLORS[d.status] || "#606070";
...
function renderAlerts(container, data) {
  ...
  if (!alertsEl) return;
  const alerts = [];
  const today = data.today_vs_yesterday;
  if (today) {
    const successChange = parseFloat(today.change?.success) || 0;
    if (successChange < -ALERT_THRESHOLD) {
      alerts.push({ icon: SVGS.warning, text: `Taxa de sucesso caiu ${Math.abs(successChange).toFixed(1)}% em rela\xE7\xE3o a ontem`, color: "#ef4444" });
    }
    const timeChange = parseFloat(today.change?.avg_time) || 0;
    if (timeChange > ALERT_THRESHOLD * 2) {
      alerts.push({ icon: SVGS.turtle, text: `Tempo m\xE9dio aumentou ${timeChange.toFixed(1)}%`, color: "#f59e0b" });
    }
  }
  ...
  if (alerts.length === 0) {
    alerts.push({ icon: SVGS.checkCircle, text: "Sistema operando dentro dos par\xE2metros normais", color: "#22c55e" });
  }
...
var distribution_default = { renderStatusDistribution, renderAlerts };
const MODULE_ID = "panels-ui-render-distribution";
