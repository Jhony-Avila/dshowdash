...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels-ui-render-summary
// PURPOSE: Panel-09 Render Summary
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   PERIODS from ../constants.js
//   formatNumber, getChangeArrow, getChangeColor, getRateColor, ARROW_SVGS from ....
//
// PROVIDES:
//   renderSuccessRate() — exported function
//   renderMiniDonut() — exported function
//   renderSparkline() — exported function
//   renderSummaryCards() — exported function
...
export function renderSuccessRate(container: HTMLElement, data: Record<string, unknown>) {
  const sr = data.success_rate as Record<string, number> | undefined;
  if (!sr) return;
  const rateEl = container.querySelector('[data-success-rate]') as HTMLElement | null;
  const changeEl = container.querySelector('[data-success-change]') as HTMLElement | null;
  if (rateEl) { rateEl.textContent = `${sr.today.toFixed(1)}%`; rateEl.style.color = getRateColor(sr.today); }
  if (changeEl) { const arrow = getChangeArrow(sr.change); const color = getChangeColor(sr.change); changeEl.innerHTML = `${arrow} ${Math.abs(sr.change).toFixed(1)}%`; changeEl.style.color = color; }
}

export function renderMiniDonut(container: HTMLElement, data: Record<string, unknown>) {
  const donutEl = container.querySelector('[data-donut]');
  if (!donutEl) return;
  const successRate = data.success_rate as Record<string, number> | undefined;
...
export function renderSparkline(container: HTMLElement, data: Record<string, unknown>) {
  const sparklineEl = container.querySelector('[data-sparkline-success]');
  if (!sparklineEl) return;
  const sparkline = data.sparkline as Record<string, number[]> | undefined;
  const sparkData = (sparkline && sparkline.success) || [];
  if (!sparkData.length) { sparklineEl.innerHTML = ''; return; }
  const max = Math.max.apply(null, sparkData.concat([1]));
  const width = 100 / sparkData.length;
  const bars = sparkData.map((v: number, i: number) => { const h = Math.max((v / max) * 20, 1); return `<rect x="${i * width}" y="${24 - h}" width="${width - 0.5}" height="${h}" fill="#22c55e" opacity="0.6"/>`; }).join('');
  sparklineEl.innerHTML = `<svg viewBox="0 0 100 24" preserveAspectRatio="none" style="width:100%;height:100%;">${bars}</svg>`;
}

export function renderSummaryCards(container: HTMLElement, data: Record<string, unknown>, activeTab: string) {
  ...
  cardsEl.innerHTML = periods.map(period => {
    const periodData = data[period] as Record<string, Record<string, unknown>> | undefined;
    if (!periodData) return '';
    const change = parseFloat((periodData.change && periodData.change.total) as string) || 0;
    const isPositive = change >= 0;
    const arrow = isPositive ? ARROW_SVGS.up : ARROW_SVGS.down;
    const color = isPositive ? '#22c55e' : '#ef4444';
    const isActive = period === activeTab;
    const periodConfig = (PERIODS as Record<string, { label: string; icon: string; tabLabel: string }>)[period];
...
export default { renderSuccessRate, renderMiniDonut, renderSparkline, renderSummaryCards };

export const MODULE_ID = 'panels-ui-render-summary';
