...
class UIComponent {
  ...
  showLoading() {
    ...
    if (this.hasData) {
      this.container.classList.add("p10-loading-overlay");
    } else {
      this.container.innerHTML = `<div class="p10-skeleton"><div class="p10-skeleton-grid">${Array(8).fill('<div class="p10-skeleton-card"></div>').join("")}</div><div class="p10-skeleton-table">${Array(5).fill('<div class="p10-skeleton-row"></div>').join("")}</div></div>`;
    }
  }
  hideLoading() {
    if (this.container) {
      this.container.classList.remove("p10-loading-overlay");
    }
  }
  showError(message) {
    if (!this.container) return;
    if (this.hasData) {
      ...
      this.container.innerHTML = `<div class="p10-error"><svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg><p class="p10-error-title">Erro ao carregar</p><p class="p10-error-message">${message || "N\xE3o foi poss\xEDvel obter dados do servidor"}</p></div>`;
    }
  }
  update(data) {
    if (!this.container) return;
    ...
    try {
      ...
      if (!payload || typeof payload !== "object") {
        this.showError("Dados inv\xE1lidos");
        return;
      }
      const p = payload;
      if (p.local || p.hostinger || p.vm_id !== void 0) {
        this.renderServerHealth(p);
      } else if (p.overview && p.recent_activity) {
        this.renderJobsPanel(p);
      } else if (Array.isArray(p)) {
        this.renderGenericList(p);
      } else {
        this.renderMetricsCards(p);
      }
      this.hasData = true;
  ...
  renderVMInfo(data) {
    ...
    const vm = data.hostinger || {};
    return `<div class="p10-vm-section"><div class="p10-section-title"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>VPS Hostinger</div><div class="p10-vm-grid"><div class="p10-vm-item"><span class="p10-vm-label">ID</span><span class="p10-vm-value">${data.vm_id || vm.id || "-"}</span></div><div class="p10-vm-item"><span class="p10-vm-label">Estado</span><span class="p10-vm-value p10-vm-state-${data.vm_state || vm.state}">${data.vm_state || vm.state || "-"}</span></div><div class="p10-vm-item"><span class="p10-vm-label">Plano</span><span class="p10-vm-value">${data.vm_plan || vm.plan || "-"}</span></div><div class="p10-vm-item"><span class="p10-vm-label">CPUs</span><span class="p10-vm-value">${data.vm_cpus || vm.cpus || "-"}</span></div><div class="p10-vm-item"><span class="p10-vm-label">Mem\xF3ria</span><span class="p10-vm-value">${this.formatMB(data.vm_memory_mb || vm.memory)}</span></div><div class="p10-vm-item"><span class="p10-vm-label">Disco</span><span class="p10-vm-value">${this.formatMB(data.vm_disk_mb || vm.disk)}</span></div><div class="p10-vm-item"><span class="p10-vm-label">IPv4</span><span class="p10-vm-value">${data.vm_ipv4 || "-"}</span></div><div class="p10-vm-item"><span class="p10-vm-label">Template</span><span class="p10-vm-value">${data.vm_template || "-"}</span></div></div></div>`;
  }
  renderJobsPanel(data) {
    const self = this;
    const overview = data.overview || {};
    const activity = data.recent_activity || [];
    this.container.innerHTML = `<div class="p10-jobs-panel"><div class="p10-metrics-grid"><div class="p10-metric-card"><div class="p10-metric-header"><span>Total Jobs</span></div><div class="p10-metric-value">${this.formatNumber(overview.total_jobs)}</div></div><div class="p10-metric-card"><div class="p10-metric-header"><span>Ativos</span></div><div class="p10-metric-value">${this.formatNumber(overview.active_jobs)}</div></div><div class="p10-metric-card"><div class="p10-metric-header"><span>Em Execu\xE7\xE3o</span></div><div class="p10-metric-value">${this.formatNumber(overview.running_jobs)}</div></div><div class="p10-metric-card"><div class="p10-metric-header"><span>Taxa Sucesso</span></div><div class="p10-metric-value">${this.formatPercent(overview.avg_success_rate)}</div></div></div><div class="p10-activity-section"><h3 class="p10-section-title">Atividade Recente</h3><div class="p10-activity-list">${activity.slice(0, 10).map((item) => `<div class="p10-activity-item p10-activity-${item.status}"><div class="p10-activity-icon">${item.status === "success" ? SVGS.check : SVGS.x}</div><div class="p10-activity-content"><div class="p10-activity-name">${item.job_name || "-"}</div><div class="p10-activity-meta"><span>${self.formatDateTime(item.execution_start)}</span><span>${self.formatDuration(item.execution_time_seconds)}</span></div></div></div>`).join("")}</div></div></div>`;
  }
  renderGenericList(items) {
    const self = this;
    if (!Array.isArray(items) || items.length === 0) {
      this.container.innerHTML = '<div class="p10-empty">Nenhum dado dispon\xEDvel</div>';
      return;
    }
  ...
  formatDuration(s) {
    ...
    return isNaN(n) ? String(s) : `${n.toFixed(2)}s`;
  }
  formatUptimeStart(seconds) {
    if (!seconds) return "--";
    const start = new Date(Date.now() - seconds * 1e3);
...
const MODULE_ID = "panels-panel-10-ui-component";
const VERSION = "9.3.0-P2-ENTERPRISE";
function info() {
  return { moduleId: MODULE_ID, version: VERSION };
}
function healthCheck() {
  return { status: "HEALTHY", moduleId: MODULE_ID, version: VERSION, checks: { componentReady: true } };
}
export {
  MODULE_ID,
  UIComponent,
