...
const MODULE_ID = "panel-10-metrics";
function MetricsGrid(container, logger) {
...
MetricsGrid.prototype.update = function(data) {
...
MetricsGrid.prototype.renderCPUCard = function(data) {
  const percent = data.cpu_percent != null ? data.cpu_percent : 0;
...
MetricsGrid.prototype.renderRAMCard = function(data) {
  const percent = data.ram_percent != null ? data.ram_percent : 0;
...
MetricsGrid.prototype.renderDiskCard = function(data) {
  const percent = data.disk_percent != null ? data.disk_percent : 0;
...
MetricsGrid.prototype.showLoading = function() {
  ...
  this.element.innerHTML = '<div class="p10-loading" role="status"><div class="p10-loading-spinner"></div><span>Carregando m\xE9tricas...</span></div>';
};
MetricsGrid.prototype.showEmpty = function() {
  if (!this.element) return;
  this.element.innerHTML = '<div class="p10-empty" role="status"><svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg><span>Nenhum dado dispon\xEDvel</span></div>';
};
MetricsGrid.prototype.showError = function(message) {
  if (!this.element) return;
...
function healthCheck() {
  return { status: "HEALTHY", moduleId: MODULE_ID, version: VERSION, checks: { classReady: typeof MetricsGrid === "function" } };
}
var metrics_default = MetricsGrid;
export {
  MODULE_ID,
  MetricsGrid,
  VERSION,
