...
import { PAINEL_ID, PANEL_TITLE, REFRESH_INTERVAL_BASE, REFRESH_INTERVAL_DEGRADED, REQUEST_TIMEOUT, MAX_CONSECUTIVE_ERRORS, CIRCUIT_BREAKER_THRESHOLD, CIRCUIT_BREAKER_TIMEOUT, STATES } from "./core/constants.js";
const MODULE_ID = "panel-11";
...
class Panel11 {
  constructor() {
    ...
    this._filters = { period: "30d", status: null, search: "" };
    ...
    this._handleFilterChange = (data) => this._onFilterChange(data);
  ...
  _onFilterChange(data) {
    if (!_isAuthenticated()) {
      _log("warn", "filter.change blocked - not authenticated");
      return;
    }
    _log("info", "filter.change", data);
    if (data?.period) this._filters.period = data.period;
    if (data?.status !== void 0) this._filters.status = data.status;
    if (data?.search !== void 0) this._filters.search = data.search;
    this.loadData();
  }
  loadData() {
    ...
    return this.apiClient.fetchData({ signal: this.abortController.signal, timeout: REQUEST_TIMEOUT, period: this._filters.period, filters: { status: this._filters.status, search: this._filters.search } }).then((result) => {
      ...
      } else throw new Error(result.error || "Resposta inv\xE1lida");
    }).catch((error) => {
  ...
  getStatus() {
    return { panelId: PAINEL_ID, version: VERSION, state: this.state, mounted: this.mounted, filters: this._filters, autoRefresh: this._autoRefreshEnabled, countdown: this._countdownValue, isAuthenticated: _isAuthenticated(), isDocumentVisible: _isDocumentVisible(), p22Compliant: true, timestamp: Date.now() };
  }
  healthCheck() {
...
var panel_11_default = { mount, unmount, destroy, getStatus, getVersion, healthCheck, info, injectPorts, getPorts };
export {
  MODULE_ID,
  PAINEL_ID as PANEL_ID,
  VERSION,
  panel_11_default as default,
  destroy,
  getPorts,
  getStatus,
  getVersion,
