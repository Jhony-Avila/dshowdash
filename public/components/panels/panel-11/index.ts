...
//   PAINEL_ID, VERSION (as CONST_VERSION), PANEL_TITLE,
//     REFRESH_INTERVAL_BASE, REFRESH_INTERVAL_DEGRADED,
//     REQUEST_TIMEOUT, MAX_CONSECUTIVE_ERRORS,
//     CIRCUIT_BREAKER_THRESHOLD, CIRCUIT_BREAKER_TIMEOUT,
//     STATES, PERIODS from ./core/constants.js
//
// PROVIDES: Panel11 class (mount/unmount/getStatus/healthCheck/info/
//   loadData/startAutoRefresh/stopAutoRefresh/startCountdown/stopCountdown),
//   mount/unmount/getStatus/getVersion/healthCheck/info,
//   PANEL_ID, MODULE_ID, VERSION, injectPorts, getPorts
//
// RECEIVES (via mount):
...
//   performance.now, setInterval/clearInterval, setTimeout/clearTimeout,
//   AbortController
// ═══════════════════════════════════════════════════════════════
// Panel-11 - Resumo Geral Enterprise Premium AAA
...
import { PAINEL_ID, VERSION as CONST_VERSION, PANEL_TITLE, REFRESH_INTERVAL_BASE, REFRESH_INTERVAL_DEGRADED, REQUEST_TIMEOUT, MAX_CONSECUTIVE_ERRORS, CIRCUIT_BREAKER_THRESHOLD, CIRCUIT_BREAKER_TIMEOUT, STATES, PERIODS } from './core/constants.js';

export const MODULE_ID = 'panel-11';
...
(() => {
  _initPorts();
  const cssPath = '/components/panels/panel-11/styles/index.css';
...
class Panel11 {
  [key: string]: any;
  constructor() {
    this.container = null; this.uiComponent = null; this.apiClient = null; this.store = null; this.circuitBreaker = null; this.eventBus = null; this.state = STATES.IDLE; this.mounted = false; this.destroyed = false; this.refreshTimer = null; this.countdownTimer = null; this.consecutiveErrors = 0; this.isDegraded = false; this.lastLoadTime = 0; this.loadCount = 0; this.abortController = null; this.unsubscribers = []; this.currentRequestId = 0; this.activeLoadRequest = null; this._filters = { period: '30d', status: null, search: '' }; this._countdownValue = 60; this._autoRefreshEnabled = true;
    ...
    this._handleFilterChange = (data: unknown) => this._onFilterChange(data as Record<string, unknown>);
    this._filteredRefreshHandler = createPanelHandler(PAINEL_ID, this._handleRefreshEvent);
    this._filteredFilterChangeHandler = createPanelHandler(PAINEL_ID, this._handleFilterChange);
  }
  ...
  setupEventListeners() {
    ...
    this.eventBus?.on?.(PANEL_INTENTS.FILTER_CHANGE, this._filteredFilterChangeHandler);
  }
  ...
  _onFilterChange(data: Record<string, unknown>) {
    if (!_isAuthenticated()) { _log('warn', 'filter.change blocked - not authenticated'); return; }
    _log('info', 'filter.change', data);
    if (data?.period) this._filters.period = data.period;
    if (data?.status !== undefined) this._filters.status = data.status;
    if (data?.search !== undefined) this._filters.search = data.search;
    this.loadData();
  }
  ...
  loadData() {
    ...
    return this.apiClient.fetchData({ signal: this.abortController.signal, timeout: REQUEST_TIMEOUT, period: this._filters.period, filters: { status: this._filters.status, search: this._filters.search } }).then((result: Record<string, unknown>) => {
      if (this.activeLoadRequest !== requestId) return;
      if (result.success && result.payload) { this.consecutiveErrors = 0; if (this.isDegraded) { this.isDegraded = false; this.setState(STATES.READY); } this.store.setData(result.payload); this.circuitBreaker.recordSuccess(); this.setState(STATES.READY); _log('debug', 'load.success'); } else throw new Error((result.error as string) || 'Resposta inválida');
  ...
  unmount() {
    ...
    return new Promise((resolve) => {
      try {
        ...
        this.eventBus?.off?.(PANEL_INTENTS.FILTER_CHANGE, this._filteredFilterChangeHandler);
        this.uiComponent?.destroy?.(); this.store?.reset?.();
        if (this.container) { this.container.innerHTML = ''; this.container = null; }
        this.mounted = false; this.destroyed = true; this.setState(STATES.DESTROYED); _log('info', 'Desmontado');
  ...
  getStatus() { return { panelId: PAINEL_ID, version: VERSION, state: this.state, mounted: this.mounted, filters: this._filters, autoRefresh: this._autoRefreshEnabled, countdown: this._countdownValue, isAuthenticated: _isAuthenticated(), isDocumentVisible: _isDocumentVisible(), p22Compliant: true, timestamp: Date.now() }; }
...
let instance: Panel11 | null = null;
const mount = (container: HTMLElement, deps: Record<string, unknown> = {}) => { if (instance?.mounted) { _log('warn', 'Instance exists'); return Promise.resolve(); } instance = new Panel11(); return instance.mount(container, deps); };
