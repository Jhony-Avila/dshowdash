...
ApiClient.prototype._delay = (ms) => new Promise((resolve) => {
...
ApiClient.prototype.fetchData = function(options = {}) {
  ...
  return fetch(url, { method: "GET", signal: abortSignal, credentials: "include", headers: { "Content-Type": "application/json", "X-Panel-Id": this.panelId, "X-Requested-With": "XMLHttpRequest" } }).then((response) => {
    ...
    if (!response.ok) {
      this.logger?.error?.("api.fetch.http-error", { status: response.status });
    ...
    if (error.name === "AbortError") {
      this.logger?.warn?.("api.fetch.aborted");
      return { success: false, error: "REQUEST_ABORTED", aborted: true };
    }
    this.logger?.error?.("api.fetch.error", { error: error.message });
    if (this.retryCount < this.maxRetries) {
      this.retryCount++;
      this.logger?.info?.("api.fetch.retry", { attempt: this.retryCount });
      return this._delay(this.retryDelay * this.retryCount).then(() => this.fetchData(options));
    }
    return { success: false, error: "NETWORK_ERROR", message: error.message, retries: this.retryCount };
  });
};
ApiClient.prototype.fetchErrors = function(options = {}) {
...
ApiClient.prototype.fetchTrend = function(options = {}) {
...
ApiClient.prototype.healthCheck = function() {
  return { moduleId: MODULE_ID, version: VERSION, panelId: this.panelId, hasActiveRequest: !!this.activeController, retryCount: this.retryCount };
};
