...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-11.telemetry.tracker
// PURPOSE: Panel-11 - Telemetry Tracker Enterprise
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
const MODULE_ID = 'panel-11.telemetry.tracker';
...
PanelTelemetryTracker.prototype.trackLoad = function(duration: number, success: boolean, itemCount: number = 0) { this.metrics.loadCount++; this.metrics.totalLoadTime += duration; if (!success) this.metrics.errorCount++; return this.track(LIFECYCLE_EVENTS.DATA_LOADED, { duration, success, itemCount, avgLoadTime: this.metrics.totalLoadTime / this.metrics.loadCount }); };
PanelTelemetryTracker.prototype.trackError = function(error: unknown, context: Record<string, unknown> = {}) { this.metrics.errorCount++; const err = error as { message?: string; name?: string }; return this.track(LIFECYCLE_EVENTS.ERROR, { error: err?.message || String(error), errorType: err?.name || 'Error', context, totalErrors: this.metrics.errorCount }); };
...
PanelTelemetryTracker.prototype.trackStateChange = function(from: string, to: string, reason: string = '') { return this.track(LIFECYCLE_EVENTS.STATE_CHANGED, { from, to, reason }); };
...
PanelTelemetryTracker.prototype.healthCheck = function() { const metrics = this.getMetrics(); const errorRate = parseFloat(metrics.errorRate); return { healthy: errorRate < 10, initialized: this.initialized, hasTelemetryCore: !!this.telemetryCore, metrics }; };
