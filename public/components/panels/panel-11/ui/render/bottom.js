...
function renderTopErrorsTable(errors) {
      ...
      ${top5.length === 0 ? `<div class="p11-table-empty">Nenhum erro registrado</div>` : `
        <div class="p11-table-wrapper">
          <table class="p11-table">
            ...
            <tbody>
              ${top5.map((e) => {
    const jobName = e.job_name || extractJobName(e.error_message) || "Desconhecido";
    const errorRate = parseFloat(e.error_rate) || 0;
...
function renderProportionDonut(success, error, total) {
  ...
  return `
    <div class="p11-card" style="padding:16px;">
      <div style="display:flex;align-items:center;gap:6px;margin-bottom:12px;">
        ...
        <span style="font-size:12px;font-weight:600;color:var(--p11-text-primary);">Propor\xE7\xE3o</span>
      </div>
      <div class="p11-donut-container">
        <svg class="p11-donut-svg" viewBox="0 0 100 100">
          ...
          <text class="p11-donut-label" x="50" y="58" text-anchor="middle">execu\xE7\xF5es</text>
        </svg>
...
var bottom_default = { renderBottomSection };
const MODULE_ID = "panels-ui-render-bottom";
