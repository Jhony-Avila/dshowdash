...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-12/icons
// PURPOSE: panel-12 - Icons (IconRegistry Adapter)
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panel-12/icons';
...
export const ICONS = new Proxy({} as Record<string, string>, {
  get(_target: Record<string, string>, prop: string | symbol) {
    if (prop === 'then') return undefined;
    const propStr = String(prop);
    return getIcon((NAME_MAP as Record<string, string>)[propStr] || propStr, 16);
  },
  has() { return true; }
});
