/* ═══════════════════════════════════════════════════════════════
 * DEPENDENCY CONTRACT — panel-13/index.js
 * @version 9.3.0-VISIBILITY-AUTH-GATING
 * @batch Batch Z (Contract #211 of 217)
 *
 * IMPORTS (EXTERNAL):
 ...
 *
 * PATTERNS:
 *   PortsFactory (createPanelPorts), ES6 Class (Panel13 — Widget de Cotacoes)
 ...
 * ═══════════════════════════════════════════════════════════════ */
// Panel-13 - Widget de Cotações em Tempo Real
// @version 9.3.0-VISIBILITY-AUTH-GATING
...
export const MODULE_ID = 'panel-13';
...
(() => {
  _initPorts();
  const cssPath = '/components/panels/panel-13/styles/index.css';
...
let instance: Panel13 | null = null;
const mount = (container: HTMLElement, deps: Record<string, unknown> = {}) => { if (instance?.mounted) { _log('warn', 'Instance exists'); return Promise.resolve(); } instance = new Panel13(); return instance.mount(container, deps); };
