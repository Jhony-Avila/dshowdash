...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-13/state/store
// PURPOSE: Panel 13 - State Store
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
const MODULE_ID = 'panel-13/state/store';
...
export class StateStore {
  [key: string]: any;
  constructor() { this.state = { data: null, loading: false, error: null, filters: {}, sort: { column: null, direction: 'asc' } }; this.listeners = new Set(); _initPorts(); }
  ...
  reset() { this.state = { data: null, loading: false, error: null, filters: {}, sort: { column: null, direction: 'asc' } }; this.notify(); }
  healthCheck() { return { status: Ports.snapshot()._initialized ? 'HEALTHY' : 'DEGRADED', moduleId: MODULE_ID, version: VERSION, state: { hasData: !!this.state.data, loading: this.state.loading, hasError: !!this.state.error }, portsInitialized: Ports.snapshot()._initialized }; }
