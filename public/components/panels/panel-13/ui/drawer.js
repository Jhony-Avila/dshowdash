...
const MODULE_ID = "panel-13.ui.drawer";
...
DrawerComponent.prototype.renderJobDetails = function(job) {
  ...
  if (subtitle) {
    ...
    subtitle.innerHTML = `<span class="p13-job-type type-${type.toLowerCase()}">${type}</span><span class="p13-status-indicator status-${_getStatusClass(health)}"><span class="p13-status-dot"></span>${getHealthText(health)}</span>`;
  }
  if (body) body.innerHTML = this._buildDetailsHTML(job);
};
DrawerComponent.prototype._buildDetailsHTML = (job) => {
  ...
  return `<div class="p13-drawer-section"><h4>M\xE9tricas</h4><div class="p13-drawer-kpis"><div class="p13-drawer-kpi"><div class="p13-drawer-kpi-label">Execu\xE7\xF5es</div><div class="p13-drawer-kpi-value">${totalExec.toLocaleString("pt-BR")}</div></div><div class="p13-drawer-kpi"><div class="p13-drawer-kpi-label">Taxa de Sucesso</div><div class="p13-drawer-kpi-value ${_getRateClassDrawer(successRate)}">${successRate.toFixed(1)}%</div></div><div class="p13-drawer-kpi"><div class="p13-drawer-kpi-label">Tempo M\xE9dio</div><div class="p13-drawer-kpi-value">${formatDuration(avgTime)}</div></div><div class="p13-drawer-kpi"><div class="p13-drawer-kpi-label">Erros</div><div class="p13-drawer-kpi-value ${errors > 0 ? "error" : ""}">${errors}</div></div></div></div><div class="p13-drawer-section"><h4>Informa\xE7\xF5es</h4><div class="p13-drawer-info"><div class="p13-drawer-info-row"><span>ID</span><span>${job.id || "--"}</span></div><div class="p13-drawer-info-row"><span>\xDAltima Execu\xE7\xE3o</span><span>${formatDateTime(job.last_execution)}</span></div><div class="p13-drawer-info-row"><span>\xDAltimo Sucesso</span><span>${formatDateTime(job.last_success)}</span></div></div></div>`;
};
DrawerComponent.prototype._buildSLARow = (label, slaData) => {
  if (!slaData) return `<div class="p13-drawer-sla-item"><span>${label}</span><span>--</span></div>`;
  const status = slaData.status || "ok";
  const value = slaData.value != null ? slaData.value : "--";
  return `<div class="p13-drawer-sla-item"><span>${label}</span><span><span class="p13-drawer-sla-dot ${status}"></span>${value}</span></div>`;
};
...
function healthCheck() {
  return { status: Ports.isInitialized() ? "HEALTHY" : "DEGRADED", moduleId: MODULE_ID, version: VERSION, checks: { drawerReady: true, portsInitialized: Ports.isInitialized(), p18IntentsAvailable: true } };
}
