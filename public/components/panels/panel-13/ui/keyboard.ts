...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-13/ui/keyboard
// PURPOSE: Panel-13 Keyboard Navigation
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
export const MODULE_ID = 'panel-13/ui/keyboard';
...
KeyboardNavigation.prototype.init = function() {
    ...
    self._keyHandler = (e: KeyboardEvent) => {
        ...
        const tbody = self.container.querySelector('.p13-tbody');
        if (!tbody) return;
...
KeyboardNavigation.prototype.navigate = function(rows: Element[], direction: number) { const newIndex = this.currentIndex + direction; if (newIndex >= 0 && newIndex < rows.length) this.goTo(rows, newIndex); };
KeyboardNavigation.prototype.goTo = function(rows: Element[], index: number) { rows.forEach((r: Element) => (r as HTMLElement).classList.remove('p13-row-selected')); this.currentIndex = index; const row = rows[index] as HTMLElement; if (row) { row.classList.add('p13-row-selected'); row.scrollIntoView({ block: 'nearest', behavior: 'smooth' }); this.onRowSelect(row); } };
KeyboardNavigation.prototype.clearSelection = function(rows: Element[]) { rows.forEach((r: Element) => (r as HTMLElement).classList.remove('p13-row-selected')); this.currentIndex = -1; };
