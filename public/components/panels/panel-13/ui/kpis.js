...
class KPIGrid {
  ...
  render() {
    ...
    this.element.className = "p13-kpi-grid";
  ...
  update(data) {
    ...
    const criticalJobs = data.criticalJobs;
    const statusClass = availability >= 99 ? "success" : availability >= 95 ? "warning" : "error";
    this.element.innerHTML = `<div class="p13-kpi ${statusClass}"><div class="p13-kpi-icon">${SVGS.barChart}</div><div class="p13-kpi-value">${availability.toFixed(2)}%</div><div class="p13-kpi-label">Disponibilidade</div></div><div class="p13-kpi info"><div class="p13-kpi-icon">${SVGS.check}</div><div class="p13-kpi-value">${this.formatNumber(totalExecutions)}</div><div class="p13-kpi-label">Total Execu\xE7\xF5es</div></div><div class="p13-kpi ${failures > 100 ? "warning" : "info"}"><div class="p13-kpi-icon">${SVGS.warning}</div><div class="p13-kpi-value">${this.formatNumber(failures)}</div><div class="p13-kpi-label">Falhas</div></div><div class="p13-kpi ${criticalJobs > 0 ? "error" : "success"}"><div class="p13-kpi-icon">${SVGS.target}</div><div class="p13-kpi-value">${criticalJobs}</div><div class="p13-kpi-label">Jobs Cr\xEDticos</div></div>`;
  }
  formatNumber(num) {
  ...
  showLoading() {
    ...
    this.element.innerHTML = '<div class="p13-loading">Carregando...</div>';
  }
  showError() {
    if (!this.element) return;
    this.element.innerHTML = `<div class="p13-error">${SVGS.warning} Erro ao carregar dados</div>`;
  }
  destroy() {
    if (this.element) {
      this.element.remove();
...
const MODULE_ID = "panels-panel-13-ui-kpis";
