...
const MODULE_ID = "panel-13/ui/toast";
...
class ToastManager {
  ...
  dismiss(toastId) {
    _getPort("toast")?.dismiss?.(toastId);
  }
...
const info = () => ({ moduleId: MODULE_ID, version: VERSION, portsInitialized: Ports.snapshot()._initialized, ports: Object.keys(Ports.snapshot()).filter((k) => k !== "_initialized") });
const healthCheck = () => {
  const toast = _getPort("toast");
  const logger = _getPort("logger");
  return { status: toast ? "HEALTHY" : "NOT_INITIALIZED", moduleId: MODULE_ID, version: VERSION, loggerReady: !!logger, portsInitialized: Ports.snapshot()._initialized };
};
export {
  MODULE_ID,
  ToastManager,
