...
const MODULE_ID = "panel-13.utils.logger";
...
Logger.prototype.healthCheck = function() {
  ...
  return { status: passed === total ? "HEALTHY" : "DEGRADED", moduleId: MODULE_ID, version: VERSION, score: `${passed}/${total}`, checks, bufferSize: this.buffer.length, timestamp: Date.now() };
};
const healthCheck = () => ({ status: "HEALTHY", moduleId: MODULE_ID, version: VERSION, checks: { moduleAvailable: true }, timestamp: Date.now() });
