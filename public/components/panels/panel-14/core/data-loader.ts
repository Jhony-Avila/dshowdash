...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-14/core/data-loader
// PURPOSE: Panel-14 - Data Loader
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   LIFECYCLE_EVENTS from /core/runtime/events/catalog/lifecycle.events.js
//   PAINEL_ID, REFRESH_INTERVAL_BASE, REFRESH_INTERVAL_DEGRADED from ./constants.js
...
export const MODULE_ID = 'panel-14/core/data-loader';
...
DataLoader.prototype.loadData = function() {
  ...
  return ctx.apiClient.fetchData({ signal: ctx.abortController.signal, timeout: REQUEST_TIMEOUT }).then((data: Record<string, unknown>) => {
    ...
    if (ctx.consecutiveErrors >= MAX_CONSECUTIVE_ERRORS && !ctx.isDegraded) { ctx.isDegraded = true; ctx.setState(STATES.DEGRADED); ctx.telemetry.track(LIFECYCLE_EVENTS.MODE_DEGRADED, { consecutiveErrors: ctx.consecutiveErrors, panelId: PAINEL_ID }); } else { ctx.setState(STATES.ERROR); }
  }).finally(() => {
