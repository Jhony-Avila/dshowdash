/* ═══════════════════════════════════════════════════════════════
 * DEPENDENCY CONTRACT — panel-14/index.js
 * @version 9.2.0-VISIBILITY-AUTH-GATING
 * @batch Batch Z (Contract #210 of 217)
 *
 * IMPORTS (EXTERNAL):
 ...
 *
 * PATTERNS:
 *   PortsFactory (createPanelPorts), ES6 Class (Panel14 — Widget de Clima)
 ...
 * ═══════════════════════════════════════════════════════════════ */
// Panel-14 - Widget de Clima em Tempo Real
// @version 9.2.0-VISIBILITY-AUTH-GATING
// @changelog v9.2.0-VISIBILITY-AUTH-GATING - Added auth/visibility checks in refresh handlers
...
export const MODULE_ID = 'panel-14';
...
const _log = (level: string, ...args: unknown[]) => { const logger = _getPort('logger'); if (!logger) return; if (!_debug() && level === 'debug') return; const fn = logger[level] || logger.info; if (typeof fn === 'function') fn(`[${PAINEL_ID}]`, ...args); };
...
class Panel14 {
  ...
  mount(container: HTMLElement, deps: Record<string, unknown> = {}) {
    ...
    return new Promise((resolve, reject) => {
      try {
        _log('info', 'mount.start', { version: VERSION }); _initPorts();
  ...
  loadData() {
    ...
    return this.apiClient.fetchData({ signal: this.abortController.signal, timeout: REQUEST_TIMEOUT }).then((result: Record<string, unknown>) => {
      ...
      if (result.success && result.payload) { this.consecutiveErrors = 0; if (this.isDegraded) { this.isDegraded = false; this.setState(STATES.READY); } this.store.setData(result.payload); this.circuitBreaker.recordSuccess(); this.setState(STATES.READY); _log('debug', 'load.success'); } else throw new Error(String(result.error) || 'Resposta inválida');
  ...
  unmount() {
    ...
    this.setState(STATES.UNMOUNTING); _log('info', 'unmount.start');
...
let instance: Panel14 | null = null;
const mount = (container: HTMLElement, deps: Record<string, unknown> = {}) => { if (instance?.mounted) { _log('warn', 'Instance exists'); return Promise.resolve(); } instance = new Panel14(); return instance.mount(container, deps); };
