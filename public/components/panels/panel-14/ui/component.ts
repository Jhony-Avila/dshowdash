...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-14:ui:component
// PURPOSE: UIComponent - Panel-14 Top Erros
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panel-14:ui:component';
...
export class UIComponent {
  ...
  _handleClick(e: MouseEvent) { const t = (e.target as HTMLElement).closest('[data-action]'); if (!t) return; if ((t as HTMLElement).dataset.action === 'export-errors') this._exportErrors(); if ((t as HTMLElement).dataset.action === 'export-jobs') this._exportJobs(); }

  showLoading() { if (this.hasData) { this.container.classList.add('p14-loading-overlay'); return; } this.container.innerHTML = renderSkeleton(); }
  hideLoading() { this.container.classList.remove('p14-loading-overlay'); }
  ...
  update(data: Record<string, unknown>) {
    ...
    try {
      ...
      if (!this.statistics.total_errors) { this.showEmpty(); return; }
        ...
        <div class="p14-kpi-grid">
          ...
          <div class="p14-kpi-card">
            ...
            <div class="p14-kpi-content">
              ...
              <span class="p14-kpi-label">Média/Dia</span>
            </div>
          </div>
        </div>
  ...
  _renderTopErrorsTable() {
    const rows = this.topErrors.slice(0, 10).map((e: Record<string, unknown>, i: number) => {
    ...
    return `<div class="p14-section"><div class="p14-section-header"><h3 class="p14-section-title">${ICONS.xCircle} <span>Top Erros</span></h3><button class="p14-btn-icon" data-action="export-errors">${ICONS.download}</button></div><div class="p14-section-body"><table class="p14-table"><thead><tr><th class="p14-th">#</th><th class="p14-th">Mensagem</th><th class="p14-th">Qtd</th><th class="p14-th">Jobs</th><th class="p14-th">Último</th></tr></thead><tbody>${rows}</tbody></table></div></div>`;
  }

  _renderJobsTable() {
    const rows = this.jobsWithErrors.slice(0, 10).map((j: Record<string, unknown>, i: number) => `<tr class="p14-tr"><td class="p14-td p14-td--rank">#${i + 1}</td><td class="p14-td">${this._escape(j.job_name)}</td><td class="p14-td"><span class="p14-badge p14-badge--error">${j.error_count}</span></td><td class="p14-td">${j.days_with_errors}d</td><td class="p14-td">${this._formatDateTime(j.last_error)}</td></tr>`).join('') || '<tr><td colspan="5" class="p14-empty-table">Nenhum job</td></tr>';
    return `<div class="p14-section"><div class="p14-section-header"><h3 class="p14-section-title">${ICONS.alertTriangle} <span>Jobs com Mais Erros</span></h3><button class="p14-btn-icon" data-action="export-jobs">${ICONS.download}</button></div><div class="p14-section-body"><table class="p14-table"><thead><tr><th class="p14-th">#</th><th class="p14-th">Job</th><th class="p14-th">Erros</th><th class="p14-th">Dias</th><th class="p14-th">Último</th></tr></thead><tbody>${rows}</tbody></table></div></div>`;
  }

  _exportErrors() { if (!this.topErrors.length) { this.toastManager?.warning('Nenhum dado'); return; } const rows = this.topErrors.map((e: Record<string, unknown>) => [e.error_message, e.occurrences, e.affected_jobs, e.last_occurrence, e.severity]); this._downloadCSV(['Mensagem', 'Ocorrências', 'Jobs Afetados', 'Última Ocorrência', 'Severidade'], rows, 'top-erros'); this.toastManager?.success('Exportado'); }
  _exportJobs() { if (!this.jobsWithErrors.length) { this.toastManager?.warning('Nenhum dado'); return; } const rows = this.jobsWithErrors.map((j: Record<string, unknown>) => [j.job_name, j.error_count, j.days_with_errors, j.last_error]); this._downloadCSV(['Job', 'Erros', 'Dias', 'Último Erro'], rows, 'jobs-com-erros'); this.toastManager?.success('Exportado'); }
  ...
  _formatDateTime(d: unknown) { if (!d) return '--'; try { return new Date(String(d)).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }); } catch { return d; } }
  _truncate(s: string, max: number) { if (!s) return '--'; return s.length > max ? `${s.slice(0, max)}...` : s; }
