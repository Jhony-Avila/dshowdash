const MODULE_ID = "panel-14-ui-component-render";
  ...
  return `
    <header class="p14__header">
      ...
      <div class="p14__header-actions">
        ...
        <button class="p14__btn p14__btn--icon" data-action="settings" title="${i18n.settings || "Configura\xE7\xF5es"}">
          ${ICONS.settings}
        </button>
      </div>
    </header>
    ...
    <div class="p14__toolbar">
      <div class="p14__filters">
        <select class="p14__select" data-filter="period">
          ...
          <option value="month" ${filters.period === "month" ? "selected" : ""}>${i18n.thisMonth || "Este m\xEAs"}</option>
        ...
        <select class="p14__select" data-filter="metric">
          <option value="all" ${filters.metric === "all" ? "selected" : ""}>${i18n.allMetrics || "Todas m\xE9tricas"}</option>
          <option value="revenue" ${filters.metric === "revenue" ? "selected" : ""}>${i18n.revenue || "Receita"}</option>
          <option value="users" ${filters.metric === "users" ? "selected" : ""}>${i18n.users || "Usu\xE1rios"}</option>
          <option value="sessions" ${filters.metric === "sessions" ? "selected" : ""}>${i18n.sessions || "Sess\xF5es"}</option>
          <option value="conversion" ${filters.metric === "conversion" ? "selected" : ""}>${i18n.conversion || "Convers\xE3o"}</option>
        </select>
      ...
      <div class="p14__view-toggle">
        <button class="p14__btn ${viewMode === "chart" ? "p14__btn--active" : ""}" data-action="set-view" data-view="chart">
          ${ICONS.chart} ${i18n.chartView || "Gr\xE1fico"}
        </button>
...
function renderChartView(data, state, i18n) {
  ...
  return `
    <div class="p14__chart-view">
        ...
        <div class="p14__chart-header">
          ...
          <div class="p14__chart-controls">
            <select class="p14__select p14__select--sm" data-action="chart-type">
              ...
              <option value="area" ${chartType === "area" ? "selected" : ""}>\xC1rea</option>
            </select>
          </div>
        </div>
      ...
      <div class="p14__secondary-charts">
        ${renderSecondaryChart("distribution", String(i18n.distribution || "Distribui\xE7\xE3o"), data)}
        ${renderSecondaryChart("comparison", String(i18n.comparison || "Comparativo"), data)}
      </div>
    </div>
  `;
}
function renderKpiCard(metric, i18n) {
  const { label, changeType, format = "number" } = metric;
  const value = metric.value;
  const change = metric.change;
  const isPositive = changeType === "positive" || change !== void 0 && change > 0;
  const isNegative = changeType === "negative" || change !== void 0 && change < 0;
...
function renderPlaceholderChart(data) {
...
function renderSecondaryChart(type, title, data) {
  return `
...
function renderMiniChart(data, type) {
  ...
  return '<div class="p14__mini-placeholder">\u{1F4CA}</div>';
}
function renderTableView(data, state, i18n) {
      ...
      <table class="p14__table">
        <thead>
          <tr>
            ...
            <th class="p14__th" data-sort="change">${i18n.change || "Varia\xE7\xE3o"}</th>
...
function renderEmpty(i18n) {
  return `<div class="p14__empty"><p>${i18n?.noData || "Nenhum dado dispon\xEDvel"}</p></div>`;
}
function renderFooter(data, i18n) {
