// ═══════════════════════════════════════════════════════════════
// DEPENDENCY CONTRACT (8.5.1-IMPORT-FIX)
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-15
// PURPOSE: Painel-15 - Overview Métricas
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panel-15';
...
const loadCSS = (path: string) => { const al = _getPort('assetLoader'); if (al?.loadCSS) al.loadCSS(path); else if (hasDocument && !document.querySelector(`link[href="${path}"]`)) { const link = document.createElement('link'); link.rel = 'stylesheet'; link.href = path; link.setAttribute('data-painel', MODULE_ID); document.head.appendChild(link); } };

(() => { _initPorts(); loadCSS('/components/panels/panel-15/styles/index.css'); })();

class PanelLogger {
  [key: string]: any;
  constructor(panelId: string, version: string) { this.panelId = panelId; this.version = version; }
  debug(msg: string, ctx?: unknown) { const logger = _getPort('logger'); (logger as Record<string, (...a: unknown[]) => void>)?.debug?.(`[${this.panelId}] ${msg}`, ctx); }
  info(msg: string, ctx?: unknown) { const logger = _getPort('logger'); (logger as Record<string, (...a: unknown[]) => void>)?.info?.(`[${this.panelId}] ${msg}`, ctx); }
  warn(msg: string, ctx?: unknown) { const logger = _getPort('logger'); (logger as Record<string, (...a: unknown[]) => void>)?.warn?.(`[${this.panelId}] ${msg}`, ctx); }
  error(msg: string, ctx?: unknown) { const logger = _getPort('logger'); (logger as Record<string, (...a: unknown[]) => void>)?.error?.(`[${this.panelId}] ${msg}`, ctx); }
}

class Telemetry {
  [key: string]: any;
  constructor(panelId: string, version: string) { this.panelId = panelId; this.version = version; }
  track(event: string, data?: Record<string, unknown>) { const telemetry = _getPort('telemetry'); telemetry?.track?.(event, { panelId: this.panelId, version: this.version, ...data }); }
}
...
class Painel15 {
  ...
  mount(container: HTMLElement, deps: Record<string, unknown> = {}) {
    ...
    return Promise.resolve().then(() => {
      ...
      this.eventBus = _getPort('eventBus'); this.abortController = new AbortController(); this.consecutiveErrors = 0; this.isDegraded = false; this.dataLoader = new (DataLoader as unknown as new (ctx: unknown) => Record<string, unknown>)(this);
      renderStructure(this.container);
      const contentElement = this.container.querySelector(`#${PAINEL_ID}-content`);
      this.uiComponent = new (UIComponent as unknown as new (container: HTMLElement | null, logger: unknown) => Record<string, unknown>)(contentElement, this.logger);
      return this.uiComponent.init();
    }).then(() => { setupStateSubscription(this); setupEventListeners(this, PAINEL_ID); return this.dataLoader.loadData(); }).then(() => {
      startCountdown(this); this.markLoaded(); this.mounted = true; this.setState(STATES.MOUNTED);
  ...
  toggleAutoRefresh() { toggleAutoRefresh(this); }
  ...
  unmount(container?: HTMLElement, deps: Record<string, unknown> = {}) {
    ...
    return Promise.resolve().then(() => {
      stopCountdown(this); cleanupEventListeners(this, PAINEL_ID);
