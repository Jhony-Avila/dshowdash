...
DrawerComponent.prototype.renderJobDetails = function(job) {
  ...
  if (subtitle) {
    ...
    subtitle.innerHTML = `<span class="p15-job-type type-${type.toLowerCase()}">${type}</span><span class="p15-status-indicator status-${_getStatusClass(health)}"><span class="p15-status-dot"></span>${getHealthText(health)}</span>`;
  }
...
DrawerComponent.prototype._buildDetailsHTML = function(job) {
  ...
  const sla = job.sla || {};
  return `<div class="p15-drawer-section"><h4 class="p15-drawer-section-title"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20V10M18 20V4M6 20v-4"/></svg> M\xE9tricas</h4><div class="p15-drawer-kpis"><div class="p15-drawer-kpi"><div class="p15-drawer-kpi-label">Execu\xE7\xF5es</div><div class="p15-drawer-kpi-value">${totalExec.toLocaleString("pt-BR")}</div></div><div class="p15-drawer-kpi"><div class="p15-drawer-kpi-label">Taxa de Sucesso</div><div class="p15-drawer-kpi-value ${_getRateClassDrawer(successRate)}">${successRate.toFixed(1)}%</div></div><div class="p15-drawer-kpi"><div class="p15-drawer-kpi-label">Tempo M\xE9dio</div><div class="p15-drawer-kpi-value">${formatDuration(avgTime)}</div></div><div class="p15-drawer-kpi"><div class="p15-drawer-kpi-label">Erros</div><div class="p15-drawer-kpi-value ${errors > 0 ? "error" : ""}">${errors}</div></div></div></div><div class="p15-drawer-section"><h4 class="p15-drawer-section-title"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><path d="M22 4L12 14.01l-3-3"/></svg> SLA</h4><div class="p15-drawer-sla">${this._buildSLARow("Taxa de Sucesso", sla.success_rate)}${this._buildSLARow("Tempo de Execu\xE7\xE3o", sla.duration)}${this._buildSLARow("Erros", sla.errors)}</div></div><div class="p15-drawer-section"><h4 class="p15-drawer-section-title"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg> Informa\xE7\xF5es</h4><div class="p15-drawer-info"><div class="p15-drawer-info-row"><span class="p15-drawer-info-label">ID</span><span class="p15-drawer-info-value">${job.id || "--"}</span></div><div class="p15-drawer-info-row"><span class="p15-drawer-info-label">\xDAltima Execu\xE7\xE3o</span><span class="p15-drawer-info-value">${formatDateTime(job.last_execution)}</span></div><div class="p15-drawer-info-row"><span class="p15-drawer-info-label">\xDAltimo Sucesso</span><span class="p15-drawer-info-value">${formatDateTime(job.last_success)}</span></div><div class="p15-drawer-info-row"><span class="p15-drawer-info-label">Criado em</span><span class="p15-drawer-info-value">${formatDateTime(job.created_at)}</span></div><div class="p15-drawer-info-row"><span class="p15-drawer-info-label">Atualizado em</span><span class="p15-drawer-info-value">${formatDateTime(job.updated_at)}</span></div></div></div>`;
};
