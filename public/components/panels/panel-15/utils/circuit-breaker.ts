...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-15/utils/circuit-breaker
// PURPOSE: Panel-15 - Circuit Breaker Enterprise
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
export const MODULE_ID = 'panel-15/utils/circuit-breaker';
...
export class CircuitBreaker {
  ...
  recordFailure(error?: Error) { this.failures++; this.lastFailureTime = Date.now(); if (this.failures >= this.threshold && this.state !== STATES.OPEN) { this.setState(STATES.OPEN); (this.logger as Record<string, (...a: unknown[]) => void>)?.warn?.('circuit.open', { failures: this.failures, threshold: this.threshold, error: error?.message }); } }
