...
const PANEL_OBSERVED_EVENTS = [
  "panel-16:refresh",
  "panel-16:filter-change",
  "panel-16:period-change",
  "panel-16:sort-change",
  "panel-16:view-change",
  "panel-16:selection-change"
];
function EventReplayClass() {
  this._isRecording = false;
  this._isReplaying = false;
  this._events = [];
  this._recordings = /* @__PURE__ */ new Map();
...
EventReplayClass.prototype.startRecording = function(name) {
  ...
  const unsubState = StateController.subscribe((change) => {
  ...
  PANEL_OBSERVED_EVENTS.forEach((eventName) => {
    const unsub = EventBusPort.on(eventName, (data) => {
      if (!self._isRecording) return;
...
EventReplayClass.prototype._recordEvent = function(type, data) {
...
EventReplayClass.prototype._serializeData = (data) => {
  try {
...
EventReplayClass.prototype._deserializeData = (data) => {
  if (data && data._type === "Set") return new Set(data.values);
  if (data && data._type === "Map") return new Map(data.entries);
  return data;
};
EventReplayClass.prototype.replay = function(name, options) {
  const self = this;
  if (!options) options = {};
  ...
  const runNext = () => {
    ...
    if (options.onProgress) options.onProgress({ current: i + 1, total: recording.events.length, event });
    i++;
...
EventReplayClass.prototype.stopReplay = function() {
  if (this._replayAbort) {
    this._replayAbort.abort();
    this._replayAbort = null;
  }
  this._isReplaying = false;
...
EventReplayClass.prototype._replayEvent = function(event) {
...
EventReplayClass.prototype._delay = (ms, signal) => new Promise((resolve) => {
...
EventReplayClass.prototype.list = function() {
  const result = [];
  this._recordings.forEach((rec, name) => {
    result.push({ name, timestamp: rec.timestamp, duration: rec.duration, eventCount: rec.eventCount });
  });
  return result;
};
EventReplayClass.prototype.get = function(name) {
  return this._recordings.get(name);
};
EventReplayClass.prototype.remove = function(name) {
  const removed = this._recordings.delete(name);
  if (removed) this._persist();
  return removed;
};
EventReplayClass.prototype.clear = function() {
  const count = this._recordings.size;
  this._recordings.clear();
  this._persist();
  return count;
};
EventReplayClass.prototype.export = function(name) {
  const recording = this._recordings.get(name);
  if (!recording) return null;
  return JSON.stringify(recording, null, 2);
};
EventReplayClass.prototype.import = function(jsonStr) {
  try {
...
EventReplayClass.prototype.isRecording = function() {
  return this._isRecording;
};
EventReplayClass.prototype.isReplaying = function() {
  return this._isReplaying;
};
EventReplayClass.prototype.getMetrics = function() {
  return Object.assign({}, this._metrics, { recordings: this._recordings.size });
};
...
const EventReplay = new EventReplayClass();
var event_replay_default = EventReplay;
export {
  EVENT_REPLAY_EVENTS,
  EventReplay,
  MODULE_ID,
  VERSION,
  event_replay_default as default
};
