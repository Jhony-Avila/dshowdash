...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-16:snapshot-manager
// PURPOSE: SnapshotManager - Panel-16 AAA
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   StateController from ../state/controller.js
//   EventBusPort, StoragePort, TelemetryPort from ../ports/index.js
//   LIFECYCLE_EVENTS from /core/runtime/events/catalog/lifecycle.events.js
//
// PROVIDES:
//   SnapshotManager — exported value
//   SNAPSHOT_EVENTS — exported value
//   MODULE_ID — module constant
//   VERSION — module constant
//
// RECEIVES (via init/options): (see init function if present)
// EMITS (eventos):
//   SNAPSHOT_EVENTS.CREATED
//   SNAPSHOT_EVENTS.RESTORED
// LISTENS (eventos):
//   (none)
// WINDOW ACCESS:
//   (none)
...
class SnapshotManagerClass {
  ...
  create(name: string | null = null, tags: string[] = []) {
    const snapshot = {
    ...
    // P24: Token fixo + dados no payload
  ...
  restore(idOrName: string, options: Record<string, unknown> = {}) {
  ...
  get(idOrName: string) {
    ...
    return this._snapshots.find((s: Record<string, unknown>) => s.id === idOrName || s.name === idOrName);
  }

  // Lista todos os snapshots
  list() {
    return this._snapshots.map((s: Record<string, unknown>) => ({
      id: s.id,
      name: s.name,
  ...
  remove(idOrName: string) {
    const idx = this._snapshots.findIndex((s: Record<string, unknown>) => s.id === idOrName || s.name === idOrName);
  ...
  diff(idOrName1: string, idOrName2: string | null = null) {
  ...
  // Export para JSON
  export(idOrName: string | null = null) {
  ...
  // Import de JSON
  import(jsonStr: string, options: { merge?: boolean } = { merge: false }) {
    try {
  ...
  // Observability
  getMetrics() { return { ...this._metrics, count: this._snapshots.length, named: this._namedSnapshots.size }; }
  ...
  healthCheck() {
    return {
      ...
      moduleId: MODULE_ID,
      version: VERSION,
      checks: { initialized: this._snapshots !== undefined, autoSaveActive: this._autoSaveEnabled },
...
export { SNAPSHOT_EVENTS, MODULE_ID, VERSION };
