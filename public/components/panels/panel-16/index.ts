/* ═══════════════════════════════════════════════════════════════
 * DEPENDENCY CONTRACT — panel-16/index.js
 * @version 9.3.0-VISIBILITY-AUTH-GATING
 * @batch Batch Z (Contract #209 of 217)
 *
 * IMPORTS (EXTERNAL):
 ...
 *
 * PATTERNS:
 *   PortsFactory (createPanelPorts), ES6 Class (Panel16 — Widget de Crypto)
 ...
 * ═══════════════════════════════════════════════════════════════ */
// Panel-16 - Widget de Crypto em Tempo Real
// @version 9.3.0-VISIBILITY-AUTH-GATING
...
(() => {
  _initPorts();
  const cssPath = '/components/panels/panel-16/styles/index.css';
  ...
  mount(container: HTMLElement, deps: Record<string, unknown> = {}) {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.apiClient = new ApiClient(PAINEL_ID, { debug: _debug }); this.store = new StateStore({ debug: _debug }); this.circuitBreaker = new (CircuitBreaker as unknown as new (t: number, to: number) => Record<string, unknown>)(CIRCUIT_BREAKER_THRESHOLD, CIRCUIT_BREAKER_TIMEOUT);
