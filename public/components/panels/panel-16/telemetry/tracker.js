...
PanelTelemetryTracker.prototype.trackError = function(error, context, severity) {
  ...
  this.metrics.errors++;
  const err = error;
  return this.track(LIFECYCLE_EVENTS.ERROR, { error: err?.message || String(error), stack: err?.stack?.slice(0, 500) || null, context, severity });
};
...
PanelTelemetryTracker.prototype.trackFornecedor = function(action, fornecedorId, data) {
  data = data || {};
  return this.track(`fornecedor.${action}`, Object.assign({ fornecedorId }, data));
};
PanelTelemetryTracker.prototype.trackRisco = function(fornecedorId, nivel, score) {
  return this.track("risco.avaliacao", { fornecedorId, nivel, score });
};
...
PanelTelemetryTracker.prototype.getMetrics = function() {
  ...
  return Object.assign({}, this.metrics, { sessionDuration: Date.now() - this.sessionStart, performance: { firstLoad: this.performance.firstLoad, lastLoad: this.performance.lastLoad, p50: this._percentile(loadTimes, 0.5), p95: this._percentile(loadTimes, 0.95), p99: this._percentile(loadTimes, 0.99) } });
};
...
PanelTelemetryTracker.prototype._log = function(...args) {
  if (this.debug) {
    const logger = _getPort("logger");
    if (logger?.debug) logger.debug(`[${MODULE_ID}]`, ...args);
  }
};
