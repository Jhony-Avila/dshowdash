...
function UIComponent(container, logger, apiClient, controlsElement) {
...
UIComponent.prototype._createHandlers = function() {
  ...
  return {
    ...
    onPeriodChange() {
      self._emitPeriodChange();
    },
    getDisplayData() {
      return self._getDisplayData();
    },
    saveFilters() {
      persistence.saveFilters(self.filters);
    },
    saveSort() {
      persistence.saveSort(self.sortColumns);
    },
    saveViewMode() {
      persistence.saveViewMode(self.viewMode);
    },
    saveColumns() {
      persistence.saveColumns(self.columns);
    },
    saveFavorites() {
      persistence.saveFavorites(self.favorites);
    },
    sortData() {
      sortData(self.useInfiniteScroll ? self.allLoadedData : self.data.list, self.sortColumns);
    },
    resetInfiniteScroll() {
      self._resetInfiniteScroll();
    },
    loadFornecedor360(id) {
      self._loadFornecedor360(id);
    },
    toggleFavorite(id) {
      self._toggleFavorite(id);
    },
    toggleExpand(id) {
      self._toggleExpand(id);
    },
    removeFilter(key) {
      self._removeFilter(key);
    },
    applyAdvancedFilters() {
      self._applyAdvancedFilters();
    },
    clearAdvancedFilters() {
      self._clearAdvancedFilters();
    },
    exportCSV() {
      exportCSV(self._getDisplayData());
    },
    exportSelected() {
      exportSelected(self._getDisplayData(), self.selectedRows);
    },
    exportXLSX() {
      exportXLSX(self._getDisplayData());
    },
    exportPDF() {
      self._exportPDF();
    },
    ...
    saveCurrentView() {
      self._saveCurrentView();
    },
    resetColumns() {
      self._resetColumns();
    },
    togglePin(colId, side) {
      self._togglePin(colId, side);
    }
  };
};
UIComponent.prototype._handleScroll = function(e) {
...
UIComponent.prototype._loadMoreData = function() {
  ...
  return this.apiClient.getList({ search: this.filters.search, status: this.filters.status, uf: this.filters.uf, tipo: this.filters.tipo, limit: INFINITE_SCROLL_CONFIG.batchSize, offset }).then((result) => {
    if (result.success && result.payload) {
      const payload = result.payload;
      const newItems = payload.list || payload.fornecedores || [];
      if (newItems.length === 0 || self.allLoadedData.length >= INFINITE_SCROLL_CONFIG.maxItems) {
        self.hasMoreData = false;
      } else {
        self.allLoadedData = self.allLoadedData.concat(newItems);
        sortData(self.allLoadedData, self.sortColumns);
        self._render();
        self._bindEvents();
      }
    } else {
...
UIComponent.prototype._updateLoadingMoreIndicator = function(show) {
  const indicator = this.container.querySelector(".p16-infinite-loading");
  if (indicator) indicator.style.display = show ? "flex" : "none";
};
UIComponent.prototype._resetInfiniteScroll = function() {
  this.allLoadedData = [];
  this.hasMoreData = true;
  this.isLoadingMore = false;
};
...
UIComponent.prototype._bindEvents = function() {
  bindEvents(this.container, this, this._createHandlers());
};
UIComponent.prototype._bindControlsEvents = function() {
  bindControlsEvents(this.controlsElement, this, this._createHandlers());
};
UIComponent.prototype._loadFornecedor360 = function(id) {
  const self = this;
  return this.apiClient.getFornecedor360(id).then((result) => {
    if (result.success && result.payload) {
      self.selectedFornecedor = result.payload;
      self._render();
      self._bindEvents();
    }
  }).catch((err) => {
...
UIComponent.prototype._toggleFavorite = function(id) {
  this.favorites.has(id) ? this.favorites.delete(id) : this.favorites.add(id);
  persistence.saveFavorites(this.favorites);
  this._render();
  this._bindEvents();
};
UIComponent.prototype._toggleExpand = function(id) {
  this.expandedRows.has(id) ? this.expandedRows.delete(id) : this.expandedRows.add(id);
  this._render();
  this._bindEvents();
};
UIComponent.prototype._removeFilter = function(key) {
...
UIComponent.prototype._clearAdvancedFilters = function() {
  const self = this;
  ["valorMin", "valorMax", "valorMinClient", "valorMaxClient", "reqMinClient", "reqMaxClient", "risco", "temPix"].forEach((k) => {
...
UIComponent.prototype._copyDoc = function(id) {
  const displayData = this._getDisplayData();
  const f = displayData.find((item) => item.id === id);
...
UIComponent.prototype._applyView = function(viewId) {
  const self = this;
  const view = this.savedViews.find((v) => v.id === viewId);
  if (!view) return;
  this.activeView = viewId;
  if (view.filters) {
    this.filters = Object.assign({}, view.filters);
    persistence.saveFilters(this.filters);
  }
  if (view.columns) {
    this.columns.forEach((c) => {
      c.visible = c.locked || view.columns.includes(c.id);
    });
...
UIComponent.prototype._saveCurrentView = function() {
  const name = prompt("Nome da vis\xE3o:");
  if (!name) return;
  const id = `custom_${Date.now()}`;
  this.savedViews.push({ id, name, icon: "bookmark", desc: "Vis\xE3o personalizada", filters: Object.assign({}, this.filters), columns: this.columns.filter((c) => c.visible).map((c) => c.id) });
  persistence.saveSavedViews(this.savedViews);
  this.activeView = id;
  this.showViewsDropdown = false;
  this._renderControls();
};
UIComponent.prototype._deleteView = function(viewId) {
...
UIComponent.prototype._resetColumns = function() {
  this.columns = COLUMNS.map((c) => Object.assign({}, c, COLUMN_TYPES[c.id] || {}));
  persistence.saveColumns(this.columns);
  this.pinnedLeft = /* @__PURE__ */ new Set(["checkbox", "nome"]);
  this.pinnedRight = /* @__PURE__ */ new Set(["action"]);
  persistence.savePinnedCols(this.pinnedLeft, this.pinnedRight);
  this._render();
  this._bindEvents();
};
UIComponent.prototype._togglePin = function(colId, side) {
  if (side === "left") {
...
UIComponent.prototype._showContextMenu = function(x, y, fornecedorId) {
  const self = this;
  this._hideContextMenu();
  const displayData = this._getDisplayData();
  const f = displayData.find((item) => item.id === fornecedorId);
  if (!f) return;
  ...
  menu.addEventListener("click", (e) => {
    ...
    if (!item) return;
    self._handleContextAction(item.dataset.ctx, item.dataset.id);
    self._hideContextMenu();
  });
};
UIComponent.prototype._hideContextMenu = function() {
  if (this.contextMenu) {
    this.contextMenu.remove();
    this.contextMenu = null;
  }
};
UIComponent.prototype._handleContextAction = function(action, id) {
  const displayData = this._getDisplayData();
  const f = displayData.find((item) => item.id === id);
  switch (action) {
    case "view":
      this._loadFornecedor360(id);
      break;
    case "expand":
    ...
    case "copy-doc":
      ...
      break;
    case "copy-nome":
      if (navigator.clipboard) navigator.clipboard.writeText(f && f.nome || "");
      break;
    case "select":
...
UIComponent.prototype._emitFilterChange = function() {
  EventBusPort.emit(PANEL_16_UI_EVENTS.FILTER_CHANGE, { filters: this.filters, pagination: { page: this.data.pagination.page, limit: this.data.pagination.limit }, source: "panel-16:ui:component", timestamp: Date.now() });
};
UIComponent.prototype._emitPeriodChange = function() {
  EventBusPort.emit(PANEL_16_UI_EVENTS.PERIOD_CHANGE, { period: this.period, source: "panel-16:ui:component", timestamp: Date.now() });
};
UIComponent.prototype._emitRefresh = () => {
  emitPanelIntent("REFRESH", PAINEL_ID);
};
UIComponent.prototype.update = function(payload) {
  if (!payload) return;
  this.lastUpdate = Date.now();
  this.isLoading = false;
  if (payload.kpis) {
    this.data.kpis = payload.kpis;
    this.data.kpis.by_status = payload.by_status;
    this.data.kpis.by_uf = payload.by_uf;
    this.data.kpis.top_valor = payload.top_valor;
  }
  ...
  if (list) {
    ...
    this.data.pagination.total = pag.total || payload.total || list.length;
    if (pag.page || payload.page) this.data.pagination.page = pag.page || payload.page;
    if (pag.limit) this.data.pagination.limit = pag.limit;
    if (this.useInfiniteScroll && this.allLoadedData.length === 0) {
      this.allLoadedData = list.slice();
      this.hasMoreData = list.length < this.data.pagination.total;
    }
...
UIComponent.prototype.showError = function(msg) {
  this.isLoading = false;
...
const MODULE_ID = "panels-panel-16-ui-component";
const VERSION = "9.3.0-P2-ENTERPRISE";
function info() {
  return { moduleId: MODULE_ID, version: VERSION };
}
function healthCheck() {
  return { status: "HEALTHY", moduleId: MODULE_ID, version: VERSION, checks: { componentReady: true }, p24Instrumented: true, timestamp: Date.now() };
}
export {
  MODULE_ID,
  UIComponent,
