...
const MODULE_ID = "panel-17/core/data-loader";
...
DataLoader.prototype.loadData = function() {
  ...
  return ctx.apiClient.fetchData({ signal: ctx.abortController.signal, timeout: REQUEST_TIMEOUT }).then((data) => {
    ...
    if (ctx.consecutiveErrors >= MAX_CONSECUTIVE_ERRORS && !ctx.isDegraded) {
      ctx.isDegraded = true;
      ctx.setState(STATES.DEGRADED);
      ctx.telemetry.track(LIFECYCLE_EVENTS.MODE_DEGRADED, { consecutiveErrors: ctx.consecutiveErrors });
    } else {
...
DataLoader.prototype.healthCheck = function() {
  return { status: "HEALTHY", moduleId: MODULE_ID, version: VERSION, checks: { loadDataReady: typeof this.loadData === "function" } };
};
function info() {
  return { moduleId: MODULE_ID, version: VERSION };
}
function healthCheck() {
  return { status: "HEALTHY", moduleId: MODULE_ID, version: VERSION, checks: { DataLoaderReady: typeof DataLoader === "function" } };
}
