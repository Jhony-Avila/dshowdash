...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-17/core/data-loader
// PURPOSE: Panel-17 - Data Loader
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panel-17/core/data-loader';

function DataLoader(this: any, context: Record<string, unknown>) { this.ctx = context; this.currentRequestId = 0; this.activeLoadRequest = null; }
...
DataLoader.prototype.loadData = function() {
  ...
  return ctx.apiClient.fetchData({ signal: ctx.abortController.signal, timeout: REQUEST_TIMEOUT }).then((data: Record<string, unknown>) => {
    ...
    if (ctx.consecutiveErrors >= MAX_CONSECUTIVE_ERRORS && !ctx.isDegraded) { ctx.isDegraded = true; ctx.setState(STATES.DEGRADED); ctx.telemetry.track(LIFECYCLE_EVENTS.MODE_DEGRADED, { consecutiveErrors: ctx.consecutiveErrors }); }
    else { ctx.setState(STATES.ERROR); }
  }).finally(() => {
...
DataLoader.prototype.healthCheck = function() { return { status: 'HEALTHY', moduleId: MODULE_ID, version: VERSION, checks: { loadDataReady: typeof this.loadData === 'function' } }; };

export function info() { return { moduleId: MODULE_ID, version: VERSION }; }
export function healthCheck() { return { status: 'HEALTHY', moduleId: MODULE_ID, version: VERSION, checks: { DataLoaderReady: typeof DataLoader === 'function' } }; }
