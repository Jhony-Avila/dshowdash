...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-17/core/template
// PURPOSE: Panel-17 - HTML Template Dashboard Métricas
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panel-17/core/template';

export function renderStructure(container: HTMLElement) {
  container.innerHTML = `
    <div class="p17-wrapper" role="region" aria-label="Top 10 Jobs">
      
      <!-- HEADER SIMPLIFICADO -->
      <header class="p17-header">
        
        <!-- Identidade -->
        <div class="p17-identity">
          <div class="p17-identity-icon">
          ...
          <h2 class="p17-title">Top 10 Jobs</h2>
        </div>

        <!-- Ações -->
        <div class="p17-actions">
          <div class="p17-auto-refresh" data-tooltip="Auto-refresh">
            <button class="p17-auto-toggle active" data-action="toggle-auto-refresh" type="button" aria-label="Toggle auto-refresh"></button>
            <span class="p17-countdown active" data-countdown>30</span>
          </div>
          
          <div class="p17-actions-cluster">
            <button class="p17-action-btn p17-action-btn--refresh" data-action="refresh" data-tooltip="Atualizar" type="button">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M23 4v6h-6M1 20v-6h6"/>
                <path d="M3.51 9a9 9 0 0114.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0020.49 15"/>
              </svg>
            </button>
            <button class="p17-action-btn p17-action-btn--print" data-action="print" data-tooltip="Imprimir" type="button">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          <span class="p17-timestamp" data-last-update data-tooltip="Última atualização">
      ...
      <!-- CONTENT AREA - UIComponent renderiza aqui -->
      <main id="${PAINEL_ID}-content" class="p17-content" aria-busy="true"></main>

      <!-- FOOTER COMPACTO -->
      <footer class="p17-footer">
        <div class="p17-footer-left">
          <span class="p17-footer-info">Dashboard Estratégico</span>
        </div>
        <div class="p17-footer-right">
          <span class="p17-footer-info" data-refresh-interval>Auto-refresh: 30s</span>
        </div>
      </footer>
    </div>
  `;
...
export function updateRefreshBtn(container: HTMLElement, text: string, loading: boolean) {
  ...
  if (!btn) return;
  if (loading) {
    btn.classList.add('p17-action-btn--loading');
    btn.setAttribute('disabled', 'true');
  } else {
    btn.classList.remove('p17-action-btn--loading');
    btn.removeAttribute('disabled');
  }
}

export function updateTimestamp(container: HTMLElement, timestamp: number) {
...
export default { 
  ...
  updateCountdown, setAutoRefreshState,
  updateStatusBadges, updateFooterStats, updateFilterCount,
  info, healthCheck
};
