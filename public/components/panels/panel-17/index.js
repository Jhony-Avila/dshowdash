...
class Panel17 {
  ...
  loadData() {
    ...
    return this.apiClient.fetchData({ signal: this.abortController.signal, timeout: REQUEST_TIMEOUT }).then((result) => {
      ...
      } else throw new Error(result.error ? String(result.error) : "Resposta inv\xE1lida");
    }).catch((error) => {
...
var panel_17_default = { mount, unmount, destroy, getStatus, getVersion, healthCheck, info, injectPorts, getPorts };
export {
  MODULE_ID,
  PAINEL_ID as PANEL_ID,
  VERSION,
  panel_17_default as default,
  destroy,
  getPorts,
  getStatus,
  getVersion,
