...
class StateStore {
  constructor() {
    this.state = { jobs: [], loading: false, error: null, filters: {}, sort: { column: "id", direction: "asc" }, selectedJob: null };
  ...
  setJobs(jobs) {
    this.setState({ jobs, loading: false, error: null });
  }
  setLoading(loading) {
  ...
  healthCheck() {
    return { status: Ports.snapshot()._initialized ? "HEALTHY" : "DEGRADED", moduleId: MODULE_ID, version: VERSION, state: { jobCount: this.state.jobs.length, loading: this.state.loading, hasError: !!this.state.error }, portsInitialized: Ports.snapshot()._initialized };
  }
  info() {
