...
const MODULE_ID = "panel-17.ui.drawer";
...
DrawerComponent.prototype.init = function() {
  ...
  this._overlayClickHandler = (e) => {
    ...
    const t = e.target;
    if (t.matches('[data-action="close-drawer"]') || t.closest('[data-action="close-drawer"]')) self.close();
    const actionBtn = t.closest("[data-action]");
    if (actionBtn && self.currentJob) {
      const action = actionBtn.dataset.action;
      if (action !== "close-drawer") self.onAction(action, self.currentJob);
    }
  };
...
DrawerComponent.prototype.updateActionButtons = function(job) {
  ...
  if (pauseBtn) {
    ...
    pauseBtn.textContent = isActive ? "Pausar" : "Ativar";
...
DrawerComponent.prototype.renderJobDetails = function(job) {
  ...
  if (subtitle) {
    ...
    subtitle.innerHTML = `<span class="p17-job-type type-${type.toLowerCase()}">${type}</span><span class="p17-status-indicator status-${_getStatusClass(health)}"><span class="p17-status-dot"></span>${getHealthText(health)}</span>`;
  }
...
DrawerComponent.prototype._buildDetailsHTML = function(job) {
  ...
  const sla = job.sla || {};
  return `<div class="p17-drawer-section"><h4>M\xE9tricas</h4><div class="p17-drawer-kpis"><div class="p17-drawer-kpi"><div class="p17-drawer-kpi-label">Execu\xE7\xF5es</div><div class="p17-drawer-kpi-value">${totalExec.toLocaleString("pt-BR")}</div></div><div class="p17-drawer-kpi"><div class="p17-drawer-kpi-label">Taxa de Sucesso</div><div class="p17-drawer-kpi-value ${_getRateClassDrawer(successRate)}">${successRate.toFixed(1)}%</div></div><div class="p17-drawer-kpi"><div class="p17-drawer-kpi-label">Tempo M\xE9dio</div><div class="p17-drawer-kpi-value">${formatDuration(avgTime)}</div></div><div class="p17-drawer-kpi"><div class="p17-drawer-kpi-label">Erros</div><div class="p17-drawer-kpi-value ${errors > 0 ? "error" : ""}">${errors}</div></div></div></div><div class="p17-drawer-section"><h4>SLA</h4><div class="p17-drawer-sla">${this._buildSLARow("Taxa de Sucesso", sla.success_rate)}${this._buildSLARow("Tempo de Execu\xE7\xE3o", sla.duration)}${this._buildSLARow("Erros", sla.errors)}</div></div><div class="p17-drawer-section"><h4>Informa\xE7\xF5es</h4><div class="p17-drawer-info"><div class="p17-drawer-info-row"><span>ID</span><span>${job.id || "--"}</span></div><div class="p17-drawer-info-row"><span>\xDAltima Execu\xE7\xE3o</span><span>${formatDateTime(String(job.last_execution || ""))}</span></div><div class="p17-drawer-info-row"><span>\xDAltimo Sucesso</span><span>${formatDateTime(String(job.last_success || ""))}</span></div></div></div>`;
};
DrawerComponent.prototype._buildSLARow = (label, slaData) => {
  if (!slaData) return `<div class="p17-drawer-sla-item"><span>${label}</span><span>--</span></div>`;
  const status = slaData.status || "ok";
  const value = slaData.value != null ? slaData.value : "--";
  return `<div class="p17-drawer-sla-item"><span>${label}</span><span><span class="p17-drawer-sla-dot ${status}"></span>${value}</span></div>`;
};
...
function healthCheck() {
  return { status: Ports.isInitialized() ? "HEALTHY" : "DEGRADED", moduleId: MODULE_ID, version: VERSION, checks: { drawerReady: true, portsInitialized: Ports.isInitialized() } };
}
