...
const MODULE_ID = "panel-17/ui/keyboard";
class KeyboardNavigation {
  ...
  init() {
    this._keyHandler = (e) => {
      ...
      switch (e.key) {
        ...
        case "Enter":
          e.preventDefault();
          if (this.currentIndex >= 0 && rows[this.currentIndex]) this.onRowActivate(rows[this.currentIndex]);
          break;
        case "Escape":
  ...
  goTo(rows, index) {
    rows.forEach((r) => r.classList.remove("p17-row-selected"));
    this.currentIndex = index;
    const row = rows[index];
    if (row) {
      row.classList.add("p17-row-selected");
  ...
  healthCheck() {
    return { status: "HEALTHY", moduleId: MODULE_ID, version: VERSION, checks: { enabled: this.enabled } };
  }
}
const info = () => ({ moduleId: MODULE_ID, version: VERSION });
const healthCheck = () => ({ status: "HEALTHY", moduleId: MODULE_ID, version: VERSION, checks: { KeyboardNavigationReady: typeof KeyboardNavigation === "function" } });
