...
const _log = (level, ...args) => {
  ...
  if (logger && typeof logger[level] === "function") logger[level](`[${MODULE_ID}]`, ...args);
};
class ToastManager {
  constructor() {
    this._ready = false;
...
const healthCheck = () => ({ status: _getPort("toast") ? "HEALTHY" : "NOT_INITIALIZED", moduleId: MODULE_ID, version: VERSION, loggerReady: !!_getPort("logger"), portsInitialized: Ports.snapshot()._initialized });
export {
  MODULE_ID,
  ToastManager,
  VERSION,
