...
const loadCSS = () => {
  const cssPath = "/components/panels/panel-18/styles/index.css";
...
class Painel18 {
  ...
  unmount(container, deps = {}) {
    ...
    return Promise.resolve().then(() => {
      ...
      eventBus?.emit?.(PANEL_EVENTS.UNMOUNTED, { panelId: PAINEL_ID, version: VERSION, timestamp: Date.now(), source: MODULE_ID });
    });
  }
  markLoaded() {
    const eventBus = _getPort("eventBus");
...
var panel_18_default = { mount, unmount, destroy, getStatus, getVersion, healthCheck, info, injectPorts, getPorts };
export {
  MODULE_ID,
  VERSION,
  panel_18_default as default,
  destroy,
  getPorts,
  getStatus,
  getVersion,
