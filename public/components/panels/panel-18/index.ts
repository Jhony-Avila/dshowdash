...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-18
// PURPOSE: Painel-18 - KPIs Taxa de Sucesso
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panel-18';
...
class Painel18 {
  ...
  mount(container: HTMLElement, deps: Record<string, unknown> = {}) {
    ...
    return Promise.resolve().then(() => {
      ...
      this.dataLoader = new (DataLoader as unknown as new (ctx: Record<string, unknown>) => Record<string, unknown>)(this); renderStructure(this.container);
  ...
  unmount(container?: HTMLElement, deps: Record<string, unknown> = {}) {
    ...
    return Promise.resolve().then(() => {
      ...
      const eventBus = _getPort('eventBus'); eventBus?.emit?.(PANEL_EVENTS.UNMOUNTED, { panelId: PAINEL_ID, version: VERSION, timestamp: Date.now(), source: MODULE_ID });
    });
  }
...
let painelInstance: Painel18 | null = null;


const mount = (container: HTMLElement, deps: Record<string, unknown> = {}) => { if (painelInstance) { const currentContainer = painelInstance.container; const contentInDOM = currentContainer && document.contains(currentContainer); const sameContainer = currentContainer === container; if (contentInDOM && sameContainer && painelInstance.mounted) return Promise.resolve(); return unmount().then(() => { painelInstance = new Painel18(); return painelInstance.mount(container, deps); }); } painelInstance = new Painel18(); return painelInstance.mount(container, deps); };
const unmount = (container?: HTMLElement, deps?: Record<string, unknown>) => { if (!painelInstance) return Promise.resolve(); return painelInstance.unmount(container as HTMLElement, deps ?? {}).then(() => { painelInstance = null; }); };
const getStatus = (): Record<string, unknown> => painelInstance?.getStatus?.() ?? { panelId: PAINEL_ID, mounted: false, p22Compliant: true };
const getVersion = () => VERSION;
const healthCheck = () => { const status = getStatus(); const checks = { instanceExists: !!painelInstance, mounted: status.mounted === true, notDestroyed: status.destroyed !== true, notDegraded: status.isDegraded !== true, lowErrorCount: ((status.consecutiveErrors as number) || 0) < 5 , abortControllerActive: !!painelInstance?.abortController && !painelInstance.abortController.signal.aborted, noOrphanTimers: status.destroyed !== true || !painelInstance?.countdownInterval, noOrphanPolling: status.destroyed !== true || !painelInstance?.autoRefreshEnabled }; const score = Object.values(checks).filter(Boolean).length; const maxScore = Object.keys(checks).length; return { status: score === maxScore ? 'HEALTHY' : score >= 4 ? 'DEGRADED' : 'UNHEALTHY', score, maxScore, scoreDisplay: `${score}/${maxScore}`, checks, panelId: PAINEL_ID, version: VERSION, p22Compliant: true, timestamp: Date.now() }; };
