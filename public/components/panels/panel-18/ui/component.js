...
const MODULE_ID = "panel-18:ui:component";
const ICONS = {
...
class UIComponent {
  ...
  showLoading() {
    if (this.hasData) {
      this.container.classList.add("p18-loading-overlay");
      return;
    }
    this.container.innerHTML = renderSkeleton();
  }
  hideLoading() {
    this.container.classList.remove("p18-loading-overlay");
  }
  showError(msg) {
  ...
  _renderDashboard() {
    ...
    const uptimeClass = a.uptime_status === "excellent" ? "p18-kpi--excellent" : a.uptime_status === "good" ? "p18-kpi--good" : "p18-kpi--poor";
    const uptimeIcon = a.uptime_status === "excellent" ? ICONS.trophy : a.uptime_status === "good" ? ICONS.check : ICONS.alert;
    this.container.innerHTML = `
      <div class="p18-dashboard">
        <div class="p18-period-badge">
          <span>Per\xEDodo: ${this.period?.from || "--"} at\xE9 ${this.period?.to || "--"} (${days} dias)</span>
        </div>
        ...
        <div class="p18-kpi-grid-2x2">
          ...
          <div class="p18-kpi-large p18-kpi--primary">
            <div class="p18-kpi-large-header">
              ...
              <span class="p18-kpi-large-title">Total Execu\xE7\xF5es</span>
            </div>
  ...
  _getStatusText(status) {
    const map = { "excellent": "Excelente", "good": "Bom", "poor": "Aten\xE7\xE3o" };
  ...
  _renderTopJobsTable() {
    ...
    const rows = jobs.map((j, i) => {
      const statusClass = j.status === "excellent" ? "p18-status--excellent" : j.status === "good" ? "p18-status--good" : j.status === "warning" ? "p18-status--warning" : "p18-status--critical";
      const statusIcon = j.status === "excellent" ? ICONS.trophy : j.status === "good" ? ICONS.check : j.status === "warning" ? ICONS.alert : ICONS.x;
      return `
        <tr class="p18-tr">
          ...
          <td class="p18-td p18-td--rate">
            <span class="p18-rate-badge ${statusClass}">${j.success_rate?.toFixed(1)}%</span>
          </td>
          ...
          <table class="p18-table">
            <thead>
              <tr>
                ...
                <th class="p18-th">Execu\xE7\xF5es</th>
                <th class="p18-th">Tempo M\xE9dio</th>
  ...
  _exportJobs() {
    ...
    const headers = ["#", "Job", "Tipo", "Taxa (%)", "Execu\xE7\xF5es", "Tempo M\xE9dio", "Status"];
    const rows = this.jobsSla.map((j, i) => [
      i + 1,
      j.job_name,
      j.job_type,
      j.success_rate,
      j.total_executions,
      j.avg_execution_time,
      j.status
    ]);
