...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-18.utils.logger
// PURPOSE: Panel-18 - Logger Enterprise
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panel-18.utils.logger';
...
Logger.prototype.shouldLog = function(level: string) { return (LOG_LEVELS as Record<string, number>)[level] >= (LOG_LEVELS as Record<string, number>)[this.minLevel]; };
...
Logger.prototype.log = function(level: string, event: string, data?: Record<string, unknown>) { if (!this.shouldLog(level)) return; const entry = this.formatEntry(level, event, data); this.buffer.push(entry); if (this.buffer.length > this.maxBuffer) this.buffer.shift(); const globalLogger = _getPort('logger') as Record<string, unknown>; if (globalLogger) { const fn = globalLogger[level]; if (typeof fn === 'function') (fn as (...a: unknown[]) => void).call(globalLogger, `[${this.panelId}] ${event}`, entry); } };
