...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-19-chart-renderer
// PURPOSE: ChartRenderer - Enterprise Clean Inline
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
//   ChartRenderer() — exported function
...
export const MODULE_ID = 'panel-19-chart-renderer';
export const VERSION = '9.3.0-P2-ENTERPRISE';

export function ChartRenderer(this: any, options: Record<string, unknown>) {
  ...
  if (options.config) {
    ...
    // @ts-expect-error strict migration — TS2339
    for (const k in cfg) { if (cfg.hasOwnProperty(k)) this._config[k] = cfg[k]; }
  }
}

ChartRenderer.prototype.setConfig = function(config: Record<string, unknown>) {
  for (const k in config) { if (config.hasOwnProperty(k)) this._config[k] = config[k]; }
  return this;
};

ChartRenderer.prototype.render = function(params: Record<string, unknown>) {
  const chartData = params.chartData;
  const period = params.period;
  const config = params.config || {};
  const cfg: Record<string, unknown> = {};
  for (const k in this._config) { if (this._config.hasOwnProperty(k)) cfg[k] = this._config[k]; }
  const configObj = config as Record<string, unknown>;
  for (const k2 in configObj) { if (configObj.hasOwnProperty(k2)) cfg[k2] = configObj[k2]; }
  const periodObj = period as Record<string, unknown> | null | undefined;
  const periodDays = (periodObj && periodObj.days) ? periodObj.days : 30;
  return `<div style="padding:16px;height:260px;max-height:260px;box-sizing:border-box;display:flex;flex-direction:column;overflow:hidden;"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;flex-shrink:0;"><span style="font-size:13px;font-weight:600;color:#f0f0f5;">Evolução da Taxa de Sucesso</span><span style="font-size:11px;color:#606070;">${periodDays} dias</span></div><div style="display:flex;gap:8px;flex:1;min-height:0;max-height:180px;overflow:hidden;"><div style="display:flex;flex-direction:column;justify-content:space-between;font-size:10px;color:#606070;width:36px;text-align:right;padding-right:4px;flex-shrink:0;"><span>100%</span><span>75%</span><span>50%</span><span>25%</span><span>0%</span></div><div style="flex:1;position:relative;background:#12121a;border-radius:4px;overflow:hidden;max-height:180px;">${this._renderSVG(chartData, cfg)}</div></div><div style="position:relative;height:24px;margin-top:8px;margin-left:44px;font-size:10px;color:#606070;flex-shrink:0;">${this._renderXAxis(chartData)}</div></div>`;
};

ChartRenderer.prototype._renderSVG = (chartData: Record<string, unknown>[], config: Record<string, unknown>) => {
  ...
  const slaY = (100 - (config.slaTarget as number)).toFixed(2);
  const pts = [];
  for (let i = 0; i < chartData.length; i++) {
    const d = chartData[i];
    pts.push({ x: ((i / (chartData.length - 1 || 1)) * 100).toFixed(2), y: (100 - (d.success_rate as number)).toFixed(2) });
  }
  const lineParts = [];
...
ChartRenderer.prototype._renderXAxis = (chartData: Record<string, unknown>[]) => {
...
ChartRenderer.prototype.renderError = (msg: string) => `<div style="height:260px;display:flex;align-items:center;justify-content:center;color:#ef4444;">${msg || 'Erro'}</div>`;
