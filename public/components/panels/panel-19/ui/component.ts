...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-19:ui:component
// PURPOSE: UIComponent - Panel-19 Gráfico Taxa de Sucesso
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panel-19:ui:component';
...
export class UIComponent {
  ...
  showLoading() { if (this.hasData) { this.container.classList.add('p19-loading-overlay'); return; } this.container.innerHTML = renderSkeleton(); }
  hideLoading() { this.container.classList.remove('p19-loading-overlay'); }
  ...
  update(data: Record<string, unknown>) {
    ...
    try {
      ...
      this.chartData = (p?.chart_data as unknown[]) || [];
      this.summary = (p?.summary as Record<string, unknown>) || {};
      this.highlights = (p?.highlights as Record<string, unknown>) || {};
      this.trend = (p?.trend as Record<string, unknown>) || {};
      this.period = (p?.period as Record<string, unknown>) || {};
      if (!this.chartData.length) { this.showEmpty(); return; }
  ...
  _renderDashboard() {
    ...
    const trendText = t.status === 'improving' ? 'Melhorando' : t.status === 'declining' ? 'Caindo' : 'Estável';
      ...
      <div class="p19-dashboard">
        <div class="p19-kpi-grid">
          <div class="p19-kpi-card">
            ...
            <div class="p19-kpi-content">
              ...
              <span class="p19-kpi-label">Taxa Média (${this.period.days || 30}d)</span>
            </div>
          </div>
          <div class="p19-kpi-card">
            ...
            <div class="p19-kpi-content">
              ...
              <span class="p19-kpi-label">Execuções</span>
            </div>
          </div>
        ...
        <div class="p19-chart-section">
          <div class="p19-section-header">
            <h3 class="p19-section-title">${ICONS.activity} <span>Evolução da Taxa de Sucesso</span></h3>
  ...
  _renderLineChart() {
    ...
    const maxRate = 100;
    const minRate = Math.min(...data.map((d: Record<string, unknown>) => d.success_rate as number)) - 5;
    const range = maxRate - minRate;
    const width = Math.max(data.length * 30, 600);
    const height = 200;
    const padding = 40;
    const points = data.map((d: Record<string, unknown>, i: number) => {
      const x = padding + (i / (data.length - 1 || 1)) * (width - padding * 2);
      const y = height - padding - (((d.success_rate as number) - minRate) / range) * (height - padding * 2);
      return { x, y, data: d };
    });
    const pathD = points.map((p: { x: number; y: number; data: Record<string, unknown> }, i: number) => `${i === 0 ? 'M' : 'L'} ${p.x} ${p.y}`).join(' ');
    ...
    const xLabels = points.filter((_: unknown, i: number) => i % Math.ceil(data.length / 8) === 0 || i === data.length - 1).map((p: { x: number; y: number; data: Record<string, unknown> }) => `<text x="${p.x}" y="${height - 10}" class="p19-axis-label">${String(p.data.date).slice(5)}</text>`).join('');
    const dots = points.map((p: { x: number; y: number; data: Record<string, unknown> }) => `<circle cx="${p.x}" cy="${p.y}" r="4" class="p19-dot" data-date="${p.data.date}" data-rate="${p.data.success_rate}"><title>${p.data.date}: ${p.data.success_rate}%</title></circle>`).join('');
  ...
  _export() { if (!this.chartData.length) { this.toastManager?.warning('Nenhum dado'); return; } const rows = this.chartData.map((d: Record<string, unknown>) => [d.date, d.total, d.success, d.errors, d.success_rate]); this._downloadCSV(['Data', 'Total', 'Sucesso', 'Erros', 'Taxa (%)'], rows, 'taxa-sucesso-diaria'); this.toastManager?.success('Exportado'); }
  ...
  _formatNumber(v: number | string | null | undefined) { if (v == null) return '--'; return parseInt(String(v)).toLocaleString('pt-BR'); }
