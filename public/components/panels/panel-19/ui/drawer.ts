...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-19.ui.drawer
// PURPOSE: Panel-19 Drawer Component
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
DrawerComponent.prototype.renderJobDetails = function(job: Record<string, unknown>) {
  ...
  if (subtitle) { const type = String(job.job_type || job.type || 'api').toUpperCase(); const health = String(job.health_status || 'inactive'); subtitle.innerHTML = `<span class="p19-job-type type-${type.toLowerCase()}">${type}</span><span class="p19-status-indicator status-${_getStatusClass(health)}"><span class="p19-status-dot"></span>${getHealthText(health)}</span>`; }
