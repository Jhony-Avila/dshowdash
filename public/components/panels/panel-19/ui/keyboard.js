...
const MODULE_ID = "panel-19/ui/keyboard";
class KeyboardNavigation {
  ...
  init() {
    this._keyHandler = (e) => {
      ...
      const tbody = this.container.querySelector(".p19-tbody");
      if (!tbody) return;
  ...
  goTo(rows, index) {
    rows.forEach((r) => r.classList.remove("p19-row-selected"));
    this.currentIndex = index;
    const row = rows[index];
    if (row) {
      row.classList.add("p19-row-selected");
