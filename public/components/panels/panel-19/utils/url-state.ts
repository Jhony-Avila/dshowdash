...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-19/utils/url-state
// PURPOSE: Panel-19 URL State - Shim para RouteStateService
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panel-19/utils/url-state';
const PANEL_ID = 'panel-19';
...
URLState.prototype.write = function(filters: Record<string, unknown>, replace: boolean) {
  ...
  if (filters) {
    for (let i = 0; i < ALLOWED_KEYS.length; i++) {
      ...
      if (filters[k] !== null && filters[k] !== '' && filters[k] !== undefined) (clean as Record<string, unknown>)[k] = filters[k];
    }
  }
...
URLState.prototype.subscribe = (callback: (...args: unknown[]) => unknown) => RouteStateService.subscribe(PANEL_ID, callback);
