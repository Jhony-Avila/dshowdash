'use strict';
// panel-ads/index.js — carregador do painel React (Ads Intelligence / Google Ads).
// @module  panels/panel-ads
// @version 1.0.0
// @created 2026-07-21
//
// Espelha panel-outlook/index.js: adaptador vanilla que o PanelPort importa e que
// SEMPRE monta a interface React dentro do container-main (sem iframe).
// CSS injetado UMA vez, escopado a [data-ads-react-root] (tokens --ads-*).
// Manifest lido com cache:'no-store' (fonte da verdade do entry hasheado).

const MODULE_ID = 'panels/panel-ads';
const VERSION   = '1.0.0';
const FLAG_KEY  = 'panel_ads_enabled';
...
const CSS_ATTR = 'data-ads-react-css';
...
function renderErro(contentEl, err) {
  console.error('[ads] falha ao carregar o React:', err);
  if (!contentEl) return;
  contentEl.innerHTML =
    '<div style="padding:32px;max-width:520px;margin:40px auto;text-align:center;'
    + 'border:1px solid var(--ads-border,#2e2e44);border-radius:12px;'
    + 'background:var(--bg-card,#1a1a2e);color:var(--text-primary,#e8e8f0)">'
    + '<strong style="display:block;font-size:15px;margin-bottom:8px">Não foi possível carregar o módulo Ads Intelligence</strong>'
...
export async function unmount() {
  try {
    ...
    console.error('[ads] erro no unmount:', err);
  } finally {
    _mounted = false;
  }
