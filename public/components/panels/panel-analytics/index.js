...
const CircuitBreaker = _CircuitBreakerImport;
import { StateStore } from "./state/store.js";
import { Logger as _LoggerImport } from "./telemetry/logger.js";
const Logger = _LoggerImport;
import { Tracker } from "./telemetry/tracker.js";
const MODULE_ID = "panels/panel-analytics";
...
let _debug = false;
const emitUIAction = (eventBus, data = {}) => {
...
class AnalyticsComponent {
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}"><div class="panel-enterprise__header"><h1 class="panel-enterprise__title">${CONFIG.svgIcon} ${CONFIG.label}</h1><span class="panel-enterprise__version">v${VERSION}</span></div><div class="panel-enterprise__content"><p class="panel-enterprise__subtitle">Este m\xF3dulo est\xE1 em desenvolvimento</p><p class="panel-enterprise__desc">Componente enterprise com arquitetura AAA completa.</p></div><div class="panel-enterprise__footer"><span class="panel-enterprise__status" data-status="ready">Ready</span></div></div>`;
  ...
  healthCheck() {
    ...
    return { status: passed === total ? "HEALTHY" : passed > total / 2 ? "DEGRADED" : "UNHEALTHY", score: passed, maxScore: total, scoreDisplay: `${passed}/${total}`, checks, version: VERSION, moduleId: MODULE_ID, p22Compliant: true, timestamp: (/* @__PURE__ */ new Date()).toISOString() };
  }
  info() {
    const portsSnapshot = Ports.snapshot();
    return { moduleId: MODULE_ID, version: VERSION, config: this.config, mounted: this._mounted, initialized: this._initialized, metrics: this._metrics, state: this.store.getState(), portsInitialized: portsSnapshot._initialized, p22Compliant: true, healthCheck: this.healthCheck() };
  }
  getState() {
...
var panel_analytics_default = { AnalyticsComponent, createAnalytics, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  AnalyticsComponent,
  MODULE_ID,
  VERSION,
  createAnalytics,
  panel_analytics_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
