'use strict';
// panel-anuncios/index.js — carregador do painel React (Anúncios / Consultor Google Ads).
// @module  panels/panel-anuncios
// @version 1.0.0
// @created 2026-07-27
//
...
// CSS injetado UMA vez, escopado a [data-anuncios-react-root] (tokens --anx-*).
...
// regras) através do proxy same-origin /api/anuncios/ask.php — o token da API
// NUNCA chega ao navegador.

const MODULE_ID = 'panels/panel-anuncios';
const VERSION   = '1.0.0';
const FLAG_KEY  = 'panel_anuncios_enabled';
...
const CSS_ATTR = 'data-anuncios-react-css';
...
async function carregarReact() {
  ...
  const base = '/components/panels/panel-anuncios/dist/';
  let entrada = null;
  try {
...
function renderErro(contentEl, err) {
  console.error('[anuncios] falha ao carregar o React:', err);
  if (!contentEl) return;
  contentEl.innerHTML =
    '<div style="padding:32px;max-width:520px;margin:40px auto;text-align:center;'
    + 'border:1px solid var(--anx-border,#2e2e44);border-radius:12px;'
    + 'background:var(--bg-card,#1a1a2e);color:var(--text-primary,#e8e8f0)">'
    + '<strong style="display:block;font-size:15px;margin-bottom:8px">Não foi possível carregar o Consultor de Anúncios</strong>'
...
export async function mount(contentEl, config = {}) {
  ...
  try {
    ...
    await mod.mountReact(contentEl, {
      ...config,
      flag: { key: FLAG_KEY, enabled: true, payload, source: 'anuncios' },
    });
    _mounted = true;
...
export async function unmount() {
  try {
    ...
    console.error('[anuncios] erro no unmount:', err);
  } finally {
    _mounted = false;
  }
