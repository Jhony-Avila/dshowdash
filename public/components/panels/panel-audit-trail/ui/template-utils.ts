...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-audit-trail-template-utils
// PURPOSE: Panel Audit Trail - Template Utils
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
export const MODULE_ID = 'panel-audit-trail-template-utils';

export function formatTimestamp(ts: string | number | Date | null | undefined) {
  if (!ts) return '-';
  try { 
...
export function truncate(str: string | null | undefined, maxLen = 100) {
  if (!str) return '';
  str = String(str);
...
export default { 
  VERSION, MODULE_ID,
  formatTimestamp, escapeHtml, truncate, getToastIcon,
  info, healthCheck
};
