'use strict';
// panel-avatar-studio/index.js — carregador do painel React (Avatar Studio).
// @module  panels/panel-avatar-studio
// @version 1.0.0
// @created 2026-07-27
//
...
// CSS injetado UMA vez, escopado a [data-avst-react-root].
// Manifest lido com cache:'no-store' (fonte da verdade do entry hasheado).

const MODULE_ID = 'panels/panel-avatar-studio';
const VERSION   = '1.0.0';
const FLAG_KEY  = 'panel_avatar_studio_enabled';
...
const CSS_ATTR = 'data-avst-react-css';
...
function renderErro(contentEl, err) {
  ...
  contentEl.innerHTML =
    ...
    + '<strong style="display:block;font-size:15px;margin-bottom:8px">Não foi possível carregar o Avatar Studio</strong>'
...
export async function mount(contentEl, config = {}) {
  ...
  try {
    ...
    await mod.mountReact(contentEl, {
      ...config,
      flag: { key: FLAG_KEY, enabled: true, payload, source: 'avatar-studio' },
    });
    _mounted = true;
