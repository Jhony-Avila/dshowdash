...
const MODULE_ID = "panels/panel-charts";
...
const CONFIG = { id: "charts", area: "panel", label: "Charts", icon: "charts", emoji: "\u{1F4C8}", kind: "panel-component" };
let _debug = false;
...
const healthCheck = () => {
  const hcBase = _currentInstance?.healthCheck() ?? { status: "UNHEALTHY", mounted: false, version: VERSION, moduleId: MODULE_ID, timestamp: Date.now() };
  const hc = { ...hcBase };
  hc.isDocumentVisible = _isDocumentVisible();
  return hc;
};
var panel_charts_default = { ChartsComponent, createCharts, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  ChartsComponent,
  MODULE_ID,
  VERSION,
  createCharts,
  panel_charts_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
