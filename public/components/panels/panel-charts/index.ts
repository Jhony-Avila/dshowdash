...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-charts
// PURPOSE: Charts - Enterprise AAA Component
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
//   createCharts() — exported function
...
export const MODULE_ID = 'panels/panel-charts';
...
const CONFIG = { id: 'charts', area: 'panel', label: 'Charts', icon: 'charts', emoji: '📈', kind: 'panel-component' };
let _debug = false;
...
export const createCharts = (options: { container?: HTMLElement | null; config?: Record<string, any>; eventBus?: Record<string, any> } = {}) => new ChartsComponent(options);
let _currentInstance: ChartsComponent | null = null;
export const mount = (container: HTMLElement, config: Record<string, any> = {}) => { if (!_isAuthenticated()) { return { success: false, moduleId: MODULE_ID, error: "not-authenticated" }; } const instance = new ChartsComponent({ container, config }); instance.init(); instance.mount(container); _currentInstance = instance; return { success: true, moduleId: MODULE_ID, instance }; };
...
export const healthCheck = () => { const hcBase = _currentInstance?.healthCheck() ?? { status: 'UNHEALTHY', mounted: false, version: VERSION, moduleId: MODULE_ID, timestamp: Date.now() }; const hc: Record<string, unknown> = { ...hcBase }; hc.isDocumentVisible = _isDocumentVisible(); return hc; };

export default { ChartsComponent, createCharts, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
