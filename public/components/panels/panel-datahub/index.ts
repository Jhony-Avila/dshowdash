...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-datahub
// PURPOSE: Data Hub - Enterprise AAA Component
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
//   createDatahub() — exported function
...
export const MODULE_ID = 'panels/panel-datahub';
...
const CONFIG = { id: 'datahub', area: 'panel', label: 'Data Hub', icon: 'datahub', emoji: '🗄️', kind: 'panel-component' };
let _debug = false;
...
export const createDatahub = (options: { container?: HTMLElement | null; config?: Record<string, any>; eventBus?: Record<string, any> } = {}) => new DatahubComponent(options);
let _currentInstance: DatahubComponent | null = null;
export const mount = (container: HTMLElement, config: Record<string, any> = {}) => { if (!_isAuthenticated()) { return { success: false, moduleId: MODULE_ID, error: "not-authenticated" }; } const instance = new DatahubComponent({ container, config }); instance.init(); instance.mount(container); _currentInstance = instance; return { success: true, moduleId: MODULE_ID, instance }; };
...
export const healthCheck = () => { const hc = _currentInstance?.healthCheck() ?? { status: 'UNHEALTHY', mounted: false, version: VERSION, moduleId: MODULE_ID, timestamp: Date.now() }; return { ...hc, isDocumentVisible: _isDocumentVisible() }; };

export default { DatahubComponent, createDatahub, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
