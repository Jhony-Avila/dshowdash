...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-datahub/telemetry/logger
// PURPOSE: Data Hub - Logger
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panels/panel-datahub/telemetry/logger';
...
export class Logger {
  [key: string]: any;
  constructor(options: { prefix?: string; level?: string; enabled?: boolean } = {}) { this.prefix = options.prefix || '[datahub]'; this.level = options.level || 'info'; this.enabled = options.enabled !== false; this.levels = { debug: 0, info: 1, warn: 2, error: 3 }; this._metrics = { debugCount: 0, infoCount: 0, warnCount: 0, errorCount: 0, lastLogAt: null }; _initPorts(); }
  ...
  _log(level: string, args: unknown[]) { if (!this.enabled || this.levels[level] < this.levels[this.level]) return; const globalLogger = _getPort('logger'); (globalLogger as Record<string, (...a: unknown[]) => void>)?.[level]?.(this.prefix, ...args); this._metrics[`${level}Count`]++; this._metrics.lastLogAt = Date.now(); }
