...
const MODULE_ID = "panel-enterprise/ui/renderer";
function renderHeader() {
  return `<div class="enterprise-header"><div class="enterprise-icon">${ICONS.building}</div><div class="enterprise-title"><h2>Enterprise Features</h2><p>Funcionalidades avan\xE7adas para sua empresa</p></div></div>`;
}
function renderFeatureGrid(features, activeFeatures) {
  const featureList = features || Object.values(FEATURES);
  return `<div class="enterprise-features">${featureList.map((f) => renderFeatureCard(f, activeFeatures?.includes(f.id))).join("")}</div>`;
}
function renderFeatureCard(feature, isActive) {
...
function renderUpgradeCard() {
  return `<div class="upgrade-card"><div class="upgrade-icon">${ICONS.star}</div><div class="upgrade-content"><h3>Upgrade para Enterprise</h3><p>Desbloqueie todas as funcionalidades avan\xE7adas</p></div><button class="upgrade-btn" data-action="upgrade">Saiba Mais</button></div>`;
}
function renderSkeleton() {
  return '<div class="enterprise-skeleton"><div class="skeleton-header"></div><div class="skeleton-grid"><div class="skeleton-card"></div><div class="skeleton-card"></div><div class="skeleton-card"></div><div class="skeleton-card"></div></div></div>';
}
...
function getStatusLabel(status) {
  const labels = { active: "Ativo", "coming-soon": "Em breve", locked: "Bloqueado", beta: "Beta" };
...
var renderer_default = { renderHeader, renderFeatureGrid, renderFeatureCard, renderUpgradeCard, renderSkeleton };
export {
  MODULE_ID,
  VERSION,
