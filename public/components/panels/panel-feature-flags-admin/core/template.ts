...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-feature-flags-admin/core/template
// PURPOSE: Panel Feature Flags Admin - HTML Template Enterprise
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panel-feature-flags-admin/core/template';
...
export function updateRefreshBtn(container: HTMLElement | null, text: string, loading: boolean) {
...
export function updateTimestamp(container: HTMLElement | null, timestamp: number | string | null) {
...
export function updateStatusBadges(container: HTMLElement | null, stats: { total?: number; active?: number } | null) {
  if (!container || !stats) return;
  const totalEl = container.querySelector('[data-total-count]');
  const activeEl = container.querySelector('[data-active-count]');
  if (totalEl) totalEl.textContent = String(stats.total !== undefined ? stats.total : 0);
  if (activeEl) activeEl.textContent = String(stats.active !== undefined ? stats.active : 0);
}

export function updateCountdown(container: HTMLElement | null, seconds: number) {
  const el = container ? container.querySelector('[data-countdown]') : null;
  if (el) el.textContent = String(seconds);
}

export function setAutoRefreshState(container: HTMLElement | null, active: boolean) {
...
export default { renderStructure, updateRefreshBtn, updateTimestamp, updateStatusBadges, updateCountdown, setAutoRefreshState, healthCheck, info };
