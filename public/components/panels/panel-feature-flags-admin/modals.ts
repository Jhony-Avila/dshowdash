...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-feature-flags-admin:modals
// PURPOSE: Panel Feature Flags Admin - Modal Templates
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
//   getCreateFormHTML() — exported function
//   getEditFormHTML() — exported function
//   parseCreateForm() — exported function
//   parseEditForm() — exported function
...
export const MODULE_ID = 'panel-feature-flags-admin:modals';
...
export function getEditFormHTML(flag: Record<string, unknown>) {
  ...
  return `<form data-form="edit-flag" style="display:flex;flex-direction:column;gap:1rem;"><div class="form-group"><label>Chave</label><input type="text" value="${flag.flag_key}" disabled style="${disabledStyle}"></div><div class="form-group"><label>Nome</label><input type="text" name="flag_name" value="${flag.flag_name || ''}" required style="${inputStyle}"></div><div class="form-group"><label>Descrição</label><textarea name="description" rows="2" style="${inputStyle}">${flag.description || ''}</textarea></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;"><div class="form-group"><label>Ambiente</label><select name="environment" style="${inputStyle}">${envSelect}</select></div><div class="form-group"><label>Rollout %</label><input type="number" name="rollout_percentage" value="${flag.rollout_percentage !== undefined ? flag.rollout_percentage : 100}" min="0" max="100" style="${inputStyle}"></div></div><label style="display:flex;align-items:center;gap:0.5rem;cursor:pointer;"><input type="checkbox" name="is_enabled"${flag.is_enabled ? ' checked' : ''}><span>Ativado</span></label></form>`;
}

export function parseCreateForm(form: HTMLFormElement) {
  return {
...
export function parseEditForm(form: HTMLFormElement) {
  return {
    flag_name: form.flag_name.value,
    description: form.description.value,
    environment: form.environment.value,
...
export default { getCreateFormHTML, getEditFormHTML, parseCreateForm, parseEditForm };
