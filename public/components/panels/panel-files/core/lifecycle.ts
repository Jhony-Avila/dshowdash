...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-files.core.lifecycle
// PURPOSE: Arquivos - Lifecycle Manager
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panel-files.core.lifecycle';
...
export class LifecycleManager {
  ...
  async _runHooks(name: string) { for (const hook of this.hooks[name] || []) { try { await (hook as (c: unknown) => Promise<void>)(this.component); } catch (e) { this._metrics.errorCount++; _getPort('logger')?.error(`[${MODULE_ID}] Hook ${name} error:`, e); } } }
