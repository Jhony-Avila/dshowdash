...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-files
// PURPOSE: Arquivos - Enterprise AAA Component
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panels/panel-files';
...
const CONFIG = { id: 'files', area: 'panel', label: 'Arquivos', icon: 'files', emoji: '📁', kind: 'panel-component' };
let _debug = false;
...
export class FilesComponent {
  ...
  _updateLoadingUI(loading: unknown) { const statusEl = this._element?.querySelector('[data-status]'); if (statusEl) { statusEl.setAttribute('data-status', loading ? 'loading' : 'ready'); statusEl.textContent = loading ? 'Loading...' : 'Ready'; } }
...
export const createFiles = (options: { container?: HTMLElement | null; config?: Record<string, any>; eventBus?: Record<string, any> } = {}) => new FilesComponent(options);
let _currentInstance: FilesComponent | null = null;
export const mount = (container: HTMLElement, config: Record<string, any> = {}) => { if (!_isAuthenticated()) { return { success: false, moduleId: MODULE_ID, error: "not-authenticated" }; } const instance = new FilesComponent({ container, config }); instance.init(); instance.mount(container); _currentInstance = instance; return { success: true, moduleId: MODULE_ID, instance }; };
...
export default { FilesComponent, createFiles, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
