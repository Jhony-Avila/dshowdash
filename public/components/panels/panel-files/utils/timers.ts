...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-files/utils/timers
// PURPOSE: Arquivos - Timer Utilities
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
export const MODULE_ID = 'panels/panel-files/utils/timers';

export function debounce(fn: (...args: unknown[]) => unknown, delay = 300) {
  let timer: ReturnType<typeof setTimeout> | undefined;
  return (...args: unknown[]) => { clearTimeout(timer); timer = setTimeout(() => fn(...args), delay); };
}

export function throttle(fn: (...args: unknown[]) => unknown, limit = 300) {
  let inThrottle: boolean = false;
  return (...args: unknown[]) => { if (!inThrottle) { fn(...args); inThrottle = true; setTimeout(() => inThrottle = false, limit); } };
}

export function delay(fn: () => unknown, ms: number) { return setTimeout(fn, ms); }
export function interval(fn: () => unknown, ms: number) { return setInterval(fn, ms); }
