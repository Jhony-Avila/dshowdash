...
const MODULE_ID = "panels/panel-footer-activity";
...
const CONFIG = { id: "footer-activity", area: "footer", label: "Activity", icon: "activity", emoji: "\u{1F4CA}", kind: "panel-component", apiEndpoint: "/api/status/activity.php", refreshInterval: 3e4 };
class FooterActivityComponent {
  constructor(options = {}) {
    _initPorts();
  ...
  _updateDisplay() {
    ...
    if (valueEl) {
      ...
      if (!isNaN(usage)) {
        if (usage < 50) valueEl.classList.add("status-ok");
        else if (usage < 80) valueEl.classList.add("status-warn");
...
const mount = (container, config) => {
  const instance = new FooterActivityComponent({ container, config });
  instance.init();
...
var panel_footer_activity_default = { FooterActivityComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  FooterActivityComponent,
  MODULE_ID,
  VERSION,
  panel_footer_activity_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
