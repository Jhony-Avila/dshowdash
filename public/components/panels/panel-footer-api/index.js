...
const MODULE_ID = "panels/panel-footer-api";
...
const CONFIG = { id: "footer-api", area: "footer", label: "APIs", icon: "api", emoji: "\u{1F50C}", kind: "panel-component", apiEndpoint: "/api/status/integrations.php", refreshInterval: 6e4 };
class FooterApiComponent {
  constructor(options = {}) {
    _initPorts();
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="APIs/Integrations Status"><span class="footer-icon">${CONFIG.emoji}</span><span class="footer-value" data-metric="usage">--</span></div>`;
...
const mount = (container, config) => {
  const instance = new FooterApiComponent({ container, config });
  instance.init();
...
var panel_footer_api_default = { FooterApiComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  FooterApiComponent,
  MODULE_ID,
  VERSION,
  panel_footer_api_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
