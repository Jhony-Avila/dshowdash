...
const MODULE_ID = "panels/panel-footer-cpu";
...
const CONFIG = { id: "footer-cpu", area: "footer", label: "CPU", icon: "cpu", emoji: "\u{1F4BB}", kind: "panel-component", apiEndpoint: "/api/status/cpu.php", refreshInterval: 3e4 };
class FooterCpuComponent {
  constructor(options = {}) {
    _initPorts();
...
const mount = (container, config) => {
  const instance = new FooterCpuComponent({ container, config });
  instance.init();
...
var panel_footer_cpu_default = { FooterCpuComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  FooterCpuComponent,
  MODULE_ID,
  VERSION,
  panel_footer_cpu_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
