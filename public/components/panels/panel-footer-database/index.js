...
const MODULE_ID = "panels/panel-footer-database";
...
const CONFIG = { id: "footer-database", area: "footer", label: "Database", icon: "database", emoji: "\u{1F5C4}\uFE0F", kind: "panel-component", apiEndpoint: "/api/status/database.php", refreshInterval: 6e4 };
class FooterDatabaseComponent {
  constructor(options = {}) {
    _initPorts();
...
const mount = (container, config) => {
  const instance = new FooterDatabaseComponent({ container, config });
  instance.init();
...
var panel_footer_database_default = { FooterDatabaseComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  FooterDatabaseComponent,
  MODULE_ID,
  VERSION,
  panel_footer_database_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
