...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-footer-database
// PURPOSE: Footer Database - Enterprise Component with Live Data
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panels/panel-footer-database';
...
const CONFIG = { id: 'footer-database', area: 'footer', label: 'Database', icon: 'database', emoji: '🗄️', kind: 'panel-component', apiEndpoint: '/api/status/database.php', refreshInterval: 60000 };

export class FooterDatabaseComponent {
  [key: string]: any;
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="Database Status"><span class="footer-icon">${CONFIG.emoji}</span><span class="footer-value" data-metric="usage">--</span></div>`;
  ...
  _updateDisplay() {
    ...
    if (valueEl) {
      ...
      else if (state.status === 'degraded') valueEl.classList.add('status-warn');
...
let _currentInstance: FooterDatabaseComponent | null = null;

// @ts-expect-error TS migration - TS2554
export const mount = (container, config) => { const instance = new FooterDatabaseComponent({ container, config }); instance.init(); instance.mount(container); _currentInstance = instance; return { success: true, moduleId: MODULE_ID, instance }; };
...
export default { FooterDatabaseComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
