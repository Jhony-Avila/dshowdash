...
const MODULE_ID = "panels/panel-footer-disk";
...
const CONFIG = { id: "footer-disk", area: "footer", label: "Disco", icon: "disk", emoji: "\u{1F4BE}", kind: "panel-component", apiEndpoint: "/api/status/disk.php", refreshInterval: 6e4 };
class FooterDiskComponent {
  constructor(options = {}) {
    _initPorts();
...
const mount = (container, config) => {
  const instance = new FooterDiskComponent({ container, config });
  instance.init();
...
var panel_footer_disk_default = { FooterDiskComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  FooterDiskComponent,
  MODULE_ID,
  VERSION,
  panel_footer_disk_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
