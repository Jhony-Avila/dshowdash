...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-footer-disk
// PURPOSE: Footer Disk - Enterprise Component with Live Data
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panels/panel-footer-disk';
...
const CONFIG = { id: 'footer-disk', area: 'footer', label: 'Disco', icon: 'disk', emoji: '💾', kind: 'panel-component', apiEndpoint: '/api/status/disk.php', refreshInterval: 60000 };

export class FooterDiskComponent {
  [key: string]: any;
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="Disk Usage"><span class="footer-icon">${CONFIG.emoji}</span><span class="footer-value" data-metric="usage">--</span></div>`;
  ...
  _updateDisplay() {
    ...
    if (valueEl) {
      ...
      if (!isNaN(usage)) { if (usage < 70) valueEl.classList.add('status-ok'); else if (usage < 90) valueEl.classList.add('status-warn'); else valueEl.classList.add('status-error'); }
    }
  }
...
let _currentInstance: FooterDiskComponent | null = null;

// @ts-expect-error TS migration - TS2554
export const mount = (container, config) => { const instance = new FooterDiskComponent({ container, config }); instance.init(); instance.mount(container); _currentInstance = instance; return { success: true, moduleId: MODULE_ID, instance }; };
...
export default { FooterDiskComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
