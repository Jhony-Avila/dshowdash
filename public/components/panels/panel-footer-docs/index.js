...
const MODULE_ID = "panels/panel-footer-docs";
...
const CONFIG = { id: "footer-docs", area: "footer", label: "Docs", emoji: "\u{1F4DA}", apiEndpoint: "/api/status/docs.php", refreshInterval: 3e5 };
class FooterDocsComponent {
  constructor(options = {}) {
    _initPorts();
    this.container = options.container || null;
    this.config = { ...CONFIG, ...options.config };
    this.eventBus = options.eventBus || _getPort("eventBus");
    this.store = new StateStore({ mounted: false, loading: false, error: null, data: null, totalDocs: 0, status: "unknown" });
  ...
  async _fetchData() {
    ...
    try {
      ...
      if (j.success && j.data) {
        this.store.setState({ loading: false, error: null, data: j.data, totalDocs: j.data.total_docs || 0, status: j.data.status || "unknown" });
        this._updateDisplay();
      }
    } catch (e) {
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="Documentation"><span class="footer-icon">${CONFIG.emoji}</span><span class="footer-value" data-metric="docs">--</span></div>`;
  ...
  _updateDisplay() {
    ...
    if (v) {
      v.textContent = s.totalDocs > 0 ? s.totalDocs : "\u2713";
      v.classList.add("status-ok");
    }
  }
  healthCheck() {
...
const mount = (container, config) => {
  const i = new FooterDocsComponent({ container, config });
  i.init();
  i.mount(container);
  _currentInstance = i;
...
var panel_footer_docs_default = { FooterDocsComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  FooterDocsComponent,
  MODULE_ID,
  VERSION,
  panel_footer_docs_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
