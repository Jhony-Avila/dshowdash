...
const MODULE_ID = "panels/panel-footer-file";
...
const CONFIG = { id: "footer-file", area: "footer", label: "Files", icon: "file", emoji: "\u{1F4C1}", kind: "panel-component", apiEndpoint: "/api/status/files.php", refreshInterval: 12e4 };
class FooterFileComponent {
  constructor(options = {}) {
    _initPorts();
  ...
  async _fetchData() {
    ...
    try {
      ...
      if ((result.ok ?? result.success) && result.data) {
        this.store.setState({ loading: false, error: null, data: result.data, totalFiles: result.data.total_files || 0, status: result.data.status || "unknown" });
        this._updateDisplay();
      }
    } catch (error) {
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="Files Status"><span class="footer-icon">${CONFIG.emoji}</span><span class="footer-value" data-metric="files">--</span></div>`;
  ...
  _updateDisplay() {
    ...
    if (valueEl) {
      valueEl.textContent = state.totalFiles > 0 ? state.totalFiles.toLocaleString() : "\u2713";
      valueEl.classList.remove("status-ok", "status-error");
  ...
  attachEvents() {
    this._unsubscribe = this.store.subscribe((state, prev) => {
      if (state.totalFiles !== prev.totalFiles) this._updateDisplay();
    });
  }
  detachEvents() {
...
const mount = (container, config) => {
  const instance = new FooterFileComponent({ container, config });
  instance.init();
...
var panel_footer_file_default = { FooterFileComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  FooterFileComponent,
  MODULE_ID,
  VERSION,
  panel_footer_file_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
