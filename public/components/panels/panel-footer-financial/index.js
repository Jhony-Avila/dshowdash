...
const MODULE_ID = "panels/panel-footer-financial";
...
const CONFIG = { id: "footer-financial", area: "footer", label: "Financial", emoji: "\u{1F4B0}", apiEndpoint: "/api/status/financial.php", refreshInterval: 3e5 };
class FooterFinancialComponent {
  constructor(options = {}) {
    _initPorts();
    this.container = options.container || null;
    this.config = { ...CONFIG, ...options.config };
    this.eventBus = options.eventBus || _getPort("eventBus");
    this.store = new StateStore({ mounted: false, loading: false, error: null, data: null, transactions: 0, status: "unknown" });
  ...
  async _fetchData() {
    ...
    try {
      ...
      if (j.success && j.data) {
        this.store.setState({ loading: false, error: null, data: j.data, transactions: j.data.month_transactions || 0, status: j.data.status || "unknown" });
        this._updateDisplay();
      }
    } catch (e) {
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="Financial"><span class="footer-icon">${CONFIG.emoji}</span><span class="footer-value" data-metric="transactions">--</span></div>`;
  ...
  _updateDisplay() {
    ...
    const v = this._element.querySelector(".footer-value");
    if (v) {
      v.textContent = s.transactions > 0 ? s.transactions : "\u2713";
      v.classList.remove("status-ok", "status-error");
      if (s.status === "online") v.classList.add("status-ok");
      else v.classList.add("status-error");
    }
  }
  healthCheck() {
...
const mount = (container, config) => {
  const i = new FooterFinancialComponent({ container, config });
  i.init();
  i.mount(container);
  _currentInstance = i;
...
var panel_footer_financial_default = { FooterFinancialComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  FooterFinancialComponent,
  MODULE_ID,
  VERSION,
  panel_footer_financial_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
