...
const MODULE_ID = "panels/panel-footer-globe";
...
const CONFIG = { id: "footer-globe", area: "footer", label: "Cloud", icon: "globe", emoji: "\u{1F310}", kind: "panel-component", apiEndpoint: "/api/status/cloud.php", refreshInterval: 6e4 };
class FooterGlobeComponent {
  constructor(options = {}) {
    _initPorts();
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="Cloud Status"><span class="footer-icon">${CONFIG.emoji}</span><span class="footer-value" data-metric="usage">--</span></div>`;
...
const mount = (container, config) => {
  const instance = new FooterGlobeComponent({ container, config });
  instance.init();
...
var panel_footer_globe_default = { FooterGlobeComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  FooterGlobeComponent,
  MODULE_ID,
  VERSION,
  panel_footer_globe_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
