...
const MODULE_ID = "panels/panel-footer-memory";
...
const CONFIG = { id: "footer-memory", area: "footer", label: "RAM", icon: "memory", emoji: "\u{1F9E0}", kind: "panel-component", apiEndpoint: "/api/status/memory.php", refreshInterval: 3e4 };
class FooterMemoryComponent {
  constructor(options = {}) {
    _initPorts();
  ...
  _updateDisplay() {
    ...
    if (valueEl) {
      ...
      if (!isNaN(usage)) {
        if (usage < 60) valueEl.classList.add("status-ok");
        else if (usage < 85) valueEl.classList.add("status-warn");
...
const mount = (container, config) => {
  const instance = new FooterMemoryComponent({ container, config });
  instance.init();
...
var panel_footer_memory_default = { FooterMemoryComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  FooterMemoryComponent,
  MODULE_ID,
  VERSION,
  panel_footer_memory_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
