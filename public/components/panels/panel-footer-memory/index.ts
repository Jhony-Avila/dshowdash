...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-footer-memory
// PURPOSE: Footer Memory - Enterprise Component with Live Data
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panels/panel-footer-memory';
...
const CONFIG = { id: 'footer-memory', area: 'footer', label: 'RAM', icon: 'memory', emoji: '🧠', kind: 'panel-component', apiEndpoint: '/api/status/memory.php', refreshInterval: 30000 };

export class FooterMemoryComponent {
  [key: string]: any;
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="Memory Usage"><span class="footer-icon">${CONFIG.emoji}</span><span class="footer-value" data-metric="usage">--</span></div>`;
  ...
  _updateDisplay() {
    ...
    if (valueEl) {
      ...
      if (!isNaN(usage)) { if (usage < 60) valueEl.classList.add('status-ok'); else if (usage < 85) valueEl.classList.add('status-warn'); else valueEl.classList.add('status-error'); }
    }
  }
...
let _currentInstance: InstanceType<typeof FooterMemoryComponent> | null = null;

// @ts-expect-error TS migration - TS2554
export const mount = (container, config) => { const instance = new FooterMemoryComponent({ container, config }); instance.init(); instance.mount(container); _currentInstance = instance; return { success: true, moduleId: MODULE_ID, instance }; };
...
export default { FooterMemoryComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
