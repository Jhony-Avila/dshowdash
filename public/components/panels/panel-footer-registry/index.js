...
const MODULE_ID = "panels/panel-footer-registry";
...
const CONFIG = { id: "footer-registry", area: "footer", label: "Storage", icon: "registry", emoji: "\u{1F5C3}\uFE0F", kind: "panel-component", apiEndpoint: "/api/status/storage.php", refreshInterval: 12e4 };
class FooterRegistryComponent {
  constructor(options = {}) {
    _initPorts();
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="Storage Status"><span class="footer-icon">${CONFIG.emoji}</span><span class="footer-value" data-metric="usage">--</span></div>`;
  ...
  _updateDisplay() {
    ...
    if (valueEl) {
      ...
      if (!isNaN(usage)) {
        if (usage < 70) valueEl.classList.add("status-ok");
        else if (usage < 90) valueEl.classList.add("status-warn");
...
const mount = (container, config) => {
  const instance = new FooterRegistryComponent({ container, config });
  instance.init();
...
var panel_footer_registry_default = { FooterRegistryComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  FooterRegistryComponent,
  MODULE_ID,
  VERSION,
  panel_footer_registry_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
