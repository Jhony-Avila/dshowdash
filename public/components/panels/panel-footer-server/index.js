...
const MODULE_ID = "panels/panel-footer-server";
...
const CONFIG = { id: "footer-server", area: "footer", label: "Server", icon: "server", emoji: "\u{1F5A5}\uFE0F", kind: "panel-component", apiEndpoint: "/api/status/server.php", refreshInterval: 6e4 };
class FooterServerComponent {
  constructor(options = {}) {
    _initPorts();
...
const mount = (container, config) => {
  const instance = new FooterServerComponent({ container, config });
  instance.init();
...
var panel_footer_server_default = { FooterServerComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  FooterServerComponent,
  MODULE_ID,
  VERSION,
  panel_footer_server_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
