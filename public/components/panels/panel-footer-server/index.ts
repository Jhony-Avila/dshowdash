...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-footer-server
// PURPOSE: Footer Server - Enterprise Component with Live Data
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panels/panel-footer-server';
...
const CONFIG = { id: 'footer-server', area: 'footer', label: 'Server', icon: 'server', emoji: '🖥️', kind: 'panel-component', apiEndpoint: '/api/status/server.php', refreshInterval: 60000 };

export class FooterServerComponent {
  [key: string]: any;
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="Server Uptime"><span class="footer-icon">${CONFIG.emoji}</span><span class="footer-value" data-metric="usage">--</span></div>`;
...
let _currentInstance: InstanceType<typeof FooterServerComponent> | null = null;

// @ts-expect-error TS migration - TS2554
export const mount = (container, config) => { const instance = new FooterServerComponent({ container, config }); instance.init(); instance.mount(container); _currentInstance = instance; return { success: true, moduleId: MODULE_ID, instance }; };
...
export default { FooterServerComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
