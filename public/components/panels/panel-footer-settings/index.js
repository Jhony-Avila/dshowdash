...
const MODULE_ID = "panels/panel-footer-settings";
...
const CONFIG = { id: "footer-settings", area: "footer", label: "Settings", emoji: "\u2699\uFE0F", apiEndpoint: "/api/status/settings.php", refreshInterval: 12e4 };
class FooterSettingsComponent {
  constructor(options = {}) {
    _initPorts();
    this.container = options.container || null;
    this.config = { ...CONFIG, ...options.config };
    this.eventBus = options.eventBus || _getPort("eventBus");
    this.store = new StateStore({ mounted: false, loading: false, error: null, data: null, theme: "--" });
  ...
  async _fetchData() {
    ...
    try {
      ...
      if ((j.ok ?? j.success) && j.data) {
        this.store.setState({ loading: false, error: null, data: j.data, theme: j.data.theme || "--" });
        this._updateDisplay();
      }
    } catch (e) {
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="Settings"><span class="footer-icon">${CONFIG.emoji}</span><span class="footer-value" data-metric="theme">--</span></div>`;
  ...
  _updateDisplay() {
    ...
    if (v) {
      v.textContent = s.theme === "dark" ? "\u{1F319}" : "\u2600\uFE0F";
...
const mount = (container, config) => {
  const i = new FooterSettingsComponent({ container, config });
  i.init();
  i.mount(container);
  _currentInstance = i;
...
var panel_footer_settings_default = { FooterSettingsComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  FooterSettingsComponent,
  MODULE_ID,
  VERSION,
  panel_footer_settings_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
