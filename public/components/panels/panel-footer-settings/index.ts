...
// ═══════════════════════════════════════════════════════════════
// MODULE: components.panels.panel-footer-settings
// PURPOSE: Footer settings panel with live data and visibility control
// ───────────────────────────────────────────────────────────────
...
// @contract MOUNT - mount() mounts component to container
// @contract UNMOUNT - unmount() unmounts component
...
export const MODULE_ID = 'panels/panel-footer-settings';
...
const CONFIG = { id: 'footer-settings', area: 'footer', label: 'Settings', emoji: '⚙️', apiEndpoint: '/api/status/settings.php', refreshInterval: 120000 };
export class FooterSettingsComponent {
  [key: string]: any;
  constructor(options: Record<string, unknown> = {}) { _initPorts(); this.container = options.container || null; this.config = { ...CONFIG, ...(options.config as Record<string, unknown>) }; this.eventBus = options.eventBus || _getPort('eventBus'); this.store = new StateStore({ mounted: false, loading: false, error: null, data: null, theme: '--' }); this.lifecycle = new LifecycleManager(this); this.circuitBreaker = new CircuitBreaker({ threshold: 3, timeout: 30000 }); this.logger = new Logger({ prefix: `[${MODULE_ID}]` }); this.tracker = new Tracker({ moduleId: MODULE_ID }); this._mounted = false; this._initialized = false; this._element = null; this._abortController = null; this._refreshTimer = null; this._metrics = { mountCount: 0, errorCount: 0, fetchCount: 0, lastFetchAt: null }; }
  ...
  init(ctx: unknown) { if (this._initialized) return this; this._ctx = ctx || {}; this._initialized = true; return this; }
  ...
  async _fetchData() { if (!this._canRefresh()) return; this.store.setState({ loading: true }); this._metrics.fetchCount++; this._metrics.lastFetchAt = Date.now(); try { const r = await fetch(this.config.apiEndpoint, { signal: this._abortController?.signal, credentials: 'include' }); if (!r.ok) throw new Error(`HTTP ${r.status}`); const j = await r.json(); if ((j.ok ?? j.success) && j.data) { this.store.setState({ loading: false, error: null, data: j.data, theme: j.data.theme || '--' }); this._updateDisplay(); } } catch (e: any) { if (e.name !== 'AbortError') { this._metrics.errorCount++; this.store.setState({ loading: false, error: e.message }); } } }
  ...
  render() { this._element = document.createElement('div'); this._element.className = `enterprise-component ${CONFIG.id}`; this._element.setAttribute('data-module-id', MODULE_ID); this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="Settings"><span class="footer-icon">${CONFIG.emoji}</span><span class="footer-value" data-metric="theme">--</span></div>`; this.container.innerHTML = ''; this.container.appendChild(this._element); }
  _updateDisplay() { if (!this._element) return; const s = this.store.getState(); const v = this._element.querySelector('.footer-value'); if (v) { v.textContent = s.theme === 'dark' ? '🌙' : '☀️'; v.classList.add('status-ok'); } }
...
let _currentInstance: InstanceType<typeof FooterSettingsComponent> | null = null;

// @ts-expect-error TS migration - TS2554
export const mount = (container, config) => { const i = new FooterSettingsComponent({ container, config }); i.init(); i.mount(container); _currentInstance = i; return { success: true, moduleId: MODULE_ID, instance: i }; };
...
export default { FooterSettingsComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
