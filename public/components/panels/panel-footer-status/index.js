...
const MODULE_ID = "panels/panel-footer-status";
...
const CONFIG = { id: "footer-status", area: "footer", label: "Web", icon: "status", emoji: "\u{1F30D}", kind: "panel-component", apiEndpoint: "/api/status/web.php", refreshInterval: 3e4 };
class FooterStatusComponent {
  ...
  async _fetchData() {
    ...
    try {
      ...
      if (result.success && result.data) {
        this.store.setState({ loading: false, error: null, data: result.data, requestsMin: result.data.requests_min || 0, status: result.data.status || "unknown" });
        this._updateDisplay();
      }
    } catch (error) {
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="Web Status"><span class="footer-icon">${CONFIG.emoji}</span><span class="footer-value" data-metric="requests">--</span></div>`;
  ...
  _updateDisplay() {
    ...
    if (valueEl) {
      valueEl.textContent = state.requestsMin > 0 ? `${state.requestsMin}/m` : "\u2713";
  ...
  attachEvents() {
    this._unsubscribe = this.store.subscribe((state, prev) => {
      if (state.requestsMin !== prev.requestsMin) this._updateDisplay();
    });
  }
  detachEvents() {
...
const mount = (container, config) => {
  const instance = new FooterStatusComponent({ container, config });
  instance.init();
...
var panel_footer_status_default = { FooterStatusComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  FooterStatusComponent,
  MODULE_ID,
  VERSION,
  panel_footer_status_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
