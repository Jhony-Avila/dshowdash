...
const MODULE_ID = "panels/panel-footer-support";
...
const CONFIG = { id: "footer-support", area: "footer", label: "Support", emoji: "\u{1F3A7}", apiEndpoint: "/api/status/support.php", refreshInterval: 12e4 };
class FooterSupportComponent {
  constructor(options = {}) {
    _initPorts();
    this.container = options.container || null;
    this.config = { ...CONFIG, ...options.config };
    this.eventBus = options.eventBus || _getPort("eventBus");
    this.store = new StateStore({ mounted: false, loading: false, error: null, data: null, ticketsOpen: 0, status: "unknown" });
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="Support"><span class="footer-icon">${CONFIG.emoji}</span><span class="footer-value" data-metric="tickets">--</span></div>`;
  ...
  _updateDisplay() {
    ...
    if (v) {
      v.textContent = s.ticketsOpen > 0 ? s.ticketsOpen : "\u2713";
      v.classList.remove("status-ok", "status-warn");
      if (s.ticketsOpen === 0) v.classList.add("status-ok");
...
const mount = (container, config) => {
  const i = new FooterSupportComponent({ container, config });
  i.init();
  i.mount(container);
  _currentInstance = i;
...
var panel_footer_support_default = { FooterSupportComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  FooterSupportComponent,
  MODULE_ID,
  VERSION,
  panel_footer_support_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
