...
export class FooterSupportComponent {
  ...
  async _fetchData() { if (!this._canRefresh()) return; this.store.setState({ loading: true }); this._metrics.fetchCount++; this._metrics.lastFetchAt = Date.now(); try { const r = await fetch(this.config.apiEndpoint, { signal: this._abortController?.signal, credentials: 'include' }); if (!r.ok) throw new Error(`HTTP ${r.status}`); const j = await r.json(); if (j.success && j.data) { this.store.setState({ loading: false, error: null, data: j.data, ticketsOpen: j.data.tickets_open || 0, status: j.data.status || 'unknown' }); this._updateDisplay(); } } catch (e) { if ((e as Error).name !== 'AbortError') { this._metrics.errorCount++; this.store.setState({ loading: false, error: (e as Error).message }); } } }
