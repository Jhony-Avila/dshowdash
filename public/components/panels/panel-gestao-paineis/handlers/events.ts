/* ═══════════════════════════════════════════════════════════════
 * panel-gestao-paineis/handlers/events.ts
 * @version 1.2.0
 * v1.2.0: open-panel action, sort dropdown, screenshot history
 * ═══════════════════════════════════════════════════════════════ */

import { CSS_PREFIX, DATA_ATTRS } from '../core/constants.js';
import { store } from '../state/store.js';
import { loadPanels, loadCategories, togglePanelActive, savePanelChanges, applyFilter, loadScreenshotHistory, reSortPanels } from './data.js';
import { handleScreenshotRequest } from './screenshot.js';
import { renderDetailModal, getModalFormData, renderScreenshotHistory } from '../ui/modal/panel-detail-modal.js';
...
import type { SortField, SortConfig } from '../ui/filters/filter-bar.js';
import type { CustomSelectOption } from '../ui/custom-select.js';
import type { PanelData } from '../core/types.js';

let _container: HTMLElement | null = null;
let _abortController: AbortController | null = null;
let _searchDebounce: ((value: string) => void) | null = null;

export function setupEventListeners(container: HTMLElement, abortController: AbortController): void {
  _container = container;
  _abortController = abortController;
  const signal = abortController.signal;

  _searchDebounce = createSearchDebounce((value: string) => {
...
function _handleClick(e: Event): void {
  ...
  const actionEl = target.closest<HTMLElement>(`[${DATA_ATTRS.action}]`);
  if (!actionEl) {
    // Check for card click (open details)
    const card = target.closest<HTMLElement>(`.${CSS_PREFIX}-card`);
    if (card && !target.closest('button') && !target.closest('a')) {
      const panelId = card.getAttribute(DATA_ATTRS.panelId);
      if (panelId) _openDetailModal(panelId);
    }
    return;
  }
  ...
  switch (action) {
    ...
    case 'open-status-select':
      ...
      break;
    /* ── MELHORIA 1: Thumbnail click opens panel route ── */
    case 'open-panel': {
      e.stopPropagation();
    ...
    /* ── MELHORIA 3: Sort dropdown ── */
    case 'open-sort-select':
      e.stopPropagation();
      _openSortDropdown(actionEl);
      break;
    /* ── MELHORIA 5: Screenshot history auto-load ── */
...
function _openCategoryDropdown(trigger: HTMLElement): void {
...
function _openStatusDropdown(trigger: HTMLElement): void {
  const state = store.getState();
  const signal = _abortController?.signal;
  ...
  openCustomSelect({
    ...
    width: 180,
    onSelect(value) {
      applyFilter({ status: value as 'active' | 'inactive' | 'all' });
      syncFiltersToURL(store.getState().filters);
      loadPanels(signal);
    },
  });
...
/* ── MELHORIA 3: Sort dropdown ── */
function _openSortDropdown(trigger: HTMLElement): void {
  ...
  const options: CustomSelectOption[] = sortOptions.map(o => ({
    value: `${o.field}`,
    label: o.label,
  }));
  ...
  openCustomSelect({
    ...
    onSelect(value) {
      ...
      // Toggle order if same field selected again
      if (currentSort.field === field) {
        order = currentSort.order === 'asc' ? 'desc' : 'asc';
      } else if (field === 'last_screenshot_at') {
        order = 'desc'; // Default most recent first for dates
      }
...
/* ── MELHORIA 5: Load screenshot history when modal section is visible ── */
async function _loadScreenshotHistoryInModal(el: HTMLElement): Promise<void> {
...
function _handleInput(e: Event): void {
  ...
  if (action === 'search' && _searchDebounce) {
    _searchDebounce((target as HTMLInputElement).value);
  }
}
...
function _openDetailModal(panelId: string): void {
...
async function _saveFromModal(panelId: string, signal?: AbortSignal): Promise<void> {
...
function _removeTag(tag: string): void {
  const panel = store.getState().selectedPanel;
  if (!panel) return;
...
function _exportPanelsJSON(): void {
  const state = store.getState();
  ...
  a.href = url;
  a.download = `paineis_export_${new Date().toISOString().slice(0, 10)}.json`;
...
function _updateSearchInput(value: string): void {
  if (!_container) return;
  const input = _container.querySelector<HTMLInputElement>(`.${CSS_PREFIX}-search-input`);
  if (input) input.value = value;
}

function _handleTagInput(e: KeyboardEvent): void {
...
export function setupTagInput(container: HTMLElement, signal: AbortSignal): void {
  container.addEventListener('keydown', (e) => {
    if ((e.target as HTMLElement).getAttribute(DATA_ATTRS.action) === 'add-tag') {
      _handleTagInput(e as KeyboardEvent);
    }
  }, { signal });
}
