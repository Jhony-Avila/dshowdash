'use strict';
// panel-google-analytics/index.js — carregador do painel React (Google Analytics).
// @module  panels/panel-google-analytics
// @version 1.0.0
// @created 2026-07-30
//
...
//
// A rota #/panel-google-analytics resolve por CONVENÇÃO no panel-paths.js
...
// root `[data-ga-react-root]` (tokens `--ga-*`), fica inerte quando o painel fecha.
//
// O manifest é lido com `cache:'no-store'` (fonte da verdade do entry hasheado).

const MODULE_ID = 'panels/panel-google-analytics';
const VERSION   = '1.0.0';
const FLAG_KEY  = 'panel_google_analytics_enabled';
...
async function carregarReact() {
  ...
  const base = '/components/panels/panel-google-analytics/dist/';
  let entrada = null;
  try {
...
function renderErro(contentEl, err) {
  ...
  contentEl.innerHTML =
    ...
    + '<strong style="display:block;font-size:15px;margin-bottom:8px">Não foi possível carregar o Google Analytics</strong>'
