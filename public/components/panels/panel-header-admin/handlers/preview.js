...
const MODULE_ID = "panel-header-admin-handlers-preview";
...
function initPreview(container) {
  previewContainer = container.querySelector("#header-preview-frame");
  if (previewContainer) updatePreview();
}
...
function getComponentsByPosition(components, groups, position) {
  const positionGroups = groups.filter((g) => g.position === position).map((g) => g.group_key);
  return components.filter((c) => positionGroups.indexOf(c.group_key) !== -1).sort((a, b) => (a.order_index || 0) - (b.order_index || 0));
}
function renderPreviewComponents(components) {
  if (!components.length) return '<span class="pha-preview-empty">\u2014</span>';
  return components.map((c) => `<div class="pha-preview-item" title="${c.label} (${c.component_key})"><span class="pha-preview-item-icon">${getIconSvg(c.icon_name)}</span><span class="pha-preview-item-label">${c.label}</span></div>`).join("");
}
...
function togglePreviewExpand(container) {
  const section = container.querySelector(".pha-preview-section");
  if (section) section.classList.toggle("pha-preview-section--expanded");
}
function info() {
