...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-header-admin/handlers/ui-actions
// PURPOSE: Header Admin - UI Actions Handler
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panels/panel-header-admin/handlers/ui-actions';
...
export class UIActionsHandler {
  ...
  async execute(name: string, payload: unknown) { const handler = this._actions.get(name); if (!handler) { _getPort('logger')?.warn(`[${MODULE_ID}] Action not found: ${name}`); return { ok: false, reason: 'not_found' }; } this._metrics.actionCount++; this._metrics.lastActionAt = Date.now(); try { const result = await handler(payload); return { ok: true, result }; } catch (e) { this._metrics.errorCount++; _getPort('logger')?.error(`[${MODULE_ID}] Action error:`, e); return { ok: false, error: (e as Error).message }; } }
