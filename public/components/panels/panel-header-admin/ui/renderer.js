...
const COMPONENT_TYPES = {
  ...
  "button": { label: "Bot\xE3o", icon: "square", color: "#f59e0b" },
...
const Renderer = {
  ...
  renderComponentCard(component, index) {
    const typeInfo = COMPONENT_TYPES[component.component_type] || COMPONENT_TYPES.info;
    const isActive = component.is_active === 1;
    const hasUarps = !!component.uarps_trigger_id;
    ...
    if (hasUarps) {
      html += `<span class="pha-card__uarps" title="UARPS: ${this.escapeHtml(component.uarps_trigger_id)}">${ICONS.shield}</span>`;
    }
    if (component.polling_enabled) {
      html += `<span class="pha-card__polling">${ICONS.refresh} ${Math.round(component.polling_interval / 1e3)}s</span>`;
    }
    html += "</div>";
  ...
  renderComponentModal(state, triggers = []) {
    ...
    html += `<div class="pha-form__field"><label>Label *</label><input type="text" name="label" value="${this.escapeHtml(c.label || "")}" required placeholder="Nome de exibi\xE7\xE3o"></div>`;
    html += "</div>";
    html += '<div class="pha-form__row">';
    html += '<div class="pha-form__field"><label>Grupo</label><select name="group_id"><option value="">Sem grupo</option>';
    groups.forEach((g) => {
      html += `<option value="${g.id}" ${c.group_id == g.id ? "selected" : ""}>${Renderer.escapeHtml(g.label)} (${g.position})</option>`;
    });
    html += "</select></div>";
    html += '<div class="pha-form__field"><label>Tipo</label><select name="component_type">';
    Object.keys(COMPONENT_TYPES).forEach((k) => {
      html += `<option value="${k}" ${c.component_type === k ? "selected" : ""}>${COMPONENT_TYPES[k].label}</option>`;
    });
    html += "</select></div>";
    html += "</div>";
    html += '<div class="pha-form__row">';
    html += `<div class="pha-form__field"><label>\xCDcone</label><input type="text" name="icon_name" value="${this.escapeHtml(c.icon_name || "")}" placeholder="ex: activity, zap"></div>`;
    html += `<div class="pha-form__field"><label>Tooltip</label><input type="text" name="tooltip" value="${this.escapeHtml(c.tooltip || "")}"></div>`;
    html += "</div>";
    html += `<div class="pha-form__field"><label>Descri\xE7\xE3o</label><input type="text" name="description" value="${this.escapeHtml(c.description || "")}"></div>`;
    html += '<div class="pha-form__row">';
    html += `<div class="pha-form__field"><label>Ordem</label><input type="number" name="order_index" value="${c.order_index || 0}" min="0"></div>`;
    html += '<div class="pha-form__field"><label>UARPS Trigger (Permiss\xE3o)</label><select name="uarps_trigger_id"><option value="">P\xFAblico (sem restri\xE7\xE3o)</option>';
    triggers.forEach((t) => {
      html += `<option value="${Renderer.escapeHtml(t.trigger_id)}" ${c.uarps_trigger_id == t.trigger_id ? "selected" : ""}>${Renderer.escapeHtml(t.label || t.trigger_id)}</option>`;
    });
    html += "</select></div>";
    html += "</div>";
