...
//   createTelemetry from ./telemetry-helper.js
//   bindUIEvents, updateCountdownDisplay,
//     renderAuthBlockedView, renderErrorView from ./ui-handlers.js
//   startAutoRefresh, stopAutoRefresh, setupEventListeners,
//     cleanupEventListeners, updatePerformanceMetrics
//     from ./auto-refresh.js
//   Store, actions, selectors from ./state/store.js
//   CircuitBreaker from ./utils/circuit-breaker.js
//   CONFIG from ./constants.js
//   injectStyles from ./styles.js
//   collectHealthData from ./collector.js
//   renderSkeleton, renderDashboard from ./render.js
//   STATES from ./core/states.js
//   CIRCUIT_BREAKER_*, DEFAULT_PERFORMANCE_METRICS,
//     REFRESH_INTERVAL, getErrorMessage from ./core/config.js
//
// PROVIDES:
//   mount(), unmount(), refresh(), healthCheck(), info(),
//   getVersion(), toggleAutoRefresh(),
//   VERSION, MODULE_ID, injectPorts(), getPorts()
//
// RECEIVES (via mount):
//   container (DOM Element or selector)
//   deps.eventBus — event bus instance
//
// BROWSER APIs (legítimo — dashboard UI):
//   document.querySelector, document.createElement — DOM
//   document.head.appendChild — CSS injection
//   document.visibilityState — auto-refresh gating
//   performance.now — mount/load timing
//   AbortController — request cancellation
//
// WINDOW ACCESS (legítimo — global API):
//   window.PanelHealthDashboard — panel global
// ═══════════════════════════════════════════════════════════════
// Panel Health Dashboard - Enterprise
// @version 9.3.0-P2-ENTERPRISE
// @changelog v8.6.0-ENTERPRISE - Added kernels to expanded categories by default
// @changelog v8.5.0-ENTERPRISE - ES6 class conversion
'use strict';
...
export const MODULE_ID = 'panel-health-dashboard';
const PANEL_ID = MODULE_ID;

(() => {
  initPorts();
  const cssPath = '/components/panels/panel-health-dashboard/styles/index.css';
  const assetLoader = getPort('assetLoader') as Record<string, any> | null;
...
function PanelHealthDashboard(this: any) {
  ...
  this.countdownValue = REFRESH_INTERVAL / 1000;
  this.countdownInterval = null;
  // Added 'kernels' to expanded categories
  this.expandedCategories = new Set(['kernels', 'core', 'components', 'panels']);
  this._handleVisibilityChange = this._handleVisibilityChange.bind(this);
  this._handleRefreshEvent = this._handleRefreshEvent.bind(this);
}

PanelHealthDashboard.prototype.mount = function(container: HTMLElement | string, deps: Record<string, unknown>) {
  const self = this;
  deps = deps || {};
  initPorts();
  ...
  return Promise.resolve().then(() => {
    const auth = getPort('auth') as Record<string, any> | null;
    ...
    renderErrorView(self.container, error.message, () => { self.refresh(); });
    return self;
  });
};
...
PanelHealthDashboard.prototype._loadData = function() {
  ...
  if (this.circuitBreaker.isOpen()) {
    ...
    renderErrorView(this.container, getErrorMessage('CIRCUIT_BREAKER_OPEN'), () => { self.refresh(); });
...
PanelHealthDashboard.prototype.toggleAutoRefresh = function(enabled: boolean) {
...
function getInstance(): Record<string, any> {
  if (!_instance) _instance = new (PanelHealthDashboard as unknown as new () => Record<string, any>)();
  return _instance;
}

const PanelHealthDashboardAPI = {
  mount(container: HTMLElement | string, deps: Record<string, unknown>) { return getInstance().mount(container, deps); },
  unmount() { return getInstance().unmount(); },
  refresh() { return getInstance().refresh(); },
  healthCheck() { return getInstance().healthCheck(); },
  info() { return getInstance().info(); },
  getVersion() { return VERSION; },
  toggleAutoRefresh(enabled: boolean) { return getInstance().toggleAutoRefresh(enabled); },
  injectPorts,
  getPorts,
  MODULE_ID,
  VERSION
...
if (typeof window !== 'undefined' && !isStrict()) (window as any).PanelHealthDashboard = PanelHealthDashboardAPI;

export const mount = (container: HTMLElement | string, deps: Record<string, unknown>) => PanelHealthDashboardAPI.mount(container, deps);
export const unmount = () => PanelHealthDashboardAPI.unmount();
export const destroy = () => unmount();
export const healthCheck = () => PanelHealthDashboardAPI.healthCheck();
export { PanelHealthDashboardAPI as PanelHealthDashboard, injectPorts, getPorts };
export default { ...PanelHealthDashboardAPI, healthCheck };
