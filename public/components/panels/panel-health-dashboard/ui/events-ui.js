...
const MODULE_ID = "panel-health-dashboard:ui:events-ui";
...
const bindUIEvents = (panel) => {
  ...
  moduleViewBtns.forEach((el) => {
    el.addEventListener("click", () => {
      showModuleDetails(el.dataset.module);
    }, { signal });
    _listenerCount++;
  });
};
const unbindUIEvents = () => {
  if (_abortController) {
...
const updateCountdownDisplay = (panel) => {
...
const renderAuthBlockedView = (container) => {
  if (!container) return;
...
const renderErrorView = (container, message, onRetry) => {
  if (!container) return;
  ...
  if (retryBtn && onRetry) {
    if (signal) {
      retryBtn.addEventListener("click", onRetry, { signal });
      _listenerCount++;
    } else {
...
const info = () => ({ moduleId: MODULE_ID, version: VERSION, listenersBound: _listenerCount, hasAbortController: _abortController !== null });
var events_ui_default = { bindUIEvents, unbindUIEvents, updateCountdownDisplay, renderAuthBlockedView, renderErrorView };
export {
  MODULE_ID,
  VERSION,
  bindUIEvents,
  events_ui_default as default,
  info,
