...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-health-dashboard-template
// PURPOSE: Panel Health Dashboard - Template
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
export const MODULE_ID = 'panel-health-dashboard-template';
...
function getStatusIcon(status: string) {
  switch (status) {
    case 'HEALTHY': return SVGS.check;
    case 'DEGRADED': return SVGS.warning;
    case 'UNHEALTHY': return SVGS.x;
    case 'UNAVAILABLE': return SVGS.circle;
    default: return '?';
  }
}

function getStatusClass(status: string) { return `phd-status-${(status || 'unknown').toLowerCase()}`; }
function formatTime(timestamp: number | null) { if (!timestamp) return '-'; return new Date(timestamp).toLocaleTimeString(); }

function renderSummary(snapshot: Record<string, any> | null) {
  if (!snapshot) return '';
...
function renderModule(name: string, data: Record<string, any>) {
...
function renderCategory(name: string, label: string, modules: Record<string, any>) {
  const entries = [];
  for (const k in modules) { if (modules.hasOwnProperty(k)) entries.push([k, modules[k]]); }
  let healthyCount = 0;
  for (let i = 0; i < entries.length; i++) { if (entries[i][1].status === 'HEALTHY') healthyCount++; }
  let modulesHtml = '';
  for (let j = 0; j < entries.length; j++) { modulesHtml += renderModule(entries[j][0], entries[j][1]); }
...
export function createTemplate(state: Record<string, any>) {
  ...
  const categoryLabels: Record<string, string> = { core: 'Core Infrastructure', components: 'Components', panels: 'Panels' };
  let categoriesHtml = '';
  if (snapshot) {
