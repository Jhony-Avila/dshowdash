...
const MODULE_ID = "panels/panel-integration-adwords";
...
const CONFIG = { id: "integration-adwords", area: "integration", label: "AdWords", emoji: "\u{1F4E2}", apiEndpoint: "/api/status/adwords.php", refreshInterval: 3e5 };
class IntegrationAdWordsComponent {
  constructor(options = {}) {
    _initPorts();
    this.container = options.container || null;
    this.config = { ...CONFIG, ...options.config };
    this.eventBus = options.eventBus || _getPort("eventBus");
    this.store = new StateStore({ mounted: false, loading: false, error: null, data: null, clicksToday: 0, status: "unknown" });
  ...
  async _fetchData() {
    ...
    try {
      ...
      if (j.success && j.data) {
        this.store.setState({ loading: false, error: null, data: j.data, clicksToday: j.data.clicks_today || 0, status: j.data.status || "unknown" });
        this._updateDisplay();
      }
    } catch (e) {
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="${CONFIG.label}"><span class="integration-icon">${CONFIG.emoji}</span><span class="integration-label">${CONFIG.label}</span><span class="integration-value" data-metric="clicks">--</span><span class="integration-indicator" data-metric="status"></span></div>`;
  ...
  _updateDisplay() {
    ...
    if (v) v.textContent = s.clicksToday > 0 ? `${s.clicksToday} clicks` : "\u2713";
    if (i) {
...
var panel_integration_adwords_default = { IntegrationAdWordsComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  IntegrationAdWordsComponent,
  MODULE_ID,
  VERSION,
  panel_integration_adwords_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
