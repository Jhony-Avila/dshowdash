...
const MODULE_ID = "panels/panel-integration-alfinete";
...
const CONFIG = { id: "integration-alfinete", area: "integration", label: "Alfinete", emoji: "\u{1F4CC}", apiEndpoint: "/api/status/alfinete.php", refreshInterval: 12e4 };
class IntegrationAlfineteComponent {
  constructor(options = {}) {
    _initPorts();
    this.container = options.container || null;
    this.config = { ...CONFIG, ...options.config };
    this.eventBus = options.eventBus || _getPort("eventBus");
    this.store = new StateStore({ mounted: false, loading: false, error: null, data: null, trackedItems: 0, status: "unknown" });
  ...
  async _fetchData() {
    ...
    try {
      ...
      if (j.success && j.data) {
        this.store.setState({ loading: false, error: null, data: j.data, trackedItems: j.data.tracked_items || 0, status: j.data.status || "unknown" });
        this._updateDisplay();
      }
    } catch (e) {
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="${CONFIG.label}"><span class="integration-icon">${CONFIG.emoji}</span><span class="integration-label">${CONFIG.label}</span><span class="integration-value" data-metric="tracked">--</span><span class="integration-indicator" data-metric="status"></span></div>`;
  ...
  _updateDisplay() {
    ...
    if (v) v.textContent = s.trackedItems > 0 ? `${s.trackedItems} items` : "\u2713";
    if (i) {
...
var panel_integration_alfinete_default = { IntegrationAlfineteComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  IntegrationAlfineteComponent,
  MODULE_ID,
  VERSION,
  panel_integration_alfinete_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
