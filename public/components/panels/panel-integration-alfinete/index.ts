...
export class IntegrationAlfineteComponent {
  ...
  async _fetchData() { if (!this._canRefresh()) return; this.store.setState({ loading: true }); this._metrics.fetchCount++; this._metrics.lastFetchAt = Date.now(); try { const r = await fetch(this.config.apiEndpoint, { signal: this._abortController?.signal, credentials: 'include' }); if (!r.ok) throw new Error(`HTTP ${r.status}`); const j = await r.json(); if (j.success && j.data) { this.store.setState({ loading: false, error: null, data: j.data, trackedItems: j.data.tracked_items || 0, status: j.data.status || 'unknown' }); this._updateDisplay(); } } catch (e: any) { if (e.name !== 'AbortError') { this._metrics.errorCount++; this.store.setState({ loading: false, error: e.message }); } } }
...
export const mount = (container: any, config: any) => { const i = new IntegrationAlfineteComponent({ container: any, config }); i.init(); i.mount(container); _currentInstance = i; return { success: true, moduleId: MODULE_ID, instance: i }; };
