...
const MODULE_ID = "panels/panel-integration-alfinete/telemetry/tracker";
...
class Tracker {
  constructor(options = {}) {
    _initPorts();
    this.moduleId = options.moduleId || "panels/panel-integration-alfinete";
  ...
  healthCheck() {
    ...
    return { status: "healthy", enabled: this.enabled, eventCount: this._events.length, version: VERSION, moduleId: MODULE_ID, portsInitialized: ps._initialized, p18IntentsAvailable: true };
  }
  info() {
    const ps = Ports.snapshot();
    return { version: VERSION, moduleId: MODULE_ID, enabled: this.enabled, eventCount: this._events.length, portsInitialized: ps._initialized, usingP18Intents: true, healthCheck: this.healthCheck() };
  }
}
