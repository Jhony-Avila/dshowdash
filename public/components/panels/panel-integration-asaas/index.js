...
const MODULE_ID = "panels/panel-integration-asaas";
...
const CONFIG = { id: "integration-asaas", area: "integration", label: "Asaas", emoji: "\u{1F4B3}", apiEndpoint: "/api/status/asaas.php", refreshInterval: 12e4 };
class IntegrationAsaasComponent {
  constructor(options = {}) {
    _initPorts();
    this.container = options.container || null;
    this.config = { ...CONFIG, ...options.config };
    this.eventBus = options.eventBus || _getPort("eventBus");
    this.store = new StateStore({ mounted: false, loading: false, error: null, data: null, pendingPayments: 0, status: "unknown" });
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="${CONFIG.label}"><span class="integration-icon">${CONFIG.emoji}</span><span class="integration-label">${CONFIG.label}</span><span class="integration-value" data-metric="pending">--</span><span class="integration-indicator" data-metric="status"></span></div>`;
  ...
  _updateDisplay() {
    ...
    if (v) v.textContent = s.pendingPayments > 0 ? `${s.pendingPayments} pend.` : "\u2713";
    if (i) {
...
var panel_integration_asaas_default = { IntegrationAsaasComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  IntegrationAsaasComponent,
  MODULE_ID,
  VERSION,
  panel_integration_asaas_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
