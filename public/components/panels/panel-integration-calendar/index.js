...
const MODULE_ID = "panels/panel-integration-calendar";
...
const CONFIG = { id: "integration-calendar", area: "integration", label: "Calendar", icon: "calendar", emoji: "\u{1F4C5}", kind: "panel-component", apiEndpoint: "/api/status/calendar.php", refreshInterval: 12e4 };
class IntegrationCalendarComponent {
  constructor(options = {}) {
    _initPorts();
  ...
  attachEvents() {
    this._unsubscribe = this.store.subscribe((state, prev) => {
      if (state.eventsToday !== prev.eventsToday || state.status !== prev.status) this._updateDisplay();
    });
  }
  detachEvents() {
...
const mount = (container, config) => {
  const instance = new IntegrationCalendarComponent({ container, config });
  instance.init();
...
var panel_integration_calendar_default = { IntegrationCalendarComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  IntegrationCalendarComponent,
  MODULE_ID,
  VERSION,
  panel_integration_calendar_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
