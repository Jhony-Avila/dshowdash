...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-integration-calendar
// PURPOSE: Integration Calendar - Enterprise Component with Live Data
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panels/panel-integration-calendar';
...
const CONFIG = { id: 'integration-calendar', area: 'integration', label: 'Calendar', icon: 'calendar', emoji: '📅', kind: 'panel-component', apiEndpoint: '/api/status/calendar.php', refreshInterval: 120000 };

export class IntegrationCalendarComponent {
  [key: string]: any;
  constructor(options: Record<string, unknown> = {}) {
    _initPorts();
    this.container = options.container || null;
    this.config = { ...CONFIG, ...(options.config as Record<string, unknown>) };
    this.eventBus = options.eventBus || _getPort('eventBus');
    this.store = new StateStore({ mounted: false, loading: false, error: null, data: null, eventsToday: 0, status: 'unknown' });
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="Calendar Integration"><span class="integration-icon">${CONFIG.emoji}</span><span class="integration-label">${CONFIG.label}</span><span class="integration-value" data-metric="events">--</span><span class="integration-indicator" data-metric="status"></span></div>`;
  ...
  _updateDisplay() {
    ...
    const valueEl = this._element.querySelector('.integration-value');
    const indicatorEl = this._element.querySelector('.integration-indicator');
    if (valueEl) valueEl.textContent = state.eventsToday > 0 ? `${state.eventsToday} eventos` : '✓';
    if (indicatorEl) indicatorEl.className = `integration-indicator ${state.status === 'online' ? 'status-ok' : 'status-error'}`;
  }

  attachEvents() { this._unsubscribe = this.store.subscribe((state: Record<string, unknown>, prev: Record<string, unknown>) => { if (state.eventsToday !== prev.eventsToday || state.status !== prev.status) this._updateDisplay(); }); }
...
let _currentInstance: IntegrationCalendarComponent | null = null;

// @ts-expect-error TS migration - TS2554
export const mount = (container, config) => { const instance = new IntegrationCalendarComponent({ container, config }); instance.init(); instance.mount(container); _currentInstance = instance; return { success: true, moduleId: MODULE_ID, instance }; };
...
export default { IntegrationCalendarComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
