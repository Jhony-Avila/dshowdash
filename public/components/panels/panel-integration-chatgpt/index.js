...
const MODULE_ID = "panels/panel-integration-chatgpt";
...
const CONFIG = { id: "integration-chatgpt", area: "integration", label: "ChatGPT", emoji: "\u{1F916}", apiEndpoint: "/api/status/chatgpt.php", refreshInterval: 6e4 };
class IntegrationChatGPTComponent {
  constructor(options = {}) {
    _initPorts();
    this.container = options.container || null;
    this.config = { ...CONFIG, ...options.config };
    this.eventBus = options.eventBus || _getPort("eventBus");
    this.store = new StateStore({ mounted: false, loading: false, error: null, data: null, queriesToday: 0, status: "unknown" });
  ...
  async _fetchData() {
    ...
    try {
      ...
      if (j.success && j.data) {
        this.store.setState({ loading: false, error: null, data: j.data, queriesToday: j.data.queries_today || 0, status: j.data.status || "unknown" });
        this._updateDisplay();
      }
    } catch (e) {
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="${CONFIG.label}"><span class="integration-icon">${CONFIG.emoji}</span><span class="integration-label">${CONFIG.label}</span><span class="integration-value" data-metric="queries">--</span><span class="integration-indicator" data-metric="status"></span></div>`;
  ...
  _updateDisplay() {
    ...
    if (v) v.textContent = s.queriesToday > 0 ? `${s.queriesToday} queries` : "\u2713";
    if (i) {
...
var panel_integration_chatgpt_default = { IntegrationChatGPTComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  IntegrationChatGPTComponent,
  MODULE_ID,
  VERSION,
  panel_integration_chatgpt_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
