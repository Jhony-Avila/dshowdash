...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-integration-chatgpt/telemetry/logger
// PURPOSE: Integration ChatGPT - Logger
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panels/panel-integration-chatgpt/telemetry/logger';
...
export class Logger {
  [key: string]: any;
  constructor(options: { prefix?: string; level?: string; enabled?: boolean } = {}) { this.prefix = options.prefix || '[integration-chatgpt]'; this.level = options.level || 'info'; this.enabled = options.enabled !== false; this.levels = { debug: 0, info: 1, warn: 2, error: 3 }; this._metrics = { debugCount: 0, infoCount: 0, warnCount: 0, errorCount: 0, lastLogAt: null }; _initPorts(); }
