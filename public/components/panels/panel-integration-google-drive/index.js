...
const MODULE_ID = "panels/panel-integration-google-drive";
...
const CONFIG = { id: "integration-google-drive", area: "integration", label: "Google Drive", emoji: "\u{1F4C1}", apiEndpoint: "/api/status/googledrive.php", refreshInterval: 3e5 };
class IntegrationGoogleDriveComponent {
  constructor(options = {}) {
    _initPorts();
    this.container = options.container || null;
    this.config = { ...CONFIG, ...options.config };
    this.eventBus = options.eventBus || _getPort("eventBus");
    this.store = new StateStore({ mounted: false, loading: false, error: null, data: null, storageUsed: "--", status: "unknown" });
  ...
  async _fetchData() {
    ...
    try {
      ...
      if ((j.ok ?? j.success) && j.data) {
        this.store.setState({ loading: false, error: null, data: j.data, storageUsed: j.data.storage_used || "--", status: j.data.status || "unknown" });
        this._updateDisplay();
      }
    } catch (e) {
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="${CONFIG.label}"><span class="integration-icon">${CONFIG.emoji}</span><span class="integration-label">${CONFIG.label}</span><span class="integration-value" data-metric="storage">--</span><span class="integration-indicator" data-metric="status"></span></div>`;
  ...
  _updateDisplay() {
    ...
    if (v) v.textContent = s.storageUsed;
    if (i) {
...
var panel_integration_google_drive_default = { IntegrationGoogleDriveComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  IntegrationGoogleDriveComponent,
  MODULE_ID,
  VERSION,
  panel_integration_google_drive_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
