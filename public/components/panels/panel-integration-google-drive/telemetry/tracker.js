...
class Tracker {
  ...
  track(event, data = {}) {
    ...
    Ports.get("eventBus")?.emit?.(TELEMETRY_INTENTS.TRACK, entry);
  }
  getEvents() {
    return [...this._events];
  }
  ...
  healthCheck() {
    return { status: "healthy", enabled: this.enabled, eventCount: this._events.length, version: VERSION, moduleId: MODULE_ID, portsInitialized: Ports.isInitialized(), p18IntentsAvailable: true };
  }
  info() {
