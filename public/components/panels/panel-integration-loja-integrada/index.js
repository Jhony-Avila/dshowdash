...
const MODULE_ID = "panels/panel-integration-loja-integrada";
...
const CONFIG = { id: "integration-loja-integrada", area: "integration", label: "Loja Integrada", emoji: "\u{1F3EA}", apiEndpoint: "/api/status/lojaintegrada.php", refreshInterval: 12e4 };
class IntegrationLojaIntegradaComponent {
  constructor(options = {}) {
    _initPorts();
    this.container = options.container || null;
    this.config = { ...CONFIG, ...options.config };
    this.eventBus = options.eventBus || _getPort("eventBus");
    this.store = new StateStore({ mounted: false, loading: false, error: null, data: null, ordersToday: 0, status: "unknown" });
  ...
  mount(container) {
    ...
    if (!_isAuthenticated()) {
      this.container.innerHTML = '<div style="padding:1rem;text-align:center;color:#F59E0B;font-size:12px;">Login</div>';
  ...
  async _fetchData() {
    ...
    try {
      ...
      if ((j.ok ?? j.success) && j.data) {
        this.store.setState({ loading: false, error: null, data: j.data, ordersToday: j.data.orders_today || 0, status: j.data.status || "unknown" });
        this._updateDisplay();
      }
    } catch (e) {
      if (e.name !== "AbortError") {
        this._metrics.errorCount++;
        this.store.setState({ loading: false, error: e.message });
      }
    }
  }
  _startRefresh() {
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="${CONFIG.label}"><span class="integration-icon">${CONFIG.emoji}</span><span class="integration-label">${CONFIG.label}</span><span class="integration-value" data-metric="orders">--</span><span class="integration-indicator" data-metric="status"></span></div>`;
  ...
  _updateDisplay() {
    ...
    const v = this._element.querySelector(".integration-value");
    const i = this._element.querySelector(".integration-indicator");
    if (v) v.textContent = s.ordersToday > 0 ? `${s.ordersToday} pedidos` : "\u2713";
    if (i) {
      i.className = `integration-indicator ${s.status === "online" ? "status-ok" : "status-error"}`;
    }
  }
  healthCheck() {
  ...
  info() {
    return { moduleId: MODULE_ID, version: VERSION, mounted: this._mounted, isAuthenticated: _isAuthenticated(), isDocumentVisible: _isDocumentVisible(), p22Compliant: true, metrics: this._metrics, state: this.store.getState() };
  }
  getState() {
...
const mount = (container, config) => {
  const i = new IntegrationLojaIntegradaComponent({ container, config });
  i.init();
  i.mount(container);
  _currentInstance = i;
...
var panel_integration_loja_integrada_default = { IntegrationLojaIntegradaComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  IntegrationLojaIntegradaComponent,
  MODULE_ID,
  VERSION,
  panel_integration_loja_integrada_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
