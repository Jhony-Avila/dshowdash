...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-integration-loja-integrada/state/validators
// PURPOSE: Integração  - State Validators
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
export const MODULE_ID = 'panels/panel-integration-loja-integrada/state/validators';

export function isValidState(state: unknown) { return state !== null && typeof state === 'object'; }
export function hasRequiredFields(state: Record<string, unknown>, fields: string[]) { return fields.every((f: string) => state.hasOwnProperty(f)); }
...
export function validate(state: Record<string, unknown>, rules: { required?: string[]; custom?: (state: unknown) => string | null } = {}) {
  const errors = [];
