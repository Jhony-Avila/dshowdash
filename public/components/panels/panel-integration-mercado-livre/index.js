...
const MODULE_ID = "panels/panel-integration-mercado-livre";
...
const CONFIG = { id: "integration-mercado-livre", area: "integration", label: "Mercado Livre", emoji: "\u{1F6D2}", apiEndpoint: "/api/status/mercadolivre.php", refreshInterval: 12e4 };
class IntegrationMercadoLivreComponent {
  constructor(options = {}) {
    _initPorts();
    this.container = options.container || null;
    this.config = { ...CONFIG, ...options.config };
    this.eventBus = options.eventBus || _getPort("eventBus");
    this.store = new StateStore({ mounted: false, loading: false, error: null, data: null, ordersPending: 0, status: "unknown" });
  ...
  async _fetchData() {
    ...
    try {
      ...
      if (j.success && j.data) {
        this.store.setState({ loading: false, error: null, data: j.data, ordersPending: j.data.orders_pending || 0, status: j.data.status || "unknown" });
        this._updateDisplay();
      }
    } catch (e) {
  ...
  _updateDisplay() {
    ...
    if (v) v.textContent = s.ordersPending > 0 ? `${s.ordersPending} pend.` : "\u2713";
    if (i) {
...
const mount = (container, config) => {
  const i = new IntegrationMercadoLivreComponent({ container, config });
  i.init();
  i.mount(container);
  _currentInstance = i;
...
var panel_integration_mercado_livre_default = { IntegrationMercadoLivreComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  IntegrationMercadoLivreComponent,
  MODULE_ID,
  VERSION,
  panel_integration_mercado_livre_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
