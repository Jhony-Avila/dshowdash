...
const MODULE_ID = "panels/panel-integration-pipedrive";
...
const CONFIG = { id: "integration-pipedrive", area: "integration", label: "Pipedrive", emoji: "\u{1F4CA}", apiEndpoint: "/api/status/pipedrive.php", refreshInterval: 12e4 };
class IntegrationPipedriveComponent {
  constructor(options = {}) {
    _initPorts();
    this.container = options.container || null;
    this.config = { ...CONFIG, ...options.config };
    this.eventBus = options.eventBus || _getPort("eventBus");
    this.store = new StateStore({ mounted: false, loading: false, error: null, data: null, dealsOpen: 0, status: "unknown" });
  ...
  async _fetchData() {
    ...
    try {
      ...
      if (j.success && j.data) {
        this.store.setState({ loading: false, error: null, data: j.data, dealsOpen: j.data.deals_open || 0, status: j.data.status || "unknown" });
        this._updateDisplay();
      }
    } catch (e) {
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="${CONFIG.label}"><span class="integration-icon">${CONFIG.emoji}</span><span class="integration-label">${CONFIG.label}</span><span class="integration-value" data-metric="deals">--</span><span class="integration-indicator" data-metric="status"></span></div>`;
  ...
  _updateDisplay() {
    ...
    if (v) v.textContent = s.dealsOpen > 0 ? `${s.dealsOpen} deals` : "\u2713";
    if (i) {
...
const mount = (container, config) => {
  const i = new IntegrationPipedriveComponent({ container, config });
  i.init();
  i.mount(container);
  _currentInstance = i;
...
var panel_integration_pipedrive_default = { IntegrationPipedriveComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  IntegrationPipedriveComponent,
  MODULE_ID,
  VERSION,
  panel_integration_pipedrive_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
