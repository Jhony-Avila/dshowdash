...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-location/api/health
// PURPOSE: Localização - API Health Check
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
export const MODULE_ID = 'panels/panel-location/api/health';

let _lastCheck: { timestamp: number; latency: number; status: string; error?: string } | null = null;
let _status = 'unknown';
...
export async function check(endpoint = '/api/health', { signal }: { signal?: AbortSignal } = {}) {
  try {
    ...
    _lastCheck = { timestamp: Date.now(), latency: -1, status: 'error', error: (error instanceof Error) ? error.message : String(error) };
    _status = 'error';
    return _lastCheck;
  }
}
