...
// PURPOSE: Panel Lotties Management - UI Renderer
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   AVAILABLE_LOTTIES, ASSIGNABLE_COMPONENTS from ../core/lifecycle.js
//
// PROVIDES:
...
interface PanelState {
  lotties: Array<{ id: string; file: string; name: string; description: string }>;
  components: Array<{ id: string; name: string; slot: string; current: string | null }>;
...
type LottieKey = keyof typeof AVAILABLE_LOTTIES;
...
export const ui = {
  ...
  _renderLottieCard(id: string, lottie: { file: string; name: string; description: string }, state: PanelState) {
  ...
  _renderComponentRow(comp: { id: string; name: string; slot: string; current: string | null }, state: PanelState) {
    const _lotties = AVAILABLE_LOTTIES as Record<string, { file: string; name: string; description: string }>;
