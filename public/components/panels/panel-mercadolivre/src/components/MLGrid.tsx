// components/MLGrid.tsx — data grid padrão do módulo (TanStack Table).
...
export function MLGrid<T>({ dados, colunas, carregando, vazio, onLinha, exportarNome }: {
  dados: T[];
  colunas: ColunaML<T>[];
  carregando?: boolean;
  ...
  return (
    <div className="ml-grid-bloco">
      <div className="ml-grid-barra">
        <div className="ml-grid-busca">
          <Search size={13} aria-hidden />
          <input value={busca} placeholder="Buscar na tabela…"
            onChange={(e) => { setBusca(e.target.value); setLimite(PAGINA); }} />
        </div>
        <span className="ml-grid-total">{fmt(linhas.length)} registro{linhas.length === 1 ? '' : 's'}</span>
        <button className="ml-btn" onClick={exportarCsv} title="Exportar CSV">
                  ...
                  {hg.headers.map((h) => {
                    ...
                    return (
                      <th key={h.id} style={{ width: meta?.largura, textAlign: meta?.alinhar === 'direita' ? 'right' : meta?.alinhar === 'centro' ? 'center' : 'left' }}>
                        <button className="ml-grid-th" onClick={h.column.getToggleSortingHandler()}
                          ...
                          {dir === 'asc' ? <ArrowUp size={11} aria-hidden />
                            ...
                            : <ArrowUpDown size={11} className="ml-grid-sort" aria-hidden />}
                        </button>
                      </th>
                    );
                  })}
      ...
      {linhas.length > limite && (
        <button className="ml-btn ml-grid-mais" onClick={() => setLimite((l) => l + PAGINA)}>
