'use strict';
// panel-metaads/index.js — carregador do painel React (Meta Ads).
// @module  panels/panel-metaads
// @version 1.0.0
// @created 2026-07-27
//
...
// CSS injetado UMA vez, escopado a [data-mads-react-root] (tokens --mads-*).
...
// regras) através do proxy same-origin /api/metaads/ask.php — o token da API
// NUNCA chega ao navegador.

const MODULE_ID = 'panels/panel-metaads';
const VERSION   = '1.0.0';
const FLAG_KEY  = 'panel_metaads_enabled';
...
const CSS_ATTR = 'data-mads-react-css';
...
async function carregarReact() {
  ...
  const base = '/components/panels/panel-metaads/dist/';
  let entrada = null;
  try {
...
function renderErro(contentEl, err) {
  ...
  contentEl.innerHTML =
    ...
    + 'border:1px solid var(--mads-border,#2e2e44);border-radius:12px;'
    + 'background:var(--bg-card,#1a1a2e);color:var(--text-primary,#e8e8f0)">'
    + '<strong style="display:block;font-size:15px;margin-bottom:8px">Não foi possível carregar o Meta Ads</strong>'
...
export async function mount(contentEl, config = {}) {
  ...
  try {
    ...
    await mod.mountReact(contentEl, {
      ...config,
      flag: { key: FLAG_KEY, enabled: true, payload, source: 'metaads' },
    });
    _mounted = true;
