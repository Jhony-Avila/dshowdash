...
const VERSION = "15.0.0-FLAT-GROUP-COLLAPSE";
const MODULE_ID = "panel-nav-admin/handlers/click-router";
const ACTIONS = {
  REFRESH: "refresh",
...
function createClickRouter(deps) {
  ...
  return function routeClick(e) {
    const el = e.target;
    const target = el.closest("[data-action]");
    const tabTarget = el.closest("[data-tab]");
    const kpiTarget = el.closest("[data-kpi]");
    if (kpiTarget && !target) {
    ...
    const sectionKeyEl = el.closest("[data-section-key]");
    ...
    switch (action) {
      ...
      case ACTIONS.EDIT_SECTION: {
        ...
        if (_isDynamic) {
          const _toast = callbacks.showToast;
          if (_toast) _toast("Este grupo \xE9 gerado dinamicamente a partir dos itens. Para editar o grupo, edite os itens que pertencem a ele.", "info");
        } else {
      ...
      case ACTIONS.FLIP_CARD: {
        ...
        if (card) flipCard(card.dataset.sectionKey);
        break;
      }
      case ACTIONS.TOGGLE_COLLAPSE: {
        const collapseCard = target.closest("[data-section-key]");
        if (collapseCard) toggleCollapse(collapseCard.dataset.sectionKey);
        break;
      }
      ...
      case ACTIONS.SELECT_ITEM:
        selectRow(itemId, target.checked);
        break;
