// ═══════════════════════════════════════════════════════════════
// DEPENDENCY CONTRACT (11.0.0-DOCUMENT-DRAG)
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-nav-admin-handlers-drag-drop
// PURPOSE: Drag & Drop Handler - Single Document Listener
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   setDragging, setDropTarget from ../renderer/items.js
//
// PROVIDES:
//   createDragDropHandlers() — exported function
//   getDraggedItemId() — exported function
//   clearDragState() — exported function
//   info() — exported function
//   healthCheck() — exported function
//   MODULE_ID — module constant
//   VERSION — module constant
//
// LISTENS (eventos):
//   'mousedown' on DOCUMENT (single listener, checks .pna-drag-handle)
//   'mousemove' on document (during drag only)
//   'mouseup' on document (during drag only)
//
// @changelog
//   11.0.0-DOCUMENT-DRAG: MAJOR — Complete rewrite. Single mousedown listener
//     on document instead of container delegation. Eliminates 534 duplicate
//     handles problem. Idempotent init — guards against double-attach.
//     No container dependency. mousemove/mouseup only during active drag.
//   10.1.0-OPTIMISTIC-REORDER: Optimistic DOM reorder after API success.
//   10.0.0-MOUSE-DRAG: mousedown+mousemove+mouseup approach.
// ═══════════════════════════════════════════════════════════════
'use strict';

import { setDragging, setDropTarget } from '../renderer/items.js';

const MODULE_ID = 'panel-nav-admin-handlers-drag-drop';
const VERSION = '15.2.0-GROUP-REORDER';

let _draggedItemId: string | null = null;
let _documentListenerAttached = false;

export function createDragDropHandlers(deps: Record<string, unknown>) {
  const store = deps.store as { get: (k: string) => Record<string, unknown>[] };
  const navAdapter = deps.navAdapter as { reorderItems: (items: Record<string, unknown>[]) => Promise<void>; updateItem: (id: unknown, payload: Record<string, unknown>) => Promise<Record<string, unknown>> };
  const showToast = deps.showToast as (msg: string, type: string) => void;
  const loadData = deps.loadData as () => void;

  // ── Drag state ──
  let _isDragging = false;
  let _dragStartX = 0;
  let _dragStartY = 0;
  let _dragGhost: HTMLElement | null = null;
  let _sourceRow: HTMLElement | null = null;

  const DRAG_THRESHOLD = 5;

  // ═══════════════════════════════════════════════════════════
  // MELHORIA 3: Group separator drag — reorder groups
  // ═══════════════════════════════════════════════════════════
  let _groupDragging = false;
  let _groupDragSource: HTMLElement | null = null;
  let _groupDragGhost: HTMLElement | null = null;

  function _onGroupDragStart(e: MouseEvent) {
    var groupHandle = (e.target as HTMLElement).closest('.pna-group-drag-handle');
    if (!groupHandle) return null;
    var separator = (e.target as HTMLElement).closest('[data-group-drag]') as HTMLElement | null;
    if (!separator) return null;
    return separator;
  }

  function _handleGroupDrop(source: HTMLElement, targetSep: HTMLElement) {
    var sourceKey = source.dataset.groupKey || source.dataset.groupSeparator || '';
    var targetKey = targetSep.dataset.groupKey || targetSep.dataset.groupSeparator || '';
    if (!sourceKey || !targetKey || sourceKey === targetKey) return;

    // console.log('[DragDrop PNA] group reorder:', { sourceGroup: sourceKey, targetGroup: targetKey }); // removed 2026-04-30 (debug temporário)

    // Collect all group separators in order
    var container = source.closest('ul.pna-list') || source.parentElement;
    if (!container) return;
    var allSeps = Array.from(container.querySelectorAll('[data-group-drag][data-group-key]')) as HTMLElement[];
    var groupKeys = allSeps.map(function(s) { return s.dataset.groupKey || s.dataset.groupSeparator || ''; });

    var fromIdx = groupKeys.indexOf(sourceKey);
    var toIdx = groupKeys.indexOf(targetKey);
    if (fromIdx === -1 || toIdx === -1) return;

    // Reorder
    groupKeys.splice(fromIdx, 1);
    groupKeys.splice(toIdx, 0, sourceKey);

    // Build PATCH payload for sidebar_groups order_index update
    var reorderPayload = groupKeys.map(function(key, idx) {
      return { group_key: key, order_index: idx + 1 };
    });

    // PATCH each group via API
    var promises = reorderPayload.map(function(item) {
      return fetch('/api/admin/navigation/sections', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': (document.querySelector('meta[name="csrf-token"]') as HTMLMetaElement)?.content || '' },
        body: JSON.stringify({
          source_table: 'ui_nav_items',
          source_id: 0, // Will be resolved by group_key
          group_key: item.group_key,
          order_index: item.order_index
        })
      });
    });

    // Use reorder endpoint instead if available
    fetch('/api/admin/navigation/reorder', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': (document.querySelector('meta[name="csrf-token"]') as HTMLMetaElement)?.content || '' },
      body: JSON.stringify({
        type: 'groups',
        items: reorderPayload
      })
    }).then(function() {
      showToast('Ordem dos grupos atualizada', 'success');
      window.dispatchEvent(new CustomEvent('navigation:items:changed', {
        detail: { source: 'panel-nav-admin', action: 'group-reorder' }
      }));
      loadData();
    }).catch(function(err: Error) {
      showToast('Erro ao reordenar grupos: ' + err.message, 'error');
      loadData();
    });
  }

  // ═══════════════════════════════════════════════════════════
  // SINGLE document mousedown — the only entry point for drag
  // ═══════════════════════════════════════════════════════════
  function _onDocumentMouseDown(e: MouseEvent) {
    if (e.button !== 0) return;

    // MELHORIA 3: Check if it's a group drag handle first
    var groupSep = _onGroupDragStart(e);
    if (groupSep) {
      _groupDragSource = groupSep;
      _dragStartX = e.clientX;
      _dragStartY = e.clientY;
      _groupDragging = false;
      document.addEventListener('mousemove', _onGroupMouseMove);
      document.addEventListener('mouseup', _onGroupMouseUp);
      e.preventDefault();
      return;
    }

    var handle = (e.target as HTMLElement).closest('.pna-drag-handle');
    if (!handle) return;

    var row = (e.target as HTMLElement).closest('.pna-list-item[data-item-id]') as HTMLElement | null;
    if (!row) return;

    _draggedItemId = row.dataset.itemId || null;
    _sourceRow = row;
    _dragStartX = e.clientX;
    _dragStartY = e.clientY;
    _isDragging = false;

    document.addEventListener('mousemove', _onDocumentMouseMove);
    document.addEventListener('mouseup', _onDocumentMouseUp);

    e.preventDefault();
  }

  // ═══════════════════════════════════════════════════════════
  // mousemove — create ghost, highlight drop target
  // ═══════════════════════════════════════════════════════════
  function _onDocumentMouseMove(e: MouseEvent) {
    if (!_draggedItemId || !_sourceRow) return;

    var dx = e.clientX - _dragStartX;
    var dy = e.clientY - _dragStartY;
    var dist = Math.sqrt(dx * dx + dy * dy);

    if (!_isDragging && dist < DRAG_THRESHOLD) return;

    if (!_isDragging) {
      _isDragging = true;
      setDragging(_draggedItemId!, true);
      _createGhost(_sourceRow, e);
    }

    if (_dragGhost) {
      _dragGhost.style.left = (e.clientX + 12) + 'px';
      _dragGhost.style.top = (e.clientY - 14) + 'px';
    }

    // Hide ghost temporarily to find element under cursor
    if (_dragGhost) _dragGhost.style.pointerEvents = 'none';
    var elementUnder = document.elementFromPoint(e.clientX, e.clientY) as HTMLElement | null;
    if (_dragGhost) _dragGhost.style.pointerEvents = '';

    var targetRow = elementUnder ? elementUnder.closest('.pna-list-item[data-item-id]') as HTMLElement | null : null;
    if (targetRow && targetRow.dataset.itemId !== _draggedItemId) {
      var rect = targetRow.getBoundingClientRect();
      var midY = rect.top + rect.height / 2;
      var isAbove = e.clientY < midY;
      setDropTarget(targetRow.dataset.itemId || null, isAbove);
    } else {
      setDropTarget(null, null);
    }
  }

  // ═══════════════════════════════════════════════════════════
  // mouseup — reorder + API save + cleanup
  // ═══════════════════════════════════════════════════════════
  function _onDocumentMouseUp(e: MouseEvent) {
    // Always cleanup move/up listeners
    document.removeEventListener('mousemove', _onDocumentMouseMove);
    document.removeEventListener('mouseup', _onDocumentMouseUp);

    _removeGhost();

    if (_draggedItemId) setDragging(_draggedItemId, false);
    setDropTarget(null, null);

    if (_isDragging && _draggedItemId) {
      // Use elementFromPoint for accurate target detection
      var elementUnder = document.elementFromPoint(e.clientX, e.clientY) as HTMLElement | null;
      var targetRow = elementUnder ? elementUnder.closest('.pna-list-item[data-item-id]') as HTMLElement | null : null;

      if (targetRow && targetRow.dataset.itemId && targetRow.dataset.itemId !== _draggedItemId) {
        var targetId = targetRow.dataset.itemId;
        var items = (store.get('items') || []) as Record<string, unknown>[];
        var draggedIndex = items.findIndex(function(i: Record<string, unknown>) { return i.id === _draggedItemId; });
        var targetIndex = items.findIndex(function(i: Record<string, unknown>) { return i.id === targetId; });

        if (draggedIndex !== -1 && targetIndex !== -1) {
          var rect = targetRow.getBoundingClientRect();
          var midY = rect.top + rect.height / 2;
          var insertBefore = e.clientY < midY;

          // v15.0.0: Cross-group detection — compare group of dragged vs target
          var sourceGroupKey = _sourceRow ? (_sourceRow.dataset.groupKey || '') : '';
          var targetGroupKey = targetRow.dataset.groupKey || '';
          var isCrossGroup = sourceGroupKey !== targetGroupKey && targetGroupKey !== '';

          // [DEBUG] MELHORIA 2 — console.log temporário (removed 2026-04-30)
          // console.log('[DragDrop PNA] drop detected:', {
          //   itemId: _draggedItemId,
          //   sourceGroup: sourceGroupKey,
          //   targetGroup: targetGroupKey,
          //   isCrossGroup: isCrossGroup,
          //   insertBefore: insertBefore
          // });

          var newItems = items.slice();
          var removed = newItems.splice(draggedIndex, 1)[0];
          var newIndex = insertBefore ? targetIndex : targetIndex + 1;
          newItems.splice(newIndex > draggedIndex ? newIndex - 1 : newIndex, 0, removed);

          // Enriquecer cada item com sourceTable/sourceId do DOM se ausente no store
          var allRows = document.querySelectorAll('.pna-list-item[data-source-table]');
          var domMap: Record<string, { sourceTable: string; sourceId: string }> = {};
          allRows.forEach(function(r) {
            var el = r as HTMLElement;
            if (el.dataset.itemId) {
              domMap[el.dataset.itemId] = {
                sourceTable: el.dataset.sourceTable || 'ui_nav_items',
                sourceId: el.dataset.sourceId || el.dataset.itemId
              };
            }
          });

          newItems.forEach(function(item: Record<string, unknown>, index: number) {
            item.order = index + 1;
            var id = String(item.id || '');
            if (!item.sourceTable && domMap[id]) {
              item.sourceTable = domMap[id].sourceTable;
              item.sourceId = domMap[id].sourceId;
            }
          });

          // v15.0.0: If cross-group drop, PATCH parent_key first, then reorder
          var reorderPromise: Promise<void>;
          if (isCrossGroup) {
            var draggedItem = removed;
            var patchPayload = {
              sourceTable: draggedItem.sourceTable || (domMap[String(draggedItem.id)] ? domMap[String(draggedItem.id)].sourceTable : 'ui_nav_items'),
              sourceId: draggedItem.sourceId || (domMap[String(draggedItem.id)] ? domMap[String(draggedItem.id)].sourceId : draggedItem.id),
              parentKey: targetGroupKey
            };
            // Update parentKey in local array so reorder sends correct data
            draggedItem.parentKey = targetGroupKey;
            draggedItem.parent_key = targetGroupKey;
            reorderPromise = navAdapter.updateItem(draggedItem.id, patchPayload).then(function(result: Record<string, unknown>) {
              if (!(result.ok || result.success)) {
                throw new Error((result.error as string) || 'Erro ao mover para outro grupo');
              }
              return navAdapter.reorderItems(newItems);
            }) as unknown as Promise<void>;
          } else {
            reorderPromise = navAdapter.reorderItems(newItems);
          }

          reorderPromise.then(function() {
            // Optimistic DOM update — immediately reorder rows and update ORDEM numbers
            var ul = document.querySelector('ul.pna-list[data-sortable="items"]');
            if (ul) {
              for (var ri = 0; ri < newItems.length; ri++) {
                var itemId = String(newItems[ri].id || '');
                var row = ul.querySelector('.pna-list-item[data-item-id="' + itemId + '"]') as HTMLElement | null;
                if (row) {
                  ul.appendChild(row);
                  var orderNum = row.querySelector('.pna-order-num');
                  if (orderNum) orderNum.textContent = String(ri + 1);
                }
              }
            }

            showToast(isCrossGroup ? 'Item movido para outro grupo' : 'Ordem atualizada', 'success');
            window.dispatchEvent(new CustomEvent('navigation:items:changed', {
              detail: { source: 'panel-nav-admin', action: isCrossGroup ? 'cross-group-move' : 'reorder' }
            }));
            loadData();
          }).catch(function(error: Error) {
            showToast('Erro ao reordenar: ' + error.message, 'error');
            loadData();
          });
        }
      }
    }

    _isDragging = false;
    _draggedItemId = null;
    _sourceRow = null;
  }

  // ═══════════════════════════════════════════════════════════
  // Ghost helpers
  // ═══════════════════════════════════════════════════════════
  function _createGhost(row: HTMLElement, e: MouseEvent) {
    _removeGhost();
    _dragGhost = document.createElement('div');
    _dragGhost.className = 'pna-drag-ghost';
    var label = row.querySelector('.pna-item-label');
    var labelText = label ? label.textContent || '' : (row.dataset.itemId || '');
    _dragGhost.innerHTML = '<div class="pna-drag-ghost-content">' +
      '<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><circle cx="9" cy="6" r="1.5"/><circle cx="15" cy="6" r="1.5"/><circle cx="9" cy="12" r="1.5"/><circle cx="15" cy="12" r="1.5"/><circle cx="9" cy="18" r="1.5"/><circle cx="15" cy="18" r="1.5"/></svg>' +
      '<span>' + labelText + '</span></div>';
    _dragGhost.style.left = (e.clientX + 12) + 'px';
    _dragGhost.style.top = (e.clientY - 14) + 'px';
    document.body.appendChild(_dragGhost);
  }

  function _removeGhost() {
    if (_dragGhost && _dragGhost.parentNode) {
      _dragGhost.parentNode.removeChild(_dragGhost);
    }
    _dragGhost = null;
  }

  // ═══════════════════════════════════════════════════════════
  // MELHORIA 3: Group drag mousemove/mouseup
  // ═══════════════════════════════════════════════════════════
  function _onGroupMouseMove(e: MouseEvent) {
    if (!_groupDragSource) return;
    var dx = e.clientX - _dragStartX;
    var dy = e.clientY - _dragStartY;
    if (!_groupDragging && Math.sqrt(dx * dx + dy * dy) < DRAG_THRESHOLD) return;

    if (!_groupDragging) {
      _groupDragging = true;
      _groupDragSource.style.opacity = '0.5';
      _groupDragGhost = document.createElement('div');
      _groupDragGhost.className = 'pna-drag-ghost';
      var gLabel = _groupDragSource.querySelector('span[style*="text-transform"]');
      _groupDragGhost.innerHTML = '<div class="pna-drag-ghost-content"><span>' + (gLabel ? gLabel.textContent : 'Grupo') + '</span></div>';
      _groupDragGhost.style.left = (e.clientX + 12) + 'px';
      _groupDragGhost.style.top = (e.clientY - 14) + 'px';
      document.body.appendChild(_groupDragGhost);
    }

    if (_groupDragGhost) {
      _groupDragGhost.style.left = (e.clientX + 12) + 'px';
      _groupDragGhost.style.top = (e.clientY - 14) + 'px';
    }

    // Highlight target group separator
    if (_groupDragGhost) _groupDragGhost.style.pointerEvents = 'none';
    var under = document.elementFromPoint(e.clientX, e.clientY) as HTMLElement | null;
    if (_groupDragGhost) _groupDragGhost.style.pointerEvents = '';
    var targetSep = under ? under.closest('[data-group-drag]') as HTMLElement | null : null;

    // Remove previous highlights
    document.querySelectorAll('.pna-group-drop-target').forEach(function(el) { el.classList.remove('pna-group-drop-target'); });
    if (targetSep && targetSep !== _groupDragSource) {
      targetSep.classList.add('pna-group-drop-target');
      targetSep.style.background = 'rgba(99,102,241,0.15)';
    }
  }

  function _onGroupMouseUp(e: MouseEvent) {
    document.removeEventListener('mousemove', _onGroupMouseMove);
    document.removeEventListener('mouseup', _onGroupMouseUp);

    if (_groupDragGhost && _groupDragGhost.parentNode) _groupDragGhost.parentNode.removeChild(_groupDragGhost);
    _groupDragGhost = null;

    document.querySelectorAll('.pna-group-drop-target').forEach(function(el) {
      el.classList.remove('pna-group-drop-target');
      (el as HTMLElement).style.background = '';
    });

    if (_groupDragSource) _groupDragSource.style.opacity = '';

    if (_groupDragging && _groupDragSource) {
      if (_groupDragGhost) _groupDragGhost.style.pointerEvents = 'none';
      var under = document.elementFromPoint(e.clientX, e.clientY) as HTMLElement | null;
      var targetSep = under ? under.closest('[data-group-drag]') as HTMLElement | null : null;
      if (targetSep && targetSep !== _groupDragSource) {
        _handleGroupDrop(_groupDragSource, targetSep);
      }
    }

    _groupDragging = false;
    _groupDragSource = null;
  }

  // ═══════════════════════════════════════════════════════════
  // init / destroy — idempotent attach/detach
  // ═══════════════════════════════════════════════════════════
  function init() {
    if (_documentListenerAttached) return;
    document.addEventListener('mousedown', _onDocumentMouseDown);
    _documentListenerAttached = true;
  }

  function destroy() {
    if (!_documentListenerAttached) return;
    document.removeEventListener('mousedown', _onDocumentMouseDown);
    document.removeEventListener('mousemove', _onDocumentMouseMove);
    document.removeEventListener('mouseup', _onDocumentMouseUp);
    _removeGhost();
    _documentListenerAttached = false;
    _isDragging = false;
    _draggedItemId = null;
    _sourceRow = null;
  }

  // Backward-compatible: return handleMouseDown/handleMouseUp as no-ops
  // so event-setup doesn't break if it still references them
  function handleMouseDown(_e: MouseEvent) { /* no-op — document listener handles it */ }
  function handleMouseUp(_e: MouseEvent) { /* no-op — document listener handles it */ }

  // Auto-init on creation
  init();

  return { handleMouseDown, handleMouseUp, init, destroy };
}

export function getDraggedItemId() { return _draggedItemId; }

export function clearDragState() {
  if (_draggedItemId) setDragging(_draggedItemId, false);
  setDropTarget(null, null);
  _draggedItemId = null;
}

export { MODULE_ID, VERSION };
export function info() { return { moduleId: MODULE_ID, version: VERSION }; }
export function healthCheck() { return { status: 'HEALTHY', moduleId: MODULE_ID, version: VERSION, checks: { dragDropReady: _documentListenerAttached } }; }
