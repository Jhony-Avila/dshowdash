...
const start = (options: Record<string, unknown> = {}) => { stop(); _initPorts(); _interval = (options.interval as number) || REFRESH_INTERVAL; _onTick = (options.onTick as ((countdown: number | null) => void)) || null; _onRefresh = (options.onRefresh as (() => Promise<unknown>)) || null; _paused = false; _refreshing = false; const seconds = Math.floor(_interval / 1000); _countdown = seconds; if (_onTick) _onTick(_countdown); _countdownId = setInterval(() => {
    /* aba oculta: nao conta nem busca */ if (typeof document !== 'undefined' && document.hidden) return; if (_paused || _refreshing) return; _countdown--; if (_onTick) _onTick(_countdown); if (_countdown <= 0) { _countdown = seconds; triggerRefresh(); } }, 1000); _log('info', 'Started: ' + seconds + 's interval'); };
