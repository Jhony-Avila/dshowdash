...
const MODULE_ID = "panel-nav-admin-icon-picker";
const VERSION = "10.1.0-INLINE-POPOVER";
let _container = null;
let _onSelect = null;
...
function _getPreviewSvg(iconName) {
  if (!iconName) return "";
...
function _render() {
  ...
  if (icons.length === 0) {
    iconsHtml = '<div class="pna-ip__empty">Nenhum \xEDcone encontrado</div>';
  }
  _container.innerHTML = `
    <div class="pna-ip ${_isOpen ? "pna-ip--open" : ""}">
      <div class="pna-ip__header">
        <h3 class="pna-ip__title">Selecionar \xCDcone</h3>
        <button class="pna-ip__close" data-action="close">&times;</button>
      </div>
      <div class="pna-ip__search">
        <input type="text" class="pna-ip__input" placeholder="Buscar \xEDcone..." value="${_escapeHtml(_searchQuery)}" data-action="search">
      </div>
      <div class="pna-ip__categories">${categoriesHtml}</div>
      <div class="pna-ip__grid">${iconsHtml}</div>
      <div class="pna-ip__footer">
        <span class="pna-ip__count">${icons.length} \xEDcone(s)</span>
...
function createPicker(targetEl, options = {}) {
  if (!targetEl) return null;
  const value = options.value;
  const onChange = options.onChange;
  const placeholder = options.placeholder || "Selecionar \xEDcone";
  ...
  const btn = wrapper.querySelector(".pna-ip-trigger__btn");
  const pickerContainer = wrapper.querySelector(".pna-ip-trigger__picker");
...
function renderIconPicker(selectedIcon, prefix) {
  prefix = prefix || "";
  ...
  return '<div class="pna-icon-picker-inline" data-icon-picker><div style="display:flex;align-items:center;gap:0.5rem;"><button type="button" class="pna-btn pna-btn--icon" data-action="open-icon-picker" title="Escolher \xEDcone" style="width:36px;height:36px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);border-radius:0.25rem;color:#fff;cursor:pointer;">' + iconSvg + '</button><input type="text" name="icon" value="' + safeIcon + '" placeholder="ex: ui:home, business:users" style="flex:1;padding:0.5rem;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);border-radius:0.25rem;color:#fff;" readonly></div><div class="pna-icon-picker-dropdown" data-icon-picker-dropdown style="display:none;position:absolute;z-index:1000;background:#1e1e2e;border:1px solid rgba(255,255,255,0.15);border-radius:0.5rem;padding:0;max-height:420px;overflow:hidden;width:380px;margin-top:0.25rem;"></div></div>';
}
function initIconPickerEvents(modalEl) {
  if (!modalEl) return;
  const pickerWrappers = modalEl.querySelectorAll("[data-icon-picker]");
  pickerWrappers.forEach((wrapper) => {
    const openBtn = wrapper.querySelector('[data-action="open-icon-picker"]');
    const dropdown = wrapper.querySelector("[data-icon-picker-dropdown]");
    const input = wrapper.querySelector('input[name="icon"]');
...
function _renderDropdown(dropdown, input, openBtn, activeNs, searchQuery) {
  const namespaces = listNamespaces();
  const nsLabels = {
    ui: "UI",
    charts: "Charts",
  ...
  tabsHtml += "</div>";
  const searchHtml = '<div style="padding:0.5rem;border-bottom:1px solid rgba(255,255,255,0.1);flex-shrink:0;"><input type="text" data-icon-search placeholder="Buscar \xEDcone..." value="' + _escapeHtml(searchQuery) + '" style="width:100%;padding:0.4rem 0.5rem;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);border-radius:0.25rem;color:#fff;font-size:0.8rem;outline:none;"></div>';
  let icons = [];
  if (activeNs === "all") {
    icons = listRegistryIcons();
  } else {
  ...
  for (const fullKey of icons) {
    ...
    const shortLabel = _shortName(fullKey);
    gridHtml += '<button type="button" data-pick-icon="' + fullKey + '" title="' + fullKey + '" style="width:40px;height:40px;display:flex;flex-direction:column;align-items:center;justify-content:center;background:' + (isSelected ? "rgba(99,102,241,0.3)" : "transparent") + ";border:1px solid " + (isSelected ? "#6366f1" : "rgba(255,255,255,0.1)") + ';border-radius:0.25rem;color:#fff;cursor:pointer;padding:2px;gap:1px;"><span style="width:20px;height:20px;display:flex;align-items:center;justify-content:center;">' + svg + "</span></button>";
  }
  if (icons.length === 0) {
    gridHtml += '<div style="padding:1rem;color:rgba(255,255,255,0.4);font-size:0.8rem;">Nenhum \xEDcone encontrado</div>';
  }
  gridHtml += "</div>";
  gridHtml += '<div style="padding:0.25rem 0;color:rgba(255,255,255,0.3);font-size:0.7rem;text-align:right;">' + icons.length + " \xEDcone(s)</div>";
  gridHtml += "</div>";
  dropdown.innerHTML = tabsHtml + searchHtml + gridHtml;
  dropdown.querySelectorAll("[data-ns-tab]").forEach((tab) => {
    tab.addEventListener("click", () => {
      const ns = tab.dataset.nsTab || "all";
      _renderDropdown(dropdown, input, openBtn, ns, searchQuery);
    });
  });
  const searchEl = dropdown.querySelector("[data-icon-search]");
  if (searchEl) {
  ...
  dropdown.addEventListener("click", (e) => {
    const pickBtn = e.target.closest("[data-pick-icon]");
    if (!pickBtn) return;
...
function _nsTab(ns, label, activeNs) {
...
function openInlinePopover(anchorEl, options) {
  closeInlinePopover();
  ...
  function renderPopoverContent() {
    ...
    let icons = activeNs === "all" ? listRegistryIcons() : listRegistryIcons(activeNs);
    if (searchQuery) {
      const q = searchQuery.toLowerCase();
      icons = icons.filter((key) => _shortName(key).toLowerCase().includes(q) || key.toLowerCase().includes(q));
    }
    ...
    popover.querySelectorAll("[data-pop-ns]").forEach((tab) => {
      tab.addEventListener("click", () => {
        activeNs = tab.dataset.popNs || "all";
        renderPopoverContent();
      });
    });
    const searchInput = popover.querySelector("[data-pop-search]");
    if (searchInput) {
    ...
    popover.addEventListener("click", (e) => {
      const pickBtn = e.target.closest("[data-pop-pick]");
      if (!pickBtn) return;
  ...
  _popoverCloseHandler = (e) => {
    if (_popoverEl && !_popoverEl.contains(e.target) && !anchorEl.contains(e.target)) {
      closeInlinePopover();
    }
  };
...
export {
  ...
  closeInlinePopover,
  createPicker,
  icon_picker_default as default,
  destroy,
  getAllIcons,
  getCategories,
  getIconsByCategory,
