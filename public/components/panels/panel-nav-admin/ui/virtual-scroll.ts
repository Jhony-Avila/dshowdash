...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-nav-admin.ui.virtual-scroll
// PURPOSE: Virtual Scroll — Renderização virtualizada para listas grandes
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
//   VirtualScroll — exported class
//   createVirtualScroll() — factory
//   info(), healthCheck()
//
// @origin Adapted from panel-01/ui/table/render-virtual.js for nav-admin lists
// @changelog
//   10.1.0-MIGRATION-PHASE3: Initial creation — FASE 3 B.1
// ═══════════════════════════════════════════════════════════════
'use strict';
...
export const MODULE_ID = 'panel-nav-admin.ui.virtual-scroll';
...
/** Default threshold above which virtual scroll activates */
export const DEFAULT_THRESHOLD = 50;

/**
 * VirtualScroll — Renderiza apenas itens visíveis na viewport.
 * Ativa automaticamente quando a contagem de itens excede o threshold.
 */
export class VirtualScroll {
  ...
  constructor(container: HTMLElement, options: Record<string, unknown> = {}) {
    ...
    this._renderRow = (options.renderRow as ((item: Record<string, unknown>, index: number) => string)) || ((item: Record<string, unknown>) => '<div class="pna-virtual-row">' + (item.label || item.id || '') + '</div>');
   ...
   */
  setRenderRow(fn: (item: Record<string, unknown>, index: number) => string) {
    this._renderRow = fn;
    if (this._active) {
      this._lastStart = -1;
      this._render();
  ...
  _render() {
    ...
    for (let i = startIdx; i < endIdx; i++) {
      html += '<div class="pna-virtual-item" style="position:absolute;top:' + (i * this._rowHeight) + 'px;width:100%;height:' + this._rowHeight + 'px;" data-index="' + i + '">' +
        this._renderRow(this._data[i], i) +
      '</div>';
    }
...
/**
 * Factory to create a VirtualScroll instance.
 * @param {HTMLElement} container
 * @param {Object} [options]
 * @returns {VirtualScroll}
 */
export function createVirtualScroll(container: HTMLElement, options: Record<string, unknown> = {}) {
...
export default { VirtualScroll, createVirtualScroll, DEFAULT_THRESHOLD, info, healthCheck, injectPorts, getPorts, VERSION, MODULE_ID };
