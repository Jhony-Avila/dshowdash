...
// ═══════════════════════════════════════════════════════════════
// MODULE: preview
// PURPOSE: Panel NavRail Admin - Preview
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
type PreviewState = { items: Record<string, unknown>[]; groups?: unknown[] };

export function init(container: HTMLElement, state: PreviewState) {
    _container = container;
    update(container, state);
}

export function update(container: HTMLElement, state: PreviewState) {
    const previewSection = container.querySelector('[data-slot="preview"]');
    if (!previewSection) return;

    const items = state.items.filter((i: Record<string, unknown>) => (i._raw as Record<string, unknown>)?.is_active === 1);

    // Agrupar por grupo
    const grouped: Record<string, Record<string, unknown>[]> = {};
    items.forEach((item: Record<string, unknown>) => {
        const group = (item.group as string) || 'outros';
    ...
    Object.keys(grouped).forEach(group => {
        grouped[group].sort((a: Record<string, unknown>, b: Record<string, unknown>) => ((a.order as number) || 0) - ((b.order as number) || 0));
    });

    // Grupos ordenados (system no final)
                ...
                ${systemGroup ? `
                    <div class="pna-preview-navrail__footer">
                        <div class="pna-preview-group" data-preview-group="system">
                            ${grouped[systemGroup].map((item: Record<string, unknown>) => `
                                <div class="pna-preview-item" title="${item.label}">
                                    <span class="pna-preview-item__icon">
                                        ${getIconSvg(item.icon as string)}
                                    </span>
...
export function toggleExpand(container: HTMLElement) {
