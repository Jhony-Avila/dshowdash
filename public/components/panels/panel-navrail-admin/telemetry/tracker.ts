...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-navrail-admin/telemetry/tracker
// PURPOSE: Navrail Admin - Event Tracker
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panels/panel-navrail-admin/telemetry/tracker';
...
export class Tracker {
  [key: string]: any;
    constructor(options: Record<string, unknown> = {}) { this.moduleId = options.moduleId || MODULE_ID; this.enabled = options.enabled !== false; this._events = []; this._maxEvents = 1000; _initPorts(); }
    ...
    info() { const ps = Ports.snapshot(); return { version: VERSION, moduleId: MODULE_ID, enabled: this.enabled, eventCount: this._events.length, portsInitialized: ps._initialized, usingP18Intents: true }; }
}
export { Tracker as PanelTracker };
