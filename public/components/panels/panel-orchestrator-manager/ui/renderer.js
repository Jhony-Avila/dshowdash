...
const MODULE_ID = "panel-orchestrator-manager-renderer";
...
const ui = {
  ...
  _getTabCount(tabId, state) {
    let count = 0;
    if (tabId === "triggers") count = Array.isArray(state["triggers"]) ? state["triggers"].length : 0;
    else if (tabId === "rules") count = Array.isArray(state["rules"]) ? state["rules"].length : 0;
    else if (tabId === "flags") count = Array.isArray(state["flags"]) ? state["flags"].length : 0;
    else return "";
  ...
  renderStatsBar(stats) {
    ...
    if (stats["triggers_by_component"] && typeof stats["triggers_by_component"] === "object") {
      const tbc = stats["triggers_by_component"];
  ...
  renderLoading() {
    return '<div class="pom-loading"><div class="pom-loading__spinner"></div><span>Carregando...</span></div>';
  },
  ...
  renderFlags(state) {
    const items = Array.isArray(state["flags"]) ? state["flags"] : [];
  ...
  renderMetrics(_state) {
    return '<div class="pom-metrics-container" data-metrics-mount><div class="pom-timeline-loading"><div class="pom-loading__spinner"></div><span>Carregando Metrics...</span></div></div>';
  },
  renderTimeline(_state) {
    return '<div class="pom-timeline-container" data-timeline-mount><div class="pom-timeline-loading"><div class="pom-loading__spinner"></div><span>Carregando Timeline...</span></div></div>';
  },
  renderAudit(state) {
    ...
    for (const a of items) {
      rows += `<tr><td>${new Date(a["created_at"]).toLocaleString("pt-BR")}</td><td><span class="pom-badge">${a["action"]}</span></td><td>${a["entity_type"]}</td><td>${a["entity_id"]}</td><td><code>${a["ip_address"] || "-"}</code></td></tr>`;
    }
    return `<table class="pom-table"><thead><tr><th>Data</th><th>Acao</th><th>Tipo</th><th>ID</th><th>IP</th></tr></thead><tbody>${rows}</tbody></table>`;
  },
  renderEmpty(type, hint) {
    return `<div class="pom-empty"><span class="pom-empty__icon">${ICONS.inboxEmpty}</span><p>Nenhum ${type} encontrado</p>${hint ? `<small>${hint}</small>` : ""}</div>`;
  },
  renderTriggerModal(item) {
    const isEdit = !!item;
    let componentOptions = "";
  ...
  renderSimulatorModal(triggers, lastResult) {
    return simulator.renderSimulatorModal(triggers, lastResult);
  },
  renderSimulatorResult(result) {
    return simulator.renderResult(result);
  },
  renderSimulatorExecuted(result) {
    return simulator.renderExecutionResult(result);
  },
  _filterItems(items, state) {
    ...
    if (filterComponent && filterComponent !== "all") {
      filtered = filtered.filter((item) => item["component"] === filterComponent);
    }
    return filtered;
  }
};
