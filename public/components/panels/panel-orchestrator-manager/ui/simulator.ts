...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-orchestrator-manager-simulator
// PURPOSE: Panel Orchestrator Manager - Intent Simulator
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
export const MODULE_ID = 'panel-orchestrator-manager-simulator';
...
export const simulator = {
  renderSimulatorModal(triggers: Record<string, unknown>[], lastResult: Record<string, unknown> | null) {
    triggers = triggers || [];
  ...
  _groupTriggersByComponent(triggers: Record<string, unknown>[]) {
    const grouped: Record<string, Record<string, unknown>[]> = {};
    for (let i = 0; i < triggers.length; i++) {
      const t = triggers[i];
      const comp = (t.component as string) || 'other';
  ...
  renderEmptyResult() { return `<div class="pom-simulator__empty"><span class="pom-simulator__empty-icon">${SVGS.target}</span><p>Selecione um trigger ou digite um intent para simular</p></div>`; },
