...
// ═══════════════════════════════════════════════════════════════
// MODULE: orchestrator-ui-controls
// PURPOSE: Orchestrator UI Controls
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   getSnapshot from ../state/selectors.js
//   createPanelListItem from ./template.js
//   logger from ../utils/logger.js
//
// PROVIDES:
//   info() — exported function
//   healthCheck() — exported function
//   OrchestratorControls() — exported function
//   orchestratorControls — exported value
...
function OrchestratorControls(this: any) {
  this.container = null;
...
OrchestratorControls.prototype.isInitialized = function() { return this.initialized && !this.destroyed; };

OrchestratorControls.prototype.init = function(container: HTMLElement) {
  const self = this;
...
OrchestratorControls.prototype._startAutoUpdate = function() {
  ...
  this.updateInterval = setInterval(() => { self.updateMetrics(); }, 5000);
  this.updateMetrics();
};
...
OrchestratorControls.prototype.updateMetrics = function() {
  ...
  if (errorsEl) { errorsEl.textContent = snapshot.errors; errorsEl.classList.toggle('has-errors', snapshot.errors > 0); }
...
OrchestratorControls.prototype._formatUptime = (ms: number) => {
...
OrchestratorControls.prototype.updateStatus = function(status: string) {
  if (!this.container) return;
  const statusEl = this.container.querySelector('.orchestrator-status');
  if (statusEl) { statusEl.dataset.status = status; const textEl = statusEl.querySelector('.status-text'); if (textEl) textEl.textContent = status; }
};

OrchestratorControls.prototype.updatePanelList = function(panels: unknown[]) {
  if (!this.container) return;
  const listEl = this.container.querySelector('#orchestrator-panel-list');
  if (!listEl) return;
  listEl.innerHTML = panels.map((panel: unknown) => createPanelListItem(panel)).join('');
  const countEl = this.container.querySelector('.panel-count');
  if (countEl) countEl.textContent = String(panels.length);
};

OrchestratorControls.prototype.updatePresetSelector = function(presetId: string) {
  if (!this.container) return;
...
OrchestratorControls.prototype.destroy = function() {
  ...
  this.initialized = false;
  this.destroyed = true;
  logger.info('Controls destruídos');
};
...
export { OrchestratorControls, orchestratorControls, VERSION, MODULE_ID };

export function info() { return { moduleId: MODULE_ID, version: VERSION }; }
export function healthCheck() { return { status: 'HEALTHY', moduleId: MODULE_ID, version: VERSION, checks: { controlsReady: true } }; }
