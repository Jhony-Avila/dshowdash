'use strict';
// panel-outlook/index.js — carregador do painel React (Integração Outlook).
// @module  panels/panel-outlook
// @version 1.0.0
// @created 2026-07-21
//
// Espelha panel-pipedrive/index.js: adaptador vanilla que o PanelPort importa e
// que SEMPRE monta a interface React dentro do container-main (sem iframe).
//
// A folha de CSS do módulo é injetada UMA vez e NUNCA removida — escopada ao root
// `[data-ol-react-root]` (tokens `--ol-*`), fica inerte quando o painel fecha.
//
// O manifest é lido com `cache:'no-store'` (fonte da verdade do entry hasheado).

const MODULE_ID = 'panels/panel-outlook';
const VERSION   = '1.0.0';
const FLAG_KEY  = 'panel_outlook_enabled';
...
const CSS_ATTR = 'data-ol-react-css';
...
async function carregarReact() {
  ...
  const base = '/components/panels/panel-outlook/dist/';
  let entrada = null;
  try {
...
function renderErro(contentEl, err) {
  console.error('[outlook] falha ao carregar o React:', err);
  if (!contentEl) return;
  contentEl.innerHTML =
    '<div style="padding:32px;max-width:520px;margin:40px auto;text-align:center;'
    + 'border:1px solid var(--ol-border,#333);border-radius:12px;'
    + 'background:var(--bg-card,#1a1a2e);color:var(--text-primary,#e8e8f0)">'
    + '<strong style="display:block;font-size:15px;margin-bottom:8px">Não foi possível carregar o módulo de E-mails</strong>'
...
export async function unmount() {
  try {
    ...
    console.error('[outlook] erro no unmount:', err);
  } finally {
    _mounted = false;
  }
