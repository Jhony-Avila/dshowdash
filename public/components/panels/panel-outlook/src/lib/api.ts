// lib/api.ts — cliente do backend /api/outlook + TanStack Query.
...
export class ApiError extends Error {
  ...
  get precisaReconectar(): boolean {
    return this.code === 'NEEDS_AUTH';
  }
}
  ...
  defaultOptions: {
    queries: {
      ...
      retry: (tentativa, erro) => {
        if (erro instanceof ApiError && (erro.ehAuth || erro.status === 404 || erro.precisaReconectar)) return false;
        return tentativa < 2;
      },
...
export const chaves = {
  ...
  accounts: ['outlook', 'accounts'] as const,
  folders: (accountId: number) => ['outlook', 'folders', accountId] as const,
  messages: (accountId: number, folder: string) => ['outlook', 'messages', accountId, folder] as const,
  message: (accountId: number, id: string) => ['outlook', 'message', accountId, id] as const,
  dashboard: (period: string) => ['outlook', 'dashboard', period] as const,
  templates: (q: string, cat: string) => ['outlook', 'templates', q, cat] as const,
  categories: ['outlook', 'categories'] as const,
  rules: ['outlook', 'rules'] as const,
};
