...
const MODULE_ID = "uarps-icons-system";
const Lucide = {
  ...
  edit: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.12 2.12 0 0 1 3 3L12 15l-4 1 1-4Z"/></svg>',
  ...
  command: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 6v12a3 3 0 1 0 3-3H6a3 3 0 1 0 3 3V6a3 3 0 1 0-3 3h12a3 3 0 1 0-3-3"/></svg>',
  ...
  powerOff: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18.36 6.64A9 9 0 0 1 20.77 15"/><path d="M6.16 6.16a9 9 0 1 0 12.68 12.68"/><path d="M12 2v4"/><path d="m2 2 20 20"/></svg>',
  ...
  toggleRight: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="20" height="12" x="2" y="6" rx="6" ry="6"/><circle cx="16" cy="12" r="2"/></svg>',
  gitCompare: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="18" cy="18" r="3"/><circle cx="6" cy="6" r="3"/><path d="M13 6h3a2 2 0 0 1 2 2v7"/><path d="M11 18H8a2 2 0 0 1-2-2V9"/></svg>',
  ...
  checkbox: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="3" rx="2"/></svg>',
  checkboxChecked: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="3" rx="2"/><path d="m9 12 2 2 4-4"/></svg>',
...
const SERVER_ICONS_PATH = "/assets/icons/system";
const TRIGGER_TO_SERVER_ICON = {
  "dash-home": "browser-25.svg",
  "dash-overview": "006-report.svg",
  "dash-analytics": "005-graph report.svg",
  "user-list": "017-astronaut.svg",
  "user-create": "001-upload.svg",
  "user-edit": "002-pen-1.svg",
  "user-delete": "004-block.svg",
  "fin-invoices": "012-credit card.svg",
  "fin-payments": "001-dollar coin.svg",
  "comm-email": "023-mail.svg",
  "comm-chat": "002-speech bubble.svg",
  "sys-config": "077-cpu.svg",
  "sys-logs": "003-archive.svg",
  "media-upload": "001-upload.svg",
  "media-gallery": "005-image.svg",
  "report-generate": "006-report.svg",
  "int-api": "coding.svg"
};
const AREA_TO_SERVER_ICON = {
  "dashboard": "browser-25.svg",
  "users": "017-astronaut.svg",
  "finance": "012-credit card.svg",
  "financial": "012-credit card.svg",
  "communication": "023-mail.svg",
  "system": "077-cpu.svg",
  "config": "077-cpu.svg",
  "media": "005-image.svg",
  "reports": "006-report.svg",
  "integrations": "coding.svg",
  "api": "coding.svg",
  "admin": "001-safebox.svg",
  "security": "001-safebox.svg",
  "default": "004-compass.svg"
};
const TRIGGER_PATTERNS = [
  { pattern: /^dash/i, icon: "browser-25.svg" },
  { pattern: /^user/i, icon: "017-astronaut.svg" },
  { pattern: /^fin|^pay|^invoice/i, icon: "012-credit card.svg" },
  { pattern: /^comm|^email|^chat|^msg/i, icon: "023-mail.svg" },
  { pattern: /^sys|^config|^setting/i, icon: "077-cpu.svg" },
  { pattern: /^media|^upload|^image|^video/i, icon: "005-image.svg" },
  { pattern: /^report|^export/i, icon: "006-report.svg" },
  { pattern: /^int|^api|^webhook/i, icon: "coding.svg" },
  { pattern: /^admin|^perm|^security/i, icon: "001-safebox.svg" }
];
function getLucideIcon(name, size) {
  if (!size) size = 24;
  const icon = Lucide[name];
  if (!icon) return Lucide.compass;
  return icon.replace(/width="24"/g, `width="${size}"`).replace(/height="24"/g, `height="${size}"`);
}
function getServerIconUrl(triggerId, area) {
  if (TRIGGER_TO_SERVER_ICON[triggerId]) {
    return `${SERVER_ICONS_PATH}/${TRIGGER_TO_SERVER_ICON[triggerId]}`;
  }
  for (let i = 0; i < TRIGGER_PATTERNS.length; i++) {
    if (TRIGGER_PATTERNS[i].pattern.test(triggerId)) {
      return `${SERVER_ICONS_PATH}/${TRIGGER_PATTERNS[i].icon}`;
    }
  }
  if (area) {
    const areaKey = area.toLowerCase().replace(/[^a-z]/g, "");
    if (AREA_TO_SERVER_ICON[areaKey]) {
      return `${SERVER_ICONS_PATH}/${AREA_TO_SERVER_ICON[areaKey]}`;
    }
  }
  return `${SERVER_ICONS_PATH}/${AREA_TO_SERVER_ICON.default}`;
}
function serverIconImg(triggerId, area, className) {
  const url = getServerIconUrl(triggerId, area);
  const cls = className || "uarps-server-icon";
  return `<img src="${url}" alt="" class="${cls}" loading="lazy" onerror="this.style.opacity='0.3'">`;
}
function getAreaLucideIcon(area) {
  const map = {
  ...
  return Lucide[map[key]] || Lucide.folder;
}
const CUSTOM_KEY = "uarps-icon-mappings";
...
function setCustomMapping(triggerId, iconFile) {
  const custom = getCustomMappings();
  custom[triggerId] = iconFile;
  localStorage.setItem(CUSTOM_KEY, JSON.stringify(custom));
}
function getIconWithCustom(triggerId, area) {
  const custom = getCustomMappings();
  if (custom[triggerId]) {
    return `${SERVER_ICONS_PATH}/${custom[triggerId]}`;
  }
  return getServerIconUrl(triggerId, area);
}
function info() {
  return {
    moduleId: MODULE_ID,
    version: VERSION,
    lucideIconsCount: Object.keys(Lucide).length,
    serverIconsCount: Object.keys(TRIGGER_TO_SERVER_ICON).length
  };
}
function healthCheck() {
  return {
    ...
    version: VERSION,
    checks: {
      lucideReady: Object.keys(Lucide).length > 0,
      serverIconsReady: Object.keys(TRIGGER_TO_SERVER_ICON).length > 0,
      getLucideIconReady: typeof getLucideIcon === "function"
    }
  };
}
const IconsSystem = {
...
var icons_system_default = IconsSystem;
export {
  AREA_TO_SERVER_ICON,
  IconsSystem,
  Lucide,
  ...
  TRIGGER_TO_SERVER_ICON,
  VERSION,
  icons_system_default as default,
  getAreaLucideIcon,
  getCustomMappings,
  getIconWithCustom,
  getLucideIcon,
