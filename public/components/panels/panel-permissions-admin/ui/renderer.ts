...
// ═══════════════════════════════════════════════════════════════
// MODULE: uarps-admin-renderer
// PURPOSE: UARPS Admin - Renderer
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   Templates from ./templates.js
//   initPorts, injectPorts, getPorts from ../core/ports.js
//   bindControlEvents, bindUserGridEvents, bindMatrixEvents, bindUserFocusEvents,...
//   renderUsers, highlightSelectedUser, renderUserFocus, renderMatrix, renderStat...
//   updateUndoRedoButtons, updateBulkUI, updateViewToggle, updateCacheIndicator, ...
//
// PROVIDES:
...
export const MODULE_ID = 'uarps-admin-renderer';

function Renderer(this: any, options: Record<string, unknown> = {}) {
...
Renderer.create = (options: Record<string, unknown>) => new (Renderer as unknown as new (o: Record<string, unknown>) => typeof Renderer.prototype)(options);
...
Renderer.prototype.render = function() {
  ...
  this._unsubscribe = this._store.subscribe((change: unknown) => self._onStoreChange(change));
...
Renderer.prototype._bindEvents = function() {
  ...
  const updateViewFn = (view: string) => updateViewToggle(self._container, view);
...
Renderer.prototype._bindGlobalEvents = function() {
  const self = this;
  const showModalFn = (opts: Record<string, unknown>) => showModal(self._elements, opts.title as string, opts.message as string, opts.requireReason as boolean);
  const hideModalFn = () => hideModal(self._elements);
  this._eventUnsubscribers = bindGlobalEvents(showModalFn, hideModalFn);
};

Renderer.prototype._onStoreChange = function(change: unknown) {
  const c = change as { key?: string };
  switch (c.key) {
    case 'users': renderUsers(this._elements, this._store); break;
    case 'selectedUser':
      renderUserFocus(this._elements, this._store);
      renderMatrix(this._elements, this._store);
      highlightSelectedUser(this._elements, this._store);
      break;
    case 'permissions': case 'triggers': case 'regions':
      renderMatrix(this._elements, this._store);
      renderStats(this._elements, this._store);
      renderUserFocus(this._elements, this._store);
      break;
    case 'loading': renderLoadingState(this._container, this._store); break;
    case 'filter': renderMatrix(this._elements, this._store); break;
    case 'history': updateUndoRedoButtons(this._elements, this._store); break;
    case 'bulk': updateBulkUI(this._elements, this._store); break;
    case 'sync': updateCacheIndicator(this._elements, this._store); break;
  }
};
...
Renderer.prototype.destroy = function() {
  ...
  for (let i = 0; i < this._eventUnsubscribers.length; i++) { if (this._eventUnsubscribers[i]) this._eventUnsubscribers[i](); }
  this._eventUnsubscribers = [];
  this._elements = {};
...
export function healthCheck() { return { status: 'HEALTHY', moduleId: MODULE_ID, version: VERSION, checks: { rendererClassReady: typeof Renderer === 'function' } }; }

export { Renderer, injectPorts, getPorts };
