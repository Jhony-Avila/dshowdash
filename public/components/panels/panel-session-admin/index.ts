...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-session-admin
// PURPOSE: Panel Session Admin - Enterprise Entry Point
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
//   (none)
// WINDOW ACCESS:
//   (window as any).PanelSessionAdmin
//   window.__dev
// ═══════════════════════════════════════════════════════════════
'use strict';
...
export const MODULE_ID = 'panel-session-admin';
...
let _instance: PanelSessionAdmin | null = null;

export class PanelSessionAdmin {
  [key: string]: any;
  constructor() {
  ...
  mount(contentEl: HTMLElement, config: Record<string, unknown> = {}) {
    ...
    return Lifecycle.mount(contentEl, config).then((result) => {
      if (!result.ok && (result as any).error !== 'AUTH_REQUIRED') {
        this._renderError((result as any).error || 'Falha na inicialização');
        return this;
      }
      this._mounted = true;
  ...
  refresh() { return Lifecycle.refresh(); }
...
export const mount = (contentEl: HTMLElement, config: Record<string, unknown> = {}) => {
  if (!_isAuthenticated()) { return Promise.resolve({ success: false, moduleId: MODULE_ID, error: "not-authenticated" }); }
  const panel = createPanel();
  return panel.mount(contentEl, config);
};

export const unmount = () => {
  if (_instance) return _instance.unmount().then(() => { _instance = null; }
);
  return Promise.resolve();
};
...
if (typeof window !== 'undefined') {
  if (!isStrict()) {
    (window as any).PanelSessionAdmin = { PanelSessionAdmin, createPanel, mount, unmount, refresh, healthCheck, info, getVersion, injectPorts, getPorts, VERSION, MODULE_ID };
    _initPorts();
    const cfg = _getPort('config');
    if (cfg?.app?.debug) { window.__dev = window.__dev || {}; window.__dev.PanelSessionAdmin = (window as any).PanelSessionAdmin; }
  } else {
...
export default { PanelSessionAdmin, createPanel, mount, unmount, destroy, refresh, healthCheck, info, getVersion, injectPorts, getPorts, VERSION, MODULE_ID };
