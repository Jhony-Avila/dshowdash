...
const MODULE_ID = "panels/panel-status-currency-btc";
...
const CONFIG = { id: "status-currency-btc", area: "status", label: "Bitcoin", icon: "bitcoin", emoji: "\u20BF", kind: "panel-component", apiEndpoint: "/api/currencies/get_btc.php", refreshInterval: 6e4 };
class StatusCurrencyBtcComponent {
  constructor(options = {}) {
    _initPorts();
  ...
  attachEvents() {
    this._unsubscribe = this.store.subscribe((state, prev) => {
      if (state.price !== prev.price) this._updateDisplay();
    });
  }
  detachEvents() {
...
var panel_status_currency_btc_default = { StatusCurrencyBtcComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  MODULE_ID,
  StatusCurrencyBtcComponent,
  VERSION,
  panel_status_currency_btc_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
