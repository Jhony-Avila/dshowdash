...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-status-currency-btc
// PURPOSE: Status Currency BTC - Enterprise Component with Live Data
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panels/panel-status-currency-btc';
...
const CONFIG = { id: 'status-currency-btc', area: 'status', label: 'Bitcoin', icon: 'bitcoin', emoji: '₿', kind: 'panel-component', apiEndpoint: '/api/currencies/get_btc.php', refreshInterval: 60000 };

export class StatusCurrencyBtcComponent {
  [key: string]: any;
  constructor(options: { container?: HTMLElement | null; config?: Record<string, unknown>; eventBus?: Record<string, unknown> } = {}) {
    ...
    this.store = new StateStore({ mounted: false, loading: false, error: null, data: null, price: '--', change: 0 });
  ...
  async _fetchData() {
    ...
    try {
      ...
      if (result.success && result.data) {
        const price = result.data.usd ? `$${(result.data.usd).toLocaleString('en-US')}` : '--';
        const change = result.data.usd_24h_change || 0;
        this.store.setState({ loading: false, error: null, data: result.data, price, change });
        this._updateDisplay();
      }
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="Bitcoin Price"><span class="status-icon">${CONFIG.emoji}</span><span class="status-value" data-metric="price">--</span><span class="status-change" data-metric="change"></span></div>`;
  ...
  _updateDisplay() {
    ...
    const changeEl = this._element.querySelector('.status-change');
    if (valueEl) valueEl.textContent = state.price;
    if (changeEl) {
      const change = parseFloat(state.change);
      if (!isNaN(change)) {
        changeEl.textContent = change >= 0 ? `▲${change.toFixed(1)}%` : `▼${Math.abs(change).toFixed(1)}%`;
        changeEl.className = `status-change ${change >= 0 ? 'status-up' : 'status-down'}`;
      }
    }
  }

  attachEvents() { this._unsubscribe = this.store.subscribe((state: Record<string, unknown>, prev: Record<string, unknown>) => { if (state.price !== prev.price) this._updateDisplay(); }); }
...
let _currentInstance: StatusCurrencyBtcComponent | null = null;

// @ts-expect-error strict migration — TS2693
export const mount = (container: any, config: any) => { const instance = new StatusCurrencyBtcComponent({ container: any, config }); instance.init(); instance.mount(container); _currentInstance = instance; return { success: true, moduleId: MODULE_ID, instance }; };
...
export default { StatusCurrencyBtcComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
