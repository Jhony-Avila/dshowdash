...
const MODULE_ID = "panels/panel-status-currency-usd-brl";
...
const CONFIG = { id: "status-currency-usd-brl", area: "status", label: "USD/BRL", icon: "dollar", emoji: "\u{1F1FA}\u{1F1F8}\u{1F1E7}\u{1F1F7}", kind: "panel-component", apiEndpoint: "/api/currencies/get_usd_brl.php", refreshInterval: 6e4 };
class StatusCurrencyUsdBrlComponent {
  constructor(options = {}) {
    _initPorts();
  ...
  _updateDisplay() {
    ...
    if (valueEl) valueEl.textContent = state.rate;
  }
  attachEvents() {
    this._unsubscribe = this.store.subscribe((state, prev) => {
      if (state.rate !== prev.rate) this._updateDisplay();
    });
  }
  detachEvents() {
...
var panel_status_currency_usd_brl_default = { StatusCurrencyUsdBrlComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  MODULE_ID,
  StatusCurrencyUsdBrlComponent,
  VERSION,
  panel_status_currency_usd_brl_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
