...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-status-currency-usd-brl
// PURPOSE: Status Currency USD/BRL - Enterprise Component with Live Data
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panels/panel-status-currency-usd-brl';
...
const CONFIG = { id: 'status-currency-usd-brl', area: 'status', label: 'USD/BRL', icon: 'dollar', emoji: '🇺🇸🇧🇷', kind: 'panel-component', apiEndpoint: '/api/currencies/get_usd_brl.php', refreshInterval: 60000 };

export class StatusCurrencyUsdBrlComponent {
  [key: string]: any;
  constructor(options: { container?: HTMLElement | null; config?: Record<string, unknown>; eventBus?: Record<string, unknown> } = {}) {
    ...
    this.store = new StateStore({ mounted: false, loading: false, error: null, data: null, rate: '--' });
  ...
  async _fetchData() {
    ...
    try {
      ...
      if (result.success && result.data) {
        const rate = result.data.rate ? `R$ ${result.data.rate.toFixed(2)}` : '--';
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="USD/BRL Exchange Rate"><span class="status-icon">${CONFIG.emoji}</span><span class="status-value" data-metric="rate">--</span></div>`;
    this.container.innerHTML = ''; this.container.appendChild(this._element);
  }

  _updateDisplay() { if (!this._element) return; const state = this.store.getState(); const valueEl = this._element.querySelector('.status-value'); if (valueEl) valueEl.textContent = state.rate; }

  attachEvents() { this._unsubscribe = this.store.subscribe((state: Record<string, unknown>, prev: Record<string, unknown>) => { if (state.rate !== prev.rate) this._updateDisplay(); }); }
...
let _currentInstance: StatusCurrencyUsdBrlComponent | null = null;

// @ts-expect-error strict migration — TS2693
export const mount = (container: any, config: any) => { const instance = new StatusCurrencyUsdBrlComponent({ container: any, config }); instance.init(); instance.mount(container); _currentInstance = instance; return { success: true, moduleId: MODULE_ID, instance }; };
...
export default { StatusCurrencyUsdBrlComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
