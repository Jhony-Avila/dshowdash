...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-status-currency-usd-cny
// PURPOSE: Status Currency USD/CNY - Enterprise Component with Live Data
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panels/panel-status-currency-usd-cny';
...
const CONFIG = { id: 'status-currency-usd-cny', area: 'status', label: 'USD/CNY', icon: 'yuan', emoji: '🇺🇸🇨🇳', kind: 'panel-component', apiEndpoint: '/api/currencies/get_usd_cny.php', refreshInterval: 60000 };

export class StatusCurrencyUsdCnyComponent {
  [key: string]: any;
  ...
  async _fetchData() {
    ...
    try {
      ...
      if (result.success && result.data) {
        const rate = result.data.rate ? `¥ ${result.data.rate.toFixed(2)}` : '--';
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="USD/CNY Exchange Rate"><span class="status-icon">${CONFIG.emoji}</span><span class="status-value" data-metric="rate">--</span></div>`;
...
let _currentInstance: StatusCurrencyUsdCnyComponent | null = null;

// @ts-expect-error strict migration — TS2693
export const mount = (container: any, config: any) => { const instance = new StatusCurrencyUsdCnyComponent({ container: any, config }); instance.init(); instance.mount(container); _currentInstance = instance; return { success: true, moduleId: MODULE_ID, instance }; };
...
export default { StatusCurrencyUsdCnyComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
