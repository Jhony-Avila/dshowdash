...
const MODULE_ID = "panels/panel-status-email-integration/telemetry/tracker";
...
class Tracker {
  ...
  healthCheck() {
    ...
    return { status: "healthy", enabled: this.enabled, eventCount: this._events.length, portsInitialized: ps._initialized, version: VERSION, moduleId: MODULE_ID };
  }
  info() {
    const ps = Ports.snapshot();
    return { version: VERSION, moduleId: MODULE_ID, enabled: this.enabled, eventCount: this._events.length, portsInitialized: ps._initialized };
  }
}
