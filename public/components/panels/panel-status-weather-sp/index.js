...
const MODULE_ID = "panels/panel-status-weather-sp";
...
const CONFIG = { id: "status-weather-sp", area: "status", label: "Weather SP", icon: "weather", emoji: "\u{1F321}\uFE0F", kind: "panel-component", apiEndpoint: "/api/weather/get_sao_paulo.php", refreshInterval: 3e5 };
class StatusWeatherSpComponent {
  constructor(options = {}) {
    _initPorts();
    this.container = options.container || null;
    this.config = { ...CONFIG, ...options.config };
    this.eventBus = options.eventBus || _getPort("eventBus");
    this.store = new StateStore({ mounted: false, loading: false, error: null, data: null, temperature: "--", icon: "\u{1F321}\uFE0F" });
  ...
  async _fetchData() {
    ...
    try {
      ...
      if (result.success && result.data) {
        const temperature = result.data.temperature !== void 0 ? `${result.data.temperature}\xB0C` : "--";
        const icon = result.data.icon || "\u{1F321}\uFE0F";
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="S\xE3o Paulo Weather"><span class="status-icon" data-metric="icon">\u{1F321}\uFE0F</span><span class="status-value" data-metric="temperature">--</span></div>`;
  ...
  attachEvents() {
    this._unsubscribe = this.store.subscribe((state, prev) => {
      if (state.temperature !== prev.temperature) this._updateDisplay();
    });
  }
  detachEvents() {
...
var panel_status_weather_sp_default = { StatusWeatherSpComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  MODULE_ID,
  StatusWeatherSpComponent,
  VERSION,
  panel_status_weather_sp_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
