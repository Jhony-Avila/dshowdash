...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-status-weather-sp
// PURPOSE: Status Weather São Paulo - Enterprise Component with Live Data
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panels/panel-status-weather-sp';
...
const CONFIG = { id: 'status-weather-sp', area: 'status', label: 'Weather SP', icon: 'weather', emoji: '🌡️', kind: 'panel-component', apiEndpoint: '/api/weather/get_sao_paulo.php', refreshInterval: 300000 };

export class StatusWeatherSpComponent {
  [key: string]: any;
  constructor(options: { container?: HTMLElement | null; config?: Record<string, unknown>; eventBus?: Record<string, unknown> } = {}) {
    ...
    this.store = new StateStore({ mounted: false, loading: false, error: null, data: null, temperature: '--', icon: '🌡️' });
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="São Paulo Weather"><span class="status-icon" data-metric="icon">🌡️</span><span class="status-value" data-metric="temperature">--</span></div>`;
  ...
  _updateDisplay() {
    ...
    const iconEl = this._element.querySelector('.status-icon');
    const valueEl = this._element.querySelector('.status-value');
    if (iconEl) iconEl.textContent = state.icon;
    if (valueEl) valueEl.textContent = state.temperature;
  }

  attachEvents() { this._unsubscribe = this.store.subscribe((state: Record<string, unknown>, prev: Record<string, unknown>) => { if (state.temperature !== prev.temperature) this._updateDisplay(); }); }
...
export default { StatusWeatherSpComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
