...
const MODULE_ID = "panels/panel-status-whatsapp-integration";
...
const CONFIG = { id: "status-whatsapp-integration", area: "status", label: "WhatsApp", icon: "whatsapp", emoji: "\u{1F4F1}", kind: "panel-component", apiEndpoint: "/api/integrations/whatsapp/status.php", refreshInterval: 3e4 };
class StatusWhatsappIntegrationComponent {
  constructor(options = {}) {
    _initPorts();
  ...
  _updateDisplay() {
    ...
    if (valueEl) {
      ...
      else if (state.conversations > 0) valueEl.textContent = `${state.conversations} \u{1F4AC}`;
  ...
  attachEvents() {
    this._unsubscribe = this.store.subscribe((state, prev) => {
      if (state.unread !== prev.unread || state.status !== prev.status) this._updateDisplay();
    });
  }
  detachEvents() {
...
var panel_status_whatsapp_integration_default = { StatusWhatsappIntegrationComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  MODULE_ID,
  StatusWhatsappIntegrationComponent,
  VERSION,
  panel_status_whatsapp_integration_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
