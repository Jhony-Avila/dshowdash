...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-status/ui/renderer
// PURPOSE: Panel Status - Renderer
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   STATUS_TYPES from ../core/constants.js
//
// PROVIDES:
//   VERSION — module constant
//   MODULE_ID — module constant
//   renderStatusCard() — exported function
//   renderStatusGrid() — exported function
//   renderOverallStatus() — exported function
...
export const MODULE_ID = 'panel-status/ui/renderer';

export function renderStatusCard(statusId: string, data: Record<string, unknown>, config: Record<string, unknown>) {
  const cfg = config || STATUS_TYPES[statusId as keyof typeof STATUS_TYPES] || {};
  const level = data?.level || 'unknown';
  const value = data?.value !== undefined ? data.value : '--';
  const loading = data?.loading;
  return `<div class="status-card status-card--${level}" data-status-id="${statusId}" style="--status-color:${cfg.color || '#6c757d'}"><div class="status-icon">${getStatusIcon(cfg.icon as string)}</div><div class="status-content"><span class="status-label">${escapeHtml((cfg.label as string) || statusId)}</span><span class="status-value">${loading ? '<span class="loading-dots">...</span>' : escapeHtml(String(value))}</span></div><div class="status-indicator status-indicator--${level}"></div></div>`;
}

export function renderStatusGrid(statuses: Record<string, Record<string, unknown>>, configs: Record<string, Record<string, unknown>>) {
  let html = '<div class="status-grid">';
  for (const id in statuses) {
    if (Object.prototype.hasOwnProperty.call(statuses, id)) { html += renderStatusCard(id, statuses[id], configs?.[id]); }
  }
  html += '</div>';
  return html;
}

export function renderOverallStatus(level: string) {
  const labels: Record<string, string> = { ok: 'Sistema Operacional', warning: 'Atenção Necessária', error: 'Problemas Detectados', unknown: 'Verificando...' };
...
export function renderSkeleton(count: number) {
  let html = '<div class="status-grid">';
  for (let i = 0; i < (count || 4); i++) { html += '<div class="status-card status-card--skeleton"><div class="skeleton-icon"></div><div class="skeleton-content"><div class="skeleton-line"></div><div class="skeleton-line"></div></div></div>'; }
  html += '</div>';
  return html;
}

export function renderError(error: { message?: string } | null) {
...
export default { renderStatusCard, renderStatusGrid, renderOverallStatus, renderSkeleton, renderError };
