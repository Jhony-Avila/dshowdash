...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-uarps-monitor
// PURPOSE: Panel UARPS Monitor - Enterprise Entry Point
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
//   state, resetState from ./core/state.js
//   fetchAPI from ./api/client.js
//   renderSkeleton, renderError, renderDashboard from ./ui/renderer.js
//   bindEvents from ./ui/events.js
//
// PROVIDES:
//   mount(), unmount(), healthCheck(), info(),
//   getVersion(), refresh(), destroy(),
...
//
// BROWSER APIs (legítimo — panel rendering + data fetch):
...
//   window.SessionManager — auth fallback
// ═══════════════════════════════════════════════════════════════
'use strict';
...
export const MODULE_ID = 'panel-uarps-monitor';
...
function render() {
  ...
  if (!_isAuthenticated()) {
    _container.innerHTML = '<div style="padding:2rem;text-align:center;color:#F59E0B;">Faça login para acessar</div>';
    return;
  }
...
export function refresh() { if (_canRefresh()) return loadData(); return Promise.resolve(false); }
export function destroy() { return unmount(); }

export default { mount, unmount, destroy, healthCheck, info, getVersion, refresh, VERSION, MODULE_ID, injectPorts, getPorts };
