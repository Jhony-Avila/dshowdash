...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-uarps-monitor:ui/renderer
// PURPOSE: UARPS Monitor UI rendering
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   formatDate, timeAgo from ../utils/formatters.js
//
// PROVIDES:
...
export const MODULE_ID = 'panel-uarps-monitor:ui/renderer';
...
function renderDivergencesTable(state: { divergences: Record<string, unknown>[]; loading: boolean; lastRefresh: Date | null }) {
  if (state.divergences.length === 0) {
    return `
      <div class="pum-empty">
        ...
        <span>O sistema UARPS está consistente com o ACL legacy</span>
      </div>
    `;
  }

  const rows = state.divergences.slice(0, 50).map((d: Record<string, unknown>) => {
    ...
    return `
      <tr>
        ...
        <td class="pum-td-date" title="${formatDate(d.created_at as string | Date | null | undefined)}">${timeAgo(d.created_at as string | Date | null | undefined)}</td>
      </tr>
    `;
  }).join('');
...
export function renderDashboard(state: { status: Record<string, unknown> | null; inventory: Record<string, unknown> | null; stats: Record<string, unknown> | null; divergences: Record<string, unknown>[]; loading: boolean; lastRefresh: Date | null }) {
  const s: Record<string, unknown> = state.status || {};
  const inv: Record<string, unknown> = state.inventory || {};
  const stats: Record<string, unknown> = state.stats || {};
  const uarpsConfig = (s.uarps_config as Record<string, unknown>) || {};
  const uarpsGate = (s.uarps_gate as Record<string, unknown>) || {};
  const uarpsShadow = (s.uarps_shadow as Record<string, unknown>) || {};

  const mode = (uarpsConfig.mode as string) || 'unknown';
    ...
    <div class="pum-container">
      ...
      <div class="pum-cards">
        ...
        <div class="pum-card ${divergenceClass}">
          ...
          <div class="pum-card-content">
            ...
            <div class="pum-card-label">Divergências</div>
          </div>
        </div>
      </div>
      ...
      <div class="pum-section">
        <div class="pum-section-header">
          <h3 class="pum-section-title">Divergências Recentes</h3>
