...
const CONFIG = { id: "user-notifications", area: "user", label: "Notifications", emoji: "\u{1F514}", apiEndpoint: "/api/user/notifications.php", refreshInterval: 3e4 };
class UserNotificationsComponent {
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="${CONFIG.label}"><span class="user-icon">${CONFIG.emoji}</span><span class="user-value" data-metric="unread">--</span></div>`;
  ...
  _updateDisplay() {
    ...
    if (v) {
      v.textContent = s.unread > 0 ? s.unread : "\u2713";
      v.classList.remove("status-ok", "status-warn");
      if (s.unread === 0) v.classList.add("status-ok");
...
var panel_user_notifications_default = { UserNotificationsComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  MODULE_ID,
  UserNotificationsComponent,
  VERSION,
  panel_user_notifications_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
