...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-user-notifications
// PURPOSE: User Notifications - Enterprise Component with Live Data
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export class UserNotificationsComponent {
  ...
  async _fetchData() {
    ...
    try { const r = await fetch(this.config.apiEndpoint, { signal: this._abortController?.signal, credentials: 'include' }); if (!r.ok) throw new Error(`HTTP ${r.status}`); const j = await r.json(); if ((j.ok ?? j.success) && j.data) { this.store.setState({ loading: false, error: null, data: j.data, unread: j.data.unread || 0 }); this._updateDisplay(); } } catch (e: any) { if (e.name !== 'AbortError') { this._metrics.errorCount++; this.store.setState({ loading: false, error: e.message }); } }
  }
  ...
  render() { this._element = document.createElement('div'); this._element.className = `enterprise-component ${CONFIG.id}`; this._element.setAttribute('data-module-id', MODULE_ID); this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="${CONFIG.label}"><span class="user-icon">${CONFIG.emoji}</span><span class="user-value" data-metric="unread">--</span></div>`; this.container.innerHTML = ''; this.container.appendChild(this._element); }
  _updateDisplay() { if (!this._element) return; const s = this.store.getState(); const v = this._element.querySelector('.user-value'); if (v) { v.textContent = s.unread > 0 ? s.unread : '✓'; v.classList.remove('status-ok', 'status-warn'); if (s.unread === 0) v.classList.add('status-ok'); else v.classList.add('status-warn'); } }
...
export default { UserNotificationsComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
