...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-user-notifications/services/api
// PURPOSE: User Notifications - API Service
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
export const MODULE_ID = 'panels/panel-user-notifications/services/api';
...
export class ApiService {
  [key: string]: any;
  constructor(options: Record<string, unknown> = {}) { this.baseUrl = options.baseUrl || '/api/notifications'; this._metrics = { requestCount: 0, errorCount: 0, lastRequestAt: null }; }
  ...
  async getById(id: string | number) { return this.fetch(`/${id}`); }
  async markRead(id: string | number) { return this.fetch(`/${id}/read`, { method: 'POST' }); }
