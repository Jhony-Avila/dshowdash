...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-user-notifications/telemetry/logger
// PURPOSE: Panel User Notifications - Logger
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
export const MODULE_ID = 'panels/panel-user-notifications/telemetry/logger';
let _debug = false;
...
export class Logger {
  ...
  constructor(options: Record<string, unknown> = {}) {
    this.prefix = options.prefix || '[user-notifications]';
  ...
  healthCheck() {
    ...
    return { status: passed === 2 ? 'HEALTHY' : 'DEGRADED', score: `${passed}/2`, checks, version: VERSION, moduleId: MODULE_ID, timestamp: new Date().toISOString() };
  }
