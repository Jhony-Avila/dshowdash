...
const MODULE_ID = "panel-user-preferences.avatar.renderer";
...
const _log = (level, msg, extra) => {
  ...
  if (typeof fn === "function") fn(`[${MODULE_ID}] ${msg}`, extra);
};
...
const getBasePath = (base, size) => {
  const half = size / 2;
  const r = half - 1;
  switch (base) {
    case AVATAR_BASES.ROUND:
      return `<circle cx="${half}" cy="${half}" r="${r}"/>`;
    case AVATAR_BASES.SQUARE: {
      const cornerRadius = size * 0.15;
      return `<rect x="1" y="1" width="${size - 2}" height="${size - 2}" rx="${cornerRadius}"/>`;
    }
    case AVATAR_BASES.HEX:
      return `<polygon points="${getHexagonPoints(half, half, r)}"/>`;
    case AVATAR_BASES.SHIELD:
      return `<path d="M${half} 2 L${size - 2} ${size * 0.25} L${size - 2} ${size * 0.6} Q${size - 2} ${size - 2} ${half} ${size - 2} Q2 ${size - 2} 2 ${size * 0.6} L2 ${size * 0.25} Z"/>`;
    case AVATAR_BASES.DIAMOND:
      return `<polygon points="${half},2 ${size - 2},${half} ${half},${size - 2} 2,${half}"/>`;
    default:
      return `<circle cx="${half}" cy="${half}" r="${r}"/>`;
  }
};
const getBackground = (bgId) => {
  if (!bgId) return AVATAR_BACKGROUNDS.GRADIENT_NEON_PURPLE;
  const bgKey = bgId.toUpperCase().replace(/-/g, "_");
  return AVATAR_BACKGROUNDS[bgKey] || AVATAR_BACKGROUNDS.GRADIENT_NEON_PURPLE;
};
const createGradientDef = (background, size) => {
...
const getIcon = (iconId) => {
  ...
  return AVATAR_ICONS[iconKey] || AVATAR_ICONS.NONE;
};
const getFrame = (frameId) => {
  if (!frameId || frameId === "none") return AVATAR_FRAMES.NONE;
  const frameKey = frameId.toUpperCase().replace(/-/g, "_");
  return AVATAR_FRAMES[frameKey] || AVATAR_FRAMES.NONE;
};
const renderFrame = (frame, size, base) => {
  const half = size / 2;
  ...
  switch (base) {
    case AVATAR_BASES.ROUND:
      framePath = `<circle cx="${half}" cy="${half}" r="${r}" fill="none" stroke="${frame.color}" stroke-width="${strokeWidth}"/>`;
      break;
    case AVATAR_BASES.HEX:
      framePath = `<polygon points="${getHexagonPoints(half, half, r)}" fill="none" stroke="${frame.color}" stroke-width="${strokeWidth}"/>`;
      break;
    default:
      framePath = `<circle cx="${half}" cy="${half}" r="${r}" fill="none" stroke="${frame.color}" stroke-width="${strokeWidth}"/>`;
  }
...
const renderBadges = (badges, avatarSize) => {
  if (!badges?.length) return "";
  const badgeSize = Math.max(12, avatarSize * 0.25);
  return badges.slice(0, 2).map((badgeId, index) => {
    const badge = AVATAR_BADGES[badgeId.toUpperCase()] || AVATAR_BADGES[badgeId];
    if (!badge) return "";
...
const renderFallback = (data, options = {}) => {
...
const renderImage = (data, options = {}) => {
  ...
  const sizeValue = AVATAR_SIZES[size] || AVATAR_SIZES.MD;
  const imageUrl = data.imageUrl || data.avatar_image_url;
  const badges = showBadges && data.badges ? data.badges : [];
...
const renderGenerated = (config, options = {}) => {
  const size = options.size || "MD";
  const className = options.className || "";
  const interactive = options.interactive || false;
  const showBadges = options.showBadges !== false;
...
const render = (avatarData, options = {}) => {
  _metrics.renders++;
  try {
    ...
    switch (type) {
      case AVATAR_TYPES.IMAGE:
        return renderImage(avatarData, options);
      case AVATAR_TYPES.GENERATED:
        return renderGenerated(avatarData.config || DEFAULT_AVATAR_CONFIG, options);
      case AVATAR_TYPES.FALLBACK:
      default:
        return renderFallback(avatarData, options);
    }
  } catch (error) {
    _metrics.errors++;
    _log("warn", `Render error: ${error.message}`);
    return renderFallback({ initials: "??", backgroundColor: "#ef4444" }, options);
  }
};
const getMetrics = () => ({ ..._metrics });
const resetMetrics = () => {
  _metrics = { renders: 0, fallbacks: 0, errors: 0 };
};
const healthCheck = () => {
  const logger = _getPort("logger");
...
var renderer_default = { render, renderFallback, renderImage, renderGenerated, getMetrics, resetMetrics, info, healthCheck, injectPorts, getPorts, VERSION, MODULE_ID };
export {
  MODULE_ID,
  VERSION,
