...
const CONFIG = { id: "user-preferences", area: "user", label: "Preferences", emoji: "\u2699\uFE0F", apiEndpoint: "/api/user/preferences.php", refreshInterval: 3e5 };
class UserPreferencesComponent {
  ...
  async _fetchData() {
    ...
    try {
      ...
      if ((j.ok ?? j.success) && j.data) {
        this.store.setState({ loading: false, error: null, data: j.data, theme: j.data.theme || "--", language: j.data.language || "--" });
        this._updateDisplay();
      }
    } catch (e) {
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="${CONFIG.label}"><span class="user-icon">${CONFIG.emoji}</span><span class="user-value" data-metric="prefs">--</span></div>`;
  ...
  _updateDisplay() {
    ...
    if (v) v.textContent = s.theme === "dark" ? "\u{1F319}" : "\u2600\uFE0F";
  }
  healthCheck() {
...
var panel_user_preferences_default = { UserPreferencesComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  MODULE_ID,
  UserPreferencesComponent,
  VERSION,
  panel_user_preferences_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
