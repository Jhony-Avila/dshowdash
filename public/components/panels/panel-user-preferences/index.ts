...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-user-preferences
// PURPOSE: User Preferences - Enterprise Component with Live Data
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
// ⚠️ Store SINGLETON, não classe: o import nomeado pegava um objeto-alias e o `new StateStore(...)`
// abaixo morria em `TypeError: StateStore is not a constructor` — o `@ts-expect-error TS2614` que
// estava aqui calava justamente o aviso que apontava isso. Mesma correção de `panel-user-profile`
// (v9.4.0-RECONNECT). Ganho extra: o singleton tem `theme`/`language` REAIS (default `system`/
// `pt-BR`), enquanto a instância descartada nascia com `'--'` — é ele que `_updateDisplay()` quer.
import Store from './state/store.js';
import { Logger } from './telemetry/logger.js';

// ⚠️ Mesma história do store: `telemetry/tracker.js` exporta um SINGLETON (`export const Tracker =
// tracker`), e o construtor fazia `new Tracker(...)` — `TypeError: Tracker is not a constructor`,
// o defeito que aparecia depois de consertar o store. Nada neste painel chama `this.tracker.*`;
// o singleton é referenciado como está, sem instanciar.
import tracker from './telemetry/tracker.js';
export const MODULE_ID = 'panels/panel-user-preferences';
export const VERSION = '9.3.0-P2-ENTERPRISE';
export const getVersion = () => VERSION;
const Ports = createPanelPorts({ moduleId: MODULE_ID });
const _initPorts = () => Ports.init(); const _getPort = (name: string) => Ports.get(name);
export const injectPorts = (p: unknown) => Ports.inject(p); export const getPorts = () => Ports.snapshot();
...
const CONFIG = { id: 'user-preferences', area: 'user', label: 'Preferences', emoji: '⚙️', apiEndpoint: '/api/user/preferences.php', refreshInterval: 300000 };
export class UserPreferencesComponent {
  [key: string]: any;
  constructor(options: Record<string, unknown> = {}) { _initPorts(); this.container = options.container || null; this.config = { ...CONFIG, ...(options.config as Record<string, unknown>) }; this.eventBus = options.eventBus || _getPort('eventBus'); this.store = Store; this.store.setState({ mounted: false, loading: false, error: null, data: null }); this.lifecycle = new LifecycleManager(this); this.circuitBreaker = new CircuitBreaker({ threshold: 3, timeout: 30000 }); this.logger = new Logger({ prefix: `[${MODULE_ID}]` }); this.tracker = tracker; this._mounted = false; this._initialized = false; this._element = null; this._abortController = null; this._refreshTimer = null; this._metrics = { mountCount: 0, errorCount: 0, fetchCount: 0, lastFetchAt: null }; }
  ...
  async _fetchData() {
    ...
    try { const r = await fetch(this.config.apiEndpoint, { signal: this._abortController?.signal, credentials: 'include' }); if (!r.ok) throw new Error(`HTTP ${r.status}`); const j = await r.json(); if ((j.ok ?? j.success) && j.data) { this.store.setState({ loading: false, error: null, data: j.data, theme: j.data.theme || '--', language: j.data.language || '--' }); this._updateDisplay(); } } catch (e: any) { if (e.name !== 'AbortError') { this._metrics.errorCount++; this.store.setState({ loading: false, error: e.message }); } }
  }
  ...
  render() { this._element = document.createElement('div'); this._element.className = `enterprise-component ${CONFIG.id}`; this._element.setAttribute('data-module-id', MODULE_ID); this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="${CONFIG.label}"><span class="user-icon">${CONFIG.emoji}</span><span class="user-value" data-metric="prefs">--</span></div>`; this.container.innerHTML = ''; this.container.appendChild(this._element); }
  _updateDisplay() { if (!this._element) return; const s = this.store.getState(); const v = this._element.querySelector('.user-value'); if (v) v.textContent = s.theme === 'dark' ? '🌙' : '☀️'; }
...
let _currentInstance: UserPreferencesComponent | null = null;

export const mount = (container: any, config: any) => { const i = new UserPreferencesComponent({ container, config }); i.init(); i.mount(container); _currentInstance = i; return { success: true, moduleId: MODULE_ID, instance: i }; };
...
export default { UserPreferencesComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
