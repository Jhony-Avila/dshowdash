...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-user-preferences/telemetry/logger
// PURPOSE: Panel User Preferences - Logger
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   (none)
//
...
export const MODULE_ID = 'panels/panel-user-preferences/telemetry/logger';

let _debug = false;
let _logBuffer: Array<{ level: string; args: unknown[]; ts: number }> = [];
...
export class Logger {
  ...
  _logMsg(level: string, ...args: unknown[]) {
  ...
  setLevel(level: string) { if ((this.levels as Record<string, number>)[level] !== undefined) this.level = level; }
  enable() { this.enabled = true; }
  disable() { this.enabled = false; }

  healthCheck() {
    const checks = { enabled: this.enabled, validLevel: this.levels[this.level] !== undefined };
    const passed = Object.values(checks).filter(Boolean).length;
    return { status: passed === 2 ? 'HEALTHY' : 'DEGRADED', score: passed, maxScore: 2, version: VERSION, moduleId: MODULE_ID };
  }

  info2() { return { version: VERSION, moduleId: MODULE_ID, enabled: this.enabled, level: this.level, metrics: this._metrics }; }
