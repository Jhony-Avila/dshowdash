...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-user-preferences:template-identity
// PURPOSE: Panel User Preferences - Identity Template
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
//   renderIdentitySection() — exported function
//   renderAvatar() — exported function
//   renderAvatarEditor() — exported function
//   renderAvatarUpload() — exported function
...
export const MODULE_ID = 'panel-user-preferences:template-identity';
...
function _s(str: string | null | undefined) { if (!str) return ''; return String(str).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;'); }
function _getInitials(user: UserData | null | undefined) { if (!user?.name && !user?.email) return '?'; const name = user.name || user.email || ''; const parts = name.split(/[\s@]+/); if (parts.length >= 2) return (parts[0][0] + parts[1][0]).toUpperCase(); return name.substring(0, 2).toUpperCase(); }

export function renderIdentitySection(user: UserData | null | undefined, options: Record<string, unknown> = {}) {
  const { showEditor = true, avatarSize = 'LG' } = options;
  if (!user) { return `<div class="pup-identity"><div class="pup-avatar-wrap"><div class="pup-avatar"><span class="pup-avatar-initials">?</span></div></div><div class="pup-user-info"><div class="pup-user-top"><h2 class="pup-user-name">Carregando...</h2></div><p class="pup-user-email">${ICONS.mail} --</p></div></div>`; }
  const name = user.name || user.username || 'Usuário'; const email = user.email || ''; const role = user.role || user.nivel || 'user'; const userId = user.id || user.user_id || '--';
  const roleLabels: Record<string, string> = { admin: 'Admin', superadmin: 'Super Admin', manager: 'Gerente', user: 'Usuário', viewer: 'Visualizador' }; const roleLabel = roleLabels[role?.toLowerCase()] || role;
...
export function renderAvatar(user: UserData | null | undefined, options: Record<string, unknown> = {}) {
  const { size = 'LG', showBadges = true, interactive = false, className = '' } = options;
  const currentAvatar = AvatarSystem.getCurrent();
  if (currentAvatar && currentAvatar.type !== 'fallback') { return AvatarSystem.render(currentAvatar, { size, showBadges, interactive, className }); }
...
function _generateColor(id: number) { const colors = ['#7c3aed', '#3b82f6', '#22c55e', '#f59e0b', '#ef4444', '#ec4899', '#06b6d4', '#8b5cf6']; return colors[id % colors.length]; }
