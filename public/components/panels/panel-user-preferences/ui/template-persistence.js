...
const MODULE_ID = "panel-user-preferences-ui-template-persistence";
const _s = (str) => !str ? "" : String(str).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
const renderPersistenceSection = (layouts, savedViews) => {
  const layoutItems = Array.isArray(layouts) ? layouts : layouts?.items ?? [];
...
const renderLayoutsCard = (layouts) => {
  ...
  if (count === 0) {
    ...
    content = layouts.slice(0, 5).map((l, i) => `<div class="pup-item ${l.is_active ? "pup-slide-in" : ""}" data-layout-key="${_s(l.layout_key)}" draggable="true" style="animation-delay:${i * 50}ms"><div class="pup-item-grip" aria-hidden="true">${ICONS.gripVertical}</div><div class="pup-item-icon">${ICONS.layout}</div><div class="pup-item-info"><span class="pup-item-name">${_s(l.layout_key)}</span><div class="pup-item-meta">${l.is_default ? `<span class="pup-item-badge default">${ICONS.star} Padr\xE3o</span>` : ""}${l.is_active ? '<span class="pup-item-badge active">Ativo</span>' : ""}</div></div><div class="pup-item-actions"><button type="button" class="pup-item-btn" data-action="apply-layout" aria-label="Aplicar ${_s(l.layout_key)}">${ICONS.check}</button><button type="button" class="pup-item-btn danger" data-action="delete-layout" aria-label="Excluir ${_s(l.layout_key)}">${ICONS.trash}</button></div></div>`).join("");
  }
...
const renderViewsCard = (views) => {
  const count = views.length;
  let content = "";
  if (count === 0) {
    content = `<div class="pup-empty"><div class="pup-empty-icon">${ICONS.eye}</div><h4 class="pup-empty-title">Nenhuma View</h4><p class="pup-empty-text">Views salvam filtros e configura\xE7\xF5es</p></div>`;
  } else {
    content = views.slice(0, 5).map((v, i) => `<div class="pup-item" data-view-id="${v.id}" style="animation-delay:${i * 50}ms"><div class="pup-item-icon">${ICONS.eye}</div><div class="pup-item-info"><span class="pup-item-name">${_s(v.view_label)}</span><div class="pup-item-meta">${v.is_default ? `<span class="pup-item-badge default">${ICONS.star} Padr\xE3o</span>` : ""}${v.is_shared ? '<span class="pup-item-badge recent">Compartilhada</span>' : ""}</div></div><div class="pup-item-actions"><button type="button" class="pup-item-btn" data-action="apply-view" aria-label="Aplicar ${_s(v.view_label)}">${ICONS.check}</button>${v.is_owner !== false ? `<button type="button" class="pup-item-btn danger" data-action="delete-view" aria-label="Excluir ${_s(v.view_label)}">${ICONS.trash}</button>` : ""}</div></div>`).join("");
  }
...
const renderBackupCard = () => {
  ...
  return `<div class="pup-persist-card"><div class="pup-persist-hdr"><span class="pup-persist-title">${ICONS.download} Backup</span></div><div class="pup-persist-body"><div class="pup-backup-header"><div class="pup-backup-ring"><svg viewBox="0 0 36 36" width="32" height="32"><circle class="pup-backup-ring-bg" cx="18" cy="18" r="13"/><circle class="pup-backup-ring-progress" cx="18" cy="18" r="13"/></svg><span class="pup-backup-ring-text">75%</span></div><div class="pup-backup-info"><h4 class="pup-backup-title">Dados Sincronizados</h4><p class="pup-backup-meta">\xDAltimo backup: ${lastBackup}</p></div></div><div class="pup-backup-actions"><button type="button" class="pup-btn secondary" data-action="export">${ICONS.download} Exportar</button><button type="button" class="pup-btn ghost" data-action="import">${ICONS.upload} Importar</button><input type="file" accept=".json" data-input="import-file" style="display:none"></div><p class="pup-backup-version">${ICONS.hash} Vers\xE3o ${CONSTANTS_VERSION}</p></div></div>`;
};
const info = () => ({ moduleId: MODULE_ID, version: VERSION, p25Compliant: true });
const healthCheck = () => ({ status: "HEALTHY", moduleId: MODULE_ID, version: VERSION, checks: { templatePersistenceReady: true }, p25Compliant: true });
var template_persistence_default = { renderPersistenceSection, info, healthCheck };
export {
  MODULE_ID,
  VERSION,
  template_persistence_default as default,
  healthCheck,
  info,
