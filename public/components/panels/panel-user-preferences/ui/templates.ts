...
// ═══════════════════════════════════════════════════════════════
// MODULE: panel-user-preferences:templates
// PURPOSE: Panel User Preferences - Templates
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   MODULE_ID, VERSION, ICONS from ../core/constants.js
//   renderIdentitySection from ./template-identity.js
//   renderInterfaceSection from ./template-interface.js
//   renderPersistenceSection from ./template-persistence.js
//   renderHistorySection from ./template-history.js
//   renderModals from ./template-modals.js
//
// PROVIDES:
//   _s() — exported function
//   _getInitials() — exported function
...
interface RenderData {
  ...
  layouts: Parameters<typeof renderPersistenceSection>[0];
  savedViews: Parameters<typeof renderPersistenceSection>[1];
  history: Parameters<typeof renderHistorySection>[0];
  isDirty: boolean;
  isSaving: boolean;
  isCompact: boolean;
}

export function _s(str: string | null | undefined) {
  if (!str) return '';
  return String(str).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
}

export function _getInitials(user: UserData | null | undefined) {
