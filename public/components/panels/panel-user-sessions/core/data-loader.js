...
const MODULE_ID = "panel-user-sessions/core/data-loader";
...
DataLoader.prototype.loadData = function() {
  ...
  return listSessions().then((data) => {
    ...
    if (data) {
      ...
      ctx.telemetry.ready({ sessionsCount: Array.isArray(data.sessions) ? data.sessions.length : 0 });
      ctx.setState(STATES.READY);
    } else {
