...
const MODULE_ID = "panels/panel-user-sessions";
...
const CONFIG = { id: "user-sessions", area: "user", label: "Sessions", emoji: "\u{1F510}", apiEndpoint: "/api/user/sessions.php", refreshInterval: 6e4 };
class UserSessionsComponent {
  constructor(options = {}) {
    ...
    this.store = new StateStore({ mounted: false, loading: false, error: null, data: null, activeSessions: 0 });
  ...
  async _fetchData() {
    ...
    try {
      ...
      if (j.success && j.data) {
        this.store.setState({ loading: false, error: null, data: j.data, activeSessions: j.data.active_sessions || 0 });
        this._updateDisplay();
      }
    } catch (e) {
  ...
  render() {
    ...
    this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="${CONFIG.label}"><span class="user-icon">${CONFIG.emoji}</span><span class="user-value" data-metric="sessions">--</span></div>`;
  ...
  _updateDisplay() {
    ...
    const v = this._element.querySelector(".user-value");
    if (v) {
      v.textContent = s.activeSessions;
      v.classList.remove("status-ok", "status-warn");
      if (s.activeSessions <= 1) v.classList.add("status-ok");
      else v.classList.add("status-warn");
    }
  }
  healthCheck() {
...
const mount = (container, config) => {
  const i = new UserSessionsComponent({ container, config });
  i.init();
  i.mount(container);
  _currentInstance = i;
...
var panel_user_sessions_default = { UserSessionsComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
export {
  MODULE_ID,
  UserSessionsComponent,
  VERSION,
  panel_user_sessions_default as default,
  destroy,
  getPorts,
  getVersion,
  healthCheck,
