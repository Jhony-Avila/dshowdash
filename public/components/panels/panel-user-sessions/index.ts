...
// ═══════════════════════════════════════════════════════════════
// MODULE: panels/panel-user-sessions
// PURPOSE: User Sessions - Enterprise Component with Live Data
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
// abaixo morria em `TypeError: StateStore is not a constructor`. Mesma correção de
// `panel-user-profile` (v9.4.0-RECONNECT) e dos irmãos notifications/preferences.
import Store from './state/store.js';
import { Logger } from './telemetry/logger.js';
import { Tracker } from './telemetry/tracker.js';
export const MODULE_ID = 'panels/panel-user-sessions';
...
const CONFIG = { id: 'user-sessions', area: 'user', label: 'Sessions', emoji: '🔐', apiEndpoint: '/api/user/sessions.php', refreshInterval: 60000 };
export class UserSessionsComponent {
  [key: string]: any;
  constructor(options: Record<string, unknown> = {}) { _initPorts(); this.container = options.container || null; this.config = { ...CONFIG, ...(options.config as Record<string, unknown>) }; this.eventBus = options.eventBus || _getPort('eventBus'); this.store = Store; this.store.setState({ mounted: false, loading: false, error: null, data: null, activeSessions: 0 }); this.lifecycle = new LifecycleManager(this); this.circuitBreaker = new CircuitBreaker({ threshold: 3, timeout: 30000 }); this.logger = new Logger({ prefix: `[${MODULE_ID}]` }); this.tracker = new Tracker({ moduleId: MODULE_ID }); this._mounted = false; this._initialized = false; this._element = null; this._abortController = null; this._refreshTimer = null; this._metrics = { mountCount: 0, errorCount: 0, fetchCount: 0, lastFetchAt: null }; }
  ...
  render() { this._element = document.createElement('div'); this._element.className = `enterprise-component ${CONFIG.id}`; this._element.setAttribute('data-module-id', MODULE_ID); this._element.innerHTML = `<div class="panel-enterprise panel-${CONFIG.id}" title="${CONFIG.label}"><span class="user-icon">${CONFIG.emoji}</span><span class="user-value" data-metric="sessions">--</span></div>`; this.container.innerHTML = ''; this.container.appendChild(this._element); }
  _updateDisplay() { if (!this._element) return; const s = this.store.getState(); const v = this._element.querySelector('.user-value'); if (v) { v.textContent = s.activeSessions; v.classList.remove('status-ok', 'status-warn'); if (s.activeSessions <= 1) v.classList.add('status-ok'); else v.classList.add('status-warn'); } }
...
let _currentInstance: UserSessionsComponent | null = null;

// @ts-expect-error TS migration - TS2554
export const mount = (container, config) => { const i = new UserSessionsComponent({ container, config }); i.init(); i.mount(container); _currentInstance = i; return { success: true, moduleId: MODULE_ID, instance: i }; };
...
export default { UserSessionsComponent, mount, unmount, destroy, healthCheck, getVersion, MODULE_ID, VERSION, injectPorts, getPorts };
