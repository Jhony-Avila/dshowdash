...
const MODULE_ID = "permissions-modules-registry";
const modulePolicies = {
  ...
  "login-modal": { roles: [], minLevel: 0, public: true },
  "preloader": { roles: [], minLevel: 0, public: true },
  "app-shell": { roles: [], minLevel: 0, public: true },
  "main": { roles: ["viewer", "editor", "manager", "admin", "super_admin"], minLevel: 20 },
  "router": { roles: [], minLevel: 0, public: true },
...
function getModulePolicy(moduleId) {
  _metrics.lookups++;
  if (!moduleId) {
    _metrics.misses++;
    return null;
  }
  const normalized = moduleId.toLowerCase().replace(/^module[:\-_]?/, "").replace(/^painel[:\-_]?/, "panel-").replace(/^panel[:\-_]?/, "panel-");
  if (modulePolicies[normalized]) {
    _metrics.hits++;
    return modulePolicies[normalized];
  }
  _metrics.misses++;
  return null;
}
function getModuleActionPolicy(moduleId, action) {
  _metrics.actionLookups++;
  const policy = getModulePolicy(moduleId);
  if (!policy) return null;
  if (policy.actions && policy.actions[action]) {
    const actionPolicy = policy.actions[action];
    return { roles: actionPolicy.roles || policy.roles, minLevel: actionPolicy.minLevel || policy.minLevel };
  }
  return { roles: policy.roles, minLevel: policy.minLevel };
}
function isPublicModule(moduleId) {
  const policy = getModulePolicy(moduleId);
  return policy?.public === true || policy?.minLevel === 0;
}
function getAllModules() {
  return Object.entries(modulePolicies).map(([moduleId, policy]) => ({ moduleId, ...policy }));
}
function getModulesByLevel(minLevel) {
  return Object.entries(modulePolicies).filter(([_, policy]) => (policy.minLevel || 0) >= minLevel).map(([moduleId, policy]) => ({ moduleId, ...policy }));
}
function getEnterprisePanels() {
  const enterpriseIds = ["panel-user-management", "panel-permissions-admin", "panel-session-admin", "panel-audit-trail", "panel-feature-flags-admin", "panel-user-preferences"];
  return enterpriseIds.map((id) => ({ moduleId: id, ...modulePolicies[id] || {} })).filter((p) => p.roles);
}
function registerModule(moduleId, policy) {
  _metrics.registrations++;
  if (!moduleId || !policy) return false;
  modulePolicies[moduleId.toLowerCase()] = policy;
  return true;
}
function validateModulePolicies(knownModules = []) {
  _metrics.validations++;
  const missing = knownModules.filter((moduleId) => !getModulePolicy(moduleId));
  return { valid: missing.length === 0, missing, checked: knownModules.length };
}
function getVersion() {
  return VERSION;
}
function info() {
  const publicCount = Object.values(modulePolicies).filter((p) => p.public || p.minLevel === 0).length;
  const panelCount = Object.keys(modulePolicies).filter((k) => k.startsWith("panel-")).length;
  const enterpriseCount = ["panel-user-management", "panel-permissions-admin", "panel-session-admin", "panel-audit-trail", "panel-feature-flags-admin"].length;
  return { moduleId: MODULE_ID, version: VERSION, totalModules: Object.keys(modulePolicies).length, publicModules: publicCount, panelModules: panelCount, enterprisePanels: enterpriseCount, metrics: { ..._metrics }, hitRate: _metrics.lookups > 0 ? `${(_metrics.hits / _metrics.lookups * 100).toFixed(1)}%` : "N/A", timestamp: Date.now() };
}
function healthCheck() {
  const checks = { policiesDefined: Object.keys(modulePolicies).length > 0, hasPublicModules: Object.values(modulePolicies).some((p) => p.public || p.minLevel === 0), hasProtectedModules: Object.values(modulePolicies).some((p) => !p.public && p.minLevel > 0), structuralModulesExist: ["header", "sidebar", "footer", "app-shell"].every((m) => modulePolicies[m]), enterprisePanelsExist: ["panel-user-management", "panel-permissions-admin"].every((m) => modulePolicies[m]), goodHitRate: _metrics.lookups === 0 || _metrics.hits / _metrics.lookups > 0.5 };
  const passed = Object.values(checks).filter(Boolean).length;
  const total = Object.keys(checks).length;
  return { status: passed === total ? "HEALTHY" : passed >= 4 ? "DEGRADED" : "UNHEALTHY", score: `${passed}/${total}`, healthy: passed >= 5, checks, totalModules: Object.keys(modulePolicies).length, version: VERSION, moduleId: MODULE_ID, timestamp: Date.now() };
}
var modules_default = { modulePolicies, getModulePolicy, getModuleActionPolicy, isPublicModule, getAllModules, getModulesByLevel, getEnterprisePanels, registerModule, validateModulePolicies, getVersion, info, healthCheck };
export {
  MODULE_ID,
  VERSION,
