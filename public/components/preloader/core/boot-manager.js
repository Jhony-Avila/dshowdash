...
const _log = function(level, ...rest) {
  ...
  if (level === "info") {
    if (logger.info) logger.info(`[${MODULE_ID}]`, args.join(" "));
    return;
  }
...
function _setAppState(state) {
  ...
  if (gs && gs.dispatch) {
    try {
      gs.dispatch({ type: "SET_APP_STATE", payload: state });
      _log("info", "GlobalState.dispatch SET_APP_STATE:", state);
    } catch (e) {
      _log("warn", "GlobalState dispatch failed:", e.message);
    }
  }
  if (document.body) {
    document.body.dataset.state = state;
    _log("debug", "body.dataset.state legacy sync:", state);
  }
}
...
BootManager.prototype._decide = function(key, defaultValue, context) {
...
BootManager.prototype._decideShouldLoadVideo = function(config) {
...
BootManager.prototype._decidePodeShowDashboard = function(status, faseAtual, force) {
...
BootManager.prototype._enterDegradedMode = function(reason, uiMessage) {
  const self = this;
  if (self._degradedMode && self._degradedReason === reason) {
    if (uiMessage) {
      self.system.degradedUI.showMessage(uiMessage, self.system.$screen, self.system.addTrace.bind(self.system));
    }
    return;
  }
  ...
  if (self.system.modeManager && typeof self.system.modeManager.setDegraded === "function") {
    self.system.modeManager.setDegraded();
  }
...
BootManager.prototype._trackDegradedMode = function(reason) {
  if (!hasWindow) return;
  try {
    const degradedHistory = JSON.parse(localStorage.getItem("boot-degraded-history") || "[]");
    degradedHistory.push({ bootId: this.system.bootId, reason, timestamp: (/* @__PURE__ */ new Date()).toISOString(), retries: this.componentsReadyTimeoutRetries });
    localStorage.setItem("boot-degraded-history", JSON.stringify(degradedHistory.slice(-10)));
  } catch (e) {
    _log("warn", "Erro ao salvar hist\xF3rico de degrada\xE7\xE3o:", e.message);
  }
};
BootManager.prototype.isDegraded = function() {
...
BootManager.prototype.startBoot = function() {
  ...
  const videoControlledExternally = videoEl && videoEl.dataset.controlledBy;
  ...
  return Promise.all([videoPromise, waitForAuthDecision(1e4)]).then((results) => {
    ...
    if (videoResult.loaded) {
      self.system.emit(PRELOADER_EVENTS.VIDEO_LOADED, { bootId: self.system.bootId, timestamp: Date.now() });
    } else {
      self.system.emit(PRELOADER_EVENTS.VIDEO_ERROR, { bootId: self.system.bootId, reason: videoResult.reason || "unknown", timestamp: Date.now() });
    }
...
BootManager.prototype._startAuthObserver = function() {
  const self = this;
  const config = self.system.config;
  const authObserver = new AuthObserver({ eventBus: self.system.eventBus, cleanupManager: self.system.cleanupManager, timers: self.system.timers, config, addTrace: self.system.addTrace.bind(self.system), transitionPhase: self.system.transitionPhase.bind(self.system), iniciarDashboard() {
    self.iniciarDashboard();
  } });
...
BootManager.prototype.iniciarDashboard = function() {
  ...
  self.system.progressMonitor = new ProgressMonitor(state);
  ...
  if (self.componentsReadyBuffer && !self.componentsReadyBufferConsumed && !self.componentsReadyProcessed) {
    ...
    timers.finalizeBuffered = setTimeout(() => {
      self.system.finalizar("components-ready-buffered");
    }, config.hideDelayMs);
    self.componentsReadyBuffer = null;
  }
};
BootManager.prototype.scheduleComponentsReadyTimeout = function(isRetry) {
  const self = this;
  const config = self.system.config;
  const timers = self.system.timers;
  const state = self.system.state;
  if (isRetry) {
    ...
    if (self.componentsReadyTimeoutRetries >= config.timeoutComponentsReadyMax) {
      ...
      setTimeout(() => {
        self.system.finalizar("max-retries-forced", true);
      }, 3e3);
      return;
    }
  }
  ...
  timers.componentsReady = setTimeout(() => {
    ...
    if (!status.temSessaoValida) {
      _log("warn", "components:ready timeout - nao autenticado");
      self._enterDegradedMode(DEGRADED_REASONS.AUTH_TIMEOUT, "waiting-auth");
      self.scheduleComponentsReadyTimeout(true);
      return;
    }
    if (!monitorAtivo) {
      _log("warn", "components:ready timeout - monitor inativo");
      self._enterDegradedMode(DEGRADED_REASONS.MONITOR_INACTIVE, "waiting-monitor");
      self.scheduleComponentsReadyTimeout(true);
      return;
    }
...
BootManager.prototype.handleComponentsReady = function(event) {
  const self = this;
  const config = self.system.config;
  const timers = self.system.timers;
  if (self.componentsReadyProcessed) return;
  self.componentsReadyReceived = true;
  if (timers.componentsReady) {
    clearTimeout(timers.componentsReady);
    delete timers.componentsReady;
  }
  _log("info", "components:ready recebido!");
  self.system.addTrace("components:ready:received", event.detail);
  if (self._degradedMode) {
    self._exitDegradedMode();
  }
  if (!self.system.progressMonitor && !self.componentsReadyBufferConsumed) {
    _log("info", "Buffering components:ready");
    self.componentsReadyBuffer = event.detail;
    return;
  }
  self.componentsReadyProcessed = true;
  if (self.system.progressMonitor) {
    self.system.progressMonitor.updateUI(100);
  }
  timers.finalize = setTimeout(() => {
    self.system.finalizar("components-ready");
  }, config.hideDelayMs);
};
BootManager.prototype.finalizar = function(motivo, force) {
  const self = this;
  motivo = motivo || "normal";
  const config = self.system.config;
  const state = self.system.state;
  const timers = self.system.timers;
  if (!force && !self.system.allowFinalize) {
    self.system.addTrace("finalize:blocked", { motivo });
    return;
  }
  ...
  if (!force && !podeShowDashboard && motivo !== "debug-force") {
    ...
    if (!status.temSessaoValida) {
      self._enterDegradedMode(DEGRADED_REASONS.AUTH_TIMEOUT, "waiting-auth");
    } else if (!self.system.progressMonitor) {
      self._enterDegradedMode(DEGRADED_REASONS.MONITOR_INACTIVE, "waiting-monitor");
    } else {
      self._enterDegradedMode(DEGRADED_REASONS.UNKNOWN, "degraded");
    }
    return;
  }
  if (self._authGraceTimer) {
    clearTimeout(self._authGraceTimer);
    self._authGraceTimer = null;
  }
  ...
  _log("info", `Decis\xF5es: Bootstrap=${self._decisionsUsed.bootstrap} Default=${self._decisionsUsed.default}`);
  ...
  if (self.system.progressMonitor) {
    self.system.progressMonitor.stop();
  }
  ...
  self.system.hide(() => {
    if (force) {
      if (status.temSessaoValida) {
        showDashboard();
        self.system.transitionPhase(PHASES.VISIBLE, true);
        self.system.addTrace("dashboard:visible:forced");
      } else {
      ...
      _log("warn", "Dashboard NAO exibido - aguardando condicoes");
      self.system.addTrace("dashboard:deferred", { motivo, reason: "waiting-conditions" });
    }
  });
};
BootManager.prototype._saveTelemetry = function(duration, motivo, force) {
  if (!hasWindow) return;
  const self = this;
  try {
    const telemetry = { bootId: self.system.bootId, schema: self.system.config.telemetrySchema, duration, motivo, forced: force, progressoFinal: self.system.state.progressoAtual, degraded: self._degradedMode, degradedReason: self._degradedReason, degradedCount: self._degradedCount, decisionsUsed: Object.assign({}, self._decisionsUsed), bootstrapControlled: BootstrapBridge.isBootstrapControlled(), timestamp: (/* @__PURE__ */ new Date()).toISOString(), bootTrace: self.system.bootTrace.slice(-50), performanceMarks: self.system.getPerformanceMarks() };
    ...
    _log("warn", "Erro telemetria:", e);
  }
};
BootManager.prototype._shouldLoadVideo = function(config) {
  return this._decideShouldLoadVideo(config);
};
BootManager.prototype.reset = function() {
  if (this._authGraceTimer) {
    clearTimeout(this._authGraceTimer);
    this._authGraceTimer = null;
  }
  this.componentsReadyBuffer = null;
  this.componentsReadyBufferConsumed = false;
