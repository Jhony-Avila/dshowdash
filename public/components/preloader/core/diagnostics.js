...
const DIAGNOSTICS_VERSION = "1.4.0-BOOTSTRAP-INTEGRATED";
...
function healthCheck(preloaderInstance) {
  _initPorts();
...
function info(preloaderInstance) {
  _initPorts();
  const progressSource = getProgressSource();
  return {
    version: VERSION,
...
function getStatus(preloaderInstance) {
  return {
    version: VERSION,
    diagnosticsVersion: DIAGNOSTICS_VERSION,
    moduleId: MODULE_ID,
...
var diagnostics_default = { healthCheck, info, getStatus, injectPorts, getPorts, DIAGNOSTICS_VERSION };
export {
  DIAGNOSTICS_VERSION,
