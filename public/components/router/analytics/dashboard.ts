...
// ═══════════════════════════════════════════════════════════════
// MODULE: router.analytics.dashboard
// PURPOSE: Router Analytics Dashboard v1.2.0-P17WI
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   createUiPorts from /core/runtime/ports-profiles.js
//   RouterMetrics from ./metrics.js
//   RouterAudit from ./audit.js
//   DeadRouteDetector from ./dead-routes.js
//
// PROVIDES:
...
const RouterAnalytics = {
  ...
  getTopRoutes(n: number) { if (!n) n = 20; return DeadRouteDetector.getTopRoutes(n); },
  getProblemRoutes() { return { dead: DeadRouteDetector.getDeadRoutes(), slow: RouterMetrics.getSlowestRoutes(10), stale: DeadRouteDetector.getStaleRoutes(7), lowAccess: DeadRouteDetector.getLowAccessRoutes(3) }; },
  getTimeline(minutes: number) { if (!minutes) minutes = 60; const since = Date.now() - (minutes * 60 * 1000); return RouterAudit.getTrail({ from: since, order: 'asc' }); },
  exportReport(format: string) {
    if (!format) format = 'json';
    const data = { generatedAt: new Date().toISOString(), dashboard: this.getDashboard(), problemRoutes: this.getProblemRoutes(), deadRouteReport: DeadRouteDetector.generateReport() };
    if (format === 'text') { return ['═══════════════════════════════════════════════════════════════', '              ROUTER ANALYTICS REPORT', '═══════════════════════════════════════════════════════════════', `Generated: ${data.generatedAt}`, '', 'SUMMARY:', JSON.stringify(this.getSummary(), null, 2), '', data.deadRouteReport].join('\n'); }
    return JSON.stringify(data, null, 2);
  },
  resetAll() { RouterMetrics.reset(); RouterAudit.clear(); DeadRouteDetector.clear(); _log('info', 'All analytics reset'); },
  getStatus() { return { version: VERSION, moduleId: MODULE_ID, portsInitialized: Ports.isInitialized(), modules: { metrics: RouterMetrics.getStatus(), audit: RouterAudit.getStatus(), deadRoutes: DeadRouteDetector.getStatus() } }; },
  healthCheck() {
...
export { RouterAnalytics, VERSION, MODULE_ID, injectPorts, getPorts };
