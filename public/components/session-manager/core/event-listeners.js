...
const MODULE_ID = "session-lifecycle:event-listeners";
let eventBusCleanups = [];
let activityListenerIds = [];
function setupAuthEventListeners() {
  ...
  const handleLoginSuccess = (data) => {
    ...
    if (data && data.csrf) TokenManager.setCsrf(data.csrf);
  ...
  const handleSessionChecked = (data) => {
    if (data && data.source === "session-manager") return;
...
function cleanupEventListeners() {
  eventBusCleanups.forEach((cleanup) => {
    try {
      cleanup();
    } catch (e) {
    }
  ...
  eventBusCleanups = [];
  activityListenerIds.forEach((id) => {
    RuntimePort.removeEventListener(id);
  });
  activityListenerIds = [];
}
function getCleanupCount() {
  return eventBusCleanups.length;
}
function getActivityListenerCount() {
  return activityListenerIds.length;
}
...
function info() {
  return { moduleId: MODULE_ID, version: VERSION, cleanups: eventBusCleanups.length, activityListeners: activityListenerIds.length, p25Compliant: true };
}
var event_listeners_default = { setupAuthEventListeners, setupActivityTracking, cleanupEventListeners, getCleanupCount, getActivityListenerCount, healthCheck, info };
export {
  MODULE_ID,
  VERSION,
  cleanupEventListeners,
  event_listeners_default as default,
  getActivityListenerCount,
  getCleanupCount,
  healthCheck,
  info,
