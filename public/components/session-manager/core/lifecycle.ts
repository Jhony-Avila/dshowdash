// ═══════════════════════════════════════════════════════════════
// DEPENDENCY CONTRACT (6.2.2-SYNTAX-FIX)
// ═══════════════════════════════════════════════════════════════
// MODULE: session-lifecycle
// PURPOSE: Session Manager - Lifecycle
// ───────────────────────────────────────────────────────────────
// IMPORTS:
...
//   initPorts as initPortsBase, getPort as getPortBase, injectPorts as injectPortsBase, getPorts as getPortsBase, isInitialized, getTracker from ./ports.js
//   LIFECYCLE_STATES, lifecycleState, stateMetrics, transitionTo, markDegraded, markFailed, markReady, isInitialized as isStateInitialized, getState, getLifecycleInfo, resetState from ./state-machine.js
//   setupAuthEventListeners, setupActivityTracking, cleanupEventListeners, getCleanupCount, getActivityListenerCount from ./event-listeners.js
//   syncToGlobalState, setupExpirationMonitor, stopRecovery, stopExpirationMonitor, hasExpirationMonitor, hasRecoveryMonitor, getMetrics as getSyncMetrics from ./globalstate-sync.js
//
// PROVIDES:
//   VERSION — module constant
//   MODULE_ID — module constant
//   SessionLifecycle — exported value
...
import { initPorts as initPortsBase, getPort as getPortBase, injectPorts as injectPortsBase, getPorts as getPortsBase, isInitialized, getTracker } from './ports.js';
import { LIFECYCLE_STATES, lifecycleState, stateMetrics, transitionTo, markDegraded, markFailed, markReady, isInitialized as isStateInitialized, getState, getLifecycleInfo, resetState } from './state-machine.js';
...
export const VERSION = '6.2.2-SYNTAX-FIX';
export const MODULE_ID = 'session-lifecycle';
...
function _log(level: string, msg: string, ctx: unknown) {
...
function init(options: Record<string, unknown> & { force?: boolean; endpoints?: Record<string, string>; timeout?: number; trackActivity?: boolean; autoRefresh?: boolean; refreshThreshold?: number; checkOnInit?: boolean }) {
  ...
  return Promise.resolve().then(() => { 
    ...
    if (options.autoRefresh !== false) setupExpirationMonitor(options.refreshThreshold || 300000); 
    ...
    const tracker = getTracker();
    if (tracker && tracker.track) {
      tracker.track('session:lifecycle:init:complete', { 
    ...
    const tracker = getTracker();
    if (tracker && tracker.track) {
      tracker.track('session:lifecycle:init:error', { error: error.message });
    }
...
function healthCheck() { 
  ...
  const checks = { 
    stateValid: Object.values(LIFECYCLE_STATES).indexOf(lifecycleState.state as any) !== -1,
