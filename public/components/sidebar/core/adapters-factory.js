...
const MODULE_ID = "sidebar-adapters-factory";
...
function createFallbackRouterAdapter() {
  ...
  return {
    ...
    onRouteChange(callback) {
      ...
      if (eb && eb.on) {
        const handler = (data) => {
          const path = data && data.path ? data.path : typeof window !== "undefined" && window.location ? window.location.hash : "#/";
    ...
    on(event, callback) {
      try {
        ...
        if (eb && eb.on) {
          ...
          return () => {
            if (eb.off) eb.off(event, callback);
          };
        }
      } catch (e) {
      }
