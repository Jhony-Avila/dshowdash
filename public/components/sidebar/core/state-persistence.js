...
const MODULE_ID = "sidebar-state-persistence";
...
function saveCollapsedState(collapsed) {
  try {
    ...
    localStorage.setItem(STORAGE_KEY_UNIFIED, value);
...
function restoreCollapsedState() {
  try {
    ...
    if (data === null) {
      ...
      if (data !== null) {
        localStorage.setItem(STORAGE_KEY_UNIFIED, data);
        _metrics.migrations++;
      }
    }
    _metrics.loads++;
...
function forceSync() {
  try {
    ...
    if (unified !== null) localStorage.setItem(STORAGE_KEY_LEGACY, unified);
      ...
      const legacy = localStorage.getItem(STORAGE_KEY_LEGACY);
      if (legacy !== null) {
        localStorage.setItem(STORAGE_KEY_UNIFIED, legacy);
        _metrics.migrations++;
      }
    }
    return { success: true };
...
function resetMetrics() {
  _metrics = { saves: 0, loads: 0, errors: 0, migrations: 0 };
}
function info() {
  return { moduleId: MODULE_ID, version: VERSION, syncStatus: checkSync(), metrics: getMetrics() };
}
...
var state_persistence_default = { saveState, loadState, clearState, saveCollapsedState, restoreCollapsedState, checkSync, forceSync, getMetrics, resetMetrics, info, healthCheck, VERSION, MODULE_ID };
export {
  MODULE_ID,
  VERSION,
  checkSync,
