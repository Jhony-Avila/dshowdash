...
// ═══════════════════════════════════════════════════════════════
// DEPENDENCY CONTRACT (5.7.0-DUAL-AAA)
// ═══════════════════════════════════════════════════════════════
// MODULE: sidebar-engine
// PURPOSE: Sidebar V2 - Engine (Domain Core)
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   SIDEBAR_EVENTS from /core/runtime/events/catalog/sidebar.events.js
//   SidebarStateMachine, LIFECYCLE_STATES from ./state-machine.js
//
// PROVIDES:
//   VERSION — module constant
//   MODULE_ID — module constant
//   createEngine() — exported function
...
export const VERSION = '5.7.0-DUAL-LOOKUP';
export const MODULE_ID = 'sidebar-engine';

export class SidebarEngine {
  [key: string]: any;
  constructor(options: { defaultCollapsed?: boolean; accordion?: DynObj } = {}) {
    ...
    this._ports = null;
    this._metrics = {
      inits: 0, mounts: 0, readys: 0, destroys: 0,
      toggles: 0, collapses: 0, expands: 0,
      sectionToggles: 0, navigations: 0, badgeUpdates: 0, eventsEmitted: 0,
      keyLookups: 0
    };
  }

  setPorts(ports: DynObj) { this._ports = ports; }
  ...
  _autoExpandNonCollapsibleSections() {
    const nonCollapsible = this._sections.filter((s: DynObj) => s.collapsible === false).map((s: DynObj) => s.id);
  ...
  getItemById(id: string) {
    // Direct match by id (numeric or exact match)
    const byId = this._items.find((item: DynObj) => item.id === id);
    if (byId) return byId;

    // Fallback: match by item_key or key (string lookup)
    if (typeof id === 'string') {
      this._metrics.keyLookups++;
      return this._items.find((item: DynObj) => item.key === id || item.item_key === id) || null;
    }

    return null;
  }

  getSectionById(id: DynObj) { return this._sections.find((section: DynObj) => section.id === id) || null; }
  getItemsBySection(sectionId: string) { return this._items.filter((item: DynObj) => item.sectionId === sectionId); }
  ...
  expandAllSections() {
    const collapsibleIds = this._sections.filter((s: DynObj) => s.collapsible !== false).map((s: DynObj) => s.id);
    return this._stateMachine.expandAllSections(collapsibleIds);
  }

  collapseAllSections() { return this._stateMachine.collapseAllSections(); }
  getExpandedSections() { return this._stateMachine.expandedSectionsArray; }
  setAccordionMode(allowMultiple: DynObj) { return this._stateMachine.setAccordionMode(allowMultiple); }
  ...
  getBadge(itemId: string) { return this._badges.get(itemId) || null; }
  removeBadge(itemId: string) { this._badges.delete(itemId); return { success: true }; }

  // STATE PROXIES
  get collapsed() { return this._stateMachine.collapsed; }
  get expanded() { return this._stateMachine.expanded; }
  get mobileOpen() { return this._stateMachine.mobileOpen; }
  get isMobile() { return this._stateMachine.isMobile; }
  get activeItemId() { return this._stateMachine.activeItemId; }
  get lifecycle() { return this._stateMachine.lifecycle; }
  get expandedSections() { return this._stateMachine.expandedSections; }
  ...
  // MOBILE STATE
  setMobile(isMobile: boolean) { return this._stateMachine.setMobile(isMobile); }
  ...
  // EVENTS
  on(event: string, handler: DynObj) {
  ...
  off(event: string, handler: DynObj) {
    if (handler) this._eventHandlers.get(event)?.delete(handler);
    else this._eventHandlers.delete(event);
  }

  _emit(event: string, data: DynObj) {
    this._metrics.eventsEmitted++;
    const handlers = this._eventHandlers.get(event);
    if (handlers) {
      handlers.forEach((fn: DynObj) => { try { fn({ event, data, timestamp: Date.now() }); } catch { /* handler error */ } });
    }
    if (this._ports?.events?.emit) this._ports.events.emit(event, data);
  }

  // STATE SUBSCRIPTION
  subscribe(callback: DynObj) { return this._stateMachine.subscribe(callback); }
  ...
  getState() {
    return {
      ...
      collapsibleSectionsCount: this._sections.filter((s: DynObj) => s.collapsible !== false).length
    };
  }
  ...
  healthCheck() {
    ...
    return {
      ...
      checks, lifecycle: this._stateMachine.lifecycle,
      itemsCount: this._items.length, sectionsCount: this._sections.length, badgesCount: this._badges.size,
      expandedSectionsCount: this._stateMachine.expandedSections.size,
      portsConnected: this._ports !== null, stateMachineHealth: smHealth,
...
export function createEngine(options: DynObj) { return new SidebarEngine(options); }
