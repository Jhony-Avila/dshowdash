...
// ═══════════════════════════════════════════════════════════════
// MODULE: loader
// PURPOSE: Accordion NCS - Accordion Loader
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   CONTAINER_ID, state, log, initPorts from ./constants.js
//   createContainer, removeContainer, waitForNavContent from ./container.js
//   hideLegacyNav, showLegacyNav from ./legacy-nav.js
//   convertModelToAccordionStructure from ./model-converter.js
//   setupNavigationHandler from ./navigation-handler.js
//   NavigationModelLoader from ../../integration/navigation-model-loader.js
//
// PROVIDES:
//   unloadAccordion() — exported function
//
...
export const MODULE_ID = 'sidebar.features.accordion-ncs.loader';
...
export async function loadAccordionFallback() {
    try {
...
export async function loadAccordion() {
    initPorts();
    ...
    try {
        ...
        if (!loadResult.success && loadResult.reason === 'already-loading') {
            ...
            log('warn', `Model load failed: ${loadResult.reason || (loadResult as any).error}`);
            return loadAccordionFallback();
        }
        ...
        if (result.success) {
            ...
            state.accordion.modelVersion = (model as DynObj).version;
            state.enabled = true;
            hideLegacyNav();
            setupNavigationHandler(result);
            ...
            return {
                ...
                modelVersion: (model as DynObj).version
            };
        } else {
...
export function unloadAccordion() {
