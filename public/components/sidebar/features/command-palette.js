...
const _state = { initialized: false, ctx: null, modal: null, input: null, results: null, commands: [], registered: [], filtered: [], selectedIndex: 0, opens: 0, executions: 0, errors: 0, cleanups: [] };
...
const DEFAULT_COMMANDS = [
  { id: "toggle-sidebar", label: "Alternar Barra Lateral", shortcut: "Ctrl+B", icon: CP_SVGS.chevronLeft, keywords: ["toggle", "sidebar", "barra", "lateral", "menu", "recolher"], action() {
    const s = _getPort("sidebar");
    if (s && s.toggle) s.toggle();
  } },
  { id: "collapse-all", label: "Recolher Todas as Se\xE7\xF5es", icon: CP_SVGS.folder, keywords: ["collapse", "recolher", "fechar", "secoes", "grupos"], action() {
    const s = _getPort("sidebar");
    if (s && s.collapseAllSections) s.collapseAllSections();
  } },
  { id: "expand-all", label: "Expandir Todas as Se\xE7\xF5es", icon: CP_SVGS.folderOpen, keywords: ["expand", "expandir", "abrir", "secoes", "grupos"], action() {
    const s = _getPort("sidebar");
    if (s && s.expandAllSections) s.expandAllSections();
  } },
  { id: "toggle-theme", label: "Alternar Tema", icon: CP_SVGS.palette, keywords: ["theme", "tema", "claro", "escuro", "dark", "light", "modo"], action() {
    const s = _getPort("sidebar");
    if (s && s.toggleTheme) s.toggleTheme();
  } },
  { id: "mini-mode", label: "Alternar Modo Compacto", icon: CP_SVGS.pin, keywords: ["mini", "compacto", "colapsar", "estreito"], action() {
    const s = _getPort("sidebar");
    if (s && s.toggleMiniMode) s.toggleMiniMode();
  } }
];
// Gera comandos de NAVEGAÇÃO a partir dos itens reais da sidebar (título pt-BR + rota),
// para que Ctrl+K encontre painéis por nome e navegue. Reconstruído a cada abertura
// (reflete a navegação atual, inclusive itens carregados dinamicamente).
  ...
  try {
    ...
    links.forEach((a) => {
      ...
      out.push({
        ...
        action() {
          // Dispara a navegação NATIVA da sidebar (handler delegado lê data-item-id →
          // routerAdapter.navigate). Clica o próprio <a> capturado; fallback re-query/href.
          if (linkEl && linkEl.isConnected && linkEl.click) { linkEl.click(); return; }
          const link = document.querySelector('.dsd-sidebar__link[href="' + href + '"]');
...
function createModal() {
  ...
  const inputHandler = () => {
    search(_state.input.value);
  };
  _state.input.addEventListener("input", inputHandler);
  _state.cleanups.push(() => {
    _state.input.removeEventListener("input", inputHandler);
  });
  const keyHandler = (e) => {
    ...
    if (e.key === "ArrowDown") {
      ...
      e.preventDefault();
      executeItem(_state.selectedIndex);
  ...
  _state.cleanups.push(() => {
    _state.input.removeEventListener("keydown", keyHandler);
  });
}
function search(query) {
  const q = query.toLowerCase();
  const filtered = _state.commands.filter((c) => c.label.toLowerCase().indexOf(q) !== -1 || c.id.toLowerCase().indexOf(q) !== -1 || (c.keywords && c.keywords.some((k) => (k || "").toLowerCase().indexOf(q) !== -1)));
  _state.selectedIndex = 0;
...
function executeItem(index) {
  const filtered = _state.filtered && _state.filtered.length ? _state.filtered : _state.commands;
...
function registerCommand(command) {
  if (!command || !command.id) return _envelope(false, null, [{ code: "INVALID", message: "Command must have id" }]);
  const existing = _state.registered.findIndex((c) => c.id === command.id);
...
var command_palette_default = { id: MODULE_ID, version: VERSION, capabilities, init, cleanup, healthCheck, info, getMetrics, show, hide, toggle, registerCommand };
export {
  MODULE_ID,
  VERSION,
  capabilities,
