...
const MODULE_ID = "sidebar-ui-constants";
const CAPABILITIES = Object.freeze({ accordion: true, badges: true, search: true, keyboard: true, mobile: true, persistence: true, tooltips: true, skeleton: true, ripple: true });
const SECTION_ICONS = Object.freeze({ main: "dashboard", operacional: "cog", analytics: "chart", management: "settings", admin: "shield", financeiro: "money", comercial: "chart", default: "nav" });
const SECTION_COLORS = Object.freeze({ main: "var(--sidebar-accent-primary, #7B6EF6)", operacional: "var(--sidebar-accent-primary, #7B6EF6)", admin: "var(--sidebar-admin-color, #F59E0B)", default: "var(--sidebar-text-muted, #6B7280)" });
...
const FALLBACK_NAV_HTML = `<div class="${CSS_CLASSES.EMPTY}"><span class="${CSS_CLASSES.EMPTY_ICON}">\u{1F4CB}</span><span class="${CSS_CLASSES.EMPTY_TEXT}">Nenhum item dispon\xEDvel</span></div>`;
const FALLBACK_SIDEBAR_HTML = `<aside class="${CSS_CLASSES.ROOT} ${CSS_CLASSES.MOD_FALLBACK}" id="sidebar" role="navigation"><div class="${CSS_CLASSES.HEADER}"><span class="${CSS_CLASSES.BRAND_TEXT}">Menu</span></div><nav class="${CSS_CLASSES.NAV}"><div class="${CSS_CLASSES.NAV_CONTENT}">${FALLBACK_NAV_HTML}</div></nav></aside>`;
function getMetrics() {
  return {
...
var constants_default = { VERSION, MODULE_ID, CAPABILITIES, SECTION_ICONS, SECTION_COLORS, CSS_CLASSES, STORAGE_KEYS, TELEMETRY_EVENTS, SELECTORS, FALLBACK_NAV_HTML, FALLBACK_SIDEBAR_HTML, getMetrics, info, healthCheck };
export {
  CAPABILITIES,
  CSS_CLASSES,
  FALLBACK_NAV_HTML,
  FALLBACK_SIDEBAR_HTML,
  MODULE_ID,
  SECTION_COLORS,
  SECTION_ICONS,
  SELECTORS,
