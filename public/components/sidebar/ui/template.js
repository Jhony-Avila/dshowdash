...
const VERSION = "5.9.0-ICON-REGISTRY";
const MODULE_ID = "sidebar-template";
...
function getIconSvg(iconName) {
  ...
  if (iconName.includes(":")) {
    const registrySvg2 = _getFromRegistry(iconName);
    if (registrySvg2) return registrySvg2;
  }
  const registrySvg = _resolveFromRegistry(iconName);
  if (registrySvg) return registrySvg;
  return ICONS.default;
}
function _getFromRegistry(fullName) {
  try {
    return registryGet(fullName);
  } catch {
    return null;
  }
}
function _resolveFromRegistry(shortName) {
  try {
...
function createTemplate(options = {}) {
  _metrics.templatesCreated++;
  return '<aside class="dsd-sidebar" id="sidebar" role="navigation" aria-label="Menu principal"><div class="dsd-sidebar__header dsd-sidebar__header--with-search"><div class="dsd-sidebar__search" role="search"><div class="dsd-sidebar__search-wrapper"><span class="dsd-sidebar__search-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg></span><input type="text" class="dsd-sidebar__search-input" placeholder="Buscar..." aria-label="Buscar p\xE1gina"/><span class="dsd-sidebar__search-clear" role="button" aria-label="Limpar busca" tabindex="0"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></span></div></div><button class="dsd-sidebar__toggle" type="button" aria-label="Recolher menu" aria-expanded="true"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 18l-6-6 6-6"/></svg></button></div><nav class="dsd-sidebar__nav"><div class="dsd-sidebar__nav-content" data-slot="nav-items"></div></nav><div class="dsd-sidebar__keyboard-hints sr-only">Use setas para navegar. Enter para selecionar. Ctrl+B para alternar sidebar.</div><div class="dsd-sidebar__announce sr-only" role="status" aria-live="polite" aria-atomic="true"></div></aside>';
}
function createTemplateElement(options = {}) {
  _metrics.templatesCreated++;
  const aside = _createElement("aside", { className: "dsd-sidebar", id: "sidebar", attributes: { "role": "navigation", "aria-label": "Menu principal" } });
  const header = _createElement("div", { className: "dsd-sidebar__header dsd-sidebar__header--with-search" });
  const search = _createElement("div", { className: "dsd-sidebar__search", attributes: { "role": "search" } });
  const searchWrapper = _createElement("div", { className: "dsd-sidebar__search-wrapper" });
  const searchIcon = _createElement("span", { className: "dsd-sidebar__search-icon" });
  ...
  const searchInput = _createElement("input", { className: "dsd-sidebar__search-input", attributes: { "type": "text", "placeholder": "Buscar...", "aria-label": "Buscar p\xE1gina" } });
  const searchClear = _createElement("span", { className: "dsd-sidebar__search-clear", attributes: { "role": "button", "aria-label": "Limpar busca", "tabindex": "0" } });
  searchClear.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>';
  searchWrapper.appendChild(searchIcon);
  searchWrapper.appendChild(searchInput);
  searchWrapper.appendChild(searchClear);
  search.appendChild(searchWrapper);
  const toggle = _createElement("button", { className: "dsd-sidebar__toggle", attributes: { "type": "button", "aria-label": "Recolher menu", "aria-expanded": "true" } });
  toggle.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 18l-6-6 6-6"/></svg>';
  header.appendChild(search);
  header.appendChild(toggle);
  const nav = _createElement("nav", { className: "dsd-sidebar__nav" });
  const navContent = _createElement("div", { className: "dsd-sidebar__nav-content", attributes: { "data-slot": "nav-items" } });
  nav.appendChild(navContent);
  const hints = _createElement("div", { className: "dsd-sidebar__keyboard-hints sr-only", textContent: "Use setas para navegar. Enter para selecionar. Ctrl+B para alternar sidebar." });
  const announce = _createElement("div", { className: "dsd-sidebar__announce sr-only", attributes: { "role": "status", "aria-live": "polite", "aria-atomic": "true" } });
  aside.appendChild(header);
  aside.appendChild(nav);
  aside.appendChild(hints);
  aside.appendChild(announce);
  return aside;
}
function createItemTemplate(item, isActive = false, badge = null) {
  ...
  const badgeHtml = badge ? `<span class="dsd-sidebar__badge dsd-sidebar__badge--${badge.type || "info"}${badge.pulse ? " dsd-sidebar__badge--pulse" : ""}" ${badge.pulse ? 'data-pulse="true"' : ""} aria-label="${badge.value} ${badge.type === "alert" ? "alertas" : "notifica\xE7\xF5es"}">${badge.value}</span>` : "";
...
function getMetrics() {
  return Object.assign({}, _metrics, { iconsCount: Object.keys(ICONS).length });
}
function info() {
  return {
    moduleId: MODULE_ID,
...
var template_default = { VERSION, MODULE_ID, createTemplate, createTemplateElement, createItemTemplate, getIconSvg, ICONS, info, getMetrics, healthCheck };
export {
  ICONS,
  MODULE_ID,
  VERSION,
