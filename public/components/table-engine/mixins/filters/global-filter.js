...
const MODULE_ID = "table-engine:global-filter";
const GF_SVGS = {
...
function applyGlobalFilter(data, query, columns, options = {}) {
  const opts = options || {};
...
function renderGlobalFilter(currentQuery, options) {
  const opts = options || {};
  const p = opts.cssPrefix || "tbl-";
  const placeholder = opts.placeholder || "Buscar em todas as colunas...";
  return `<div class="${p}global-filter"><div class="${p}global-filter-wrapper"><span class="${p}global-filter-icon">\u{1F50D}</span><input type="text" class="${p}global-filter-input" placeholder="${placeholder}" value="${currentQuery || ""}" data-action="global-search" />${currentQuery ? `<button class="${p}global-filter-clear" data-action="clear-global-search" title="Limpar">${GF_SVGS.clear}</button>` : ""}</div>${opts.showHint ? `<span class="${p}global-filter-hint">Pressione Enter para buscar</span>` : ""}</div>`;
}
...
var global_filter_default = { applyGlobalFilter, renderGlobalFilter, highlightMatches, info, healthCheck, VERSION, MODULE_ID };
export {
  MODULE_ID,
  VERSION,
  applyGlobalFilter,
  global_filter_default as default,
  healthCheck,
  highlightMatches,
  info,
  renderGlobalFilter
