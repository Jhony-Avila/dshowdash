...
// ═══════════════════════════════════════════════════════════════
// MODULE: table-engine:render
// PURPOSE: Table Engine - Render Module
// ───────────────────────────────────────────────────────────────
// IMPORTS:
//   renderCell from ./cell-renderers.js
//   renderSkeleton, renderEmpty, renderError, renderLoadingOverlay, renderNoResults from ./templates.js
...
export const MODULE_ID = 'table-engine:render';
...
export function renderTable(state: Record<string, unknown> & { get: (key: string) => unknown; getFilteredData: () => Record<string, unknown>[] }, options: Record<string, unknown>) {
  const opts = options || {};
  const p = opts.cssPrefix || 'tbl-';
  const loading = state.get('loading');
  const error = state.get('error');
  const data = state.getFilteredData();
  const columns = (state.get('visibleColumns') || state.get('columns') || []) as Record<string, unknown>[];
  ...
  if (!data.length) { if (searchQuery) return renderNoResults(Object.assign({}, opts, { searchQuery })); return renderEmpty(opts); }
...
export function renderHeader(columns: Record<string, unknown>[], state: Record<string, unknown>, options: Record<string, unknown>) {
  const opts = options || {};
  const p = opts.cssPrefix || 'tbl-';
  const sort = ((state as Record<string, unknown> & { getSort?: () => Record<string, unknown> }).getSort?.() || {}) as Record<string, unknown>;
  const selectable = opts.selectable !== false;
  let cells = '';
  if (selectable) {
    const stateWithMethods = state as Record<string, unknown> & { getSelection?: () => Set<string>; getFilteredData?: () => Record<string, unknown>[] };
  ...
  columns.forEach(col => {
    ...
    cells += `<th class="${p}th ${sortable ? `${p}sortable` : ''} ${sortDir ? `${p}sorted-${sortDir}` : ''}" data-col="${col.id}" ${width}><div class="${p}th-content">${reorderable ? `<span class="${p}th-grip" draggable="true">::</span>` : ''}<span class="${p}th-label" ${sortable ? `data-action="sort" data-sort="${col.id}"` : ''}>${formatters.escapeHtml((col.label || col.id) as string | number)}</span>${sortIcon ? `<span class="${p}th-sort-icon">${sortIcon}</span>` : ''}${resizable ? `<div class="${p}th-resizer"></div>` : ''}</div></th>`;
  });
...
export function renderBody(data: Record<string, unknown>[], columns: Record<string, unknown>[], state: Record<string, unknown>, options: Record<string, unknown>) {
  const opts = options || {};
  const p = opts.cssPrefix || 'tbl-';
  const stateEx = state as Record<string, unknown> & { getPagination?: () => { page: number; pageSize: number }; getSelection?: () => Set<string>; getExpanded?: () => Set<string> };
  const pagination = (stateEx.getPagination?.() || { page: 1, pageSize: 25 });
  const start = (pagination.page - 1) * pagination.pageSize;
  const end = start + pagination.pageSize;
  const pageData = data.slice(start, end);
  const selection = (stateEx.getSelection?.() || new Set<string>());
  const expanded = (stateEx.getExpanded?.() || new Set<string>());
  const selectable = opts.selectable !== false;
  ...
  pageData.forEach((row, index) => {
    ...
    if (opts.hoverActions !== false) {
      const actions = (opts.rowActions || [{ action: 'view', icon: '👁', title: 'Ver' }, { action: 'edit', icon: '✏️', title: 'Editar' }, { action: 'delete', icon: '🗑', title: 'Excluir', danger: true }]) as Record<string, unknown>[];
      cells += `<td class="${p}td ${p}td-hover-actions"><div class="${p}hover-actions">${actions.map((a: Record<string, unknown>) => `<button class="${p}hover-btn ${a.danger ? `${p}hover-btn-danger` : ''}" data-action="${a.action}" data-id="${rowId}" title="${a.title}">${a.icon}</button>`).join('')}</div></td>`;
    }
    ...
    if (isExpanded && opts.renderExpansion) {
      const expansionContent = (opts.renderExpansion as (row: Record<string, unknown>, ctx: Record<string, unknown>) => string)(row, { formatters, cssPrefix: p });
...
export function renderFooter(data: Record<string, unknown>[], columns: Record<string, unknown>[], state: Record<string, unknown>, options: Record<string, unknown>) {
  const opts = options || {};
  const p = opts.cssPrefix || 'tbl-';
  const selectable = opts.selectable !== false;
  ...
  columns.forEach(col => {
    let content = '';
    if ((totals as Record<string, unknown>)[col.id as string] !== undefined) { content = renderCell((totals as Record<string, unknown>)[col.id as string], col, {}, opts); }
    else if (col.aggregate) {
      const values = data.map(row => parseFloat(row[col.id as string] as string) || 0);
      let aggregated = 0;
...
export function renderPagination(state: Record<string, unknown>, options: Record<string, unknown>) {
  const opts = options || {};
  const p = opts.cssPrefix || 'tbl-';
  const pagination = ((state as Record<string, unknown> & { getPagination?: () => { page: number; pageSize: number; total: number } }).getPagination?.() || { page: 1, pageSize: 25, total: 0 });
  const totalPages = Math.ceil(pagination.total / pagination.pageSize) || 1;
  const page = pagination.page;
  const pageSize = pagination.pageSize;
  const total = pagination.total;
  const pageSizes = (opts.pageSizes || [10, 25, 50, 100]) as number[];
  ...
  return `<div class="${p}pagination"><div class="${p}pagination-info">Mostrando ${start}–${end} de ${total} registros</div><div class="${p}pagination-controls"><button class="${p}page-btn ${p}page-prev" data-action="page" data-page="${page - 1}" ${page <= 1 ? 'disabled' : ''}>${RENDER_SVGS.prevPage}</button>${pageButtons}<button class="${p}page-btn ${p}page-next" data-action="page" data-page="${page + 1}" ${page >= totalPages ? 'disabled' : ''}>${RENDER_SVGS.nextPage}</button></div><div class="${p}pagination-size"><select class="${p}page-size" data-action="page-size">${pageSizes.map((size: number) => `<option value="${size}" ${size === pageSize ? 'selected' : ''}>${size} por página</option>`).join('')}</select></div></div>`;
}

export function renderToolbar(state: Record<string, unknown> & { get: (key: string) => unknown; getSelection?: () => Set<string> }, options: Record<string, unknown>) {
  const opts = options || {};
  const p = opts.cssPrefix || 'tbl-';
  const selection = (state.getSelection && state.getSelection()) || new Set();
  const selectionCount = selection.size;
  return `<div class="${p}toolbar"><div class="${p}toolbar-left"><div class="${p}search-wrapper"><input type="text" class="${p}search-input" placeholder="Buscar..." value="${state.get('search') || ''}" /><button class="${p}search-clear" data-action="clear-search">${RENDER_SVGS.clear}</button></div></div><div class="${p}toolbar-center">${selectionCount > 0 ? `<div class="${p}bulk-bar ${p}visible"><span class="${p}bulk-count">${selectionCount} selecionado${selectionCount > 1 ? 's' : ''}</span><button class="${p}bulk-btn" data-action="bulk-export">📤 Exportar</button><button class="${p}bulk-btn ${p}bulk-danger" data-action="bulk-delete">🗑 Excluir</button><button class="${p}bulk-btn" data-action="clear-selection">${RENDER_SVGS.clear} Limpar</button></div>` : ''}</div><div class="${p}toolbar-right"><button class="${p}toolbar-btn" data-action="export-csv" title="Exportar CSV">📄</button><button class="${p}toolbar-btn" data-action="export-excel" title="Exportar Excel">📊</button><button class="${p}toolbar-btn" data-action="toggle-columns" title="Colunas">⚙️</button><button class="${p}toolbar-btn" data-action="refresh" title="Atualizar">🔄</button></div></div>`;
}

export function info() { return { moduleId: MODULE_ID, version: VERSION }; }
export function healthCheck() { return { status: 'HEALTHY', moduleId: MODULE_ID, version: VERSION }; }

export default { renderTable, renderHeader, renderBody, renderFooter, renderPagination, renderToolbar, renderSkeleton, renderEmpty, renderError, info, healthCheck, VERSION, MODULE_ID };
