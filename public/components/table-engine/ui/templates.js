...
const MODULE_ID = "table-engine:templates";
const TEMPLATE_SVGS = {
...
function renderSkeleton(options) {
  ...
  const rows = opts.skeletonRows || 5;
  const cols = opts.columns && opts.columns.length || 5;
  let headerCells = "";
...
function renderEmpty(options) {
  const opts = options || {};
  const p = opts.cssPrefix || "tbl-";
  const icon = opts.emptyIcon || "\u{1F4ED}";
...
function renderError(options) {
  ...
  const message = opts.errorMessage || opts.error && opts.error.message || "Ocorreu um erro inesperado. Por favor, tente novamente.";
...
function renderLoadingOverlay(options) {
  const opts = options || {};
  const p = opts.cssPrefix || "tbl-";
...
function renderNoResults(options) {
  const opts = options || {};
  const p = opts.cssPrefix || "tbl-";
  const query = opts.searchQuery || "";
  return `<div class="${p}no-results"><div class="${p}no-results-icon">\u{1F50D}</div><h3 class="${p}no-results-title">Nenhum resultado para "${query}"</h3><p class="${p}no-results-message">Tente buscar por outros termos.</p><button class="${p}no-results-clear" data-action="clear-search">Limpar busca</button></div>`;
}
function info() {
  return { moduleId: MODULE_ID, version: VERSION, templates: ["skeleton", "empty", "error", "loading", "noResults"] };
}
function healthCheck() {
  return { status: "HEALTHY", moduleId: MODULE_ID, version: VERSION };
}
var templates_default = { renderSkeleton, renderEmpty, renderError, renderLoadingOverlay, renderNoResults, info, healthCheck, VERSION, MODULE_ID };
export {
  MODULE_ID,
  VERSION,
