...
const MODULE_ID = "ticker.ui.modal";
...
class NewsModal {
  constructor(options = {}) {
    ...
    this._metrics = { opens: 0, closes: 0, shares: 0, linkClicks: 0 };
    this._boundKeyHandler = this._onKeyDown.bind(this);
    this._boundClickHandler = this._onOverlayClick.bind(this);
  }
  init() {
    this._injectStyles();
    this._createModal();
    _log("debug", "NewsModal initialized");
    return this;
  }
  _injectStyles() {
    if (!hasDocument || document.getElementById("ticker-modal-styles")) return;
    const style = document.createElement("style");
    style.id = "ticker-modal-styles";
  ...
  _createModal() {
    ...
    this._overlay.className = "ticker-modal-overlay";
    this._overlay.setAttribute("role", "dialog");
    this._overlay.setAttribute("aria-modal", "true");
    this._overlay.setAttribute("aria-hidden", "true");
    this._modal = document.createElement("div");
    this._modal.className = "ticker-modal";
    this._overlay.appendChild(this._modal);
    document.body.appendChild(this._overlay);
    this._overlay.addEventListener("click", this._boundClickHandler);
  }
  open(item) {
    if (!item || this._isOpen || !hasDocument) return;
    this._currentItem = item;
    this._previousFocus = document.activeElement;
    this._renderContent(item);
    this._overlay.classList.add("ticker-modal-overlay--visible");
    ...
    setTimeout(() => {
      const closeBtn = this._modal.querySelector(".ticker-modal-close");
      if (closeBtn) closeBtn.focus();
    }, 100);
    _log("debug", "Modal opened", { title: item.title?.substring(0, 30) });
  }
  close() {
    if (!this._isOpen || !hasDocument) return;
    this._overlay.classList.remove("ticker-modal-overlay--visible");
    this._overlay.setAttribute("aria-hidden", "true");
    this._isOpen = false;
    this._currentItem = null;
    this._metrics.closes++;
  ...
  healthCheck() {
    const logger = _getPort("logger");
    const checks = { hasOverlay: !!this._overlay, hasModal: !!this._modal, loggerReady: !!logger, portsInitialized: Ports.isInitialized() };
    const passed = Object.values(checks).filter(Boolean).length;
    return { status: passed === 4 ? "HEALTHY" : "DEGRADED", score: `${passed}/4`, isOpen: this._isOpen, checks, metrics: { ...this._metrics }, version: VERSION, moduleId: MODULE_ID, portsInitialized: Ports.isInitialized() };
  }
  info() {
    return { version: VERSION, moduleId: MODULE_ID, isOpen: this._isOpen, metrics: { ...this._metrics }, portsInitialized: Ports.isInitialized() };
  }
}
function getVersion() {
  return VERSION;
}
var modal_default = NewsModal;
export {
  MODULE_ID,
  NewsModal,
  VERSION,
