...
// ── A) flag ON ──────────────────────────────────────────────────────
{
  const { navegador: b, pagina: p, erros } = await abrir({
    ...
    init: () => { localStorage.setItem('dshow.avst.flags.v1', JSON.stringify({ 'as5.novo_shell': true, 'as6.tax_v2': false, 'as6.dock_inferior': false, 'as5.palco3d': false })); },
  });
  try {
...
// ── B) rollback §651 ────────────────────────────────────────────────
{
  const { navegador: b, pagina: p, erros } = await abrir({
    viewport: { width: 1500, height: 940 },
    init: () => { localStorage.setItem('dshow.avst.flags.v1', JSON.stringify({ 'as5.novo_shell': true, 'as6.tax_v2': false, 'as6.dock_inferior': false, 'as5.palco3d': false, 'as6.contexto': false })); },
  });
  try {
    await irParaHarness(p, 'avst-harness.html', 1200);
    await escreverBusca(p, 'neon');
