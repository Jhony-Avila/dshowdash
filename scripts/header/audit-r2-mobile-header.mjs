...
// @version 1.0.1  @created 2026-09-11  (lote as6.mobile_header_v2 — doc 23, decisões #80–#87)
// 1.0.1: MH2_EXTRA_FLAGS (ex.: as6.shell_layout_v2) p/ medir a composição do canário; com o layout v2 ON o
//        fluxo do rodapé é dele (gate FOOTER_IN_NORMAL_FLOW aceita data-shell-layout-v2="on").
//
...
const EXTRA_FLAGS = String(process.env.MH2_EXTRA_FLAGS || '').split(',').map((s) => s.trim()).filter(Boolean); // ex.: as6.shell_layout_v2 (composição u75)
...
async function cenario({ vw, vh, theme, route }) {
  ...
  await ctx.addInitScript(({ theme, extras }) => { try { localStorage.setItem('cm_theme', theme); localStorage.setItem('dshowdash_theme_prefs', JSON.stringify({ theme, density: 'comfortable' })); const k = 'dshow.avst.flags.v1'; const cur = JSON.parse(localStorage.getItem(k) || '{}'); for (const f of ['as6.visual_composer', 'as6.vc_3d', 'as6.shell_vc3d', 'as6.vestuario_separado', 'as6.catalogo_v2']) cur[f] = true; cur['as6.mobile_header_v2'] = true; for (const f of (extras || [])) cur[f] = true; localStorage.setItem(k, JSON.stringify(cur)); localStorage.setItem('avst.vc.onboarded', '1'); } catch {} }, { theme, extras: EXTRA_FLAGS });
...
gate('FOOTER_IN_NORMAL_FLOW=YES', todos(dash, (s) => (s.fechado.flow.attr === 'on' || s.fechado.flow.lv2 === 'on') && s.fechado.flow.footerPos === 'static' && s.fechado.flow.footerInnerPos === 'static' && s.fechado.flow.mainPos === 'static' && s.fechado.flow.shell && ((s.fechado.flow.shell.display === 'flex' && s.fechado.flow.shell.dir === 'column') || (s.fechado.flow.lv2 === 'on' && s.fechado.flow.shell.display === 'grid')) && parseFloat(s.fechado.flow.shell.minH) >= s.vh - 1), dash.map((s) => `${s.tag}: flow=${s.fechado.flow.attr}${s.fechado.flow.lv2 === 'on' ? '(layout v2)' : ''} main=${s.fechado.flow.mainPos} footer=${s.fechado.flow.footerPos}/${s.fechado.flow.footerInnerPos} shell=${s.fechado.flow.shell?.display}/${s.fechado.flow.shell?.dir} min-height=${s.fechado.flow.shell?.minH} padding-top=${s.fechado.flow.shell?.padTop}`).join(' · '));
