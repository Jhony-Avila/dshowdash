...
const parity = d0 && d1 && ['headerRegion', 'ticker', 'main'].every((k) => JSON.stringify(d0.m1.rects[k]) === JSON.stringify(d1.m1.rects[k])) && d0.m1.controls.length === d1.m1.controls.length && d0.m1.controls.every((c, i) => c.sel === d1.m1.controls[i].sel && c.w === d1.m1.controls[i].w && c.h === d1.m1.controls[i].h && Math.abs(c.x - d1.m1.controls[i].x) <= 1 && Math.abs(c.y - d1.m1.controls[i].y) <= 1) && d1.m1.html.mode === 'wide';
gate('DESKTOP_PARITY', parity, d0 && d1 ? `1440: regiões ${['headerRegion', 'ticker', 'main'].map((k) => JSON.stringify(d0.m1.rects[k]) === JSON.stringify(d1.m1.rects[k]) ? 'ok' : k + '≠').join('/')} · controles ${d0.m1.controls.length}→${d1.m1.controls.length} · modo=${d1.m1.html.mode}` : 'cenário 1440 ausente');
...
if (safe) {
  ...
  const ok = cs.every((s) => s.m1.headerOverlap === 0 && s.m1.stackGap === 0 && s.m1.underSafe === 0 && s.m1.rects.headerRegion.h === 56 + Number(safe.safeTop) && s.m1.rects.siteHeader.y === Number(safe.safeTop));
