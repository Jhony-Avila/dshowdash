...
// shell é relatada em INFO SHELL_PERF_TELEMETRY e NÃO conta no gate — qualquer outro console.error continua sendo FAIL.
...
const perr = []; const cerr = []; const perfShell = [];
page.on('pageerror', (e) => perr.push(String(e && e.message || e).slice(0, 200)));
page.on('console', (m) => { const c = classificarConsole(m); if (c.perfShell) { perfShell.push(c.texto); return; } if (c.tipo === 'error' || /Already mounted/.test(c.texto)) cerr.push(c.tipo + ': ' + c.texto); });
...
gates.push(`INFO SHELL_PERF_TELEMETRY — ${perfShell.length} ocorrência(s) do bootstrap do shell (não conta no gate)${perfShell.length ? ' ' + JSON.stringify(perfShell.slice(0, 2)) : ''}`);
const fails = gates.filter((g) => g.startsWith('FAIL')).length;
const txt = `# SMOKE panel-gestao-paineis — ${new Date().toISOString()} — base=${BASE}\n` + gates.join('\n') + `\n# ${fails ? fails + ' FAIL' : 'TODOS PASS'} (${gates.filter((g) => !g.startsWith('INFO')).length} gates)\n`;
